...
describe('migrateV1ToV2', () => {
  it('groups v1 flat tabs by workspace slug', () => {
    ...
    expect(v2.byWorkspace.butter.activeTabId).toBe('t3'); 
...
describe('useTabStore actions', () => {
  ...
  it('closeTab on the last tab in a workspace reseeds the default tab', () => {
    ...
    expect(s.byWorkspace.acme.tabs).toHaveLength(1);
    expect(s.byWorkspace.acme.tabs[0].url).toBe('/acme/issues');
    expect(s.byWorkspace.acme.tabs[0].id).not.toBe(onlyTabId); 
  });
  ...
  it('ignores updates addressed to a tab after it has been closed', () => {
    ...
    expect(useTabStore.getState().byWorkspace.acme.tabs.some((t) => t.id === closedTabId)).toBe(
      false,
    );
  });
...
describe('togglePin', () => {
  ...
  it('appends a second pinned tab after the first pinned tab', () => {
    ...
    const tabs = useTabStore.getState().byWorkspace.acme.tabs;
    expect(tabs.map((t) => t.id)).toEqual([agentsId, projectsId, tabs[2].id]);
...
describe('moveTab boundary clamp', () => {
  ...
  it('reorders freely within the same zone', () => {
    ...
    store.moveTab(2, 0);
    const tabs = useTabStore.getState().byWorkspace.acme.tabs;
    expect(tabs.map((t) => t.url)).toEqual(['/acme/agents', '/acme/issues', '/acme/projects']);
  });
});
...
describe('pinTabOnce (#348, the capabilities tab after onboarding)', () => {
  const CAPS = '/acme/capabilities';

  it('pins the tab into a workspace this client has never opened', () => {
    const id = useTabStore.getState().pinTabOnce('acme', CAPS, '');

    const group = useTabStore.getState().byWorkspace.acme;
    expect(group.tabs[0].id).toBe(id);
    expect(group.tabs[0].pinned).toBe(true);
    expect(group.tabs[0].url).toBe(CAPS);
    expect(group.activeTabId).toBe(id);
    expect(group.tabs.map((t) => t.url)).toEqual([CAPS, '/acme/issues']);
  });

  it('does not yank the window to another workspace', () => {
    const store = useTabStore.getState();
    store.switchWorkspace('other');
    store.pinTabOnce('acme', CAPS, '');
    expect(useTabStore.getState().activeWorkspaceSlug).toBe('other');
  });

  it('is a no-op the second time — a closed tab is never forced back', () => {
    const store = useTabStore.getState();
    const id = store.pinTabOnce('acme', CAPS, '');
    store.closeTab(id);
    expect(useTabStore.getState().byWorkspace.acme.tabs.map((t) => t.url)).not.toContain(CAPS);

    store.pinTabOnce('acme', CAPS, '');
    expect(useTabStore.getState().byWorkspace.acme.tabs.map((t) => t.url)).not.toContain(CAPS);
  });

  it('pins an already-open tab instead of duplicating it', () => {
    const store = useTabStore.getState();
    store.switchWorkspace('acme');
    const openedId = store.openTab(CAPS, 'Capabilities');

    const id = store.pinTabOnce('acme', CAPS, '');

    expect(id).toBe(openedId);
    const group = useTabStore.getState().byWorkspace.acme;
    expect(group.tabs.filter((t) => t.url === CAPS)).toHaveLength(1);
    expect(group.tabs[0].id).toBe(openedId);
    expect(group.tabs[0].pinned).toBe(true);
  });

  it('keeps the pinned-first invariant when the group already has pinned tabs', () => {
    const store = useTabStore.getState();
    store.switchWorkspace('acme');
    const issuesId = useTabStore.getState().byWorkspace.acme.tabs[0].id;
    store.addTab('/acme/projects', 'Projects');
    ...
    const tabs = useTabStore.getState().byWorkspace.acme.tabs;
    expect(tabs.map((t) => t.pinned)).toEqual([true, true, false]);
    expect(tabs[1].url).toBe(CAPS);
  });

  it('survives a persisted marker across a restart', () => {
    const merged = mergePersistedTabs(
      {
        activeWorkspaceSlug: 'acme',
        byWorkspace: {},
        autoPinned: { [`acme::${CAPS}`]: true },
      },
      {
        activeWorkspaceSlug: null,
        byWorkspace: {} as Record<string, WorkspaceTabGroup>,
        autoPinned: {} as Record<string, true>,
      },
    );
    expect(merged.autoPinned).toEqual({ [`acme::${CAPS}`]: true });
  });

  it('reads a payload written before #348 as "nothing auto-pinned yet"', () => {
    const merged = mergePersistedTabs(
      { activeWorkspaceSlug: 'acme', byWorkspace: {} },
      {
        activeWorkspaceSlug: null,
        byWorkspace: {} as Record<string, WorkspaceTabGroup>,
        autoPinned: {} as Record<string, true>,
    ...
    expect(merged.autoPinned).toEqual({});
  });

  it('rejects a path that is not workspace-scoped, creating no group', () => {
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
    expect(useTabStore.getState().pinTabOnce('acme', '/issues', '')).toBe('');
    expect(useTabStore.getState().byWorkspace.acme).toBeUndefined();
    expect(useTabStore.getState().autoPinned).toEqual({});
    warn.mockRestore();
  });

  it('gives the next member on this machine their own pin', () => {
    const store = useTabStore.getState();
    store.pinTabOnce('acme', CAPS, '');
    store.reset(); 
    expect(useTabStore.getState().autoPinned).toEqual({});

    useTabStore.getState().pinTabOnce('acme', CAPS, '');
    expect(useTabStore.getState().byWorkspace.acme.tabs.some((t) => t.url === CAPS)).toBe(true);
  });
});
...
describe('mergePersistedTabs (rehydration, MUL-4370)', () => {
  ...
  } => ({ activeWorkspaceSlug: null, byWorkspace: {}, autoPinned: {} });
