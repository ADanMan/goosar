// Тесты двух регрессий сброса отчёта о зависании: подтверждение должно
// приходить по факту доставки, а не при передаче в SDK.
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest';

import { buildFreezeEventProps, flushFreezeBreadcrumb, FREEZE_ACK_GRACE_MS } from './freeze-flush';
...
function setup(overrides: Partial<FreezeBreadcrumb> | null = {}) {
  ...
  const options = capture.mock.calls[0]?.[2] as
    { onCaptured?: () => void; sendInstantly?: boolean } | undefined;
...
describe('telemetry props carry no raw identifiers', () => {
  ...
  it('drops workspace slug, tab id and window url if a context still carries them', () => {
    const leaky: FreezeBreadcrumb = {
      ...hang,
      context: {
        windowUrl: 'file:///Users/someone/Applications/Goosar.app/index.html',
        desktopRoute: {
          surface: 'tab',
          path: '/:slug/issues',
          workspaceSlug: 'acme',
        ...
        stack: [
          {
            ...
            url: 'file:///Users/someone/Applications/Goosar.app/out/renderer/main.js',
