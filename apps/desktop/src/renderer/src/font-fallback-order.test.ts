...
function expectJapaneseScopedOverride(source: string) {
  ...
  const lastChinese = Math.max(...chineseFonts.map((font) => source.lastIndexOf(font)));
...
describe('CJK font fallback order', () => {
  it('keeps desktop Chinese font fallbacks before Korean font fallbacks', () => {
    const desktopCss = readFileSync(resolve(process.cwd(), 'src/renderer/src/globals.css'), 'utf8');
