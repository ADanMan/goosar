...
vi.mock('@/stores/tab-store', () => {
  ...
  const useTabStore = (selector: (s: ReturnType<typeof state>) => unknown) => selector(state());
...
describe('DesktopRouteErrorPage', () => {
  ...
  it('brands React Router route errors and offers tab reload', async () => {
    ...
    expect(await screen.findByRole('alert')).toHaveTextContent('Something went wrong in this tab');
  ...
  describe('404 Not Found', () => {
    it('renders a Not Found page, not a crash report, for an unroutable path', async () => {
      ...
      expect(screen.queryByRole('button', { name: /report error/i })).not.toBeInTheDocument();
      expect(screen.queryByRole('button', { name: /reload tab/i })).not.toBeInTheDocument();
    });

    it('shows the unroutable path so the user can see what was wrong with the link', async () => {
      renderUnmatchedRoute('/Users/whoever/Desktop/shot.png');
      expect(await screen.findByRole('alert')).toHaveTextContent('/Users/whoever/Desktop/shot.png');
    });
    ...
    it('takes the recovery entry from the active workspace, NEVER from the bad pathname', async () => {
      ...
      expect(navigateActiveSession).not.toHaveBeenCalledWith('/Users/issues', expect.anything());
    });
    ...
    it('offers no recovery entry at all when there is no active workspace', async () => {
      ...
      expect(screen.queryByRole('button', { name: /go to issues/i })).not.toBeInTheDocument();
