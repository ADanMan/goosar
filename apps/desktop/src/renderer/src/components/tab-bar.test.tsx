...
describe('TabBar hover action buttons', () => {
  ...
  it('renders the resource leading visual for every tab', () => {
    ...
    const { getByLabelText } = render(<TabBar />);
    expect(getByLabelText('Autopilots').querySelector('[data-testid="tab-leading"]')).toBeTruthy();
    expect(getByLabelText('Project').querySelector('[data-testid="tab-leading"]')).toBeTruthy();
  });
...
describe('TabBar overflow', () => {
  ...
  it('uses a directional mask instead of a visible scrollbar', async () => {
    ...
    const { result } = renderHook(() => useScrollFade(tabScrollRef, 24, 'horizontal'));
  ...
  it('scrolls only the tab strip when the active tab moves out of view', () => {
    ...
    const tabScroller = container.querySelector('[data-tab-scroll-container]') as HTMLDivElement;
  ...
  it('smoothly reveals a newly added active tab', () => {
    ...
    expect(getByLabelText('Tab 6')).toHaveAttribute('data-tab-entering', 'true');
