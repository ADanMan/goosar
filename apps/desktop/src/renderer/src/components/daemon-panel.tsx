...
export function DaemonPanel({ open, onOpenChange, status, runtimeCount }: DaemonPanelProps) {
  ...
  const handleCopy = useCallback(async () => {
    ...
    if (await copyText(text)) {
      toast.success(t(($) => $.desktop.daemon.logs.copied, { count: filtered.length }));
    } else {
      toast.error(t(($) => $.desktop.daemon.logs.copy_failed));
    }
  }, [filtered, t]);
      ...
      >
        ...
        <div className="flex shrink-0 items-center justify-between gap-3 border-b px-4 py-3">
          ...
          <button
            ...
            aria-label={t(($) => $.desktop.daemon.logs.close)}
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2 border-b px-4 py-2">
          ...
          <div className="relative w-56">
            ...
            <input
              ...
              placeholder={t(($) => $.desktop.daemon.logs.search_placeholder)}
        ...
        <div className="flex shrink-0 items-center justify-between border-t bg-muted/30 px-4 py-1.5 text-xs text-muted-foreground">
          <span className="tabular-nums">
            {t(($) => $.desktop.daemon.logs.showing, {
              shown: filtered.length,
              total: logs.length,
            })}
            {logs.length === MAX_LOG_LINES && (
              <span className="ml-1 text-muted-foreground/60">
                {t(($) => $.desktop.daemon.logs.buffer_full)}
              </span>
            )}
          </span>
          {!autoScroll && (
            ...
            >
              ...
              {t(($) => $.desktop.daemon.logs.jump_to_latest)}
            </button>
          )}
        </div>
      </DialogContent>
    ...
    return (
      <>
        ...
        >
          <span>···</span>
          <span>
            {t(($) => $.desktop.daemon.logs.expand_repeated, {
              count: rest.length,
  ...
  return (
    <>
      ...
      >
        <span>···</span>
        <span>
          {t(($) => $.desktop.daemon.logs.collapse_repeated, {
            count: rest.length + 1,
          })}
        </span>
      </button>
...
function EmptyState({
  ...
  let subtitle: string;
  if (hasFilter) {
    title = t(($) => $.desktop.daemon.logs.empty_filtered_title);
    subtitle = t(($) => $.desktop.daemon.logs.empty_filtered_subtitle);
  } else if (!isRunning) {
    title = t(($) => $.desktop.daemon.logs.empty_stopped_title);
    subtitle = t(($) => $.desktop.daemon.logs.empty_stopped_subtitle);
  } else if (!hasLogs) {
    title = t(($) => $.desktop.daemon.logs.empty_waiting_title);
    subtitle = t(($) => $.desktop.daemon.logs.empty_waiting_subtitle);
  } else {
    title = '';
    subtitle = '';
  }
