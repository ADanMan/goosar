...
function getTabElement(scroller: HTMLDivElement, tabId?: string): HTMLElement | null {
  if (!tabId) {
    return scroller.querySelector<HTMLElement>('[data-tab-active="true"]');
  }

  return (
    Array.from(scroller.querySelectorAll<HTMLElement>('[data-tab-id]')).find(
      (candidate) => candidate.dataset.tabId === tabId,
    ) ?? null
  );
}

function getTabScrollTarget(scroller: HTMLDivElement, tab: HTMLElement): number {
  ...
  const visibleLeft = scrollerRect.left + (hasHiddenLeft ? TAB_SCROLL_FADE_SIZE : 0);
  const visibleRight = scrollerRect.right - (hasHiddenRight ? TAB_SCROLL_FADE_SIZE : 0);

  if (tabRect.left < visibleLeft) {
    return Math.max(0, scroller.scrollLeft - (visibleLeft - tabRect.left));
  }
  if (tabRect.right > visibleRight) {
    return Math.min(maxScrollLeft, scroller.scrollLeft + (tabRect.right - visibleRight));
  }
    ...
    >
      ...
      >
        {isActive ? (
          <span
            aria-hidden
            className={cn('pointer-events-none absolute inset-0', isDragging && 'opacity-60')}
          >
...
function NewTabEdgeFeedback({
  ...
  return (
    <motion.div
      ...
      onAnimationComplete={() => {
        setSignal((current) => (current?.sequence === signal.sequence ? null : current));
      }}
    />
  );
}
...
export function TabBar() {
  ...
  const tabFadeStyle = useScrollFade(tabScrollRef, TAB_SCROLL_FADE_SIZE, 'horizontal');
  ...
  const tabLayoutKey = tabs
    ...
    .join('\0');
  const addedTabIds = getAddedTabIds(previousTabsRef.current, activeWorkspaceSlug, tabIds);
  const addedTabIdSet = new Set(addedTabIds);
  const newestTabId = addedTabIds.at(-1) ?? null;
  const backgroundAddedTabId = newestTabId && newestTabId !== activeTabId ? newestTabId : null;
  ...
  useLayoutEffect(() => {
    ...
    if (newlyAddedIds.length > 0) {
      if (newlyAddedIds.includes(activeTabId)) {
        keepTabVisible(tabScrollRef.current, activeTabId, shouldReduceMotion ? 'auto' : 'smooth');
      }
      return;
    }

    keepTabVisible(tabScrollRef.current);
  }, [activeWorkspaceSlug, activeTabId, shouldReduceMotion, tabLayoutKey, tabOrder]);
              ...
              {tabs.map((tab, index) => {
                ...
                return (
                  <Fragment key={tab.id}>
                    ...
                    />
                    {tab.pinned && index === pinnedCount - 1 && unpinnedCount > 0 && (
                      <div
                        aria-hidden
