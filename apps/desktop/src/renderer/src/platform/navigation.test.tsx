...
describe('push', () => {
  ...
  it('routes /workspaces/join to the role-picker overlay', () => {
    const getAdapter = renderProvider();
    const before = acmeGroup();

    getAdapter().push('/workspaces/join');

    expect(overlay.open).toHaveBeenCalledWith({ type: 'join-workspace' });
    expect(acmeGroup()).toBe(before);
  });

  it('routes /onboarding to the onboarding overlay without touching sessions', () => {
    const getAdapter = renderProvider();
    const before = acmeGroup();

    getAdapter().push('/onboarding');

    expect(overlay.open).toHaveBeenCalledWith({ type: 'onboarding' });
    expect(acmeGroup()).toBe(before);
  });

  it('routes /onboarding?replay=1 to the same overlay without touching sessions', () => {
    const getAdapter = renderProvider();
    const before = acmeGroup();

    getAdapter().push('/onboarding?replay=1');
...
describe('routeContentLinkPath (links inside content — MUL-5208)', () => {
  ...
  it('switches workspace instead of opening the path under the active group', () => {
    ...
    expect(state.byWorkspace.acme.tabs.some((t) => t.url === '/butter/issues/BUT-1')).toBe(false);
  });
});
