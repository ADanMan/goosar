// Главный процесс решает, показывать ли нативное уведомление: только он знает,
// есть ли окно Goosar на переднем плане, и схлопывает одно событие от N рендереров.
...
export function parseNativeNotificationPayload(value: unknown): NativeNotificationPayload | null {
  ...
  for (const key of Object.keys(limits) as (keyof NativeNotificationPayload)[]) {
    ...
    if (typeof field !== 'string' || (!mayBeEmpty && !field.trim()) || field.length > limits[key]) {
      return null;
    }
    result[key] = field;
  }
