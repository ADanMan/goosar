...
describe('decideIdleRestartAction (issue #189: provisioning-triggered daemon restart)', () => {
  it('returns not_running when health payload is null', () => {
    expect(decideIdleRestartAction(null)).toBe('not_running');
  });

  it("returns not_running when status is not 'running'", () => {
    expect(decideIdleRestartAction({ status: 'stopped' })).toBe('not_running');
  });

  it('returns restart when the daemon is running and idle', () => {
    expect(decideIdleRestartAction({ status: 'running', active_task_count: 0 })).toBe('restart');
  });

  it('treats a missing active_task_count as idle (0)', () => {
    expect(decideIdleRestartAction({ status: 'running' })).toBe('restart');
  });

  it('returns defer when the daemon is running but busy', () => {
    expect(decideIdleRestartAction({ status: 'running', active_task_count: 3 })).toBe('defer');
  });

  it('transitions defer → restart as tasks drain', () => {
    expect(decideIdleRestartAction({ status: 'running', active_task_count: 2 })).toBe('defer');
    expect(decideIdleRestartAction({ status: 'running', active_task_count: 1 })).toBe('defer');
    expect(decideIdleRestartAction({ status: 'running', active_task_count: 0 })).toBe('restart');
  });
});
