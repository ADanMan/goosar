...
const GITHUB_LATEST_BASE = 'https://github.com/adanman/goosar/releases/latest/download';

function binaryName(): string {
  return process.platform === 'win32' ? 'goosar.exe' : 'goosar';
}
...
async function installFresh(): Promise<string> {
  ...
  const workDir = join(tmpdir(), `goosar-cli-${Date.now()}`);
...
export async function ensureManagedCli(options: { forceInstall?: boolean } = {}): Promise<string> {
  ...
  if (updaterHardDisabled()) {
    throw new Error(
      'managed CLI download is disabled in this build (perimeter delivery); ' +
        'reinstall the app package or place the goosar binary manually',
    );
  }
  return installFresh();
}
