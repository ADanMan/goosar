...
export { AUTH_SESSION_STATE_CHANNEL, parseAuthSessionUserId } from '../shared/auth-session';

export class AuthSessionCoordinator<T> {
  private mainUserId: AuthSessionUserId | undefined;
  private readonly issueUserIds = new Map<T, AuthSessionUserId | undefined>();
  ...
  isCurrentIssueSession(window: T): boolean {
    return typeof this.mainUserId === 'string' && this.issueUserIds.get(window) === this.mainUserId;
  }

  reportMain(userId: AuthSessionUserId): boolean {
    const becameLoggedOut = userId === null && this.mainUserId !== null;
    const accountChanged = this.mainUserId !== undefined && this.mainUserId !== userId;
  ...
  reportIssue(window: T, userId: AuthSessionUserId): void {
    ...
    if (userId === null || (this.mainUserId !== undefined && this.mainUserId !== userId)) {
      this.closeIssue(window);
    }
  }
