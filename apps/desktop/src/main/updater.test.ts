...
const ctx = vi.hoisted(() => ({
  ...
  ipcOnHandlers: new Map<string, Handler[]>(),
  ipcHandle: vi.fn(),
...
vi.mock('electron', () => ({
  ...
  ipcMain: {
    ...
    on: vi.fn((channel: string, handler: Handler) => {
      const handlers = ctx.ipcOnHandlers.get(channel) ?? [];
      handlers.push(handler);
      ctx.ipcOnHandlers.set(channel, handlers);
    }),
  },
}));

import { configureMacX64UpdateChannel, setupAutoUpdater } from './updater';
import { updaterPreferencesPath } from './updater-preferences';
import { UPDATE_CONTROL_CHANNEL } from '../shared/update-control';
...
function emitIpcOn(channel: string, ...args: unknown[]) {
  const handlers = ctx.ipcOnHandlers.get(channel);
  if (!handlers?.length) throw new Error(`Missing IPC listener: ${channel}`);
...
describe('setupAutoUpdater', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    ctx.userDataPath = mkdtempSync(join(tmpdir(), 'goosar-updater-test-'));
    ctx.handlers.clear();
    ctx.ipcHandlers.clear();
    ctx.ipcOnHandlers.clear();
  ...
  it('persists the automatic update preference and stops future background checks', async () => {
    ...
    await expect(invokeIpc('updater:set-automatic-updates', false)).resolves.toEqual({
      automaticUpdates: false,
    });
    expect(JSON.parse(readFileSync(updaterPreferencesPath(ctx.userDataPath), 'utf-8'))).toEqual({
      automaticUpdates: false,
    });
  ...
  it('skips update progress when webContents.send loses a destroy race', () => {
    const { win, send } = makeWindowWithThrowingSend(new TypeError('Object has been destroyed'));
    setupAutoUpdater(() => win);
  ...
  it('rethrows non-destroy errors from webContents.send', () => {
    ...
    expect(() => emitUpdater('download-progress', { percent: 42 })).toThrow('boom');
  });
});

describe('setupAutoUpdater delivery-profile gating', () => {
  beforeEach(() => {
    vi.useFakeTimers();
    ctx.userDataPath = mkdtempSync(join(tmpdir(), 'goosar-updater-gate-test-'));
    ctx.handlers.clear();
    ctx.ipcHandlers.clear();
    ctx.ipcOnHandlers.clear();
    ctx.ipcHandle.mockClear();
    ...
    ctx.downloadUpdate.mockClear();
    ctx.quitAndInstall.mockClear();
    delete process.env.GOOSAR_DESKTOP_NO_UPDATER;
  });
  ...
  it('stops background checks when the server reports the perimeter profile', async () => {
    setupAutoUpdater(() => null);
    await invokeIpc('updater:get-preferences');

    emitIpcOn(UPDATE_CONTROL_CHANNEL, { perimeterProfile: true });

    await vi.advanceTimersByTimeAsync(2 * 60 * 60 * 1000);
    expect(ctx.checkForUpdates).not.toHaveBeenCalled();
  });

  it('refuses manual checks while the perimeter profile is reported', async () => {
    setupAutoUpdater(() => null);
    await invokeIpc('updater:get-preferences');
    emitIpcOn(UPDATE_CONTROL_CHANNEL, { perimeterProfile: true });
    ...
    expect(result).toEqual({
      ok: false,
      error: expect.stringContaining('perimeter'),
    });
    expect(ctx.checkForUpdates).not.toHaveBeenCalled();
    await expect(invokeIpc('updater:download')).rejects.toThrow('perimeter');
    await invokeIpc('updater:install');
    expect(ctx.quitAndInstall).not.toHaveBeenCalled();
  });

  it('re-arms background checks when the server stops reporting perimeter', async () => {
    setupAutoUpdater(() => null);
    await invokeIpc('updater:get-preferences');
    emitIpcOn(UPDATE_CONTROL_CHANNEL, { perimeterProfile: true });
    ...
    emitIpcOn(UPDATE_CONTROL_CHANNEL, { perimeterProfile: false });

    await vi.advanceTimersByTimeAsync(5_000);
    expect(ctx.checkForUpdates).toHaveBeenCalled();
  });

  it('treats a malformed control payload as cloud (default-on preserved)', async () => {
    setupAutoUpdater(() => null);
    await invokeIpc('updater:get-preferences');
  ...
  it('hard-disables everything when the build-time flag is set', async () => {
    ...
    const result = (await invokeIpc('updater:check')) as { ok: boolean };
