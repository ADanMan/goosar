...
describe('handleAppShortcut — reload blocking', () => {
  ...
  it('ignores non-keyDown events', () => {
    const wc = makeWc();
    expect(handleAppShortcut({ ...key('r', { meta: true }), type: 'keyUp' }, wc, 'darwin')).toBe(
      false,
    );
  });
});
...
describe('handleAppShortcut — unrelated keys pass through', () => {
  ...
  it('rejects extra secondary modifiers for owned shortcuts', () => {
    const wc = makeWc();
    expect(handleAppShortcut(key('w', { meta: true, control: true }), wc, 'darwin')).toBe(false);
    expect(handleAppShortcut(key('-', { control: true, alt: true }), wc, 'win32')).toBe(false);
  });
});
