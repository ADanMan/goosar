...
async function validateLocalDirectory(path: string): Promise<ValidateLocalDirectoryResult> {
...
export function setupLocalDirectory(windowGetter: () => BrowserWindow | null): void {
  ipcMain.handle(
  ...
  ipcMain.handle(
    ...
    (_event, path: string): Promise<ValidateLocalDirectoryResult> => validateLocalDirectory(path),
  );
}
