// Тесты: кадры стека пересобираются на обоих концах — при захвате и при отправке,
// потому что промежуточный файл почти не валидируется.
import { describe, expect, it } from 'vitest';

import { HANG_STACK_MAX_FRAMES, redactFrameUrl, sanitizeHangStackFrames } from './hang-stack';
...
describe('redactFrameUrl', () => {
  it('strips the install path, which can contain the OS username', () => {
    expect(
      redactFrameUrl('file:///Users/someone/Applications/Goosar.app/out/renderer/main.js'),
    ).toBe('renderer/main.js');
  });

  it('is idempotent, so re-sanitizing does not erode the value', () => {
    const once = redactFrameUrl(
      'file:///Users/someone/Applications/Goosar.app/out/renderer/main.js',
    );
