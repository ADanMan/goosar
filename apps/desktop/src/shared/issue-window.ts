export const ISSUE_WINDOW_ARGUMENT_PREFIX = '--goosar-issue-window=';
...
export type DesktopWindowContext = { kind: 'main' } | IssueWindowContext;
...
export function parseIssueWindowPath(
  ...
  let url: URL;
  try {
    url = new URL(value, 'https://desktop.goosar.invalid');
  } catch {
    return null;
  }
  if (url.origin !== 'https://desktop.goosar.invalid') return null;
...
export function readDesktopWindowContext(argv: readonly string[]): DesktopWindowContext {
  const argument = argv.find((item) => item.startsWith(ISSUE_WINDOW_ARGUMENT_PREFIX));
