...
import { MainRendererMessageQueue, parseMainRendererChannelState } from './main-renderer-messages';
...
describe('parseMainRendererChannelState', () => {
  it('accepts only allowlisted channels with an explicit boolean', () => {
    expect(parseMainRendererChannelState({ channel: 'auth:token', ready: true })).toEqual({
      channel: 'auth:token',
      ready: true,
    });
    expect(
    ...
    expect(parseMainRendererChannelState({ channel: 'auth:token', ready: 'yes' })).toBeNull();
  });
});
