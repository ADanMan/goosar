...
function binaryNameForPlatform(platform) {
  return platform === 'win32' ? 'goosar.exe' : 'goosar';
}
...
if (goAvailable) {
  const version = git('describe', '--tags', '--match', 'v[0-9]*', '--always', '--dirty') || 'dev';
  ...
  execFileSync('go', ['build', '-ldflags', ldflags, '-o', srcBinary, './cmd/goosar'], {
    cwd: serverDir,
    stdio: 'inherit',
    env: {
      ...process.env,
...
if (!(await exists(srcBinary))) {
  ...
  if (process.env.GOOSAR_RELEASE_BUILD === '1') {
    console.error(
      `[bundle-cli] release build requires the bundled goosar CLI: ${srcBinary} not present ` +
        `(go ${goAvailable ? 'built nothing' : 'not found in PATH'})`,
    );
    process.exit(1);
  }
  console.warn(
