---
title: Architecture
description: Technical architecture of the Goosar platform.
---

## Overview

Goosar is a Go backend + monorepo frontend (pnpm workspaces + Turborepo) with shared packages.
...
- **Entry points** (`cmd/`): `server` (HTTP API), `goosar` (CLI + daemon), `migrate`
...
- **Agent SDK** (`pkg/agent/`): Unified `Backend` interface for executing prompts via any supported AI coding tool
