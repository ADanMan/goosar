---
title: Contributing
description: Local development workflow for contributors working on the Goosar codebase.
---
...
- The main checkout usually uses `.env` and `POSTGRES_DB=goosar`
- Each Git worktree uses its own `.env.worktree`
- Every checkout connects to the same PostgreSQL host: `localhost:5433`
...
| | Main | Worktree |
|---|---|---|
| Database | `goosar` | `goosar_my_feature_702` |
| Backend port | `8081` | generated (e.g. `18782`) |
| Frontend port | `3001` | generated (e.g. `13702`) |
...
pnpm install
pnpm dev:web          # Next.js dev server (port 3001)
...
# Backend (Go)
make dev              # Run Go server (port 8081)
