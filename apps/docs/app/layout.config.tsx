...
// Docs-local stateless Goosar mark — matches @goosar/ui's GoosarIcon
...
// packages/ui/components/common/goosar-icon.tsx if the mark changes.
const GOOSAR_CLIP = `polygon(
...
function GoosarMark() {
  return (
    <span className="inline-block size-[1em]" aria-hidden="true">
      <span className="block size-full bg-current" style={{ clipPath: GOOSAR_CLIP }} />
    </span>
  );
}
...
// desktop). Leading icon = brand identity (GitHub mark / Goosar asterisk);
...
export const baseOptions: BaseLayoutProps = {
  nav: {
    title: <span className="font-semibold text-base">Goosar Docs</span>,
  },
  links: [
    {
      icon: <GitHubMark />,
