...
// navigation inside English (or any non-default-language) docs stays in
// that language. Without this, `[xx](/security)` written in a `*.en.mdx`
// renders as `<a href="/security">`, which Next's basePath rewrites to
// `/docs/security` and the docs middleware then routes to Russian —
