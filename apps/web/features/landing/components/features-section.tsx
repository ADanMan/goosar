...
import { StatusIcon, PriorityIcon } from '@goosar/views/issues/components';
import { STATUS_CONFIG } from '@goosar/core/issues/config/status';
import { PRIORITY_CONFIG } from '@goosar/core/issues/config/priority';
import type { IssueStatus, IssuePriority } from '@goosar/core/types';
...
function TeammatesVisual() {
  ...
  const [assignee, setAssignee] = useState<Assignee>(allAssignees[3]!); 
    ...
    <div className="relative aspect-video overflow-hidden rounded-lg border bg-background text-foreground shadow-2xl">
      ...
      <div className="flex h-10 shrink-0 items-center border-b bg-background px-4 text-sm">
        <div className="flex items-center gap-1.5 min-w-0 text-xs">
          <span className="text-muted-foreground">Goosar Demo</span>
...
const mockToolCalls = [
  {
    ...
    content: 'Analyzing the error handling patterns across all 14 handler files…',
  },
  { type: 'tool_use' as const, tool: 'Read', summary: 'server/internal/handler/issue.go' },
  {
    type: 'tool_result' as const,
    preview: 'func (h *IssueHandler) Create(w http.ResponseWriter, r *http.Request) { …',
  },
  {
  ...
  {
    type: 'thinking' as const,
    content: 'Now checking handler/comment.go for the same inconsistent patterns…',
  },
  { type: 'tool_use' as const, tool: 'Read', summary: 'server/internal/handler/comment.go' },
  {
    type: 'tool_result' as const,
    preview: 'func (h *CommentHandler) Create(w http.ResponseWriter, r *http.Request) { …',
  },
  {
    type: 'tool_use' as const,
    tool: 'Bash',
  ...
  {
    type: 'tool_result' as const,
    preview: 'ok  \tgithub.com/goosar/server/internal/handler\t0.847s',
  },
];
                  ...
                  <div className="space-y-3 text-xs">
                    ...
                    <div className="space-y-2 text-muted-foreground leading-relaxed">
                      ...
                      <p>
                        ...
                        Validates against the current database state and generates both up and down
                        migrations.
                      </p>
...
const mockRuntimeList = [
  ...
  },
  {
    ...
    mode: 'cloud' as const,
    status: 'online' as const,
    device: 'api.runtime-provider.example',
    lastSeen: 'Just now',
  },
  {
    name: 'Linux Server',
...
export function FeaturesSection() {
  ...
  return (
    <section id="features" className="bg-white text-[#1c1917]">
              ...
              {features.map((f, i) => (
                <button
                  ...
                  className={cn(
                    ...
                    i === activeIndex
                      ? 'text-[#1c1917]'
                      : 'text-[#1c1917]/36 hover:text-[#1c1917]/60',
                  )}
                >
                  <span
                    className={cn(
                      'size-2 shrink-0 rounded-full transition-colors',
                      i === activeIndex ? 'bg-[#1c1917]' : 'bg-transparent',
                    )}
                  />
                  {f.label}
                </button>
          ...
          <div className="flex-1">
            {features.map((feature, i) => (
              <div
                ...
                className={cn(
                  'py-20 lg:py-28',
                  i < features.length - 1 && 'border-b border-[#1c1917]/8',
                )}
              >
                {/* Title + description */}
                <h2 className="font-[family-name:var(--font-serif)] text-[2.6rem] leading-[1.05] tracking-[-0.03em] text-[#1c1917] sm:text-[3.4rem] lg:text-[4.2rem]">
                  {feature.title}
                </h2>
                <p className="mt-5 max-w-[640px] text-[15px] leading-7 text-[#1c1917]/60 sm:text-[16px]">
                  {feature.description}
                </p>
                ...
                <div className="mt-14 sm:mt-18">
                  {feature.visual ? (
                    ...
                    <div className="relative overflow-hidden border border-[#1c1917]/8 bg-[#f5f5f5]">
                      <div className="aspect-[16/9] w-full" />
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="flex flex-col items-center gap-4 text-center">
                          <div className="grid size-14 place-items-center rounded-2xl border border-[#1c1917]/8 bg-white shadow-sm">
                            <ImageIcon className="size-6 text-[#1c1917]/30" />
                          </div>
                          <p className="text-[13px] text-[#1c1917]/36">
                ...
                <div className="mt-14 grid gap-8 sm:mt-18 md:grid-cols-3 md:gap-10">
                  {feature.cards.map((card) => (
                    <div key={card.title}>
                      <h3 className="text-[15px] font-semibold leading-snug text-[#1c1917] sm:text-[16px]">
                        {card.title}
                      </h3>
                      <p className="mt-2.5 text-[14px] leading-[1.7] text-[#1c1917]/56 sm:text-[15px]">
                        {card.description}
                      </p>
                    </div>
                  ))}
