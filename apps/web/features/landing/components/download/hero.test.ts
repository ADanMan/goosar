...
describe('resolveContent', () => {
  ...
  it('keeps the macOS CTA active from GOOSAR_MAC_DMG_URL when GitHub releases are unavailable', () => {
    const content = resolveContent(
    ...
    expect(content.primary).toMatchObject({
      href: 'https://downloads.test/goosar.dmg',
      disabled: false,
    });
  });
});
