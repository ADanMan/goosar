    ...
    <SafeAreaView className="flex-1 bg-background">
      <ScrollView contentContainerClassName="px-6 py-6 gap-6">
        ...
        <View className="gap-3">
          <Text className="text-2xl font-semibold text-foreground">Select a workspace</Text>
          ...
          {isLoading ? (
            ...
            <Text className="text-sm text-muted-foreground">
              You don&apos;t belong to any workspaces yet. Contact your workspace admin to be
              invited.
            </Text>
          ) : (
            <View className="gap-3">
              {data.map((ws) => (
                <CardPressable key={ws.id} onPress={() => onSelect(ws.id, ws.slug)}>
                  <Text className="text-base font-semibold text-foreground">{ws.name}</Text>
                  <Text className="text-xs text-muted-foreground mt-1">/{ws.slug}</Text>
                  {ws.description ? (
                    <Text className="text-sm text-muted-foreground mt-2">{ws.description}</Text>
                  ) : null}
                </CardPressable>
              ))}
            </View>
