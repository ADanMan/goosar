// Лист фильтра по статусу и приоритету (formSheet), общий для «Моих задач»
// и задач воркспейса; стор выбирается параметром scope.
  ...
  return (
    <View className="flex-1">
      <View className="flex-row items-center justify-between px-4 pt-4 pb-3">
        ...
        {hasActive ? (
          <Pressable onPress={onClearFilters} hitSlop={8} className="px-2 py-1 active:opacity-60">
        ...
        {ALL_STATUSES.map((status) => {
          ...
          return (
            ...
            >
              ...
              <Text className="flex-1 text-sm text-foreground">{STATUS_LABEL[status]}</Text>
        ...
        {PRIORITY_ORDER.map((priority) => {
          ...
          return (
            ...
            >
              ...
              <Text className="flex-1 text-sm text-foreground">{PRIORITY_LABEL[priority]}</Text>
              <CheckMark checked={checked} />
            </Pressable>
          );
        })}
