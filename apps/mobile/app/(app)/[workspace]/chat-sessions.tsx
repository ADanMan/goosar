// Лист переключения сессии чата (formSheet). Читает список сессий из кэша чата
// и пишет выбор в общий стор активной сессии.
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        {sessions.length === 0 ? (
          <View className="px-4 py-8">
            <Text className="text-sm text-muted-foreground text-center">No chats yet.</Text>
          </View>
        ) : (
          sessions.map((session) => {
            ...
            return (
              ...
              >
                ...
                <View className="flex-1">
                  <Text
                    className={cn('text-sm text-foreground', session.has_unread && 'font-semibold')}
                    numberOfLines={1}
                  >
                    {session.title || 'Untitled chat'}
                  </Text>
                  {archived ? (
                    <Text className="text-xs text-muted-foreground mt-0.5">archived</Text>
                  ) : null}
                </View>
                {selected ? <Text className="text-sm text-primary font-semibold">✓</Text> : null}
              </Pressable>
            );
          })
        )}
