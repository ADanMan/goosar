// Страница всех задач воркспейса: зеркалит веб-страницу issues — области
// all / members / agents, группировка по статусу, фильтр по статусу и приоритету.
...
import { useIssuesViewStore, type IssuesScope } from '@/data/stores/issues-view-store';
...
export default function IssuesPage() {
  ...
  useClearFiltersOnWorkspaceChange(useIssuesViewStore.getState().clearFilters, wsId);

  const { data, isLoading, error, refetch, isRefetching } = useQuery(issueListOptions(wsId));
    ...
    <View className="flex-1 bg-background">
      ...
      {hasActiveFilters ? (
        <ActiveFilterChips
          ...
          onClearStatus={(s) => useIssuesViewStore.getState().toggleStatusFilter(s)}
          onClearPriority={(p) => useIssuesViewStore.getState().togglePriorityFilter(p)}
        />
      ) : null}
      {isLoading ? (
        <IssuesLoading />
        ...
        <EmptyState
          message={
            hasActiveFilters ? 'No issues match the current filters.' : emptyMessageForScope(scope)
          }
        />
      ) : (
        <SectionList
  ...
  return (
    <View style={{ position: 'relative' }} className="ml-2">
      <Button
        ...
        className="w-9 px-0"
      >
        <Ionicons name="options-outline" size={16} color={THEME[colorScheme].mutedForeground} />
      </Button>
      {hasActiveFilters ? (
        <View
          pointerEvents="none"
