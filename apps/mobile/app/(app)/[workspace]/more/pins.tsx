// Список закреплённых элементов — аналог секции «Закреплённые» в сайдбаре веба.
// PinnedItem несёт только тип и id; название подтягивается из кэша сущности.
import { useMemo } from 'react';
import { ActivityIndicator, Pressable, RefreshControl, ScrollView, View } from 'react-native';
import { useQuery } from '@tanstack/react-query';
import { router } from 'expo-router';
import { Ionicons } from '@expo/vector-icons';
import type { Issue, PinnedItem, Project } from '@goosar/core/types';
...
export default function PinsPage() {
  ...
  const { data, isLoading, error, refetch, isRefetching } = useQuery(pinListOptions(wsId, userId));

  const pins = useMemo(() => [...(data ?? [])].sort((a, b) => a.position - b.position), [data]);
    ...
    return (
      <View className="flex-1 bg-background px-4 gap-3 pt-4">
        <Text className="text-sm text-destructive">
          Failed to load pins: {error instanceof Error ? error.message : 'unknown error'}
        </Text>
...
function ProjectPinRow({
  ...
  if (isLoading) return <SkeletonRow />;
  if (!project) return <MissingPinRow itemType="project" itemId={pin.item_id} />;
...
function MissingPinRow({ itemType, itemId }: { itemType: 'issue' | 'project'; itemId: string }) {
  ...
  return (
    ...
    >
      <Ionicons name="alert-circle-outline" size={18} color={THEME[colorScheme].mutedForeground} />
