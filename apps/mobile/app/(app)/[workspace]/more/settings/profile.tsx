// Подэкран профиля — имя и аватар. Аватар открывает нативный ActionSheet
// (снять / выбрать / удалить).
...
export default function ProfileSettingsScreen() {
  ...
  const removeAvatar = async () => {
    ...
    try {
      ...
      Alert.alert('Remove failed', err instanceof Error ? err.message : 'Could not remove avatar.');
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Alert.alert('Save failed', err instanceof Error ? err.message : 'Could not update profile.');
    } finally {
      setSaving(false);
    }
  };
