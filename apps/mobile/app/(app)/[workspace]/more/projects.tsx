// Страница проектов: плоский список, заголовок и кнопка + в нативной шапке Stack.
...
export default function ProjectsPage() {
  ...
  const { data, isLoading, error, refetch, isRefetching } = useQuery(projectListOptions(wsId));
...
function PlusButton({ onPress }: { onPress: () => void }) {
  return <IconButton name="add" onPress={onPress} accessibilityLabel="New project" />;
}

function EmptyState({ onCreate }: { onCreate: () => void }) {
  return (
    <View className="flex-1 items-center justify-center px-6 gap-4">
      <Text className="text-base font-medium text-foreground">No projects yet</Text>
