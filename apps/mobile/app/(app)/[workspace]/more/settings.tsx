// Страница настроек: аккаунт, переключение воркспейса, оформление, подэкраны
// профиля и уведомлений, выход.
...
export default function SettingsPage() {
  ...
  const onSignOut = () => {
    Alert.alert('Sign out', "You'll need to sign in again to use Goosar on this device.", [
  ...
  const goNotifications = () => router.push(`/${currentSlug}/more/settings/notifications`);
    ...
    <ScrollView className="flex-1 bg-background" contentContainerClassName="px-4 py-4 gap-6">
      ...
      <SectionGroup title="Workspaces">
        {isLoading ? (
          ...
          <View className="p-4">
            <Text className="text-sm text-destructive">Failed to load workspaces</Text>
          </View>
        ) : (
          data?.map((ws, idx) => {
            ...
            return (
              <View key={opt.value}>
                ...
                >
                  ...
                  <Text className="flex-1 text-base font-medium text-foreground">{opt.label}</Text>
                </Pressable>
                {!isLast ? <Separator /> : null}
              </View>
            );
...
function NavRow({
  ...
  return (
    <Pressable
      onPress={onPress}
      className={cn('flex-row items-center px-4 py-3.5 active:bg-secondary gap-3')}
    >
      {leading}
...
function SectionGroup({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <View className="gap-2">
      <Text className="text-xs uppercase tracking-wider text-muted-foreground px-1">{title}</Text>
...
function WorkspaceRow({
  ...
  return (
    ...
    >
      ...
      <Ionicons name={isActive ? 'checkmark' : 'chevron-forward'} size={18} color={iconColor} />
    </Pressable>
  );
}
