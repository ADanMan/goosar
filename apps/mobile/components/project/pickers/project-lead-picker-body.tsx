/**
 * Тело пикера лида проекта — одиночный выбор среди участников и агентов,
 * плюс строка «Не назначен». Заголовок и поиск владеет нативный iOS-хедер
 * маршрута, само тело — плоский FlatList.
 */
      ...
      renderItem={({ item }) => (
        ...
        >
          {item.kind === 'unassigned' ? (
            ...
            >
              <Ionicons name="close-circle-outline" size={20} color={MOBILE_PLACEHOLDER_COLOR} />
            </View>
          ) : item.kind === 'member' ? (
            <ActorAvatar type="member" id={item.member.user_id} size={AVATAR_SIZE} showPresence />
          ) : (
            <ActorAvatar type="agent" id={item.agent.id} size={AVATAR_SIZE} showPresence />
          )}
      ...
      ListEmptyComponent={
        <View className="px-3 py-8 items-center">
          <Text className="text-sm text-muted-foreground text-center">
            {query ? 'No matches.' : 'No members or agents in this workspace yet.'}
          </Text>
        </View>
      }
    />
