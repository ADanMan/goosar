/**
 * Секция ресурсов проекта — список внешних ссылок (сейчас: репозитории
 * GitHub). Тап открывает URL в системном браузере, лонг-тап удаляет.
 * resource_ref типизирован как unknown, чтобы неизвестный resource_type
 * рендерился обычной строкой, а не падал.
 */

import { ActivityIndicator, Alert, Linking, Pressable, View } from 'react-native';
import { useQuery } from '@tanstack/react-query';
import { Ionicons } from '@expo/vector-icons';
import type { GithubRepoResourceRef, ProjectResource } from '@goosar/core/types';
...
export function ProjectResourcesSection({ projectId, onAdd }: Props) {
  ...
  const { data: resources, isLoading } = useQuery(projectResourcesOptions(wsId, projectId));
  ...
  const onLongPress = (resource: ProjectResource) => {
    Alert.alert('Detach resource?', describeResource(resource), [
    ...
    <View>
      ...
      {isLoading ? (
        ...
        <View className="px-4 py-3">
          <Text className="text-sm text-muted-foreground/70">No resources attached.</Text>
        </View>
      ) : (
