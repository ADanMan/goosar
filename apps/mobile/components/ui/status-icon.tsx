/**
 * Иконка статуса задачи для мобильного клиента (react-native-svg).
 * Геометрия (14×14, центр 7,7) повторяет веб-версию, чтобы статус
 * выглядел одинаково на всех платформах. Компонент собственный для
 * мобильного — веб-компонент не переиспользуется напрямую.
 */
...
function ProgressCircle({
  ...
  return (
    <>
      <Circle cx={CX} cy={CY} r={OUTER_R} fill="none" stroke={color} strokeWidth={1.5} />
      {progress === 1 ? (
