/**
 * Визуальная часть строки инбокса, без обработки жестов. Жест свайпа
 * и нажатия — в обёртке swipeable-inbox-row.tsx, этот файл остаётся
 * чисто презентационным.
 */
        ...
        <View className="flex-1 min-w-0">
          ...
          <View className="flex-row items-center gap-2">
            <View className="flex-row items-center gap-1.5 flex-1 min-w-0">
              {isUnread ? <View className="size-1.5 rounded-full bg-brand shrink-0" /> : null}
              <Text
                className={cn(
                  'flex-1 text-sm',
            ...
            {item.issue_status ? <StatusIcon status={item.issue_status} size={14} /> : null}
          </View>
          ...
          <View className="flex-row items-center gap-2 mt-0.5">
            <View className="flex-1 min-w-0">
              <InboxDetailLabel
                item={item}
                className={isUnread ? 'text-muted-foreground' : 'text-muted-foreground/60'}
              />
            </View>
            <Text
              className={cn(
