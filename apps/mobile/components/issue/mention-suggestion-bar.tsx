/**
 * Подсказки для @упоминаний над полем ввода. Два режима: comment
 * (по умолчанию, композер комментария и описание новой задачи) и chat
 * (композер чата). В comment — секции @all, участники, агенты, отряды.
 * В chat упоминание ссылается на ресурс для агента: недавние задачи и
 * мои задачи. Закрытые задачи показываются приглушённо, но остаются
 * кликабельными.
 */
...
import { canAssignAgentToIssue } from '@goosar/core/permissions';
...
export function MentionSuggestionBar({ visible, query, onSelect, mode = 'comment' }: Props) {
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    if (isChat) {
      const issueMatches = (i: Issue) =>
        !q || i.identifier.toLowerCase().includes(q) || i.title.toLowerCase().includes(q);
  ...
  return (
    <View className="bg-background border-b border-border" style={{ maxHeight: 220 }}>
      <FlatList
        data={rows}
        ...
        renderItem={({ item, index }) => {
          if (item.kind === 'section') {
            return (
              <View className={cn('px-3 pt-2 pb-1', index > 0 && 'border-t border-border/60 mt-1')}>
          ...
          if (item.kind === 'all') {
            return (
              <Pressable
                onPress={() => onSelect({ type: 'all', id: 'all', name: 'all' })}
              ...
              >
                ...
                <Text className="flex-1 text-sm text-foreground">Everyone</Text>
                <Badge label="All" />
              </Pressable>
            );
          }
          if (item.kind === 'member') {
            return (
              ...
              >
                ...
                <Text className="flex-1 text-sm text-foreground">{item.member.name}</Text>
                <Badge label="Member" />
              </Pressable>
            );
          }
          ...
          if (item.kind === 'squad') {
            return (
              ...
              >
                ...
                <Text className="flex-1 text-sm text-foreground">{item.squad.name}</Text>
                <Badge label="Squad" tone="outline" />
              </Pressable>
            );
          }
          const closed = item.issue.status === 'done' || item.issue.status === 'cancelled';
          return (
            <Pressable
              onPress={() =>
                onSelect({
            ...
            >
              ...
              <Text className="text-sm font-medium text-foreground">{item.issue.identifier}</Text>
...
function Badge({ label, tone = 'muted' }: { label: string; tone?: 'muted' | 'brand' | 'outline' }) {
  return (
    <View
      className={cn(
