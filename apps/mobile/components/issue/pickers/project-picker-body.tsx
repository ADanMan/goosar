/**
 * Тело пикера проекта задачи — одиночный выбор. Заголовок и поиск —
 * нативный iOS-заголовок навигации, тело — простой FlatList без обвязки.
 */
...
export function ProjectPickerBody({ value, query, onChange }: Props) {
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    const selected = projectRows.find((r) => r.kind === 'project' && r.project.id === value?.id);
    return [
      { kind: 'none' },
      ...(selected ? [selected] : []),
      ...projectRows.filter((r) => !(r.kind === 'project' && r.project.id === value?.id)),
    ];
  ...
  return (
    <FlatList
      ...
      keyExtractor={(row) => (row.kind === 'none' ? 'none' : `p:${row.project.id}`)}
      renderItem={({ item }) => (
        <Pressable
          onPress={() => (item.kind === 'none' ? onChange(null) : onChange(item.project))}
          className="flex-row items-center gap-3 px-4 py-3 active:bg-secondary"
        >
          {item.kind === 'none' ? (
            <Ionicons name="close-circle-outline" size={28} color={MOBILE_PLACEHOLDER_COLOR} />
          ) : (
      ...
      ListEmptyComponent={
        <View className="px-3 py-8 items-center">
          <Text className="text-sm text-muted-foreground text-center">
            {query ? 'No matches.' : 'No projects in this workspace yet.\nCreate them on web.'}
          </Text>
        </View>
      }
    />
