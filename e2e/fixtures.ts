// TestApiClient — лёгкий API-помощник для подготовки и очистки данных E2E-тестов
// на чистом fetch.
...
export class TestApiClient {
  ...
  async login(email: string, name: string) {
    ...
    try {
      ...
      const configuredDevCode = process.env.GOOSAR_DEV_VERIFICATION_CODE?.trim();
  ...
  async cleanup() {
    if (this.seededIssueIds.length > 0 && this.workspaceId) {
      ...
      await client.connect();
      try {
        await client.query(`DELETE FROM issue WHERE workspace_id = $1 AND id = ANY($2::uuid[])`, [
          this.workspaceId,
          this.seededIssueIds,
        ]);
      } finally {
