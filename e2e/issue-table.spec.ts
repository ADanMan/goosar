...
test.describe('Issue Table server grouping', () => {
  ...
  test('groups 1,001 issues exactly without materializing the full result', async ({ page }) => {
    const run = Date.now();
    ...
    const groupsResponsePromise = page.waitForResponse(
      (response) =>
        apiPath(response.request()) === '/api/issues/table/groups' && response.status() === 200,
    );
    ...
    expect(Object.fromEntries(groups.groups.map((group) => [group.key, group.count]))).toEqual({
      'status:backlog': 501,
      'status:todo': 300,
      'status:done': 200,
    });

    const backlogGroup = page.getByRole('row').filter({ hasText: 'Backlog' }).first();
    await expect(backlogGroup).toContainText('501');
    await expect(page.getByText(/Grouping and hierarchy are paused/)).toHaveCount(0);
  ...
  test('keeps same-group children nested and cross-group children at the group root', async ({
    ...
    const todoRootPromise = page.waitForResponse((response) => {
      ...
      return (
        body.group_key === 'status:todo' && body.parent_id === null && response.status() === 200
      );
    });
    ...
    const doneRootPromise = page.waitForResponse((response) => {
      ...
      const body = tableBody(response.request());
      return (
        body.group_key === 'status:done' && body.parent_id === null && response.status() === 200
      );
    });

    await groupByStatus(page);
    const [todoRootResponse, todoChildrenResponse, doneRootResponse] = await Promise.all([
      todoRootPromise,
      todoChildrenPromise,
      doneRootPromise,
    ]);
    const todoRoot = (await todoRootResponse.json()) as TableRowsResponse;
    const todoChildren = (await todoChildrenResponse.json()) as TableRowsResponse;
    ...
    expect(todoChildren.rows.map((row) => row.issue.title)).toEqual([sameGroupTitle]);
    expect(doneRoot.rows.map((row) => row.issue.title)).toEqual([crossGroupTitle]);

    const sameGroupRow = page.getByRole('row').filter({ hasText: sameGroupTitle });
    const crossGroupRow = page.getByRole('row').filter({ hasText: crossGroupTitle });
  ...
  test('drops stale branch cursors after a realtime sort-boundary update', async ({ page }) => {
    const run = Date.now();
    ...
    await expect(page.getByRole('button', { name: moved.title }).first()).toBeVisible();
    ...
    const collectResponse = async (response: Response) => {
      if (apiPath(response.request()) !== '/api/issues/table/rows' || response.status() !== 200) {
        return;
      }
      ...
      .poll(
        () =>
          postUpdateResponses.find(
            ({ body }) => body.page?.cursor === null || body.page?.cursor === undefined,
          )?.payload.next_cursor,
        { timeout: 10000 },
      )
      .not.toBeUndefined();
    ...
    await expect
      .poll(
        () => postUpdateResponses.find(({ body }) => body.page?.cursor === freshCursor)?.payload,
        { timeout: 10000 },
      )
      .not.toBeUndefined();
    ...
    const refreshedIds = [...(freshHead?.rows ?? []), ...(freshTail?.rows ?? [])].map(
      (row) => row.issue.id,
    );
  ...
  test('shows a branch-level retry and never falls back to client grouping', async ({ page }) => {
    ...
    await page.route('**/api/issues/table/rows', async (route) => {
      ...
      if (body.group?.kind === 'status' && body.group_key === 'status:todo' && !allowRows) {
        forcedFailures += 1;
        await route.fulfill({
          status: 500,
