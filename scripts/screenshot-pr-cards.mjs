...
const DB =
  ...
  'postgres://goosar:goosar@localhost:5432/goosar_goosar_101?sslmode=disable';
...
async function main() {
  ...
  await page.evaluate((t) => localStorage.setItem('goosar_token', t), token);
  ...
  for (const n of ISSUES) {
    ...
    const closeChat = page
      .locator('[aria-label="Minimize" i], [aria-label="Close" i], button[title="Minimize" i]')
      .first();
