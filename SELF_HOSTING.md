...
| `GOOSAR_UPLOAD_GC_GRACE` | `168h` (7 дней) | Сколько вложение, не привязанное ни к issue, ни к комментарию, ни к сообщению чата, остаётся нетронутым, прежде чем уборщик удалит строку и сохранённый объект. Композер сохраняет байты до появления строки-владельца, а черновики в браузере не истекают, поэтому окно должно переживать возврат к черновику после долгих выходных. `0` отключает уборку. |
...
| `GOOSAR_RETENTION_CLOSED_ISSUES` | `0` | Возраст, с которого удаляется issue в завершённом состоянии вместе с комментариями, реакциями, метками, элементами входящих и активностью |
...
3. Существующий аккаунт сопоставляется **сначала по subject каталога, потом по адресу**, поэтому переименование ящика человека сохраняет его аккаунт, issue и членства, а не заводит незаметно второй аккаунт.
...
| --- | --- | --- |
| PostgreSQL (том `pgdata`) | рабочие пространства, пользователи, issue, комментарии, агенты, сессии, зашифрованные учётные данные интеграций | `db.dump` (`pg_dump -Fc`) |
...
> ```
>
> **Окно потери данных — всё, что записано между копией и откатом:** каждые issue, комментарий, задача, вложение и настройка, созданные или изменённые после запуска `scripts/backup.sh`. Частичного пути нет: откат через миграцию — это восстановление. Поэтому копия обязательна и должна сниматься непосредственно перед обновлением, а не по ночному расписанию.
>
