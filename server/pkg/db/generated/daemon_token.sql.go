...
const deleteDaemonTokensByRuntimeOwner = `-- name: DeleteDaemonTokensByRuntimeOwner :many
...
func (q *Queries) DeleteDaemonTokensByRuntimeOwner(ctx context.Context, ownerID pgtype.UUID) ([]string, error) {
	rows, err := q.db.Query(ctx, deleteDaemonTokensByRuntimeOwner, ownerID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
