...
VALUES ($1, $2, $3)
RETURNING id, workspace_id, user_id, role, created_at, perimeter_access
`
...
SELECT id, workspace_id, user_id, role, created_at, perimeter_access FROM member
WHERE id = $1
`
...
const updateMemberPerimeterAccess = `-- name: UpdateMemberPerimeterAccess :one
UPDATE member SET perimeter_access = $2
WHERE id = $1
RETURNING id, workspace_id, user_id, role, created_at, perimeter_access
`

type UpdateMemberPerimeterAccessParams struct {
	ID              pgtype.UUID `json:"id"`
	PerimeterAccess bool        `json:"perimeter_access"`
}

func (q *Queries) UpdateMemberPerimeterAccess(ctx context.Context, arg UpdateMemberPerimeterAccessParams) (Member, error) {
	row := q.db.QueryRow(ctx, updateMemberPerimeterAccess, arg.ID, arg.PerimeterAccess)
	var i Member
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
