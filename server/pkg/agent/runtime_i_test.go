...
func TestNewReturnsRuntimeIBackend(t *testing.T) {
	t.Parallel()
	b, err := New("runtime-i", Config{ExecutablePath: "/nonexistent/grok"})
	if err != nil {
		t.Fatalf("New(grok) error: %v", err)
	}
	if _, ok := b.(*runtimeIBackend); !ok {
		t.Fatalf("expected *grokBackend, got %T", b)
	}
}

func fakeRuntimeIACPScript() string {
	return `#!/bin/sh
seen_stdio=
for arg in "$@"; do
...
func TestRuntimeIBackendStreamsAndCompletes(t *testing.T) {
	t.Parallel()
	fakePath := filepath.Join(t.TempDir(), "grok")
	writeTestExecutable(t, fakePath, []byte(fakeRuntimeIACPScript()))

	backend, err := New("runtime-i", Config{ExecutablePath: fakePath, Logger: slog.Default()})
	if err != nil {
...
func TestRuntimeIBlockedArgsFiltering(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	...
	backend, err := New("runtime-i", Config{
...
func TestRuntimeISetModelFailureFailsTask(t *testing.T) {
	t.Parallel()
	fakePath := filepath.Join(t.TempDir(), "grok")
	writeTestExecutable(t, fakePath, []byte(fakeRuntimeIACPScript()))
...
func TestRuntimeIUsesSessionLoadForResume(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
...
func TestRuntimeIAuthenticatesBeforeSession(t *testing.T) {
	t.Parallel()
...
func TestRuntimeIAuthFailureFailsTask(t *testing.T) {
	t.Parallel()
	fakePath := filepath.Join(t.TempDir(), "grok")
	writeTestExecutable(t, fakePath, []byte(fakeRuntimeIACPScript()))
...
func TestRuntimeINoUsableAuthMethodFailsBeforeSession(t *testing.T) {
...
func TestRuntimeIUsesAdvertisedAPIKeyMethod(t *testing.T) {
	tempDir := t.TempDir()
	requestsFile := filepath.Join(tempDir, "requests.jsonl")
	fakePath := filepath.Join(tempDir, "grok")
	writeTestExecutable(t, fakePath, []byte(fakeRuntimeIACPScript()))
...
func TestRuntimeIDrainsNotificationsAfterPromptResponse(t *testing.T) {
	fakePath := filepath.Join(t.TempDir(), "grok")
	writeTestExecutable(t, fakePath, []byte(fakeRuntimeIACPScript()))
	backend, err := New("runtime-i", Config{
		ExecutablePath: fakePath,
...
func TestRuntimeIPropagatesMCPAndUsage(t *testing.T) {
	tempDir := t.TempDir()
	requestsFile := filepath.Join(tempDir, "requests.jsonl")
	fakePath := filepath.Join(tempDir, "grok")
	writeTestExecutable(t, fakePath, []byte(fakeRuntimeIACPScript()))
...
func TestRuntimeIAttributesUsageOnResumeWithoutConfiguredModel(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	fakePath := filepath.Join(tempDir, "grok")
	writeTestExecutable(t, fakePath, []byte(fakeRuntimeIACPScript()))
...
func TestRuntimeITimeoutAndCancellation(t *testing.T) {
	for _, tc := range []struct {
		name       string
...
func TestDiscoverRuntimeIModelsWaitsForAdvertisedAuth(t *testing.T) {
	tempDir := t.TempDir()
	requestsFile := filepath.Join(tempDir, "requests.jsonl")
	fakePath := filepath.Join(tempDir, "grok")
	writeTestExecutable(t, fakePath, []byte(fakeRuntimeIACPScript()))
	...
	models, err := discoverRuntimeIModels(context.Background(), fakePath)
	if err != nil {
...
func TestDiscoverRuntimeIModelsStopsOnAuthFailures(t *testing.T) {
	for _, tc := range []struct {
		name     string
		methods  string
		authFail string
...
func TestRuntimeIThinkingCatalogIsPerModel(t *testing.T) {
	models := staticModelsForRuntime("runtime-i")
...
func TestRuntimeIValidateThinkingLevelUsesPerModelCatalog(t *testing.T) {
	for _, tc := range []struct {
		...
		got, err := ValidateThinkingLevel(context.Background(), "runtime-i", "/nonexistent/grok", tc.model, tc.level)
		if err != nil {
...
func TestRuntimeISelectAuthMethod(t *testing.T) {
	t.Parallel()
	cases := []struct {
		name    string
		methods []string
	...
	for _, tc := range cases {
		got, err := selectRuntimeIAuthMethod(tc.methods, tc.haveKey)
...
func TestRuntimeIIsKnownThinkingValue(t *testing.T) {
	t.Parallel()
	for _, level := range []string{"", "low", "medium", "high"} {
		if !IsKnownThinkingValue("runtime-i", level) {
