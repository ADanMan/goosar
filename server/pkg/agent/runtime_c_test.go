...
func TestRuntimeCHandleAssistantText(t *testing.T) {
	t.Parallel()

	b := &runtimeCBackend{cfg: Config{Logger: slog.Default()}}
	ch := make(chan Message, 10)

	сообщение := runtimeCSDKMessage{
		Type: "assistant",
		Message: mustMarshal(t, runtimeCMessageContent{
			Role: "assistant",
			Content: []runtimeCContentBlock{
				{Type: "text", Text: "Hello world"},
			},
		}),
	}

	output, tools := b.handleAssistant(сообщение, ch, make(map[string]TokenUsage))
...
func TestRuntimeCHandleAssistantToolUse(t *testing.T) {
	t.Parallel()
...
func TestRuntimeCHandleUserToolResult(t *testing.T) {
	t.Parallel()
...
func TestRuntimeCHandleControlRequestAutoApproves(t *testing.T) {
	t.Parallel()
	...
	сообщение := runtimeCSDKMessage{
		...
		RequestID: "req-42",
		Request: mustMarshal(t, runtimeCControlRequestPayload{
			Subtype:  "tool_use",
			ToolName: "Bash",
	...
	b.handleControlRequest(сообщение, &written)
...
func TestRuntimeCHandleControlRequestForcesBackgroundToolsForeground(t *testing.T) {
	t.Parallel()
...
func TestRuntimeCHandleUserDetectsAsyncLaunchedToolResult(t *testing.T) {
	t.Parallel()
...
func TestRuntimeCHandleUserIgnoresAsyncLaunchedTextOutput(t *testing.T) {
	t.Parallel()
...
func TestRuntimeCHandleAssistantInvalidJSON(t *testing.T) {
	t.Parallel()
...
func TestBuildRuntimeCArgsInheritsMCPByDefault(t *testing.T) {
	t.Parallel()

	args := buildRuntimeCArgs(ExecOptions{}, slog.Default())
	expected := []string{
		"-p",
...
func TestBuildRuntimeCArgsUsesStrictMCPForManagedConfig(t *testing.T) {
	t.Parallel()

	args := buildRuntimeCArgs(ExecOptions{McpConfig: json.RawMessage(`{}`)}, slog.Default())
...
func TestBuildRuntimeCArgsPassesThroughCustomArgs(t *testing.T) {
	t.Parallel()
...
func TestBuildRuntimeCArgsFiltersBlockedCustomArgs(t *testing.T) {
	t.Parallel()
...
func TestBuildRuntimeCInputEncodesUserMessage(t *testing.T) {
	t.Parallel()

	data, err := buildRuntimeCInput("say pong")
	if err != nil {
	...
	сообщение, ok := payload["message"].(map[string]any)
	if !ok {
		t.Fatalf("expected message object, got %T", payload["message"])
	}
	if сообщение["role"] != "user" {
		t.Fatalf("expected role user, got %v", сообщение["role"])
	}

	content, ok := сообщение["content"].([]any)
	if !ok || len(content) != 1 {
		t.Fatalf("expected one content block, got %v", сообщение["content"])
	}
...
func TestMergeEnvDeduplicatesKeysLastWins(t *testing.T) {
	t.Parallel()
	...
	for key, count := range seen {
		if count > 1 {
			t.Fatalf("duplicate env key %q (%d entries): %v", key, count, env)
		}
	}

	if seen["http_proxy"] != 1 || seen["HTTPS_PROXY"] != 1 {
		t.Fatalf("expected both spellings to survive as distinct keys: %v", env)
	}
	for _, entry := range env {
		if strings.HasPrefix(entry, "HTTPS_PROXY=") &&
			entry != "HTTPS_PROXY=http://127.0.0.1:3128" {
			t.Fatalf("extra layer must win the HTTPS_PROXY slot, got %q", entry)
		}
	}
}

func TestMergeEnvFiltersRuntimeCCodeVars(t *testing.T) {
	t.Parallel()
	...
	env := mergeEnv([]string{
		...
		"GOOSAR_SERVER_URL=https://daemon.example",
	}, map[string]string{
		"FOO":               "bar",
	...
	if !found["GOOSAR_SERVER_URL=https://task.example"] || !found["GOOSAR_TOKEN=mat_task"] {
		t.Fatalf("expected explicit task GOOSAR_* values to be appended, got %v", env)
	}
}
...
func TestRuntimeCRootSudoPreflight(t *testing.T) {
	t.Parallel()

	t.Run("sandbox bypass allowed", func(t *testing.T) {
		t.Parallel()

		err := runtimeCRootSudoPreflight(
...
func TestBuildRuntimeCArgsBlocksMcpConfig(t *testing.T) {
	t.Parallel()
...
func TestWriteMcpConfigToTemp(t *testing.T) {
	...
	path, err := writeMcpConfigToTemp(raw, slog.Default())
	if err != nil {
...
func TestWriteMcpConfigToTempFiltersDisabledEntries(t *testing.T) {
	t.Parallel()

	raw := json.RawMessage(`{"mcpServers":{"on":{"command":"echo"},"off":{"command":"mcp-atlassian","enabled":false}}}`)
	path, err := writeMcpConfigToTemp(raw, slog.Default())
	if err != nil {
		t.Fatalf("writeMcpConfigToTemp: %v", err)
	}
	defer cleanupMcpConfigTemp(path)
	...
	if _, kept := document.McpServers["off"]; kept {
		t.Fatalf("disabled entry leaked into the temp config: %s", data)
	}
	if _, kept := document.McpServers["on"]; !kept {
		t.Fatalf("enabled entry missing from the temp config: %s", data)
	}
}
...
func TestRuntimeCExecuteSurfacesStderrWhenChildExitsEarly(t *testing.T) {
	t.Parallel()
	if runtime.GOOS == "windows" {
...
func TestRuntimeCExecuteReportsResumeRejectedOnAccountBinding(t *testing.T) {
	t.Parallel()
	if runtime.GOOS == "windows" {
		t.Skip("shell-script fixture is POSIX-only")
	}
...
func TestRuntimeCExecuteKeepsSessionOnNetworkFailure(t *testing.T) {
	t.Parallel()
	if runtime.GOOS == "windows" {
		t.Skip("shell-script fixture is POSIX-only")
	}
...
func TestRuntimeCExecuteRecordsResultModelUsage(t *testing.T) {
	t.Parallel()
	if runtime.GOOS == "windows" {
		t.Skip("shell-script fixture is POSIX-only")
	}
...
func TestBuildRuntimeCArgsExtraArgsBeforeCustomArgsAndFiltersBoth(t *testing.T) {
	args := buildRuntimeCArgs(ExecOptions{
...
func TestBuildRuntimeCArgsManagedSkillSettingsWins(t *testing.T) {
	args := buildRuntimeCArgs(ExecOptions{
		RuntimeCSettingsPath: "/tmp/goosar-claude-settings.json",
	...
	if !strings.Contains(joined, "--settings /tmp/goosar-claude-settings.json") {
...
func TestBuildRuntimeCArgsSettingsIsAlwaysBlocked(t *testing.T) {
	args := buildRuntimeCArgs(ExecOptions{
		CustomArgs: []string{"--settings", "/tmp/agent.json", "--max-turns", "7"},
	}, slog.Default())
	joined := strings.Join(args, " ")
	if strings.Contains(joined, "--settings") {
		t.Fatalf("--settings from custom_args must be dropped: %v", args)
	}
	if !strings.Contains(joined, "--max-turns 7") {
		t.Fatalf("unrelated custom arg was dropped: %v", args)
	}
