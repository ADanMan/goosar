...
func TestNewReturnsRuntimeQBackend(t *testing.T) {
	t.Parallel()
	backend, err := New("runtime-q", Config{ExecutablePath: "/nonexistent/qwen"})
	if err != nil {
		t.Fatalf("New(qwen): %v", err)
	}
	if _, ok := backend.(*runtimeQBackend); !ok {
		t.Fatalf("New(qwen) = %T, want *qwenBackend", backend)
	}
}

func TestBuildRuntimeQArgsKeepsProtocolManaged(t *testing.T) {
	t.Parallel()
	args := buildRuntimeQArgs("task prompt", ExecOptions{
...
func TestBuildRuntimeQArgsYoloAlwaysPresent(t *testing.T) {
	t.Parallel()

	args := buildRuntimeQArgs("task", ExecOptions{}, slog.Default())
	if !strings.Contains(strings.Join(args, " "), "--yolo") {
		t.Fatalf("--yolo missing from base args %v", args)
	}
}

func fakeRuntimeQScript() string {
	return `#!/bin/sh
...
func newFakeRuntimeQBackend(t *testing.T, env map[string]string) *runtimeQBackend {
	t.Helper()
	if runtime.GOOS == "windows" {
		t.Skip("shell fixture is POSIX-only")
	}
	path := filepath.Join(t.TempDir(), "qwen")
	writeTestExecutable(t, path, []byte(fakeRuntimeQScript()))
	return &runtimeQBackend{cfg: Config{ExecutablePath: path, Logger: slog.Default(), Env: env}}
}

func awaitRuntimeQResult(t *testing.T, session *Session) ([]Message, Result) {
	t.Helper()
...
func TestRuntimeQBackendStreamsNativeEvents(t *testing.T) {
	t.Parallel()
	backend := newFakeRuntimeQBackend(t, nil)
	session, err := backend.Execute(context.Background(), "reply PONG", ExecOptions{Model: "qwen-test", Timeout: 5 * time.Second})
	if err != nil {
		t.Fatalf("Execute: %v", err)
	}
	messages, result := awaitRuntimeQResult(t, session)
	...
	for _, сообщение := range messages {
		...
		case MessageThinking:
			thinking = сообщение.Content == "considering"
		case MessageToolUse:
			toolUse = сообщение.Tool == "list_directory" && сообщение.CallID == "call-1" && сообщение.Input["path"] == "/work"
		case MessageToolResult:
			toolResult = сообщение.CallID == "call-1" && сообщение.Output == "Listed 1 item"
		case MessageText:
			text = сообщение.Content == "PONG"
		}
	}
...
func TestRuntimeQBackendPreservesSuccessfulResumeSession(t *testing.T) {
	t.Parallel()
	backend := newFakeRuntimeQBackend(t, nil)
	...
	_, result := awaitRuntimeQResult(t, session)
	if result.Status != "completed" || result.Output != "PONG" || result.SessionID != "sess-qwen-1" {
		t.Fatalf("resumed result = %+v", result)
	}
}

func newRuntimeQTestContext() (context.Context, context.CancelFunc) {
	return context.WithCancel(context.Background())
}

func TestRuntimeQBackendFailureTimeoutAndCancellation(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("shell fixture is POSIX-only")
	}
	resumeFixture, err := filepath.Abs(filepath.Join("testdata", "runtime-q-0.20.0-resume-not-found.stderr.txt"))
	if err != nil {
	...
	for _, tc := range []struct {
		...
		{"result error", map[string]string{"QWEN_MODE": "error"}, newRuntimeQTestContext, ExecOptions{}, "failed", "synthetic Qwen authentication failure", "sess-error", false},
		{"process error", map[string]string{"QWEN_MODE": "exit"}, newRuntimeQTestContext, ExecOptions{}, "failed", "synthetic qwen stderr", "", false},
		{"missing resume", map[string]string{"QWEN_MODE": "resume-missing", "QWEN_STDERR_FIXTURE": resumeFixture}, newRuntimeQTestContext, ExecOptions{ResumeSessionID: "session-redacted"}, "failed", "No saved session found", "", true},
		{"timeout", map[string]string{"QWEN_MODE": "spin"}, newRuntimeQTestContext, ExecOptions{Timeout: 20 * time.Millisecond}, "timeout", "timed out", "", false},
		{"cancel", map[string]string{"QWEN_MODE": "spin"}, newRuntimeQTestContext, ExecOptions{}, "aborted", "cancelled", "", false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			session, err := newFakeRuntimeQBackend(t, tc.env).Execute(ctx, "task", tc.opts)
			if err != nil {
				t.Fatalf("Execute: %v", err)
			}
			_, result := awaitRuntimeQResult(t, session)
...
func TestRuntimeQBackendPassesManagedMCPThroughTempFile(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("shell fixture is POSIX-only")
	}
	...
	backend := newFakeRuntimeQBackend(t, map[string]string{"QWEN_ARGS_FILE": argsPath, "QWEN_MCP_CAPTURE_FILE": capturePath})
...
func TestRuntimeQCode020FixtureParses(t *testing.T) {
	t.Parallel()
	data, err := os.ReadFile(filepath.Join("testdata", "runtime-q-0.20.0-stream-json.jsonl"))
	if err != nil {
		t.Fatal(err)
	}
	state := runtimeQStreamState{usage: make(map[string]TokenUsage)}
	...
	for _, line := range strings.Split(strings.TrimSpace(string(data)), "\n") {
		...
		handleRuntimeQEvent(event, messages, &state)
	}
...
func TestRuntimeQCode020ErrorFixturePreservesErrorMessage(t *testing.T) {
	t.Parallel()
	data, err := os.ReadFile(filepath.Join("testdata", "runtime-q-0.20.0-error-stream-json.jsonl"))
	if err != nil {
		t.Fatal(err)
	}
	state := runtimeQStreamState{usage: make(map[string]TokenUsage)}
