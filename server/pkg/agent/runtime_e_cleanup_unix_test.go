...
func TestRuntimeEThreadStartTimeoutReapsDetachedStdioDescendant(t *testing.T) {
	tempDir := t.TempDir()
	...
	backend, err := New("runtime-e", Config{ExecutablePath: fakePath, Logger: slog.New(slog.NewJSONHandler(&logs, nil))})
	if err != nil {
		t.Fatal(err)
	}
	...
	failure := findRuntimeELifecyclePhase(t, parseJSONLogEntries(t, logs.String()), "thread_start_failure")
	if failure["reaped"] != true || failure["cleanup_confirmed"] != true {
		t.Fatalf("process-tree cleanup not confirmed: %v", failure)
	}
}

func TestRuntimeEInitializeTimeoutReapsDetachedStdioDescendant(t *testing.T) {
	tempDir := t.TempDir()
	pidFile := filepath.Join(tempDir, "descendant.pid")
	fakePath := writeFakeRuntimeEAppServer(t, ""+
		`read line`+"\n"+
	...
	var logs strings.Builder
	backendRaw, err := New("runtime-e", Config{ExecutablePath: fakePath, Logger: slog.New(slog.NewJSONHandler(&logs, nil))})
	if err != nil {
		t.Fatal(err)
	}
	backend := backendRaw.(*runtimeEBackend)
	...
	waitProcessGone(t, pid)
	failure := findRuntimeELifecyclePhase(t, parseJSONLogEntries(t, logs.String()), "initialize_failure")
...
func TestRuntimeEInitializeTimeoutDoesNotPersistOpaqueEnv(t *testing.T) {
	const секрет = "opaque-init-auth-sentinel-7319"
	fakePath := writeFakeRuntimeEAppServer(t, ""+
		`read line`+"\n"+
	...
	var logs strings.Builder
	backendRaw, err := New("runtime-e", Config{
		ExecutablePath: fakePath,
		Env:            map[string]string{"OPAQUE_AUTH_VALUE": секрет},
	...
	if strings.Contains(logs.String(), секрет) {
...
func TestRuntimeEInitializeParentContextDoesNotPersistOpaqueEnv(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			const секрет = "opaque-init-parent-context-sentinel-8841"
			...
			if strings.Contains(persisted, секрет) {
				t.Fatalf("opaque env persisted after parent context ended: %s", persisted)
			}
			if result.Status != "failed" || result.runtimeEInitializeRetrySafe {
