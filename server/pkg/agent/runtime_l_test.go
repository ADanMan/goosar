...
func TestNewReturnsRuntimeLBackend(t *testing.T) {
	t.Parallel()
	b, err := New("runtime-l", Config{ExecutablePath: "/nonexistent/kiro-cli"})
	if err != nil {
		t.Fatalf("New(kiro) error: %v", err)
	}
	if _, ok := b.(*runtimeLBackend); !ok {
		t.Fatalf("expected *kiroBackend, got %T", b)
	}
}

func TestRuntimeLToolNameFromTitle(t *testing.T) {
	t.Parallel()
	tests := []struct {
		title string
		want  string
	...
	for _, tt := range tests {
		got := runtimeLToolNameFromTitle(tt.title)
		if got != tt.want {
...
func fakeRuntimeLACPScript() string {
	return `#!/bin/sh
...
func TestRuntimeLBackendSetModelFailureFailsTask(t *testing.T) {
	t.Parallel()

	fakePath := filepath.Join(t.TempDir(), "kiro-cli")
	writeTestExecutable(t, fakePath, []byte(fakeRuntimeLACPScript()))

	backend, err := New("runtime-l", Config{ExecutablePath: fakePath, Logger: slog.Default()})
	if err != nil {
...
func TestRuntimeLBackendAttributesUsageToCurrentModel(t *testing.T) {
	t.Parallel()
...
func fakeRuntimeLACPGoalCompleteCloseErrorScript(goalStatus string) string {
	return `#!/bin/sh
while IFS= read -r line; do
  id=$(printf '%s' "$line" | sed -n 's/.*"id":\([0-9]*\).*/\1/p')
  case "$line" in
...
func TestRuntimeLBackendTreatsGoalCompleteCloseErrorAsCompleted(t *testing.T) {
	t.Parallel()

	fakePath := filepath.Join(t.TempDir(), "kiro-cli")
	writeTestExecutable(t, fakePath, []byte(fakeRuntimeLACPGoalCompleteCloseErrorScript("completed")))
...
func TestRuntimeLBackendDoesNotCompleteAfterFailedGoalComplete(t *testing.T) {
	t.Parallel()

	fakePath := filepath.Join(t.TempDir(), "kiro-cli")
	writeTestExecutable(t, fakePath, []byte(fakeRuntimeLACPGoalCompleteCloseErrorScript("failed")))
...
func fakeRuntimeLACPIssueCommentCloseErrorScript() string {
	return `#!/bin/sh
while IFS= read -r line; do
  id=$(printf '%s' "$line" | sed -n 's/.*"id":\([0-9]*\).*/\1/p')
  case "$line" in
    ...
    *'"method":"session/new"'*)
      ...
      ;;
    *'"method":"session/prompt"'*)
      printf '{"jsonrpc":"2.0","method":"session/notification","params":{"sessionId":"ses_comment_done","update":{"type":"ToolCall","toolCallId":"tc-comment","name":"Shell","status":"pending","parameters":{"command":"goosar issue comment add issue-1 --content-file ./reply.md"}}}}\n'
...
func TestRuntimeLBackendTreatsCommentAddCloseErrorAsCompleted(t *testing.T) {
	t.Parallel()

	fakePath := filepath.Join(t.TempDir(), "kiro-cli")
	writeTestExecutable(t, fakePath, []byte(fakeRuntimeLACPIssueCommentCloseErrorScript()))
...
func TestRuntimeLIssueCommentAddCommand(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{"goosar issue comment add issue-1 --content-file ./reply.md", true},
		{"./goosar issue comment add issue-1 --content-file ./reply.md", true},
		{"/usr/local/bin/goosar issue comment add issue-1 --content-file ./reply.md", true},
		{"GOOSAR_TOKEN=x goosar issue comment add issue-1 --content-file ./reply.md", true},
		{"FOO=1 BAR=2 ./goosar issue comment add issue-1", true},
		{`sh -c "goosar issue comment add issue-1 --content-file ./reply.md"`, true},
		{`bash -c 'goosar issue comment add issue-1'`, true},
		{`/bin/sh -c "goosar issue comment add issue-1"`, true},
		{"goosar issue get issue-1", false},
		{"echo goosar issue comment add issue-1", false},
		{`sh -c "echo goosar issue comment add issue-1"`, false},
		{"FOO=bar", false},
		{"", false},
	}
	for _, tt := range tests {
		if got := isRuntimeLIssueCommentAddCommand(tt.command); got != tt.want {
			t.Errorf("isKiroIssueCommentAddCommand(%q) = %v, want %v", tt.command, got, tt.want)
		}
	}
}

func TestRuntimeLIssueCommentAddToolIgnoresToolTitle(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		сообщение := Message{
			Type:  MessageToolUse,
			Tool:  tt.tool,
			Input: map[string]any{"command": "goosar issue comment add issue-1 --content-file ./reply.md"},
		}
		if got := isRuntimeLIssueCommentAddTool(сообщение); got != tt.want {
	...
	if isRuntimeLIssueCommentAddTool(notComment) {
		t.Errorf("isKiroIssueCommentAddTool should reject a non-comment-add command")
	}
}

func runRuntimeLCloseErrorScript(t *testing.T, script string) Result {
	t.Helper()
	fakePath := filepath.Join(t.TempDir(), "kiro-cli")
...
func fakeRuntimeLACPRunningToolCloseErrorScript(toolName, command string) string {
	params := `{}`
	if command != "" {
...
func TestRuntimeLBackendDoesNotCompleteRunningCommentAddWithoutResult(t *testing.T) {
	t.Parallel()

	result := runRuntimeLCloseErrorScript(t, fakeRuntimeLACPRunningToolCloseErrorScript(
		"execute_bash",
		"goosar issue comment add issue-1 --content-file ./reply.md",
	))
	if result.Status != "failed" {
...
func TestRuntimeLBackendDoesNotCompleteRunningGoalCompleteWithoutResult(t *testing.T) {
	t.Parallel()

	result := runRuntimeLCloseErrorScript(t, fakeRuntimeLACPRunningToolCloseErrorScript("goal_complete", ""))
	if result.Status != "failed" {
		t.Fatalf("expected status=failed for a result-less running goal_complete, got %q (error=%q)", result.Status, result.Error)
	}
}

func fakeRuntimeLACPCompletedToolCloseErrorScript(toolName, command string) string {
	params := `{}`
	if command != "" {
		params = `{"command":"` + command + `"}`
	}
...
func TestRuntimeLBackendTreatsCompletedCommentAddWithNonTerminalTitleAsCompleted(t *testing.T) {
	t.Parallel()

	result := runRuntimeLCloseErrorScript(t, fakeRuntimeLACPCompletedToolCloseErrorScript(
		"execute_bash",
		"goosar issue comment add issue-1 --content-file ./reply.md",
	))
	if result.Status != "completed" {
...
func fakeRuntimeLACPRealGPT56SolCloseErrorScript(command string) string {
	return `#!/bin/sh
while IFS= read -r line; do
  id=$(printf '%s' "$line" | sed -n 's/.*"id":\([0-9]*\).*/\1/p')
  case "$line" in
...
func TestRuntimeLBackendPreservesCompletionOnRealGPT56SolFrames(t *testing.T) {
	t.Parallel()

	result := runRuntimeLCloseErrorScript(t, fakeRuntimeLACPRealGPT56SolCloseErrorScript(
		"goosar issue comment add issue-1 --content-file ./reply.md",
	))
	if result.Status != "completed" {
...
func fakeRuntimeLACPTwoCommentCloseErrorScript(firstStatus, secondStatus string) string {
	return `#!/bin/sh
while IFS= read -r line; do
  id=$(printf '%s' "$line" | sed -n 's/.*"id":\([0-9]*\).*/\1/p')
  case "$line" in
    ...
    *'"method":"session/new"'*)
      ...
      ;;
    *'"method":"session/prompt"'*)
      printf '{"jsonrpc":"2.0","method":"session/notification","params":{"sessionId":"ses_two","update":{"type":"ToolCall","toolCallId":"tc-1","name":"terminal","status":"pending","parameters":{"command":"goosar issue comment add issue-1 --content-file ./progress.md"}}}}\n'
      ...
      printf '{"jsonrpc":"2.0","method":"session/notification","params":{"sessionId":"ses_two","update":{"type":"ToolCall","toolCallId":"tc-2","name":"terminal","status":"pending","parameters":{"command":"goosar issue comment add issue-1 --content-file ./final.md"}}}}\n'
...
func TestRuntimeLBackendFailedFinalCommentOverridesEarlierSuccess(t *testing.T) {
	t.Parallel()

	result := runRuntimeLCloseErrorScript(t, fakeRuntimeLACPTwoCommentCloseErrorScript("completed", "failed"))
	if result.Status != "failed" {
...
func TestRuntimeLBackendCompletedRetryAfterEarlierFailureIsCompleted(t *testing.T) {
	t.Parallel()

	result := runRuntimeLCloseErrorScript(t, fakeRuntimeLACPTwoCommentCloseErrorScript("failed", "completed"))
	if result.Status != "completed" {
...
func fakeRuntimeLACPFailedCommentAddCloseErrorScript() string {
	return `#!/bin/sh
while IFS= read -r line; do
  id=$(printf '%s' "$line" | sed -n 's/.*"id":\([0-9]*\).*/\1/p')
  case "$line" in
    ...
    *'"method":"session/new"'*)
      ...
      ;;
    *'"method":"session/prompt"'*)
      printf '{"jsonrpc":"2.0","method":"session/notification","params":{"sessionId":"ses_failed","update":{"type":"ToolCall","toolCallId":"tc-fail","name":"execute_bash","status":"pending","parameters":{"command":"goosar issue comment add issue-1 --content-file ./reply.md"}}}}\n'
...
func TestRuntimeLBackendDoesNotCompleteAfterFailedCommentAddResult(t *testing.T) {
	t.Parallel()

	result := runRuntimeLCloseErrorScript(t, fakeRuntimeLACPFailedCommentAddCloseErrorScript())
	if result.Status != "failed" {
...
func fakeRuntimeLACPStaleLoadSetModelScript() string {
	return `#!/bin/sh
while IFS= read -r line; do
  id=$(printf '%s' "$line" | sed -n 's/.*"id":\([0-9]*\).*/\1/p')
  case "$line" in
...
func TestRuntimeLBackendClearsSessionIDWhenSetModelSessionNotFound(t *testing.T) {
	t.Parallel()

	fakePath := filepath.Join(t.TempDir(), "kiro-cli")
	writeTestExecutable(t, fakePath, []byte(fakeRuntimeLACPStaleLoadSetModelScript()))
...
func fakeRuntimeLACPStalePromptScript() string {
	return `#!/bin/sh
while IFS= read -r line; do
  id=$(printf '%s' "$line" | sed -n 's/.*"id":\([0-9]*\).*/\1/p')
  case "$line" in
...
func TestRuntimeLBackendClearsSessionIDWhenPromptSessionNotFound(t *testing.T) {
	t.Parallel()

	fakePath := filepath.Join(t.TempDir(), "kiro-cli")
	writeTestExecutable(t, fakePath, []byte(fakeRuntimeLACPStalePromptScript()))
...
func TestRuntimeLBackendInvokesACPWithTrustAllTools(t *testing.T) {
	t.Parallel()
	...
	backend, err := New("runtime-l", Config{
	...
	session, err := backend.Execute(ctx, "prompt-ignored", ExecOptions{
		Model:      "bogus-model",
		Timeout:    5 * time.Second,
		CustomArgs: []string{"acp", "--trust-tools", "shell", "-a", "--agent", "goosar"},
	})
	if err != nil {
		t.Fatalf("execute: %v", err)
	}
	...
	if strings.Join(lines, "\n") != strings.Join([]string{"acp", "--trust-all-tools", "--agent", "goosar"}, "\n") {
		t.Errorf("unexpected argv after filtering: %q", lines)
	}
}

func TestRuntimeLBackendUsesSessionLoadForResume(t *testing.T) {
	t.Parallel()
...
func TestRuntimeLLoadIncludesMcpServersFromConfig(t *testing.T) {
	t.Parallel()
...
func TestIsRuntimeLOversizedHistoryImage(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := isRuntimeLOversizedHistoryImage(tt.err); got != tt.want {
...
func fakeRuntimeLACPOversizedHistoryImageScript() string {
	return `#!/bin/sh
while IFS= read -r line; do
  id=$(printf '%s' "$line" | sed -n 's/.*"id":\([0-9]*\).*/\1/p')
  case "$line" in
...
func runRuntimeLScriptWithOpts(t *testing.T, script string, opts ExecOptions) Result {
	t.Helper()
	fakePath := filepath.Join(t.TempDir(), "kiro-cli")
	writeTestExecutable(t, fakePath, []byte(script))
...
func TestRuntimeLResumedOversizedHistoryImageSignalsResumeRejected(t *testing.T) {
	t.Parallel()

	result := runRuntimeLScriptWithOpts(t, fakeRuntimeLACPOversizedHistoryImageScript(), ExecOptions{
...
func TestRuntimeLFreshOversizedHistoryImageDoesNotSignalResumeRejected(t *testing.T) {
	t.Parallel()
