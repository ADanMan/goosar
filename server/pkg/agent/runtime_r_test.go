...
func TestNewReturnsRuntimeRBackend(t *testing.T) {
	t.Parallel()
	b, err := New("runtime-r", Config{ExecutablePath: "/nonexistent/traecli"})
	if err != nil {
		t.Fatalf("New(traecli) error: %v", err)
	}
	if _, ok := b.(*runtimeRBackend); !ok {
		t.Fatalf("expected *traecliBackend, got %T", b)
	}
}

func fakeRuntimeRACPScript() string {
	return `#!/bin/sh
...
func TestRuntimeRBackendStreamsAndCompletes(t *testing.T) {
	t.Parallel()
	fakePath := filepath.Join(t.TempDir(), "traecli")
	writeTestExecutable(t, fakePath, []byte(fakeRuntimeRACPScript()))

	backend, err := New("runtime-r", Config{ExecutablePath: fakePath, Logger: slog.Default()})
	if err != nil {
...
func TestRuntimeRBlockedArgsFiltering(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	...
	backend, err := New("runtime-r", Config{
...
func TestRuntimeRSetModelFailureFailsTask(t *testing.T) {
	t.Parallel()
	fakePath := filepath.Join(t.TempDir(), "traecli")
	writeTestExecutable(t, fakePath, []byte(fakeRuntimeRACPScript()))
...
func TestRuntimeRUsesSessionLoadForResume(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
