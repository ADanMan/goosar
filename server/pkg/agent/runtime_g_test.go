...
func TestNewReturnsRuntimeGBackend(t *testing.T) {
	t.Parallel()
	b, err := New("runtime-g", Config{ExecutablePath: "/nonexistent/cursor-agent"})
	if err != nil {
		t.Fatalf("New(cursor) error: %v", err)
	}
	if _, ok := b.(*runtimeGBackend); !ok {
		t.Fatalf("expected *cursorBackend, got %T", b)
	}
}

func TestBuildRuntimeGArgs(t *testing.T) {
	t.Parallel()

	args := buildRuntimeGArgs(ExecOptions{
		Cwd:   "/tmp/work",
		Model: "composer-1.5",
	}, slog.Default())
...
func TestBuildRuntimeGArgsWithResume(t *testing.T) {
	t.Parallel()
...
func TestBuildRuntimeGArgsMinimal(t *testing.T) {
	t.Parallel()

	args := buildRuntimeGArgs(ExecOptions{}, slog.Default())
...
func TestBuildRuntimeGArgsIgnoresSystemPromptAndMaxTurns(t *testing.T) {
	t.Parallel()
...
func TestBuildRuntimeGArgsCustomArgs(t *testing.T) {
	t.Parallel()
...
func TestNormalizeRuntimeGStreamLine(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range tests {
		got := normalizeRuntimeGStreamLine(tc.input)
		if got != tc.want {
...
func TestObservedRuntimeGEventTypeIsBoundedAndContentFree(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			if got := observedRuntimeGEventType(tc.value); got != tc.want {
...
func TestRuntimeGHandleAssistantText(t *testing.T) {
	t.Parallel()

	b := &runtimeGBackend{cfg: Config{Logger: slog.Default()}}
	ch := make(chan Message, 10)
	var output strings.Builder

	evt := &runtimeGStreamEvent{
		Type: "assistant",
		Message: mustMarshal(t, runtimeGAssistantMessage{
			Model: "composer-1.5",
			Content: []runtimeGContentBlock{
	...
	b.handleRuntimeGAssistant(evt, ch, &output)
...
func TestRuntimeGHandleAssistantToolUse(t *testing.T) {
	t.Parallel()
...
func TestRuntimeGErrorText(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{"error field", runtimeGStreamEvent{ErrorMsg: "bad request"}, "bad request"},
		{"detail field", runtimeGStreamEvent{Detail: "not found"}, "not found"},
		{"result field", runtimeGStreamEvent{ResultText: "failed"}, "failed"},
		{"empty", runtimeGStreamEvent{}, ""},
	}

	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := runtimeGErrorText(&tc.evt)
			if got != tc.want {
...
func TestRuntimeGUsageModelFallback(t *testing.T) {
	t.Parallel()
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := runtimeGUsageModel(tc.evtModel, tc.configuredModel)
			if got != tc.want {
...
func TestRuntimeGAccumulateResultUsageUsesConfiguredModel(t *testing.T) {
	t.Parallel()
...
func TestRuntimeGAccumulateResultUsage(t *testing.T) {
	t.Parallel()
...
func TestRuntimeGUsageOnlyFromResult(t *testing.T) {
	t.Parallel()
...
func TestRuntimeGStepFinishParsing(t *testing.T) {
	t.Parallel()

	part := runtimeGStepFinishPart{}
...
func TestRuntimeGUsageNoDoubleCount(t *testing.T) {
	t.Parallel()
...
func TestRuntimeGStreamEventUnmarshalTopLevelCamelCase(t *testing.T) {
	t.Parallel()
...
func TestRuntimeGStreamEventUnmarshalNestedCamelCase(t *testing.T) {
	t.Parallel()
...
func TestRuntimeGStreamEventUnmarshalLegacyUsage(t *testing.T) {
	t.Parallel()
...
func TestRuntimeGThinkingStreamForwardsDeltasAndSeparatesBlocks(t *testing.T) {
	t.Parallel()

	var stream runtimeGThinkingStream
...
func TestRuntimeGThinkingStreamDropsEmptyDeltas(t *testing.T) {
	t.Parallel()
...
func TestParseRuntimeGToolCall(t *testing.T) {
	t.Parallel()
...
func TestRuntimeGToolPayloadKeyIsDeterministic(t *testing.T) {
	t.Parallel()
	...
	for i := 0; i < 20; i++ {
		if got := runtimeGToolPayloadKey(envelope); got != "readToolCall" {
			t.Fatalf("iteration %d: key = %q, want readToolCall", i, got)
		}
	}
	if got := runtimeGToolPayloadKey(map[string]json.RawMessage{"toolCallId": json.RawMessage(`"x"`)}); got != "" {
