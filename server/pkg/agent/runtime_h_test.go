...
func TestNewReturnsRuntimeHBackend(t *testing.T) {
	t.Parallel()
	b, err := New("runtime-h", Config{ExecutablePath: "/nonexistent/deveco"})
	if err != nil {
		t.Fatalf("New(deveco) error: %v", err)
	}
	if _, ok := b.(*runtimeHBackend); !ok {
		t.Fatalf("expected *devecoBackend, got %T", b)
	}
}

func fakeRuntimeHScript() string {
	return `#!/bin/sh
...
func TestRuntimeHBackendArgvShapeAndNoPrompt(t *testing.T) {
	t.Parallel()

	tempDir := t.TempDir()
	argsFile := filepath.Join(tempDir, "argv.txt")
	fakePath := filepath.Join(tempDir, "deveco")
	writeTestExecutable(t, fakePath, []byte(fakeRuntimeHScript()))

	workDir := t.TempDir()

	backend, err := New("runtime-h", Config{
...
func TestRuntimeHEventParsingTextFixture(t *testing.T) {
	t.Parallel()
...
func TestRuntimeHEventParsingStepFinishTokensFixture(t *testing.T) {
	t.Parallel()
...
func TestRuntimeHProcessEventsHappyPath(t *testing.T) {
	t.Parallel()

	b := &runtimeHBackend{cfg: Config{Logger: slog.Default()}}
...
func TestRuntimeHProcessEventsErrorCausesFailedStatus(t *testing.T) {
	t.Parallel()
...
func TestRuntimeHHandleErrorEventNilError(t *testing.T) {
	t.Parallel()
	...
	b.handleErrorEvent(runtimeHEvent{Type: "error"}, ch, &status, &errMsg)
...
func runtimeHMainPackageNative(prefix string) string {
	return filepath.Join(prefix, "node_modules", "@deveco", "deveco-code", "bin", "deveco.exe")
}

func TestResolveRuntimeHNativeFromShimPrefersMainPackageCopy(t *testing.T) {
	t.Parallel()

	prefix := filepath.Join("C:\\nvm4w", "nodejs")
	shim := filepath.Join(prefix, "deveco.cmd")
	native := runtimeHMainPackageNative(prefix)

	got := resolveRuntimeHNativeFromShim(shim, fakeStat(native))
	if got != native {
		t.Errorf("got %q, want %q", got, native)
	}
}

func TestResolveRuntimeHNativeFromShimFallsBackToPlatformPackage(t *testing.T) {
	t.Parallel()
	...
	got := resolveRuntimeHNativeFromShim(shim, fakeStat(platform))
	if got != platform {
		t.Errorf("got %q, want %q", got, platform)
	}
}

func TestResolveRuntimeHNativeFromShimFallsBackToBaseline(t *testing.T) {
	t.Parallel()
	...
	got := resolveRuntimeHNativeFromShim(shim, fakeStat(baseline))
	if got != baseline {
		t.Errorf("got %q, want %q", got, baseline)
	}
}

func TestResolveRuntimeHNativeFromShimReturnsEmptyWhenNativeMissing(t *testing.T) {
	t.Parallel()

	shim := filepath.Join("C:\\nvm4w", "nodejs", "deveco.cmd")

	got := resolveRuntimeHNativeFromShim(shim, fakeStat())
	if got != "" {
		t.Errorf("got %q, want empty (missing native binary)", got)
	}
}

func TestResolveRuntimeHNativeFromShimSkipsNonCmdPath(t *testing.T) {
	t.Parallel()
	...
	for _, p := range cases {
		if got := resolveRuntimeHNativeFromShim(p, fakeStat("anything")); got != "" {
			t.Errorf("path %q: got %q, want empty", p, got)
		}
	}
}

func TestResolveRuntimeHNativeFromShimAcceptsUppercaseExtension(t *testing.T) {
	t.Parallel()
