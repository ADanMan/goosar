package agent

import (
	"context"
	"log/slog"
	"os"
	"path/filepath"
	"reflect"
	"runtime"
	"testing"
)

func TestParseRuntimeCEffortHelp_OldFormat(t *testing.T) {
	t.Parallel()

	help := `Usage: claude [options]

Options:
  --model <model>     Model to use
  --effort <level>    Effort level for the current session (low, medium, high, max)
  --verbose
`
	got := parseRuntimeCEffortHelp(help)
	want := []string{"low", "medium", "high", "max"}
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("parseClaudeEffortHelp: got %v, want %v", got, want)
	}
}

func TestParseRuntimeCEffortHelp_NewFormat(t *testing.T) {
	t.Parallel()

	help := `Usage: claude [options]

Options:
  --effort <level>    Effort level for the current session (low, medium, high, xhigh, max)
`
	got := parseRuntimeCEffortHelp(help)
	want := []string{"low", "medium", "high", "xhigh", "max"}
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("parseClaudeEffortHelp: got %v, want %v", got, want)
	}
}

func TestParseRuntimeCEffortHelp_Missing(t *testing.T) {
	t.Parallel()
	help := `Usage: claude [options]

Options:
  --model <model>     Model to use
  --verbose
`
	got := parseRuntimeCEffortHelp(help)
	if got != nil {
		t.Fatalf("parseClaudeEffortHelp: expected nil, got %v", got)
	}
}

func TestRuntimeCEffortLevelsFromHelp_DriftedFormatFallsBackToFullSuperset(t *testing.T) {
	t.Parallel()

	help := `Usage: claude [options]

Options:
  --effort <level>    Choose how hard the model thinks
`
	got := runtimeCEffortLevelsFromHelp(help)
	if !reflect.DeepEqual(got, runtimeCStaticEffortFullSuperset) {
		t.Fatalf("claudeEffortLevelsFromHelp: got %v, want full superset %v", got, runtimeCStaticEffortFullSuperset)
	}
}

func TestRuntimeCEffortLevelsFromHelp_PreEffortCLIReturnsNoLevels(t *testing.T) {
	t.Parallel()

	help := `Usage: claude [options]

Options:
  --model <model>     Model to use
  --verbose
`
	if got := runtimeCEffortLevelsFromHelp(help); got != nil {
		t.Fatalf("claudeEffortLevelsFromHelp: expected nil for pre-effort CLI, got %v", got)
	}
}

func TestProjectRuntimeCLevels_PerModelSubset(t *testing.T) {
	t.Parallel()
	superset := []string{"low", "medium", "high", "xhigh", "max"}

	got := projectRuntimeCLevels(superset, runtimeCModelEffortAllow["claude-sonnet-4-6"])
	values := make([]string, 0, len(got))
	for _, lvl := range got {
		values = append(values, lvl.Value)
	}
	want := []string{"low", "medium", "high", "max"}
	if !reflect.DeepEqual(values, want) {
		t.Fatalf("projectClaudeLevels: got %v, want %v", values, want)
	}

	got = projectRuntimeCLevels(superset, runtimeCModelEffortAllow["claude-opus-4-7"])
	values = values[:0]
	for _, lvl := range got {
		values = append(values, lvl.Value)
	}
	if !reflect.DeepEqual(values, superset) {
		t.Fatalf("projectClaudeLevels for Opus: got %v, want %v", values, superset)
	}

	got = projectRuntimeCLevels(superset, runtimeCModelEffortAllow["claude-opus-5"])
	values = values[:0]
	for _, lvl := range got {
		values = append(values, lvl.Value)
	}
	if !reflect.DeepEqual(values, superset) {
		t.Fatalf("projectClaudeLevels for Opus 5: got %v, want %v", values, superset)
	}
}

func TestRuntimeEDebugModelsArgs_Pinned(t *testing.T) {
	t.Parallel()
	want := []string{"debug", "models", "--bundled"}
	if !reflect.DeepEqual(runtimeEDebugModelsArgs, want) {
		t.Fatalf("codexDebugModelsArgs drifted: got %v, want %v", runtimeEDebugModelsArgs, want)
	}
	for _, arg := range runtimeEDebugModelsArgs {
		if arg == "--output" || arg == "-o" {
			t.Errorf("--output / -o leaked back into argv (codex CLI does not accept it): %v", runtimeEDebugModelsArgs)
		}
	}
}

func TestRunRuntimeEDebugModels_ArgvSeenByBinary(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("shell-script fake binary requires a POSIX shell")
	}
	t.Parallel()

	dir := t.TempDir()
	argvFile := filepath.Join(dir, "argv.txt")
	fake := filepath.Join(dir, "codex")
	script := "#!/bin/sh\n" +
		"printf '%s\\n' \"$@\" > '" + argvFile + "'\n" +
		"echo '{\"models\":[]}'\n"

	writeTestExecutable(t, fake, []byte(script))

	raw, err := runRuntimeEDebugModels(context.Background(), fake)
	if err != nil {
		t.Fatalf("runCodexDebugModels: %v (output=%q)", err, raw)
	}

	data, err := os.ReadFile(argvFile)
	if err != nil {
		t.Fatalf("read argv file: %v", err)
	}
	got := splitNonEmptyLines(string(data))
	want := []string{"debug", "models", "--bundled"}
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("fake codex received argv %v, want %v", got, want)
	}
}

func splitNonEmptyLines(s string) []string {
	var out []string
	start := 0
	for i := 0; i < len(s); i++ {
		if s[i] == '\n' {
			if i > start {
				out = append(out, s[start:i])
			}
			start = i + 1
		}
	}
	if start < len(s) {
		out = append(out, s[start:])
	}
	return out
}

func TestRuntimeESupportsDebugModels(t *testing.T) {
	t.Parallel()
	for _, tc := range []struct {
		version string
		want    bool
	}{
		{"codex-cli 0.121.0", false},
		{"codex-cli 0.122.0", true},
		{"codex-cli 0.144.1", true},
		{"invalid", false},
	} {
		if got := runtimeESupportsDebugModels(tc.version); got != tc.want {
			t.Errorf("codexSupportsDebugModels(%q) = %v, want %v", tc.version, got, tc.want)
		}
	}
}

func TestParseRuntimeEModelCatalog(t *testing.T) {
	t.Parallel()
	raw := []byte(`{
		"models": [
			{
				"slug": "gpt-5.6-sol",
				"display_name": "GPT-5.6-Sol",
				"visibility": "list",
				"default_reasoning_level": "low",
				"supported_reasoning_levels": [
					{"effort": "low", "description": "Fast"},
					{"effort": "max", "description": "Maximum"},
					{"effort": "ultra", "description": "Delegates"},
					{"effort": "future", "description": "New CLI value"}
				],
				"service_tiers": [
					{"id": "priority", "name": "Fast", "description": "1.5x speed, increased usage"}
				]
			},
			{
				"slug": "hidden-model",
				"display_name": "Hidden",
				"visibility": "hide",
				"supported_reasoning_levels": [{"effort": "low"}]
			},
			{
				"slug": "no-reasoning",
				"display_name": "No Reasoning",
				"visibility": "list",
				"supported_reasoning_levels": []
			}
		]
	}`)
	got, err := parseRuntimeEModelCatalog(raw)
	if err != nil {
		t.Fatalf("parseCodexModelCatalog: %v", err)
	}
	if len(got) != 2 {
		t.Fatalf("expected two visible models, got %+v", got)
	}
	if got[0].ID != "gpt-5.6-sol" || got[0].Label != "GPT-5.6-Sol" || !got[0].Default {
		t.Errorf("unexpected first model: %+v", got[0])
	}
	if got[0].Thinking == nil || got[0].Thinking.DefaultLevel != "low" || !hasThinkingLevel(got[0].Thinking, "max") || !hasThinkingLevel(got[0].Thinking, "ultra") || !hasThinkingLevel(got[0].Thinking, "future") {
		t.Errorf("unexpected per-model thinking catalog: %+v", got[0].Thinking)
	}
	if len(got[0].ServiceTiers) != 1 || got[0].ServiceTiers[0].ID != "priority" || got[0].ServiceTiers[0].Name != "Fast" {
		t.Errorf("unexpected service-tier catalog: %+v", got[0].ServiceTiers)
	}
	if got[1].ID != "no-reasoning" || got[1].Thinking != nil {
		t.Errorf("model without reasoning should remain selectable without a thinking picker: %+v", got[1])
	}
}

func TestParseRuntimeEModelCatalogMalformed(t *testing.T) {
	t.Parallel()
	if _, err := parseRuntimeEModelCatalog([]byte("not json")); err == nil {
		t.Fatal("expected malformed catalog error")
	}
}

func TestDiscoverRuntimeEModelsVersionGateAndFallback(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("shell-script fake binary requires a POSIX shell")
	}

	t.Run("supported version uses bundled catalog", func(t *testing.T) {
		dir := t.TempDir()
		fake := filepath.Join(dir, "codex")
		script := `#!/bin/sh
if [ "$1" = "--version" ]; then
  echo "codex-cli 0.122.0"
  exit 0
fi
printf '%s\n' "$@" > "` + filepath.Join(dir, "argv.txt") + `"
echo '{"models":[{"slug":"runtime-model","display_name":"Runtime Model","visibility":"list","default_reasoning_level":"high","supported_reasoning_levels":[{"effort":"high","description":"Live"}]}]}'
`
		writeTestExecutable(t, fake, []byte(script))

		got := discoverRuntimeEModels(context.Background(), fake)
		if len(got) != 1 || got[0].ID != "runtime-model" || got[0].Thinking == nil || !hasThinkingLevel(got[0].Thinking, "high") {
			t.Fatalf("expected runtime catalog, got %+v", got)
		}
	})

	t.Run("old version uses static fallback", func(t *testing.T) {
		dir := t.TempDir()
		fake := filepath.Join(dir, "codex")
		script := "#!/bin/sh\n" +
			"if [ \"$1\" = \"--version\" ]; then echo 'codex-cli 0.121.0'; exit 0; fi\n" +
			"exit 99\n"
		writeTestExecutable(t, fake, []byte(script))

		got := discoverRuntimeEModels(context.Background(), fake)
		if len(got) == 0 || got[0].ID != "gpt-5.6-sol" {
			t.Fatalf("expected static fallback, got %+v", got)
		}
	})

	t.Run("debug command failure uses static fallback", func(t *testing.T) {
		dir := t.TempDir()
		fake := filepath.Join(dir, "codex")
		script := "#!/bin/sh\n" +
			"if [ \"$1\" = \"--version\" ]; then echo 'codex-cli 0.144.1'; exit 0; fi\n" +
			"exit 1\n"
		writeTestExecutable(t, fake, []byte(script))

		got := discoverRuntimeEModels(context.Background(), fake)
		if len(got) == 0 || got[0].ID != "gpt-5.6-sol" || got[0].Thinking == nil {
			t.Fatalf("expected model + thinking fallback, got %+v", got)
		}
	})
}

func TestValidateThinkingLevelRuntimeEPerModelFallbackCatalog(t *testing.T) {
	t.Parallel()
	for _, tc := range []struct {
		model string
		level string
		want  bool
	}{
		{model: "gpt-5.6-sol", level: "ultra", want: true},
		{model: "gpt-5.6-terra", level: "ultra", want: true},
		{model: "gpt-5.6-luna", level: "max", want: true},
		{model: "gpt-5.6-luna", level: "ultra", want: false},
		{model: "gpt-5.3-codex", level: "xhigh", want: true},
		{model: "gpt-5.3-codex", level: "max", want: false},
	} {
		got, err := ValidateThinkingLevel(context.Background(), "runtime-e", "/nonexistent/codex", tc.model, tc.level)
		if err != nil {
			t.Fatalf("ValidateThinkingLevel(%q, %q): %v", tc.model, tc.level, err)
		}
		if got != tc.want {
			t.Errorf("ValidateThinkingLevel(%q, %q) = %v, want %v", tc.model, tc.level, got, tc.want)
		}
	}
}

func TestParseRuntimeEModelCatalog_PreservesFutureEfforts(t *testing.T) {
	t.Parallel()
	raw := []byte(`{
		"models": [
			{
				"slug": "gpt-5.6-sol",
				"display_name": "GPT-5.6-Sol",
				"visibility": "list",
				"default_reasoning_level": "high",
				"supported_reasoning_levels": [
					{"effort": "medium"},
					{"effort": "high"},
					{"effort": "max"},
					{"effort": "ultra"},
					{"effort": "hyper"}
				]
			},
			{
				"slug": "gpt-5.6-luna",
				"display_name": "GPT-5.6-Luna",
				"visibility": "list",
				"default_reasoning_level": "medium",
				"supported_reasoning_levels": [
					{"effort": "medium"},
					{"effort": "max"}
				]
			}
		]
	}`)
	got, err := parseRuntimeEModelCatalog(raw)
	if err != nil {
		t.Fatalf("parseCodexModelCatalog: %v", err)
	}
	byID := make(map[string]Model, len(got))
	for _, model := range got {
		byID[model.ID] = model
	}

	sol := byID["gpt-5.6-sol"]
	if sol.Thinking == nil {
		t.Fatalf("missing gpt-5.6-sol thinking entry: %+v", got)
	}
	if !hasThinkingLevel(sol.Thinking, "hyper") {
		t.Errorf("future effort should be preserved for sol: %+v", sol.Thinking.SupportedLevels)
	}
	if !IsKnownThinkingValue("runtime-e", "hyper") {
		t.Error("future safe Codex effort should pass the server token gate")
	}
	luna := byID["gpt-5.6-luna"]
	if luna.Thinking == nil {
		t.Fatalf("missing gpt-5.6-luna thinking entry: %+v", got)
	}
	if hasThinkingLevel(luna.Thinking, "hyper") {
		t.Errorf("future effort must remain model-specific: %+v", luna.Thinking.SupportedLevels)
	}
}

func hasThinkingLevel(mt *ModelThinking, value string) bool {
	for _, lvl := range mt.SupportedLevels {
		if lvl.Value == value {
			return true
		}
	}
	return false
}

func TestIsKnownThinkingValue(t *testing.T) {
	t.Parallel()
	tests := []struct {
		provider string
		value    string
		want     bool
	}{
		{"runtime-c", "", true},
		{"runtime-c", "low", true},
		{"runtime-c", "xhigh", true},
		{"runtime-c", "max", true},
		{"runtime-c", "none", false},
		{"runtime-e", "", true},
		{"runtime-e", "none", true},
		{"runtime-e", "minimal", true},
		{"runtime-e", "xhigh", true},
		{"runtime-e", "max", true},
		{"runtime-e", "ultra", true},
		{"runtime-e", "future-level", true},
		{"runtime-e", ".hidden", false},
		{"runtime-e", "bad value", false},
		{"runtime-m", "", true},
		{"runtime-m", "max", true},
		{"runtime-m", "fast-mode", true},
		{"runtime-m", ".hidden", false},
		{"runtime-m", "bad value", false},
		{"runtime-j", "", true},
		{"runtime-j", "low", false},
		{"runtime-i", "", true},
		{"runtime-i", "low", true},
		{"runtime-i", "medium", true},
		{"runtime-i", "high", true},
		{"runtime-i", "none", false},
		{"runtime-i", "minimal", false},
		{"runtime-i", "xhigh", false},
		{"runtime-i", "max", false},
		{"runtime-i", "bogus", false},
	}
	for _, tc := range tests {
		if got := IsKnownThinkingValue(tc.provider, tc.value); got != tc.want {
			t.Errorf("IsKnownThinkingValue(%q, %q) = %v, want %v",
				tc.provider, tc.value, got, tc.want)
		}
	}
}

func TestRuntimeEAdvertisedLevelsArePersistable(t *testing.T) {
	t.Parallel()
	for effort := range runtimeEEffortLabel {
		if !IsKnownThinkingValue("runtime-e", effort) {
			t.Errorf("Codex advertises effort %q but IsKnownThinkingValue rejects it; "+
				"keep the dynamic Codex token gate compatible so it can be saved", effort)
		}
	}
}

func TestValidateThinkingLevel_EmptyModelResolvesToDefault(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("shell-script fake binary requires a POSIX shell")
	}

	fakeRuntimeC := writeFakeRuntimeCHelpBinary(t)
	resetThinkingCacheForTests()
	defer resetThinkingCacheForTests()

	ctx := context.Background()

	t.Run("valid level on default model passes", func(t *testing.T) {

		ok, err := ValidateThinkingLevel(ctx, "runtime-c", fakeRuntimeC, "", "high")
		if err != nil {
			t.Fatalf("unexpected err: %v", err)
		}
		if !ok {
			t.Errorf("default-model high should be valid for claude; got false")
		}
	})

	t.Run("invalid level on default model fails", func(t *testing.T) {

		ok, err := ValidateThinkingLevel(ctx, "runtime-c", fakeRuntimeC, "", "xhigh")
		if err != nil {
			t.Fatalf("unexpected err: %v", err)
		}
		if ok {
			t.Errorf("xhigh should be invalid on sonnet (the default model); got true")
		}
	})

	t.Run("empty value always valid", func(t *testing.T) {

		ok, err := ValidateThinkingLevel(ctx, "runtime-c", fakeRuntimeC, "", "")
		if err != nil {
			t.Fatalf("unexpected err: %v", err)
		}
		if !ok {
			t.Errorf("empty value must always be valid")
		}
	})
}

func TestValidateThinkingLevel_ExplicitModel(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("shell-script fake binary requires a POSIX shell")
	}

	fakeRuntimeC := writeFakeRuntimeCHelpBinary(t)
	resetThinkingCacheForTests()
	defer resetThinkingCacheForTests()

	ctx := context.Background()

	ok, err := ValidateThinkingLevel(ctx, "runtime-c", fakeRuntimeC, "claude-opus-4-7", "xhigh")
	if err != nil {
		t.Fatalf("unexpected err: %v", err)
	}
	if !ok {
		t.Errorf("xhigh should be valid on opus-4-7; got false")
	}

	for _, level := range []string{"low", "medium", "high", "xhigh", "max"} {
		ok, err := ValidateThinkingLevel(ctx, "runtime-c", fakeRuntimeC, "claude-opus-5", level)
		if err != nil {
			t.Fatalf("unexpected err for opus-5 %q: %v", level, err)
		}
		if !ok {
			t.Errorf("%q should be valid on opus-5; got false", level)
		}
	}

	ok, err = ValidateThinkingLevel(ctx, "runtime-c", fakeRuntimeC, "claude-sonnet-4-6", "xhigh")
	if err != nil {
		t.Fatalf("unexpected err: %v", err)
	}
	if ok {
		t.Errorf("xhigh must not be valid on sonnet-4-6; got true")
	}

	ok, err = ValidateThinkingLevel(ctx, "runtime-c", fakeRuntimeC, "claude-nonexistent", "high")
	if err != nil {
		t.Fatalf("unexpected err: %v", err)
	}
	if ok {
		t.Errorf("unknown model must fail closed; got true")
	}
}

func TestValidateThinkingLevel_RuntimeEEmptyModelFailsClosed(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("shell-script fake binary requires a POSIX shell")
	}

	fakeRuntimeE := writeFakeRuntimeEModelsBinary(t)
	ctx := context.Background()

	check := func(model, value string, want bool) {
		t.Helper()
		ok, err := ValidateThinkingLevel(ctx, "runtime-e", fakeRuntimeE, model, value)
		if err != nil {
			t.Fatalf("ValidateThinkingLevel(codex, %q, %q): unexpected err: %v", model, value, err)
		}
		if ok != want {
			t.Errorf("ValidateThinkingLevel(codex, %q, %q) = %v, want %v", model, value, ok, want)
		}
	}

	check("gpt-5.6-sol", "ultra", true)
	check("gpt-5.6-terra", "ultra", true)
	check("gpt-5.6-luna", "ultra", false)
	check("gpt-5.6-luna", "max", true)
	check("gpt-5.6-luna", "medium", true)

	check("", "ultra", false)
	check("", "medium", false)

	check("", "", true)
	check("gpt-5.6-luna", "", true)
}

func TestValidateThinkingLevel_PreEffortCLIRejectsAllLevels(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("shell-script fake binary requires a POSIX shell")
	}

	fakeRuntimeC := writeFakeRuntimeCPreEffortHelpBinary(t)
	resetThinkingCacheForTests()
	defer resetThinkingCacheForTests()

	ctx := context.Background()

	for _, level := range []string{"low", "medium", "high", "xhigh", "max"} {
		ok, err := ValidateThinkingLevel(ctx, "runtime-c", fakeRuntimeC, "claude-fable-5", level)
		if err != nil {
			t.Fatalf("unexpected err for %q: %v", level, err)
		}
		if ok {
			t.Errorf("level %q must be invalid on a pre-effort CLI; got true", level)
		}
	}

	ok, err := ValidateThinkingLevel(ctx, "runtime-c", fakeRuntimeC, "claude-fable-5", "")
	if err != nil {
		t.Fatalf("unexpected err: %v", err)
	}
	if !ok {
		t.Errorf("empty value must always be valid")
	}
}

func TestValidateThinkingLevel_RuntimeMEmptyModelUsesAdvertisedVariants(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("shell-script fake binary requires a POSIX shell")
	}

	modelCacheMu.Lock()
	delete(modelCache, "runtime-m")
	modelCacheMu.Unlock()
	defer func() {
		modelCacheMu.Lock()
		delete(modelCache, "runtime-m")
		modelCacheMu.Unlock()
	}()

	dir := t.TempDir()
	fake := filepath.Join(dir, "opencode")
	script := `#!/bin/sh
if [ "$1" = "models" ]; then
  cat <<'EOF'
opencode/deepseek-v4
{
  "id": "deepseek-v4",
  "reasoning": true,
  "variants": {
    "high": {},
    "max": {}
  }
}
EOF
  exit 0
fi
echo "opencode 9.9.9"
`
	writeTestExecutable(t, fake, []byte(script))

	ctx := context.Background()
	ok, err := ValidateThinkingLevel(ctx, "runtime-m", fake, "", "max")
	if err != nil {
		t.Fatalf("unexpected err: %v", err)
	}
	if !ok {
		t.Fatalf("expected empty-model opencode max to pass when any advertised model supports it")
	}

	ok, err = ValidateThinkingLevel(ctx, "runtime-m", fake, "", "xhigh")
	if err != nil {
		t.Fatalf("unexpected err: %v", err)
	}
	if ok {
		t.Fatalf("xhigh should fail when no advertised OpenCode model exposes it")
	}
}

func writeFakeRuntimeCHelpBinary(t *testing.T) string {
	t.Helper()
	dir := t.TempDir()
	path := filepath.Join(dir, "claude")
	script := "#!/bin/sh\n" +
		"cat <<'EOF'\n" +
		"Usage: claude [options]\n" +
		"\n" +
		"Options:\n" +
		"  --model <model>     Model to use\n" +
		"  --effort <level>    Effort level for the current session (low, medium, high, xhigh, max)\n" +
		"EOF\n"

	writeTestExecutable(t, path, []byte(script))
	return path
}

func writeFakeRuntimeCPreEffortHelpBinary(t *testing.T) string {
	t.Helper()
	dir := t.TempDir()
	path := filepath.Join(dir, "claude")
	script := "#!/bin/sh\n" +
		"cat <<'EOF'\n" +
		"Usage: claude [options]\n" +
		"\n" +
		"Options:\n" +
		"  --model <model>     Model to use\n" +
		"  --verbose\n" +
		"EOF\n"
	writeTestExecutable(t, path, []byte(script))
	return path
}

func writeFakeRuntimeEModelsBinary(t *testing.T) string {
	t.Helper()
	dir := t.TempDir()
	path := filepath.Join(dir, "codex")
	script := "#!/bin/sh\n" +
		"if [ \"$1\" = \"debug\" ]; then\n" +
		"cat <<'EOF'\n" +
		`{"models":[` +
		`{"slug":"gpt-5.6-sol","default_reasoning_level":"high","supported_reasoning_levels":[{"effort":"low"},{"effort":"medium"},{"effort":"high"},{"effort":"xhigh"},{"effort":"max"},{"effort":"ultra"}],"service_tiers":[{"id":"priority","name":"Fast","description":"1.5x speed"}]},` +
		`{"slug":"gpt-5.6-terra","default_reasoning_level":"high","supported_reasoning_levels":[{"effort":"low"},{"effort":"medium"},{"effort":"high"},{"effort":"xhigh"},{"effort":"max"},{"effort":"ultra"}],"service_tiers":[{"id":"priority","name":"Fast"}]},` +
		`{"slug":"gpt-5.6-luna","default_reasoning_level":"medium","supported_reasoning_levels":[{"effort":"low"},{"effort":"medium"},{"effort":"high"},{"effort":"xhigh"},{"effort":"max"}]}` +
		`]}` + "\n" +
		"EOF\n" +
		"exit 0\n" +
		"fi\n" +
		"echo 'codex-cli 0.144.1'\n"
	writeTestExecutable(t, path, []byte(script))
	return path
}

func TestValidateServiceTierRuntimeEPerModelCatalog(t *testing.T) {
	t.Parallel()
	fake := writeFakeRuntimeEModelsBinary(t)
	for _, tc := range []struct {
		provider string
		model    string
		tier     string
		want     bool
	}{
		{provider: "runtime-e", model: "gpt-5.6-sol", tier: "priority", want: true},
		{provider: "runtime-e", model: "gpt-5.6-luna", tier: "priority", want: false},
		{provider: "runtime-e", model: "", tier: "priority", want: false},
		{provider: "runtime-c", model: "gpt-5.6-sol", tier: "priority", want: false},
		{provider: "runtime-e", model: "gpt-5.6-sol", tier: "", want: true},
	} {
		got, err := ValidateServiceTier(context.Background(), tc.provider, fake, tc.model, tc.tier)
		if err != nil {
			t.Fatalf("ValidateServiceTier(%q, %q, %q): %v", tc.provider, tc.model, tc.tier, err)
		}
		if got != tc.want {
			t.Errorf("ValidateServiceTier(%q, %q, %q) = %v, want %v", tc.provider, tc.model, tc.tier, got, tc.want)
		}
	}
}

func TestIsKnownServiceTier(t *testing.T) {
	t.Parallel()
	for _, tc := range []struct {
		provider string
		value    string
		want     bool
	}{
		{provider: "runtime-e", value: "", want: true},
		{provider: "runtime-c", value: "", want: true},
		{provider: "runtime-e", value: "priority", want: true},
		{provider: "runtime-e", value: "future.fast", want: true},
		{provider: "runtime-e", value: "../priority", want: false},
		{provider: "runtime-c", value: "priority", want: false},
	} {
		if got := IsKnownServiceTier(tc.provider, tc.value); got != tc.want {
			t.Errorf("IsKnownServiceTier(%q, %q) = %v, want %v", tc.provider, tc.value, got, tc.want)
		}
	}
}

func TestThinkingCacheKeyDistinct(t *testing.T) {

	resetThinkingCacheForTests()
	defer resetThinkingCacheForTests()

	a := thinkingCacheKey{provider: "runtime-c", executablePath: "/bin/claude", cliVersion: "2.1.121"}
	b := thinkingCacheKey{provider: "runtime-c", executablePath: "/bin/claude", cliVersion: "2.1.122"}
	c := thinkingCacheKey{provider: "runtime-c", executablePath: "/opt/claude", cliVersion: "2.1.121"}

	thinkingCachePut(a, map[string]*ModelThinking{"x": {DefaultLevel: "a"}})
	thinkingCachePut(b, map[string]*ModelThinking{"x": {DefaultLevel: "b"}})
	thinkingCachePut(c, map[string]*ModelThinking{"x": {DefaultLevel: "c"}})

	assertLevel := func(name string, key thinkingCacheKey, want string) {
		t.Helper()
		models, ok := thinkingCacheGet(key)
		if !ok {
			t.Fatalf("cache key %s: entry missing", name)
		}
		model, ok := models["x"]
		if !ok || model == nil {
			t.Fatalf("cache key %s: model x missing", name)
		}
		if model.DefaultLevel != want {
			t.Errorf("cache key %s: got %q, want %q", name, model.DefaultLevel, want)
		}
	}

	assertLevel("A", a, "a")
	assertLevel("B", b, "b")
	assertLevel("C", c, "c")
}

type runtimeEReasoningCase struct {
	name  string
	level string
}

var runtimeEReasoningCases = []runtimeEReasoningCase{
	{"empty-level-is-noop", ""},
	{"low", "low"},
	{"medium", "medium"},
	{"high", "high"},
	{"xhigh", "xhigh"},
	{"none-codex-only", "none"},
}

func TestApplyRuntimeEReasoningEffort_ThreePoints(t *testing.T) {
	t.Parallel()
	for _, tc := range runtimeEReasoningCases {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()

			startParams := map[string]any{
				"model": "gpt-5.5",
				"cwd":   "/work",
			}
			applyRuntimeEReasoningEffort(startParams, tc.level)
			assertRuntimeEThreadConfigEffort(t, "thread/start", startParams, tc.level)

			resumeParams := map[string]any{
				"threadId": "thr_prior",
				"cwd":      "/work",
				"model":    "gpt-5.5",
			}
			applyRuntimeEReasoningEffort(resumeParams, tc.level)
			assertRuntimeEThreadConfigEffort(t, "thread/resume", resumeParams, tc.level)

			turnParams := map[string]any{
				"threadId": "thr_x",
				"input":    []map[string]any{{"type": "text", "text": "hi"}},
			}
			applyRuntimeEReasoningEffort(turnParams, tc.level)
			assertRuntimeETurnEffort(t, "turn/start", turnParams, tc.level)
		})
	}
}

func assertRuntimeEThreadConfigEffort(t *testing.T, method string, params map[string]any, want string) {
	t.Helper()
	cfgAny, hasCfg := params["config"]
	if want == "" {

		if !hasCfg {
			return
		}
		cfg, _ := cfgAny.(map[string]any)
		if _, has := cfg["model_reasoning_effort"]; has {
			t.Errorf("%s: empty level must not emit model_reasoning_effort, got %v", method, cfg["model_reasoning_effort"])
		}
		return
	}
	if !hasCfg {
		t.Fatalf("%s: expected config block when level=%q", method, want)
	}
	cfg, ok := cfgAny.(map[string]any)
	if !ok {
		t.Fatalf("%s: config has wrong type %T", method, cfgAny)
	}
	got, ok := cfg["model_reasoning_effort"]
	if !ok {
		t.Fatalf("%s: missing config.model_reasoning_effort for level=%q (params=%+v)", method, want, params)
	}
	if got != want {
		t.Errorf("%s: config.model_reasoning_effort = %v, want %q", method, got, want)
	}

	if _, leaked := params["effort"]; leaked {
		t.Errorf("%s: top-level effort key leaked into thread params: %+v", method, params)
	}
}

func assertRuntimeETurnEffort(t *testing.T, method string, params map[string]any, want string) {
	t.Helper()
	got, has := params["effort"]
	if want == "" {
		if has {
			t.Errorf("%s: empty level must not emit effort, got %v", method, got)
		}

		if cfg, hasCfg := params["config"]; hasCfg {
			t.Errorf("%s: turn-shape params must not gain a config block, got %v", method, cfg)
		}
		return
	}
	if !has {
		t.Fatalf("%s: missing top-level effort for level=%q (params=%+v)", method, want, params)
	}
	if got != want {
		t.Errorf("%s: effort = %v, want %q", method, got, want)
	}

	if cfg, hasCfg := params["config"]; hasCfg {
		cfgMap, _ := cfg.(map[string]any)
		if _, leaked := cfgMap["model_reasoning_effort"]; leaked {
			t.Errorf("%s: config.model_reasoning_effort leaked into turn params: %+v", method, params)
		}
	}
}

func TestApplyRuntimeEReasoningEffort_NilParamsSafe(t *testing.T) {
	t.Parallel()

	applyRuntimeEReasoningEffort(nil, "high")
}

func TestApplyRuntimeEReasoningEffort_PreservesPreExistingConfig(t *testing.T) {
	t.Parallel()

	startParams := map[string]any{
		"model": "gpt-5.5",
		"config": map[string]any{
			"some_future_key": "preserve_me",
		},
	}
	applyRuntimeEReasoningEffort(startParams, "high")
	cfg, _ := startParams["config"].(map[string]any)
	if cfg["some_future_key"] != "preserve_me" {
		t.Errorf("pre-existing config key was clobbered: %+v", cfg)
	}
	if cfg["model_reasoning_effort"] != "high" {
		t.Errorf("reasoning effort not injected: %+v", cfg)
	}
}

func TestApplyRuntimeEServiceTier_ThreePoints(t *testing.T) {
	t.Parallel()
	for _, tier := range []string{"", "priority", "future-fast"} {
		t.Run(tier, func(t *testing.T) {
			for _, params := range []map[string]any{
				{"model": "gpt-5.6-sol", "cwd": "/work"},
				{"threadId": "prior", "cwd": "/work"},
				{"threadId": "thread", "input": []map[string]any{{"type": "text", "text": "hi"}}},
			} {
				applyRuntimeEServiceTier(params, tier)
				got, exists := params["serviceTier"]
				if tier == "" {
					if exists {
						t.Errorf("empty tier emitted serviceTier=%v", got)
					}
				} else if !exists || got != tier {
					t.Errorf("serviceTier = %v (exists=%v), want %q", got, exists, tier)
				}
			}
		})
	}
	applyRuntimeEServiceTier(nil, "priority")
}

func TestBuildRuntimeCArgs_InjectsEffort(t *testing.T) {
	t.Parallel()
	args := buildRuntimeCArgs(ExecOptions{Model: "claude-opus-4-7", ThinkingLevel: "xhigh"}, slog.Default())
	if !containsAdjacent(args, "--effort", "xhigh") {
		t.Errorf("expected --effort xhigh in args: %v", args)
	}

	modelIdx := argIndexOf(args, "--model")
	effortIdx := argIndexOf(args, "--effort")
	if modelIdx < 0 || effortIdx < 0 || modelIdx > effortIdx {
		t.Errorf("expected --model before --effort: %v", args)
	}
}

func TestBuildRuntimeCArgs_OmitsEffortWhenEmpty(t *testing.T) {
	t.Parallel()
	args := buildRuntimeCArgs(ExecOptions{Model: "claude-sonnet-4-6"}, slog.Default())
	if argIndexOf(args, "--effort") >= 0 {
		t.Errorf("expected no --effort when level empty: %v", args)
	}
}

func TestBuildRuntimeCArgs_BlocksUserEffortOverride(t *testing.T) {
	t.Parallel()
	args := buildRuntimeCArgs(ExecOptions{
		Model:         "claude-opus-4-7",
		ThinkingLevel: "high",
		CustomArgs:    []string{"--effort", "max", "--keep-me"},
	}, slog.Default())

	if !containsAdjacent(args, "--effort", "high") {
		t.Errorf("daemon-injected --effort high should remain: %v", args)
	}

	count := 0
	for _, a := range args {
		if a == "--effort" {
			count++
		}
	}
	if count != 1 {
		t.Errorf("expected exactly one --effort, got %d: %v", count, args)
	}
	if argIndexOf(args, "max") >= 0 {
		t.Errorf("filtered user --effort value still appears: %v", args)
	}

	if argIndexOf(args, "--keep-me") < 0 {
		t.Errorf("non-blocked custom arg was dropped: %v", args)
	}
}

func containsAdjacent(haystack []string, a, b string) bool {
	for i := 0; i < len(haystack)-1; i++ {
		if haystack[i] == a && haystack[i+1] == b {
			return true
		}
	}
	return false
}

func argIndexOf(slice []string, target string) int {
	for i, v := range slice {
		if v == target {
			return i
		}
	}
	return -1
}
