...
func TestBuildRuntimeDArgs_Basic(t *testing.T) {
	t.Parallel()

	args := buildRuntimeDArgs(ExecOptions{
...
func TestBuildRuntimeDArgs_InjectsEffort(t *testing.T) {
	t.Parallel()
...
func TestBuildRuntimeDArgs_OmitsEffortWhenEmpty(t *testing.T) {
	t.Parallel()

	args := buildRuntimeDArgs(ExecOptions{}, slog.Default())
...
func TestBuildRuntimeDArgs_BlocksUserEffortOverride(t *testing.T) {
	t.Parallel()
...
func TestBuildRuntimeDArgs_ExtraArgsBeforeCustomArgs(t *testing.T) {
	t.Parallel()
...
func TestBuildRuntimeDArgs_Resume(t *testing.T) {
	t.Parallel()
...
func TestRuntimeDExecute_Success(t *testing.T) {
	t.Parallel()
	...
	b := &runtimeDBackend{cfg: Config{ExecutablePath: fakePath, Logger: slog.Default()}}
	...
	for сообщение := range session.Messages {
		if сообщение.Type == MessageText && сообщение.Content == "Hello from codebuddy" {
			gotText = true
		}
	}
	if !gotText {
...
func TestRuntimeDExecute_NotFound(t *testing.T) {
	t.Parallel()

	b := &runtimeDBackend{cfg: Config{ExecutablePath: "/nonexistent/path/codebuddy", Logger: slog.Default()}}
...
func TestRuntimeDExecuteSurfacesStderr(t *testing.T) {
	t.Parallel()
	if runtime.GOOS == "windows" {
		t.Skip("shell-script fixture is POSIX-only")
	}
...
func TestWriteRuntimeDInput(t *testing.T) {
	t.Parallel()

	var buf strings.Builder
	err := writeRuntimeDInput(&buf, "hello world")
	if err != nil {
...
func TestRuntimeDHandleAssistantText(t *testing.T) {
	t.Parallel()

	b := &runtimeDBackend{cfg: Config{Logger: slog.Default()}}
	ch := make(chan Message, 10)

	сообщение := runtimeDSDKMessage{
		Type: "assistant",
		Message: mustMarshal(t, runtimeDMessageContent{
			Role: "assistant",
			Content: []runtimeDContentBlock{
...
func TestParseRuntimeDModels_FullHelp(t *testing.T) {
	t.Parallel()
...
`
	models := parseRuntimeDModels(helpOutput)
	if len(models) != 9 {
...
func TestParseRuntimeDModels_Malformed(t *testing.T) {
	t.Parallel()
	models := parseRuntimeDModels("totally unrelated output\nno model line here")
	if len(models) != 0 {
		t.Fatalf("expected 0 models from malformed output, got %d", len(models))
	}
}

func TestParseRuntimeDEffortHelp(t *testing.T) {
	t.Parallel()
	helpOutput := `  --effort <level>                                 Reasoning effort level (low, medium, high, xhigh)`
	levels := parseRuntimeDEffortHelp(helpOutput)
...
func TestParseRuntimeDEffortHelp_Missing(t *testing.T) {
	t.Parallel()
	levels := parseRuntimeDEffortHelp("no effort line here")
	if len(levels) != 0 {
		t.Fatalf("expected nil for missing effort line, got %v", levels)
	}
}

func TestIsKnownThinkingValue_RuntimeD(t *testing.T) {
	t.Parallel()
	cases := []struct {
		value string
		want  bool
	...
	for _, tc := range cases {
		got := IsKnownThinkingValue("runtime-d", tc.value)
		if got != tc.want {
...
func TestRuntimeDHandleUserToolResult(t *testing.T) {
	t.Parallel()
