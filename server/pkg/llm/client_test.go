...
func TestAPIKeyWithoutBaseURLIsRefused(t *testing.T) {

	c := New(Config{APIKey: "sk-test"})
	if c.Enabled() {
		t.Fatal("expected disabled client when API key is set without a base URL")
	}
	if _, err := c.Chat(context.Background(), openai.ChatCompletionNewParams{}); err != ErrMissingBaseURL {
		t.Fatalf("Chat: expected ErrMissingBaseURL, got %v", err)
	}
	if _, err := c.ChatStream(context.Background(), openai.ChatCompletionNewParams{}); err != ErrMissingBaseURL {
		t.Fatalf("ChatStream: expected ErrMissingBaseURL, got %v", err)
	}
	if _, err := c.GenerateText(context.Background(), "", "", "hi"); err != ErrMissingBaseURL {
		t.Fatalf("GenerateText: expected ErrMissingBaseURL, got %v", err)
	}

	for _, name := range []string{"GOOSAR_LLM_API_KEY", "GOOSAR_LLM_BASE_URL"} {
		if !strings.Contains(ErrMissingBaseURL.Error(), name) {
			t.Fatalf("ErrMissingBaseURL must name %s, got: %v", name, ErrMissingBaseURL)
		}
	}
}

func TestEnabledWithKeyAndBaseURL(t *testing.T) {
	c := New(Config{APIKey: "sk-test", BaseURL: "http://localhost:1234"})
	if !c.Enabled() {
		t.Fatal("expected enabled client when both API key and base URL are set")
	}
}
