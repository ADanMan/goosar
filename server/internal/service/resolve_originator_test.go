...
func seedOriginatorFanout(t *testing.T, pool *pgxpool.Pool) (memberCommentID, agentCommentID, taskAID, userID, workspaceUUID pgtype.UUID) {
	...
	if err := pool.QueryRow(ctx, `
		...
		VALUES ('Resolve Originator User', 'resolve-originator-fanout@goosar.test')
		RETURNING id
	...
	t.Cleanup(func() {
		pool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'resolve-originator-fanout@goosar.test'`)
	})
...
func seedForeignWorkspaceTask(t *testing.T, pool *pgxpool.Pool) pgtype.UUID {
	t.Helper()
	ctx := context.Background()
	...
	if err := pool.QueryRow(ctx, `
		INSERT INTO "user" (name, email) VALUES ('Foreign WS Human', $1) RETURNING id
	`, fmt.Sprintf("cross-ws-origin-%d@goosar.test", suffix)).Scan(&userID); err != nil {
		t.Fatalf("seed foreign user: %v", err)
	}
	t.Cleanup(func() { pool.Exec(context.Background(), `DELETE FROM "user" WHERE id = $1`, userID) })

	if err := pool.QueryRow(ctx, `
		INSERT INTO workspace (name, slug) VALUES ('cross-ws-origin', $1) RETURNING id
	`, fmt.Sprintf("cross-ws-origin-%d", suffix)).Scan(&workspaceID); err != nil {
	...
	if err := pool.QueryRow(ctx, `
		...
		VALUES ($1, 'r', 'cloud', 'codex', 'online', '', '{}'::jsonb, $2) RETURNING id
	...
	if err := pool.QueryRow(ctx, `
		...
		VALUES ($1, 'foreign-agent', 'cloud', '{}'::jsonb, $2, 'workspace', 1, $3, '', '{}'::jsonb, '[]'::jsonb) RETURNING id
	...
	if err := pool.QueryRow(ctx, `
		...
		VALUES ($1, $2, 'completed', 0, $3, $3) RETURNING id
	`, agentID, runtimeID, userID).Scan(&taskID); err != nil {
		t.Fatalf("seed foreign task: %v", err)
	}
	return util.MustParseUUID(taskID)
}

func TestResolveOriginatorForIssueTask_CrossWorkspaceOriginStaysUnattributed(t *testing.T) {
	pool := newResolveOriginatorPool(t)

	_, _, _, _, workspaceW1 := seedOriginatorFanout(t, pool)
	foreignTaskW2 := seedForeignWorkspaceTask(t, pool)
	svc := &TaskService{Queries: db.New(pool)}

	issue := db.Issue{
		WorkspaceID: workspaceW1,
		CreatorType: "agent",
		OriginType:  pgtype.Text{String: "agent_create", Valid: true},
		OriginID:    foreignTaskW2,
	}

	got := svc.resolveOriginatorForIssueTask(context.Background(), issue, pgtype.UUID{})
	if got.Valid {
		t.Fatalf("cross-workspace origin resolved originator %s; want unattributed (H2 fail-closed)",
...
func TestEnqueueTaskForIssueStoresRuntimeMCPOverlayInQueuedRow(t *testing.T) {
	...
	q := db.New(pool)
	suffix := time.Now().UnixNano()
	email := fmt.Sprintf("runtime-overlay-insert-%d@goosar.test", suffix)
