...
This skill states WHAT a mention link does in the Goosar backend, traced to
...
- a person → `goosar workspace member list --output json` → use `user_id`
- an agent → `goosar agent list --output json` → use `id`
- a squad  → `goosar squad list --output json` → use `id`
...
The mention trigger set is computed by `resolveMentionedAgentCommentTriggers`
(`server/internal/handler/comment.go`); `computeCommentAgentTriggers` calls it
when the comment carries an explicit @agent/@squad mention and enqueues the
result via `enqueueCommentAgentTriggers`.
...
And it is special at trigger time: `computeCommentAgentTriggers`
(`server/internal/handler/comment.go`) returns early with no triggers when
`util.HasMentionAll` sees an `@all` mention — BEFORE explicit @agent/@squad
mentions are even resolved. A comment carrying `@all` is a pure broadcast: it
suppresses the assignee's automatic on-comment trigger AND any explicit
@agent/@squad mentions in the same comment — nothing is enqueued at all. Use
`@all` to announce; never combine it with a mention you expect to trigger a
run.
...
- **A private agent you cannot access:** skipped — the mention path gates on
  ...
  one; the child-done wake is ungated — see the goosar-squads skill).
...
Correct:
  1. `goosar workspace member list --output json`  → Alice's `user_id` = 7f3a…
