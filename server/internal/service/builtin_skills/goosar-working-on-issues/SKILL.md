---
name: goosar-working-on-issues
description: "Use when working on a Goosar issue after the runtime has provided the trigger context — to apply the product contracts the runtime brief does not encode: how PR linking differs from close intent, how to read a linked PR's real state via the pull-requests CLI, which metadata keys are high-signal, what status changes trigger on the server, how sub-issue create status (todo vs backlog) controls whether assigned agents start immediately, and the exact command that posts the result comment (`goosar issue comment add` — there is no `create` verb — with the body in a file, not on stdin)."
user-invocable: false
allowed-tools: Bash(goosar *), Bash(git *), Bash(gh *)
---

# Working on Goosar issues
...
For building mention links, load `goosar-mentioning` instead — not this skill.
...
This is the link that `goosar issue pull-requests` reads back — but see the
...
**excluded from `goosar issue pull-requests`** (and the issue's right-side PR
...
is to open or update a PR before posting the final Goosar issue comment, unless
...
## Posting that comment: the verb is `add`, and the body comes from a file

There is no `goosar issue comment create`. Guessing it costs turns — the CLI
answers `unknown flag: --content` or prints the `goosar issue comment` help
instead of failing on the verb. The working command is:

```bash
goosar issue comment add <issue-id> --content-file ./reply.md
```

Write the body to a UTF-8 file with your file-write tool first, then point
`--content-file` at it. That is the mode that carries a **multi-line** body,
backticks, `$()`, and quotes through unchanged (MUL-2904). The path must sit
inside the current working directory — an outside path like `/tmp/reply.md` is
rejected so another run's stale file cannot leak in (MUL-4252). If you truly
must read a file from outside the working directory, either copy it into the
cwd first (preferred), or pass `--allow-external-file` as a rare, deliberate
override. Remove the file after posting.

Exactly one body source per call; combining two is an error:

- `--content-file <path>` — the default for any body you authored.
- `--content "..."` — short single-line bodies. It does decode `\n`, `\r`,
  `\t` and `\\`, so `--content "line one\n\nline two"` really does produce a
  blank line, but the shell still rewrites backticks, `$()`, and quotes on the
  way in.
- `--content-stdin` — reads this process's stdin, and only works when the same
  command line pipes bytes into it. A bare `--content-stdin` gets no body from
  an agent shell tool: stdin is either empty (`stdin content for
  --content-stdin is empty`, exit 1) or never closes and the call hangs. A
  HEREDOC pipe does reach the CLI, but the heredoc/flag boundary silently
  swallows trailing flags (#4182). Use `--content-file` instead.

`--output` already defaults to `json` on this command. Add `--parent
<comment-id>` to reply inside a thread; a comment-triggered run must reply
under its trigger comment, and the server rejects a top-level comment from one.

## Reading a linked PR's real state

When a step depends on PR state, query Goosar's link table — do not infer it
...
```bash
goosar issue metadata set <issue-id> --key pr_url --value <url>
goosar issue metadata delete <issue-id> --key <stale-key>
```
...
- Read what exists before writing: `goosar property list` shows the catalog;
  `goosar issue property list <issue-id>` shows values set on the issue.
- Set values by property name and option name — the CLI translates to ids:

```bash
goosar issue property set <issue-id> --name Environment --value staging
goosar issue property set <issue-id> --name Platforms --value "iOS,Android"
goosar issue property unset <issue-id> --name Environment
```
...
```bash
goosar issue create --title "..." --parent <issue-id> --assignee <agent> --status todo
```

Strictly serial children — park later steps, promote one at a time:

```bash
goosar issue create --title "Step 2: ..." --parent <issue-id> --assignee <agent> --status backlog
goosar issue status <child-id> todo   # promote when the previous step is truly done
```
...
goosar issue create --title "Research A" --parent <id> --assignee <agent> --stage 1 --status todo
goosar issue create --title "Research B" --parent <id> --assignee <agent> --stage 1 --status todo
goosar issue create --title "Build"      --parent <id> --assignee <agent> --stage 2 --status backlog
goosar issue create --title "Ship"       --parent <id> --assignee <agent> --stage 3 --status backlog
```
...
```bash
goosar issue children <parent-id>             # sub-issues grouped by stage
goosar issue status <stage-2-child-id> todo   # promote when its deps are met
```
...
```bash
# incorrect — there is no `create` verb on `issue comment`
goosar issue comment create <issue-id> --content "..."

# incorrect — nothing pipes into stdin, so the body is empty (or the call hangs)
goosar issue comment add <issue-id> --content-stdin

# correct — body written to a UTF-8 file in the workdir first
goosar issue comment add <issue-id> --content-file ./reply.md
```
...
goosar issue create --title "Step 2" --parent <issue-id> --assignee <agent> --status todo
goosar issue create --title "Step 3" --parent <issue-id> --assignee <agent> --status todo

# correct — stage them; Stage 1 runs, later stages park and are promoted as
# each stage's barrier closes
goosar issue create --title "Step 1" --parent <issue-id> --assignee <agent> --stage 1 --status todo
goosar issue create --title "Step 2" --parent <issue-id> --assignee <agent> --stage 2 --status backlog
goosar issue create --title "Step 3" --parent <issue-id> --assignee <agent> --stage 3 --status backlog
```
...
`issue children` CLI, the `issue comment add` verb + body-source flags with
their empty-stdin / workdir guards, and the metadata CLI. Re-derive before
