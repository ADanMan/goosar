---
name: goosar-autopilots
description: "Use when creating, updating, inspecting, triggering, or debugging Goosar autopilots. Covers the full chain: schedule/webhook/manual trigger, create_issue vs run_only execution, agent/squad leader admission, runs, created issues/tasks, webhook URL rotation, and side-effect boundaries."
...
```bash
goosar autopilot list --output json
goosar autopilot get <autopilot-id> --output json
goosar autopilot runs <autopilot-id> --output json
```
...
- `create_issue` creates a Goosar issue, making the run visible as issue state.
...
goosar autopilot list --output json
goosar autopilot get <autopilot-id> --output json
goosar autopilot create --title "<title>" --description "<task prompt>" --agent <agent-name-or-id> --mode create_issue|run_only --output json
goosar autopilot update <autopilot-id> --status active|paused --output json
goosar autopilot runs <autopilot-id> --output json
goosar autopilot trigger-add <autopilot-id> --kind schedule --cron "0 9 * * *" --timezone Asia/Shanghai --output json
goosar autopilot trigger-add <autopilot-id> --kind webhook --label "ci" --output json
goosar autopilot trigger <autopilot-id> --output json
goosar autopilot trigger-rotate-url <autopilot-id> <trigger-id> --yes --output json
```
...
1. `goosar autopilot get <id> --output json` — status, mode, assignee, triggers.
2. `goosar autopilot runs <id> --output json` — run status and failure reason.
3. If assigned to a squad, inspect the squad: `goosar squad get <squad-id> --output json`; execution goes to the leader.
4. Inspect the target agent/runtime: `goosar agent get <agent-id> --output json` and `goosar runtime list --output json`.
