---
name: goosar-squads
description: "Use when creating, inspecting, updating, assigning, mentioning, or debugging Goosar squads. Explains what squads are, squad/member fields, CLI commands, leader routing, issue assignment, comments, mentions, autopilot behavior, leader briefing, side effects, and product-gap handling."
...
goosar squad get <squad-id> --output json
goosar squad member list <squad-id> --output json
goosar issue comment list <issue-id> --recent 10 --output json
```
...
```bash
goosar squad --help
goosar squad member --help
goosar issue update --help
...
A Goosar squad is a workspace routing and coordination object.
...
```bash
goosar squad list --output json
goosar squad get <squad-id> --output json
goosar squad create --name <name> --leader <agent-name-or-id> --output json
goosar squad update <squad-id> --instructions "<leader coordination policy>" --output json
goosar squad delete <squad-id>
```
...
```bash
goosar squad member list <squad-id> --output json
goosar squad member add <squad-id> --member-id <id> --type agent|member --role <role> --output json
goosar squad member remove <squad-id> --member-id <id> --type agent|member
goosar squad member set-role <squad-id> --member-id <id> --member-type agent|member --role <role> --output json
```

Squad leader evaluation command:

```bash
goosar squad activity <issue-id> action|no_action|failed --reason "<why>" --output json
```
...
```bash
goosar issue get <issue-id> --output json
goosar issue update <issue-id> --help
goosar issue comment list <issue-id> --output json
goosar issue comment add <issue-id> --help
```
...
For squad leader tasks, Goosar appends a squad leader briefing to the leader
...
members carry no skills segment. Builtin `goosar-*` skills are not listed —
...
- recording squad activity with `goosar squad activity`;
...
- Getting the leader briefing does NOT imply status authority. A squad
  ...
  delegation rules yes, `goosar issue status` no.
