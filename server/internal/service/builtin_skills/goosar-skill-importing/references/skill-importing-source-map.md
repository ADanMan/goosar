...
Evidence layer for `goosar-skill-importing`. Every behavioral claim in `SKILL.md`
...
grep -n "func runSkillImport"           server/cmd/goosar/cmd_skill.go
...
## CLI: `goosar skill import --url` / `--file`

| Behavior | File:line |
|---|---|
| `skill import` command def | `server/cmd/goosar/cmd_skill.go:59-63` |
| `--url` flag | `server/cmd/goosar/cmd_skill.go:143` |
| `--file` flag (local `.skill` / `.zip`; mutually exclusive with `--url`) | `server/cmd/goosar/cmd_skill.go:144` |
| `--on-conflict` flag (default `fail`) | `server/cmd/goosar/cmd_skill.go:145` |
| `--output` flag (default `json`) | `server/cmd/goosar/cmd_skill.go:146` |
| `runSkillImport` | `server/cmd/goosar/cmd_skill.go:412` |
| Requires exactly one of `--url` / `--file` | `server/cmd/goosar/cmd_skill.go:420-427` |
| `--file` reads the archive and posts multipart via `ImportSkillFile` | `server/cmd/goosar/cmd_skill.go:436-447`, client method `server/internal/cli/client.go:535` |
| `POST /api/skills/import` (URL, JSON body) | `server/cmd/goosar/cmd_skill.go:455` |
| Structured HTTP error body handling | `server/cmd/goosar/cmd_skill.go:437-440`, `handleSkillImportError` at `:454` |
| Prints structured result (`json` or table) | `server/cmd/goosar/cmd_skill.go:443`, helper at `:497` |
...
| CLI normalizes legacy `{existing_skill}` body into `status:"conflict"` | `server/cmd/goosar/cmd_skill.go:454-482`, helper at `:484` |
...
## Deployment-disabled sources (`GOOSAR_SKILL_SOURCES`)

| Behavior | File:line |
|---|---|
| `skillsources` package (env parsing, profile defaults, fail-closed reads) | `server/internal/skillsources/skillsources.go` (`Parse` at `:80`, `EnabledFromEnv` at `:128`, `DisabledMessage` at `:167`) |
| Import gate: 403 naming `GOOSAR_SKILL_SOURCES` before any fetch | `server/internal/handler/skill.go:837` (`requireImportSourceEnabled`), called from `ImportSkill` at `:2259`; source mapping `skillSourceFor` at `:822` |
| Search gate: `GET /api/skills/search` 403 `{code:"source_disabled"}` when clawhub is off | `server/internal/handler/skill.go:319` |
| `GITHUB_TOKEN` never attached while github is disabled | `server/internal/handler/skill.go:1740-1758` (`addGitHubAuthHeader`) |
| `/api/config` advertises the enabled list as `skill_sources` (absent = all enabled) | `server/internal/handler/config.go:89` (field), `:134-144` (emission) |
| Agent templates: vendored offline skills + gated network fallback | `server/internal/handler/agent_template.go:306` (vendored resolve), `:317-329` (gate), `server/internal/agenttmpl/vendored.go:81` (`LoadVendored`) |
| Startup validation (typo = hard failure) | `server/cmd/server/router.go` (`deliveryProfileFromEnv`) |
| Tests (gate matrix, token, offline templates) | `server/internal/handler/skill_sources_gate_test.go`, `server/internal/handler/agent_template_offline_test.go`, `server/internal/skillsources/skillsources_test.go` |

Note: inserting the gate shifted line anchors elsewhere in
`server/internal/handler/skill.go`; anchors in the older tables predate that
change — re-grep the named symbols to re-derive.
...
| CLI `agent skills add` def ("without replacing existing assignments") | `server/cmd/goosar/cmd_agent.go:125-130` |
| `runAgentSkillsAdd` → `POST .../skills/add` | `server/cmd/goosar/cmd_agent.go:797`; POST `:818` |
| CLI `agent skills set` def ("replaces all current assignments") | `server/cmd/goosar/cmd_agent.go:118-123` |
| `runAgentSkillsSet` → `PUT .../skills` | `server/cmd/goosar/cmd_agent.go:772`; PUT `:790` |
| CLI `agent skills list` | `server/cmd/goosar/cmd_agent.go:740`; GET `:750` |
