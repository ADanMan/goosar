---
name: goosar-skill-importing
description: "Use when a user provides a skill URL, slug, or clear intent to import/install a specific skill into the current Goosar workspace. Teaches the workspace import API/CLI path (POST /api/skills/import), the supported URL source families, --on-conflict fail|overwrite|rename|skip behavior and structured import results, additive agent binding vs replace-all, and the reserved SKILL.md supporting-file rule. Do not use it to decide which skill the user needs, and never treat an external local installer like npx skills add as the final Goosar install."
user-invocable: false
allowed-tools: Bash(goosar *)
---

# Importing skills into Goosar

Use this skill when the user already provided a skill URL, slug, or a clear intent
to import a specific skill into the current Goosar workspace.
...
A skill is installed for Goosar only when it exists in the current workspace's
...
```bash
goosar skill import --url <url> --output json              # hosted source
goosar skill import --file <path-to.skill> --output json   # local archive
```
...
environment, not the Goosar workspace DB, so Goosar cannot manage or bind it.

## Supported URL source families

`detectImportSource` accepts these hosts (and `www.` variants). Pass any of these
forms to `goosar skill import --url <url> --output json`:

```bash
goosar skill import --url clawhub.ai/owner/skill --output json
goosar skill import --url skills.sh/owner/repo/skill --output json
goosar skill import --url github.com/owner/repo --output json
goosar skill import --url github.com/owner/repo/tree/main/path/to/skill --output json
goosar skill import --url github.com/owner/repo/blob/main/path/to/SKILL.md --output json
```
...
## Deployment-disabled sources

Each source family can be switched off per deployment via the server env
`GOOSAR_SKILL_SOURCES` (comma list of `clawhub`, `github`, `skillssh`, or
`none`; unset means all enabled on the cloud profile and all disabled on the
perimeter profile). Importing from a disabled source returns `403` with an
error naming `GOOSAR_SKILL_SOURCES`, before any network fetch happens:

```json
{ "error": "the clawhub skill source is disabled on this deployment (GOOSAR_SKILL_SOURCES)" }
```

Treat that as deployment policy, not a transient failure: do not retry, do
not try another URL form for the same host. Report the message and, when the
skill content is available locally, offer the `--file` archive path instead —
local archive and runtime-local imports stay available in every
configuration. `GET /api/config` advertises the enabled list as
`skill_sources` (field absent = every source enabled).

## Local archive import (`.skill` / `.zip`)

`goosar skill import --file <path> --output json` imports a skill from a local
...
After the final `goosar agent skills list <agent-id> --output json`, verify the
target skill id is present before claiming the skill is available to that agent.

## Additive add vs replace-all set

`goosar agent skills add` is additive: the server inserts the assignments without
clearing existing ones (`AddAgentSkills`).

`goosar agent skills set` is replace-all: the server clears every current
...
Default behavior is safe: `goosar skill import --url <url>` is equivalent to
...
goosar skill import --url https://skills.sh/acme/repo/review-helper --output json

# Replace the existing same-name skill, preserving its ID and agent bindings.
goosar skill import --url https://skills.sh/acme/repo/review-helper --on-conflict overwrite --output json

# Keep the existing skill and import a copy such as review-helper-2.
goosar skill import --url https://skills.sh/acme/repo/review-helper --on-conflict rename --output json

# Batch-friendly behavior: leave the existing skill alone and mark it skipped.
goosar skill import --url https://skills.sh/acme/repo/review-helper --on-conflict skip --output json
```
...
```bash
goosar skill get <skill-id> --output json
```
...
Incorrect (bypasses Goosar):

```bash
npx skills add https://skills.sh/owner/repo/skill
```

The skill may exist locally, but Goosar cannot manage it as a workspace skill.
...
```bash
goosar skill import --url https://skills.sh/owner/repo/skill --output json
```
