---
name: goosar-runtimes-and-repos
description: "Use when inspecting or debugging Goosar runtimes, daemon task claiming, agent not running, workdir/session reuse, or repository checkout. Covers runtime online/offline state, daemon heartbeat/claim chain, task-scoped repo checkout, project repo context, local_directory caveats, and safe diagnostic commands."
user-invocable: false
allowed-tools: Bash(goosar *)
---

# Goosar Runtimes and Repos
...
7. `goosar repo checkout` talks to the local daemon, not directly to GitHub.

## CLI

```bash
goosar runtime list --output json
goosar runtime usage <runtime-id> --output json
goosar runtime activity <runtime-id> --output json
goosar runtime update <runtime-id> --target-version <version> --output json
goosar runtime delete <runtime-id>
goosar repo checkout <url>
goosar repo checkout <url> --ref <branch-or-sha>
```
...
4. Is the runtime online? `goosar runtime list --output json`.
