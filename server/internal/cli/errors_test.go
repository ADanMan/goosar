...
func TestClassifyNetworkError(t *testing.T) {
	cases := []struct {
		...
		{"dns", &net.DNSError{Err: "no such host", Name: "goosar.ru", IsNotFound: true}, KindNetworkDNS},
		...
		{"x509 hostname", x509.HostnameError{Host: "goosar.ru"}, KindNetworkTLS},
		{"timeout string fallback", errors.New("Get \"https://x\": net/http: request canceled (Client.Timeout exceeded)"), KindNetworkTimeout},
		{"dns string fallback", errors.New("dial tcp: lookup goosar.ru: no such host"), KindNetworkDNS},
...
func TestFormatErrorAllKinds(t *testing.T) {
	...
	for _, lang := range []Language{LangEN, LangZH, LangRU} {
...
func TestFormatErrorNetwork(t *testing.T) {
	...
	raw := errors.New("Get \"https://goosar.ru/api/issues/abc\": context deadline exceeded")
	...
	if strings.Contains(got, "goosar.ru") || strings.Contains(got, "resolve issue") {
...
func TestFormatErrorRussianLocale(t *testing.T) {
	withLang(t, "ru_RU.UTF-8")
	netErr := &NetworkError{Kind: KindNetworkDNS, Err: errors.New("no such host")}
	got := FormatError(netErr, false)
	if !strings.Contains(got, "Не удалось определить адрес") {
		t.Errorf("expected Russian DNS message, got %q", got)
	}
}

func TestFormatErrorRussianViaLCAll(t *testing.T) {
	withLang(t, "en_US.UTF-8")
	t.Setenv("LC_ALL", "ru_RU.UTF-8")
	httpErr := &HTTPError{StatusCode: 401}
	got := FormatError(httpErr, false)
	if !strings.Contains(got, "goosar login") || !strings.Contains(got, "Сессия") {
		t.Errorf("expected Russian auth message, got %q", got)
	}
}
...
func TestDetectLanguage(t *testing.T) {
	cases := []struct {
		...
		{"lang ru", map[string]string{"LANG": "ru_RU.UTF-8"}, LangRU},
		{"lc_all ru", map[string]string{"LC_ALL": "ru_RU.UTF-8", "LANG": "en_US.UTF-8"}, LangRU},
		{"lc_messages ru", map[string]string{"LC_MESSAGES": "ru_RU.UTF-8"}, LangRU},
		{"lc_all wins over ru lang", map[string]string{"LC_ALL": "en_US.UTF-8", "LANG": "ru_RU.UTF-8"}, LangEN},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
...
func TestHTTPTimeout(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Setenv("GOOSAR_HTTP_TIMEOUT", tc.val)
...
func TestFormatErrorActionableHints(t *testing.T) {
	cases := []struct {
		...
		{401, []string{"goosar login", "self-hosted", "administrator"}, []string{"goosar login", "自托管", "管理员"}},
...
func TestUserMessageError(t *testing.T) {
	withLang(t, "en_US.UTF-8")
	const hint = "Could not sign in with that token — make sure it is valid and not expired, then run `goosar login --token <token>` again."
	...
	t.Run("wrapped NetworkError classifies as network", func(t *testing.T) {
		...
		err := WithUserMessage("Sign-in did not complete: the server did not accept the new credential. Run `goosar login` again.", underlying)
