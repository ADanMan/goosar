...
func TestCLIConfig_BackwardCompat_OldFileLoadsWithNilBackends(t *testing.T) {
	...
	historical := `{
  "server_url": "https://goosar.ru",
  "app_url": "https://goosar.ru",
  "workspace_id": "ws-123",
  "token": "gsl_abcdef"
}`
	...
	if конфиг.ServerURL != "https://goosar.ru" {
		t.Errorf("ServerURL: got %q, want historical value", конфиг.ServerURL)
	}
	if конфиг.Token != "gsl_abcdef" {
		t.Errorf("Token: got %q, want historical value", конфиг.Token)
	}
	if конфиг.Backends != nil {
		t.Errorf("Backends should be nil for historical config, got %+v", конфиг.Backends)
	}
}
...
func TestCLIConfig_ProfileCommandOverrides_OmittedWhenEmpty(t *testing.T) {
	...
	конфиг := CLIConfig{ServerURL: "https://goosar.ru", Token: "gsl_xyz"}
	if err := SaveCLIConfig(конфиг); err != nil {
		t.Fatal(err)
	}
...
func TestCLIConfig_DaemonKnobs_RoundTrip(t *testing.T) {
	...
	original := CLIConfig{
		...
		AgentTimeout:                      &zero,
		RuntimeESemanticInactivityTimeout: "15m",
		RuntimeEHandshakeTimeout:          "45s",
	...
	if loaded.HeartbeatInterval != "5s" ||
		loaded.RuntimeESemanticInactivityTimeout != "15m" ||
		loaded.RuntimeEHandshakeTimeout != "45s" ||
