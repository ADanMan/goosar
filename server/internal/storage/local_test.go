...
func TestLocalStorage_ServeFile_NoSidecarFallback(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(tmpDir, key), []byte("body"), 0o644); err != nil {
...
func TestLocalStorage_ServeFile_RejectsPathTraversal(t *testing.T) {
	...
	if err := os.WriteFile(leakedBase+metaSuffix, []byte(`{"filename":"leaked.xlsx","content_type":"text/plain"}`), 0o644); err != nil {
...
func TestLocalStorage_RefusesExportArchives(t *testing.T) {
	tmpDir := t.TempDir()
	t.Setenv("LOCAL_UPLOAD_DIR", tmpDir)
	...
	if err := os.MkdirAll(filepath.Join(tmpDir, ExportsSubdir), 0o700); err != nil {
		t.Fatalf("mkdir: %v", err)
	}
	archive := "workspace-ws-job.tar.gz"
	if err := os.WriteFile(filepath.Join(tmpDir, ExportsSubdir, archive), []byte("the whole workspace"), 0o600); err != nil {
		t.Fatalf("write archive: %v", err)
	}

	keys := []string{
		ExportsSubdir + "/" + archive,
		ExportsSubdir + "/" + archive + ".part",
		"/" + ExportsSubdir + "/" + archive,
		"other/../" + ExportsSubdir + "/" + archive,
	}
	for _, key := range keys {
		req := httptest.NewRequest(http.MethodGet, "/uploads/"+key, nil)
		rec := httptest.NewRecorder()
		store.ServeFile(rec, req, key)
		if rec.Code != http.StatusNotFound {
			t.Errorf("ServeFile(%q) status = %d, want 404 (body %q)", key, rec.Code, rec.Body.String())
		}
		if _, err := store.GetReader(context.Background(), key); err == nil {
			t.Errorf("GetReader(%q) succeeded, want refusal", key)
		}
	}

	if _, err := store.Upload(context.Background(), "ok.txt", []byte("body"), "text/plain", "ok.txt"); err != nil {
		t.Fatalf("Upload failed: %v", err)
	}
	req := httptest.NewRequest(http.MethodGet, "/uploads/ok.txt", nil)
	rec := httptest.NewRecorder()
	store.ServeFile(rec, req, "ok.txt")
	if rec.Code != http.StatusOK {
		t.Fatalf("ordinary object status = %d, want 200", rec.Code)
	}
}
