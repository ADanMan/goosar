...
func deliverEnvelope(hub *Hub, daemonRuntime DaemonRuntimeDeliverer, ev envelope) {
	...
	case ScopeUser:
		...
		if ev.EventType == EventConnectionRevoked {
			hub.DisconnectUser(ev.ScopeID, ev.WorkspaceID)
			return
		}
...
var (
	_ Broadcaster    = (*RedisRelay)(nil)
	_ Broadcaster    = (*DualWriteBroadcaster)(nil)
	_ RelayPublisher = (*RedisRelay)(nil)
)
