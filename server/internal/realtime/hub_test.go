...
func (m *mockMembershipChecker) IsMember(_ context.Context, _, _ string, _ *int32) bool {
	return true
}
...
func TestCheckOrigin(t *testing.T) {
	...
	cases := []struct {
		...
		{"empty origin allowed", "goosar.ru", "", "", "1.2.3.4:5678", true},
		{"same-origin allowed (native client default)", "localhost:8080", "http://localhost:8080", "", "1.2.3.4:5678", true},
		{"same-origin allowed (https)", "goosar.ru", "https://goosar.ru", "", "1.2.3.4:5678", true},
		{"same-origin allowed (case-insensitive host, RFC 7230)", "Goosar.RU", "https://goosar.ru", "", "1.2.3.4:5678", true},
		{"whitelisted origin allowed (web cross-origin)", "localhost:8080", "http://localhost:3000", "", "1.2.3.4:5678", true},
		{"whitelisted origin allowed (prod web)", "localhost:8080", "https://goosar.ru", "", "1.2.3.4:5678", true},
		{"unknown origin rejected (CSWSH defense)", "goosar.ru", "https://evil.com", "", "1.2.3.4:5678", false},
		{"different port rejected", "localhost:8080", "http://localhost:9999", "", "1.2.3.4:5678", false},
		{"X-Forwarded-Host from trusted proxy matches origin", "internal.proxy", "https://goosar.ru", "goosar.ru", "127.0.0.1:5678", true},
		{"X-Forwarded-Host from trusted proxy case-insensitive", "internal.proxy", "https://Goosar.RU", "goosar.ru", "10.0.0.1:5678", true},
		{"X-Forwarded-Host from untrusted source rejected", "internal.proxy", "https://example.com", "example.com", "1.2.3.4:5678", false},
		{"X-Forwarded-Host from trusted proxy but evil origin rejected", "internal.proxy", "https://evil.com", "goosar.ru", "127.0.0.1:5678", false},
		{"X-Forwarded-Host present but origin matches direct Host", "goosar.ru", "https://goosar.ru", "other.host", "1.2.3.4:5678", true},
		{"X-Forwarded-Host spoofed by attacker rejected", "internal.proxy", "https://evil.com", "evil.com", "1.2.3.4:5678", false},
		{"X-Forwarded-Host from trusted CIDR range matches origin", "internal.proxy", "https://goosar.ru", "goosar.ru", "10.5.6.7:5678", true},
		{"X-Forwarded-Host from trusted IPv6 proxy matches origin", "internal.proxy", "https://goosar.ru", "goosar.ru", "[::1]:5678", true},
		{"X-Forwarded-Host comma list uses first (client-facing) value", "internal.proxy", "https://goosar.ru", "goosar.ru, proxy.internal", "127.0.0.1:5678", true},
		{"X-Forwarded-Host comma list ignores trailing values", "internal.proxy", "https://staging.goosar.ru", "proxy.internal, staging.goosar.ru", "127.0.0.1:5678", false},
	}
...
func TestAuthenticateToken_RefusesMFATicket(t *testing.T) {
	ticket := jwt.NewWithClaims(jwt.SigningMethodHS256, jwt.MapClaims{
		"sub": testUserID,
		"typ": "mfa",
		"exp": jwt.NewNumericDate(time.Now().Add(5 * time.Minute)),
	})
	signed, err := ticket.SignedString(auth.JWTSecret())
	if err != nil {
		t.Fatalf("sign: %v", err)
	}

	uid, _, errMsg := authenticateToken(signed, nil, context.Background())
	if errMsg == "" || uid != "" {
		t.Fatalf("mfa ticket must be refused, got uid=%q errMsg=%q", uid, errMsg)
	}
}
