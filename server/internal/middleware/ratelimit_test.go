...
func TestRateLimit_NilStore(t *testing.T) {
...
func TestRateLimit_AllowsUnderLimit(t *testing.T) {
	...
	mw := RateLimit(NewRateLimitStore(rdb), 3, time.Minute, nil)
	handler := mw(okHandler)
...
func TestRateLimit_BlocksOverLimit(t *testing.T) {
	rdb := newRedisTestClient(t)
	mw := RateLimit(NewRateLimitStore(rdb), 2, time.Minute, nil)
	handler := mw(okHandler)
...
func TestRateLimit_RetryAfterHeader(t *testing.T) {
	rdb := newRedisTestClient(t)
	mw := RateLimit(NewRateLimitStore(rdb), 1, 2*time.Minute, nil)
	handler := mw(okHandler)
...
func TestRateLimit_DifferentIPs(t *testing.T) {
	rdb := newRedisTestClient(t)
	mw := RateLimit(NewRateLimitStore(rdb), 1, time.Minute, nil)
	handler := mw(okHandler)
...
func TestRateLimit_LuaScript_SetsTTL(t *testing.T) {
	rdb := newRedisTestClient(t)
	mw := RateLimit(NewRateLimitStore(rdb), 10, 30*time.Second, nil)
	handler := mw(okHandler)
	...
	key := "mul:ratelimit:auth:send-code:ip:10.0.99.1"
