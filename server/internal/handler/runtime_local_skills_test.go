...
func createRuntimeLocalSkillTestRuntime(t *testing.T, ownerID string) string {
	...
	if err := testPool.QueryRow(context.Background(), `
		...
		VALUES ($1, $2, $3, 'local', 'runtime-c', 'online', 'Runtime Local Skills Test', '{}'::jsonb, $4, now())
		RETURNING id
...
func createRuntimeLocalSkillTestMember(t *testing.T, role string) string {
	...
	email := fmt.Sprintf("runtime-local-skills-%d@goosar.ru", time.Now().UnixNano())
...
func TestListLocalSkills_RejectsNonMember(t *testing.T) {
	...
	email := fmt.Sprintf("runtime-local-skills-outsider-%d@goosar.ru", time.Now().UnixNano())
	if err := testPool.QueryRow(context.Background(), `
		INSERT INTO "user" (name, email)
