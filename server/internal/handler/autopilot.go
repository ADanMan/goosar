...
func autopilotToResponse(a db.Autopilot, subscribers []db.AutopilotSubscriber) AutopilotResponse {
	...
	return AutopilotResponse{
		...
		IsTemplate: a.ExternalKey.Valid &&
			strings.HasPrefix(a.ExternalKey.String, templateAutopilotExternalKeyPrefix),
		Subscribers: subResp,
	}
}

func broadcastTriggerResponse(tr AutopilotTriggerResponse) AutopilotTriggerResponse {
	tr.WebhookToken = nil
	tr.WebhookPath = nil
	tr.WebhookURL = nil
	return tr
...
func (h *Handler) CreateAutopilotTrigger(w http.ResponseWriter, r *http.Request) {
	...
	case "webhook":
		...
		h.publish(protocol.EventAutopilotUpdated, workspaceID, "member", userID, map[string]any{
			"autopilot_id": uuidToString(ap.ID),
			"trigger":      broadcastTriggerResponse(resp),
		})
		writeJSON(w, http.StatusCreated, resp)
		return
	}
...
func (h *Handler) validateAutopilotAssignee(w http.ResponseWriter, r *http.Request, assigneeType string, assigneeID, workspaceID pgtype.UUID) bool {
	...
	case "agent":
		...
		if !h.canInvokeAgent(r.Context(), agent, actorType, actorID, h.invokeAuthorityFromRequest(r, actorType, actorID, noInvokeScope), util.UUIDToString(workspaceID)) {
			writeError(w, http.StatusForbidden, "cannot assign autopilot to private agent")
			return false
		}
		return true
	case "squad":
		...
		actorType, actorID := h.resolveActor(r, requestUserID(r), util.UUIDToString(workspaceID))
		if !h.canInvokeAgent(r.Context(), leader, actorType, actorID, h.invokeAuthorityFromRequest(r, actorType, actorID, noInvokeScope), util.UUIDToString(workspaceID)) {
