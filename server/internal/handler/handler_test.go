...
const (
	handlerTestEmail         = "handler-test@goosar.ru"
	handlerTestName          = "Handler Test User"
	handlerTestWorkspaceSlug = "handler-tests"
)

func requireTestDB() bool {
	return os.Getenv("GOOSAR_REQUIRE_TEST_DB") == "1" || os.Getenv("CI") != ""
}

func TestMain(m *testing.M) {
	ctx := context.Background()

	for _, v := range []string{"RESEND_API_KEY", "SMTP_HOST"} {
		if os.Getenv(v) != "" {
			fmt.Printf("FATAL: %s is set — handler tests would call a real mail service. Run via scripts/test-go.sh (which scrubs mail env) or unset it.\n", v)
			os.Exit(1)
		}
	}
	...
	if err != nil {
		if requireTestDB() {
			fmt.Printf("FATAL: could not connect to database (GOOSAR_REQUIRE_TEST_DB/CI set, refusing to skip): %v\n", err)
			os.Exit(1)
		}
	...
	if err := pool.Ping(ctx); err != nil {
		pool.Close()
		if requireTestDB() {
			fmt.Printf("FATAL: database not reachable (GOOSAR_REQUIRE_TEST_DB/CI set, refusing to skip): %v\n", err)
			os.Exit(1)
		}
...
func cleanupHandlerTestFixture(ctx context.Context, pool *pgxpool.Pool) error {
	...
	if _, err := pool.Exec(ctx, `DELETE FROM agent WHERE owner_id IN (SELECT id FROM "user" WHERE email = $1)`, handlerTestEmail); err != nil {
		return fmt.Errorf("sweep leftover agents owned by test user: %w", err)
	}
	...
	}{
		{
			...
			req: newRequest("POST", "/api/daemon/register", map[string]any{
				...
				"runtimes": []map[string]any{
					{"name": "runtime-e", "type": "runtime-e", "status": "online"},
				},
			}),
...
func TestSendCode(t *testing.T) {
	w := httptest.NewRecorder()
	body := map[string]string{"email": "sendcode-test@goosar.ru"}
	var buf bytes.Buffer
	json.NewEncoder(&buf).Encode(body)
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(), `DELETE FROM verification_code WHERE email = $1`, "sendcode-test@goosar.ru")
	})
}
...
func TestSendCodeDbError(t *testing.T) {
	...
	w := httptest.NewRecorder()
	body := map[string]string{"email": "dberror-test@goosar.ru"}
	var buf bytes.Buffer
	json.NewEncoder(&buf).Encode(body)
	req := httptest.NewRequest("POST", "/auth/send-code", &buf)
	req.Header.Set("Content-Type", "application/json")
...
func TestSendCodeRateLimit(t *testing.T) {
	const email = "ratelimit-test@goosar.ru"
	t.Cleanup(func() {
...
func TestVerifyCode(t *testing.T) {
	const email = "verify-test@goosar.ru"
	ctx := context.Background()
...
func TestVerifyCodeRejectsDevCodeUnlessExplicitlyConfigured(t *testing.T) {
	...
	const email = "dev-code-disabled-test@goosar.ru"
	ctx := context.Background()
...
func TestVerifyCodeAcceptsConfiguredDevCodeOutsideProduction(t *testing.T) {
	...
	const email = "dev-code-enabled-test@goosar.ru"
	ctx := context.Background()
...
func TestVerifyCodeRejectsConfiguredDevCodeInProduction(t *testing.T) {
	...
	const email = "dev-code-production-test@goosar.ru"
	ctx := context.Background()
...
func TestVerifyCodeWrongCode(t *testing.T) {
	...
	const email = "wrong-code-test@goosar.ru"
	ctx := context.Background()
...
func TestVerifyCodeBruteForceProtection(t *testing.T) {
	...
	const email = "bruteforce-test@goosar.ru"
	ctx := context.Background()
...
func TestVerifyCodeNewUserHasNoWorkspace(t *testing.T) {
	const email = "workspace-verify-test@goosar.ru"
	ctx := context.Background()
...
func TestResolveActor(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "agent ID without task ID or actor-source returns member",
			agentIDHeader: agentID,
			wantActorType: "member",
		},
		{

			name:          "valid agent + valid task WITHOUT actor-source returns member",
			agentIDHeader: agentID,
			taskIDHeader:  taskID,
			wantActorType: "member",
		},
		{
			name:            "valid agent + valid task WITH task_token actor-source returns agent",
			agentIDHeader:   agentID,
			taskIDHeader:    taskID,
			actorSourceTask: true,
			wantActorType:   "agent",
			wantIsAgent:     true,
		},
		{

			name:            "any agent ID WITH task_token actor-source returns agent",
...
func TestDaemonRegisterMissingWorkspaceReturns404(t *testing.T) {
	...
	req := httptest.NewRequest("POST", "/api/daemon/register", bytes.NewBufferString(`{
		...
		"runtimes":[{"name":"Local Codex","type":"runtime-e","version":"1.0.0","status":"online"}]
	}`))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("X-User-ID", testUserID)
