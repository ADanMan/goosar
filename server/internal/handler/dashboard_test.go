...
func TestDashboardFailuresCountNeverStartedTasks(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx, `
		SELECT ((CURRENT_DATE)::timestamp + interval '12 hours') AT TIME ZONE 'UTC',
		       ((CURRENT_DATE)::timestamp + interval '11 hours') AT TIME ZONE 'UTC'
	`).Scan(&completed, &started); err != nil {
		t.Fatalf("anchor fixture times to UTC day: %v", err)
	}
	...
	mkTask("failed", "queued_expired", nil)
	mkTask("failed", nil, started)
