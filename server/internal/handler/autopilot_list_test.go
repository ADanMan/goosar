...
func TestListAutopilots_IsTemplate(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}
	ctx := context.Background()

	agentID := createHandlerTestAgent(t, "autopilot-is-template-agent", []byte(`[]`))
	templated := insertListTestAutopilot(t, agentID, "list-is-template-seeded")

	foreign := insertListTestAutopilot(t, agentID, "list-is-template-foreign")
	own := insertListTestAutopilot(t, agentID, "list-is-template-own")
	for _, kv := range [][2]string{
		{templated, templateAutopilotExternalKey("hr", "weekly-plan")},
		{foreign, "integration:hr:receivables"},
	} {
		if _, err := testPool.Exec(ctx,
			`UPDATE autopilot SET external_key = $1 WHERE id = $2`, kv[1], kv[0]); err != nil {
	...
	for id, want := range map[string]bool{templated: true, foreign: false, own: false} {
		row, ok := rows[id]
		if !ok {
			t.Fatalf("autopilot %s missing from list", id)
		}
		if got, _ := row["is_template"].(bool); got != want {
			t.Errorf("is_template for %s: expected %v, got %v", row["title"], want, row["is_template"])
		}
	}
}
