...
func TestValidateClientUsageRuntime(t *testing.T) {
	tests := []struct {
		...
		{name: "successful mixed config", probe: clientUsageRuntimeProbe{ProbeResult: "success", RuntimeCount: ptrInt32(3), ProviderSummary: map[string]int{"runtime-c": 2, "runtime-e": 1}, OnlineCount: ptrInt32(1), OfflineCount: ptrInt32(2)}},
		...
		{name: "state total mismatch", probe: clientUsageRuntimeProbe{ProbeResult: "success", RuntimeCount: ptrInt32(2), ProviderSummary: map[string]int{"runtime-e": 2}, OnlineCount: ptrInt32(1), OfflineCount: ptrInt32(0)}, wantErr: true},
		{name: "provider total mismatch", probe: clientUsageRuntimeProbe{ProbeResult: "success", RuntimeCount: ptrInt32(2), ProviderSummary: map[string]int{"runtime-e": 1}, OnlineCount: ptrInt32(1), OfflineCount: ptrInt32(1)}, wantErr: true},
...
func TestUpsertClientUsageKeepsRuntimeSnapshotOnActivityRefresh(t *testing.T) {
	...
	w := report(map[string]any{
		...
		"runtime": map[string]any{
			...
			"provider_summary": map[string]int{"runtime-e": 1},
