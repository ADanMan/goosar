...
const (
	...
	goosarShellEnvBeginMarker = "# BEGIN goosar-managed shell-environment (do not edit; regenerated by daemon)"
	goosarShellEnvEndMarker   = "# END goosar-managed shell-environment"
)

var shellEnvBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(goosarShellEnvBeginMarker) +
		`.*?^` + regexp.QuoteMeta(goosarShellEnvEndMarker) + `\n*`)
...
func renderGoosarShellEnvBlock(includeOnly []string) (string, error) {
	...
	b.WriteString(goosarShellEnvBeginMarker)
	b.WriteByte('\n')
	b.WriteString("[")
	...
	b.WriteByte('\n')
	b.WriteString(goosarShellEnvEndMarker)
	b.WriteByte('\n')
	return b.String(), nil
}
...
func EnsureCodexShellEnvPolicyConfig(configPath string, includeOnly []string, logger *slog.Logger) error {
	...
	block, err := renderGoosarShellEnvBlock(includeOnly)
	if err != nil {
		return err
	}
