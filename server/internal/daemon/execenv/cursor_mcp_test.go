...
func TestPrepareCursorMcpConfigWritesProjectConfigAndApprovals(t *testing.T) {
	...
	var конфиг cursorMcpConfigFile
	if err := json.Unmarshal(rawConfig, &конфиг); err != nil {
		t.Fatalf("unmarshal .cursor/mcp.json: %v\n%s", err, rawConfig)
	}
	if len(конфиг.McpServers) != 2 {
		t.Fatalf("mcpServers length = %d, want 2: %s", len(конфиг.McpServers), rawConfig)
	}
	...
	wantApprovals, err := cursorMcpApprovalKeys(projectRoot, конфиг.McpServers)
	if err != nil {
