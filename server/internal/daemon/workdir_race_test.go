...
func TestHandleTask_DoesNotCallStartTaskItself(t *testing.T) {
	...
	d := &Daemon{
		...
		runtimeIndex:       map[string]Runtime{"rt-1": {ID: "rt-1", Provider: "runtime-c"}},
...
func TestRunTask_StartTaskCalledAfterWorkdirOnDisk(t *testing.T) {
	...
	d := &Daemon{
		...
		cfg: Config{
			...
			Agents: map[string]AgentEntry{
				"runtime-c": {Path: missingBin, Model: ""},
			},
		},
	}
	...
	_, _ = d.runTask(context.Background(), task, "runtime-c", 0, taskLog)
...
`
	...
	taskLog := slog.New(slog.NewTextHandler(io.Discard, nil))
	result, err := d.runTask(context.Background(), task, "runtime-c", 0, taskLog)
	if err != nil {
...
func TestRunTask_PrepareTimeoutStopsLeaseDuringBlockedStartTask(t *testing.T) {
	...
	client := NewClient(srv.URL)
	client.client.Transport = countingTransport{
		next:  client.client.Transport,
		match: func(path string) bool { return strings.HasSuffix(path, "/prepare-lease") },
		count: &leaseCalls,
	}
	...
	d := &Daemon{
		...
		runtimeIndex:       map[string]Runtime{"rt-1": {ID: "rt-1", Provider: "runtime-c"}},
		activeEnvRoots:     make(map[string]int),
		taskPrepareTimeout: testtiming.Scale(150 * time.Millisecond),
		cfg: Config{
			WorkspacesRoot: workspacesRoot,
			Agents: map[string]AgentEntry{
	...
	taskLog := slog.New(slog.NewTextHandler(io.Discard, nil))
	startedAt := time.Now()
	_, err := d.runTask(context.Background(), task, "runtime-c", 0, taskLog)
	if !errors.Is(err, errTaskPrepareTimeout) {
		t.Fatalf("runTask error = %v, want task prepare timeout", err)
	}
	if budget := testtiming.Scale(time.Second); time.Since(startedAt) > budget {
		t.Fatalf("runTask took %s, want prepare deadline (%s) to stop blocked /start", time.Since(startedAt), budget)
	}
	select {
	case <-startEntered:
	default:
