...
func TestEnsureDaemonID_Persists(t *testing.T) {
	...
	path := filepath.Join(home, ".goosar", "daemon.id")
...
func TestEnsureDaemonID_SharedAcrossProfiles(t *testing.T) {
	...
	profileFile := filepath.Join(home, ".goosar", "profiles", "staging", "daemon.id")
...
func TestEnsureDaemonID_PromotesPreChangeProfileFile(t *testing.T) {
	...
	profileDir := filepath.Join(home, ".goosar", "profiles", "staging")
	...
	data, err := os.ReadFile(filepath.Join(home, ".goosar", "daemon.id"))
	if err != nil {
...
func TestLegacyDaemonUUIDs_ScansProfileDirs(t *testing.T) {
	...
	for name, id := range map[string]string{"prod": uuidA, "desktop-goosar": uuidB} {
		dir := filepath.Join(home, ".goosar", "profiles", name)
		if err := os.MkdirAll(dir, 0o755); err != nil {
	...
	corruptDir := filepath.Join(home, ".goosar", "profiles", "corrupt")
