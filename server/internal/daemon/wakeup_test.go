...
func TestTaskWakeupURL(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			runtimeIDs: []string{"runtime-j", "runtime-a"},
			want:       "ws://localhost:8080/api/daemon/ws?runtime_ids=runtime-a%2Cruntime-j",
		},
		{
			name:       "https base",
		...
		{
			name:       "base path",
			baseURL:    "https://api.example.com/goosar",
			runtimeIDs: []string{"runtime-1"},
			want:       "wss://api.example.com/goosar/api/daemon/ws?runtime_ids=runtime-1",
		},
		{
...
func TestReadTaskWakeupMessagesTimesOutWithoutPeerTraffic(t *testing.T) {
	overrideTaskWakeupTimings(t, 60*time.Millisecond, 20*time.Millisecond, defaultTaskWakeupTimings.backoffResetAfter)
...
func TestReadTaskWakeupMessagesExtendsDeadlineOnServerPing(t *testing.T) {
	overrideTaskWakeupTimings(t, 120*time.Millisecond, 50*time.Millisecond, defaultTaskWakeupTimings.backoffResetAfter)
...
func TestReadTaskWakeupMessagesAcceptsLargeRPCResponse(t *testing.T) {
	overrideTaskWakeupTimings(t, time.Second, 100*time.Millisecond, defaultTaskWakeupTimings.backoffResetAfter)
...
func TestShouldResetTaskWakeupBackoffRequiresStableConnection(t *testing.T) {
	overrideTaskWakeupTimings(t, defaultTaskWakeupTimings.pongWait, defaultTaskWakeupTimings.writeWait, 10*time.Second)
...
func overrideTaskWakeupTimings(t *testing.T, pongWait, writeWait, backoffResetAfter time.Duration) {
	t.Helper()
	old := taskWakeupTimingsHook.Load()
	taskWakeupTimingsHook.Store(&taskWakeupTimings{
		pongWait:          pongWait,
		writeWait:         writeWait,
		backoffResetAfter: backoffResetAfter,
	})
	t.Cleanup(func() {
		taskWakeupTimingsHook.Store(old)
	})
}
...
func mustProtocolFrame(t *testing.T, сообщение protocol.Message) []byte {
	t.Helper()
	frame, err := json.Marshal(сообщение)
	if err != nil {
...
func TestTaskWakeupDialerTrustsTheConfiguredCA(t *testing.T) {
	...
	if err == nil || strings.Contains(err.Error(), "certificate") || strings.Contains(err.Error(), "x509") {
		t.Fatalf("want a non-TLS handshake outcome, got %v", err)
	}
}
