...
func TestPruneWorktree_SerializesWithCreateWorktree(t *testing.T) {
	...
	waitFor := func(what string, ch <-chan struct{}) {
		...
		select {
		case <-ch:
		case <-time.After(testtiming.Scale(5 * time.Second)):
			t.Fatalf("timed out waiting for %s", what)
		}
	}

	waitFor("pruneWorktree to acquire repo lock", blockingCache.entered)
	...
	waitFor("CreateWorktree to start", blockingCache.createCalled)

	select {
	case err := <-createDone:
		t.Fatalf("CreateWorktree should wait for GC lock, returned early with err=%v", err)
	case <-time.After(testtiming.Scale(200 * time.Millisecond)):
	}
	...
	waitFor("pruneWorktree to finish", pruneDone)
}
...
func (c *blockingRepoCache) CreateWorktree(params repocache.WorktreeParams) (*repocache.WorktreeResult, error) {
	if c.createCalled != nil {
		close(c.createCalled)
	}
