...
func noSleepRetry(t *testing.T) func() {
	t.Helper()
	prev := retrySleepHook.Load()
	var instant retrySleepFunc = func(ctx context.Context, _ time.Duration) error {
		if err := ctx.Err(); err != nil {
			return err
		}
		return nil
	}
	retrySleepHook.Store(&instant)
	return func() { retrySleepHook.Store(prev) }
}
