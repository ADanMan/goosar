...
`
	...
	servers, supported, err := listRuntimeLocalMcpServers("runtime-e", nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestListRuntimeLocalMcpServersClaudeMissingConfig(t *testing.T) {
	...
	servers, supported, err := listRuntimeLocalMcpServers("runtime-c", nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestListRuntimeLocalMcpServersClaudeEnabledPlugin(t *testing.T) {
	...
	if servers[0].Source != "Runtime C plugin · paper-desktop" {
...
func TestListRuntimeLocalMcpServersUnknownProvider(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	servers, supported, err := listRuntimeLocalMcpServers("future-runtime", nil)
	if err != nil {
		t.Fatal(err)
	}
	if supported || len(servers) != 0 {
...
func TestMergeRuntimeAndAgentMcpConfigClaudeCombinesAndAgentWins(t *testing.T) {
	...
	merged, _, err := mergeRuntimeAndAgentMcpConfig("runtime-c", json.RawMessage(`{"mcpServers":{"shared":{"command":"agent-shared"},"agent-only":{"url":"https://agent.example/mcp"}}}`), nil)
	if err != nil {
		t.Fatal(err)
	}
	var document struct {
...
func TestMergeRuntimeAndAgentMcpConfigSkipsDisabledEntries(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)

	runtimeConfig := `{"mcpServers":{
		"runtime-on":{"command":"runtime-cmd"},
		"runtime-off":{"command":"runtime-cmd","enabled":false},
		"shared":{"command":"runtime-shared"}
	}}`
	if err := os.WriteFile(filepath.Join(home, ".claude.json"), []byte(runtimeConfig), 0o600); err != nil {
		t.Fatal(err)
	}

	merged, _, err := mergeRuntimeAndAgentMcpConfig("runtime-c", json.RawMessage(`{"mcpServers":{
		"shared":{"command":"agent-shared","enabled":false},
		"agent-off":{"command":"mcp-atlassian","disabled":true},
		"agent-on":{"command":"agent-cmd"}
	}}`), nil)
	if err != nil {
		t.Fatal(err)
	}
	...
	want := []string{"agent-on", "runtime-on"}
	if len(document.McpServers) != len(want) {
		t.Fatalf("merged servers = %#v, want exactly %v", document.McpServers, want)
	}
	for _, name := range want {
		if _, ok := document.McpServers[name]; !ok {
			t.Fatalf("expected %q in merged servers: %#v", name, document.McpServers)
		}
	}
}
...
`
	...
	merged, _, err := mergeRuntimeAndAgentMcpConfig("runtime-e", json.RawMessage(`{"mcpServers":{"agent":{"command":"node","args":["agent.js"]}}}`), nil)
	if err != nil {
		t.Fatal(err)
	}
	var document struct {
...
func TestMergeRuntimeAndAgentMcpConfigNullKeepsNativeInheritance(t *testing.T) {
	...
	for _, raw := range []json.RawMessage{nil, json.RawMessage("null"), json.RawMessage(" null ")} {
		merged, _, err := mergeRuntimeAndAgentMcpConfig("runtime-c", raw, nil)
		if err != nil {
			t.Fatal(err)
		}
