...
const (
	integrationTestEmail         = "integration-test@goosar.ru"
...
func TestSendCodeAndVerify(t *testing.T) {
	const email = "integration-sendcode@goosar.ru"
	ctx := context.Background()
...
func TestLoginLinkVerifyAndReplay(t *testing.T) {
	const email = "integration-loginlink@goosar.ru"
	ctx := context.Background()
	...
	const linkToken = "integration-test-link-token-0123456789abcdef"
	sum := sha256.Sum256([]byte(linkToken))
	tag, err := testPool.Exec(ctx,
		`UPDATE verification_code SET link_token_hash = $1 WHERE email = $2`,
		hex.EncodeToString(sum[:]), email)
	if err != nil {
		t.Fatalf("inject link token hash: %v", err)
	}
	if tag.RowsAffected() == 0 {
		t.Fatal("send-code did not create a verification_code row")
	}

	body, _ = json.Marshal(map[string]string{"link_token": linkToken})
	resp, err = http.Post(testServer.URL+"/auth/verify-link", "application/json", bytes.NewReader(body))
	if err != nil {
		t.Fatalf("verify-link failed: %v", err)
	}
	if resp.StatusCode != http.StatusOK {
		respBody, _ := io.ReadAll(resp.Body)
		resp.Body.Close()
		t.Fatalf("verify-link: expected 200, got %d: %s", resp.StatusCode, respBody)
	}
	var loginResp struct {
		Token string `json:"token"`
		User  struct {
	...
	readJSON(t, resp, &loginResp)
	if loginResp.Token == "" {
		t.Fatal("verify-link: expected non-empty token")
	}
	if loginResp.User.Email != email {
		t.Fatalf("verify-link: logged in as %q, want %q", loginResp.User.Email, email)
	}
	...
	resp, err = http.Post(testServer.URL+"/auth/verify-link", "application/json", bytes.NewReader(body))
	if err != nil {
		t.Fatalf("verify-link replay failed: %v", err)
	}
	resp.Body.Close()
	if resp.StatusCode != http.StatusBadRequest {
		t.Fatalf("verify-link replay: expected 400, got %d", resp.StatusCode)
	}
}

func TestVerifyCodeNewUserHasNoWorkspace(t *testing.T) {
	const email = "new-integration-verify@goosar.ru"
	ctx := context.Background()
...
func TestAgentsThroughRouter(t *testing.T) {
	...
	if агент["id"] != agentID {
		t.Fatalf("expected agent id %s, got %s", agentID, агент["id"])
	}
