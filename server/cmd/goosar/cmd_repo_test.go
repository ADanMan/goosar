...
func TestRunRepoCheckoutForwardsManagedCheckoutMode(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got := r.Header.Get("Authorization"); got != "Bearer mat_repo_checkout_test" {
			t.Errorf("Authorization = %q, want task-scoped bearer", got)
		}
	...
	t.Setenv("GOOSAR_DAEMON_PORT", strings.TrimPrefix(srv.URL, "http://127.0.0.1:"))
	t.Setenv("GOOSAR_WORKSPACE_ID", "ws-1")
	t.Setenv("GOOSAR_AGENT_NAME", "Test Agent")
	t.Setenv("GOOSAR_TASK_ID", "task-1")
	t.Setenv("GOOSAR_TOKEN", "mat_repo_checkout_test")
	t.Setenv("GOOSAR_REPO_CHECKOUT_MODE", "isolated")
...
func TestRunRepoCheckoutRequiresTaskCredential(t *testing.T) {
	t.Setenv("GOOSAR_DAEMON_PORT", "12345")
	t.Setenv("GOOSAR_TOKEN", "")

	err := runRepoCheckout(&cobra.Command{}, []string{"https://github.com/org/repo.git"})
	if err == nil || !strings.Contains(err.Error(), "GOOSAR_TOKEN not set") {
