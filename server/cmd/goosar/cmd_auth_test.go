...
func TestResolveAppURL(t *testing.T) {
	...
	t.Run("prefers GOOSAR_APP_URL", func(t *testing.T) {
		t.Setenv("GOOSAR_APP_URL", "http://localhost:14000")
...
func TestBrowserLoginInstructionsSSHRemoteHint(t *testing.T) {
	const loginURL = "https://goosar.ru/login?cli_callback=http%3A%2F%2Flocalhost%3A43689%2Fcallback"
...
func TestLoginTokenFlagWiring(t *testing.T) {
	...
	if tokenFlag.NoOptDefVal != tokenPromptSentinel {
		t.Fatalf("loginCmd --token NoOptDefVal = %q, want %q (legacy `goosar login --token` prompt mode would break)", tokenFlag.NoOptDefVal, tokenPromptSentinel)
	}
}
...
func TestLoginTokenHelpOutputRendersCleanly(t *testing.T) {
	...
	if want := "чтобы ввести его в диалоге."; !strings.Contains(help, want) {
...
func TestLoginTokenFlagParsing(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			argv: []string{"--token", auth.PATPrefix + "xxx"},
			want: want{changed: true, resolvedToken: auth.PATPrefix + "xxx"},
		},
		{
			name: "equals-separated value",
			argv: []string{"--token=" + auth.PATPrefix + "yyy"},
			want: want{changed: true, resolvedToken: auth.PATPrefix + "yyy"},
		},
		{
...
func TestValidateLoginTokenPrefix(t *testing.T) {
	cases := []struct {
		...
		{name: "gsl_ PAT", token: auth.PATPrefix + "abc123", wantErr: false},
		{name: "gsln_ Cloud Node PAT", token: auth.CloudPATPrefix + "abc123", wantErr: false},
		...
		{name: "case-sensitive: GSL_ rejected", token: "GSL_abc123", wantErr: true},
		{name: "leading whitespace not allowed (callers TrimSpace first)", token: " " + auth.PATPrefix + "abc", wantErr: true},
	}
	for _, tc := range cases {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
	...
	for _, p := range []string{auth.PATPrefix, auth.CloudPATPrefix} {
