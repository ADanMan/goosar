...
func TestNewAPIClient_WorkdirParentEscapeFailsClosed(t *testing.T) {
	...
	if err := cli.SaveCLIConfig(cli.CLIConfig{Token: "gsl_owner_pat"}); err != nil {
	...
	t.Setenv("GOOSAR_DAEMON_PORT", "")
	t.Setenv("GOOSAR_TOKEN", "")
	t.Setenv("GOOSAR_SERVER_URL", "http://127.0.0.1:8080")
...
func TestResolveWorkspaceID_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("agent context with explicit env uses env", func(t *testing.T) {
		t.Setenv("GOOSAR_AGENT_ID", "agent-123")
		t.Setenv("GOOSAR_TASK_ID", "task-456")
		t.Setenv("GOOSAR_DAEMON_PORT", "")
		t.Setenv("GOOSAR_WORKSPACE_ID", "env-ws")
	...
	t.Run("daemon port marker also skips config", func(t *testing.T) {
		t.Setenv("GOOSAR_AGENT_ID", "")
		t.Setenv("GOOSAR_TASK_ID", "")
		t.Setenv("GOOSAR_DAEMON_PORT", "27182")
		t.Setenv("GOOSAR_WORKSPACE_ID", "")
...
func TestResolveToken_AgentContextSkipsConfig(t *testing.T) {
	...
	if err := cli.SaveCLIConfig(cli.CLIConfig{Token: "gsl_profile_token"}); err != nil {
		t.Fatalf("seed config: %v", err)
	}
	...
	t.Run("outside agent context falls back to config", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "gsl_profile_token" {
	...
	t.Run("agent context without env never reads config", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "" {
			t.Fatalf("resolveToken() = %q, want empty in agent context without GOOSAR_TOKEN", got)
		}
	})
	...
	t.Run("daemon port marker without env never reads config", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "" {
			t.Fatalf("resolveToken() = %q, want empty in daemon-managed context without GOOSAR_TOKEN", got)
		}
	})
	...
	t.Run("agent context uses explicit task token env", func(t *testing.T) {
		...
		t.Setenv("GOOSAR_TASK_ID", "task-456")
		t.Setenv("GOOSAR_DAEMON_PORT", "")
		t.Setenv("GOOSAR_TOKEN", "mat_task_token")

		if got := resolveToken(testCmd()); got != "mat_task_token" {
			t.Fatalf("resolveToken() = %q, want GOOSAR_TOKEN", got)
		}
	})
	...
	t.Run("daemon port set without agent context avoids config fallback", func(t *testing.T) {
		...
		t.Setenv("GOOSAR_TASK_ID", "")
		t.Setenv("GOOSAR_TOKEN", "")
		t.Setenv("GOOSAR_DAEMON_PORT", "19514")
	...
	t.Run("daemon port set with explicit task token uses task token", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "mat_task_token" {
			t.Fatalf("resolveToken() = %q, want GOOSAR_TOKEN (task token wins over daemon signal)", got)
		}
	})

	t.Run("GOOSAR_SERVER_URL alone does not block config fallback", func(t *testing.T) {
		t.Setenv("GOOSAR_AGENT_ID", "")
		t.Setenv("GOOSAR_TASK_ID", "")
		t.Setenv("GOOSAR_TOKEN", "")
		t.Setenv("GOOSAR_DAEMON_PORT", "")
		t.Setenv("GOOSAR_SERVER_URL", "https://goosar.ru")
...
func TestNewAPIClient_AgentContextRequiresTaskToken(t *testing.T) {
	t.Setenv("GOOSAR_SERVER_URL", "http://127.0.0.1:8080")
	t.Setenv("GOOSAR_WORKSPACE_ID", "workspace-123")
	t.Setenv("GOOSAR_AGENT_ID", "agent-123")
	t.Setenv("GOOSAR_TASK_ID", "task-456")
	...
	t.Run("member token fails closed", func(t *testing.T) {
		t.Setenv("GOOSAR_TOKEN", "gsl_member_token")
...
func TestNewAPIClient_DaemonPortRequiresTaskToken(t *testing.T) {
	...
	if err := cli.SaveCLIConfig(cli.CLIConfig{Token: "gsl_profile_token", WorkspaceID: "config-file-ws"}); err != nil {
		t.Fatalf("seed config: %v", err)
	}
...
func TestAgentUpdateNoFieldsErrorPointsAtEnvCommand(t *testing.T) {
	...
	t.Setenv("GOOSAR_WORKSPACE_ID", "test-ws")
	...
	сообщение := err.Error()
	if !strings.Contains(сообщение, "goosar agent env set") {
		t.Fatalf("no-fields error must direct users to `goosar agent env set`; got: %q", сообщение)
	}
}
...
func TestAgentUpdateDoesNotExposeCustomEnvFlags(t *testing.T) {
	for _, flag := range []string{"custom-env", "custom-env-stdin", "custom-env-file"} {
		if agentUpdateCmd.Flag(flag) != nil {
			t.Errorf("agent update must NOT expose --%s after MUL-2600; use `goosar agent env set` instead", flag)
		}
	}
}
...
func TestParseCustomEnvErrorSanitization(t *testing.T) {
	...
	secretish := `{"SECRET_TOKEN":verySensitiveValue}`
	...
	for _, leak := range []string{"SECRET_TOKEN", "verySensitiveValue"} {
		if strings.Contains(сообщение, leak) {
			t.Fatalf("parseCustomEnv error leaked input fragment %q: %q", leak, сообщение)
		}
	}
}

func TestParseCustomArgsErrorSanitization(t *testing.T) {
	secretish := `["--api-key=verySensitiveValue", oops]`
	...
	for _, leak := range []string{"--api-key", "verySensitiveValue", "oops"} {
		if strings.Contains(сообщение, leak) {
			t.Fatalf("parseCustomArgs error leaked input fragment %q: %q", leak, сообщение)
		}
	}
}
...
func TestResolveCustomEnv(t *testing.T) {
	...
	origStdinCheck := stdinHasNoPipedInput
	stdinHasNoPipedInput = func() (bool, error) { return false, nil }
	t.Cleanup(func() { stdinHasNoPipedInput = origStdinCheck })

	t.Run("no piped input fails fast", func(t *testing.T) {
		cmd := freshAgentEnvSetCmd()
		if err := cmd.Flags().Set("custom-env-stdin", "true"); err != nil {
			t.Fatal(err)
		}
		orig := stdinHasNoPipedInput
		stdinHasNoPipedInput = func() (bool, error) { return true, nil }
		defer func() { stdinHasNoPipedInput = orig }()
		if _, _, err := resolveCustomEnv(cmd); err == nil {
			t.Fatal("expected fail-fast on no piped input")
		}
	})
...
func TestParseMcpConfigErrorSanitization(t *testing.T) {
	secretish := `{"mcpServers":{"x":{"env":{"TOKEN":verySensitiveValue}}}}`
	...
	for _, leak := range []string{"TOKEN", "verySensitiveValue", "mcpServers"} {
		if strings.Contains(сообщение, leak) {
			t.Fatalf("parseMcpConfig error leaked input fragment %q: %q", leak, сообщение)
		}
	}
}
...
func TestResolveMcpConfig(t *testing.T) {
	...
	t.Run("no piped input fails fast", func(t *testing.T) {
		...
		if err := cmd.Flags().Set("mcp-config-stdin", "true"); err != nil {
			t.Fatal(err)
		}
		orig := stdinHasNoPipedInput
		stdinHasNoPipedInput = func() (bool, error) { return true, nil }
		defer func() { stdinHasNoPipedInput = orig }()
		if _, _, err := resolveMcpConfig(cmd); err == nil {
			t.Fatal("expected fail-fast on no piped input")
		}
	})
