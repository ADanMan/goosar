...
func TestApplyConfigSetSupportsDaemonKeys(t *testing.T) {
	...
	pairs := []struct{ key, val string }{
		...
		{"runtime_e_semantic_inactivity_timeout", "15m"},
		{"runtime_e_handshake_timeout", "45s"},
	...
	if cfg.DeviceName != "vm-1-custom-name" ||
		...
		cfg.RuntimeESemanticInactivityTimeout != "15m" ||
		cfg.RuntimeEHandshakeTimeout != "45s" ||
...
func TestApplyConfigSetPositiveDurationRoundTripsToDaemonResolver(t *testing.T) {
	const envName = "TEST_GOOSAR_PERSISTED_DURATION"
	t.Setenv(envName, "")
	...
	cases := []struct {
		...
		{"runtime_e_semantic_inactivity_timeout", func(cfg cli.CLIConfig) string { return cfg.RuntimeESemanticInactivityTimeout }},
		{"runtime_e_handshake_timeout", func(cfg cli.CLIConfig) string { return cfg.RuntimeEHandshakeTimeout }},
...
func TestApplyConfigSetRejectsBadValues(t *testing.T) {
	...
	cases := []struct {
		...
		{"runtime-e semantic zero", "runtime_e_semantic_inactivity_timeout", "0s", "positive"},
		{"runtime-e handshake bad", "runtime_e_handshake_timeout", "10", "duration"},
