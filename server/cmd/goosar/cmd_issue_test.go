...
func TestResolveTextFlag(t *testing.T) {
	...
	t.Run("empty stdin error names the working alternatives", func(t *testing.T) {
		...
		pipeStdin(t, "", func() {
			...
			for _, want := range []string{"nothing was piped in", "--content-file", `--content "..."`} {
				if !strings.Contains(err.Error(), want) {
					t.Errorf("empty-stdin error should mention %s, got %v", want, err)
				}
			}
		})
	})

	t.Run("character-device stdin fails fast with the same actionable error", func(t *testing.T) {
		...
		var resultErr error
		go func() {
			_, _, resultErr = resolveTextFlag(c, "content")
			close(done)
		}()
		select {
		case <-done:
		case <-time.After(2 * time.Second):
			t.Fatal("resolveTextFlag did not return within 2s for a character-device stdin")
		}
		if resultErr == nil {
			t.Fatalf("expected an error for a character-device stdin")
		}
		for _, want := range []string{"nothing was piped in", "--content-file", `--content "..."`} {
			if !strings.Contains(resultErr.Error(), want) {
				t.Errorf("character-device stdin error should mention %s, got %v", want, resultErr)
			}
		}
	})

	t.Run("noInput result skips the blocking read entirely", func(t *testing.T) {
		r, w, err := os.Pipe()
		if err != nil {
			t.Fatalf("pipe: %v", err)
		}
		...
		origCheck := stdinHasNoPipedInput
		stdinHasNoPipedInput = func() (bool, error) { return true, nil }
		defer func() { stdinHasNoPipedInput = origCheck }()
		...
		case <-done:
		case <-time.After(2 * time.Second):
			t.Fatal("resolveTextFlag blocked on io.ReadAll despite a noInput=true stat result")
		}
		if resultErr == nil || !strings.Contains(resultErr.Error(), "nothing was piped in") {
			t.Errorf("expected the actionable empty-stdin error, got %v", resultErr)
		}
	})
...
func TestRunIssueCommentAddRejectsExternalAttachmentWithZeroUploads(t *testing.T) {
	...
	t.Setenv("GOOSAR_TOKEN", "mat_test-token")
...
func TestRunIssueCreateSendsAllowDuplicate(t *testing.T) {
	...
	t.Setenv("GOOSAR_SERVER_URL", srv.URL)
...
func TestRunIssueCreateSendsExistingAttachmentIDs(t *testing.T) {
	...
	t.Setenv("GOOSAR_WORKSPACE_ID", "ws-1")
	t.Setenv("GOOSAR_TOKEN", "test-token")
	t.Setenv("GOOSAR_QUICK_CREATE_ATTACHMENT_IDS", `["att-env","att-shared"]`)
...
func TestRunIssuePullRequestsListsLinkedPRsAsJSON(t *testing.T) {
		...
		case "/api/issues/issue-uuid/pull-requests":
			json.NewEncoder(w).Encode(map[string]any{
				"pull_requests": []map[string]any{
					{
						"url":    "https://github.com/adanman/goosar/pull/42",
						"number": float64(42),
						"state":  "open",
	...
	if pr["url"] != "https://github.com/adanman/goosar/pull/42" || pr["number"] != float64(42) || pr["state"] != "open" || pr["title"] != "MUL-2818 add issue PR CLI" {
...
func TestRunIssuePullRequestsTableIncludesCoreFields(t *testing.T) {
	...
	out, _ := io.ReadAll(r)
	text := string(out)
	for _, want := range []string{"NUMBER", "STATE", "TITLE", "URL", "42", "open", "MUL-2818 add issue PR CLI", "https://github.com/adanman/goosar/pull/42"} {
...
func TestResolveIssueRef(t *testing.T) {
	...
	t.Run("short UUID prefix is rejected without any HTTP call", func(t *testing.T) {
		...
		if сообщение := err.Error(); !strings.Contains(сообщение, "short UUID prefix") {
			t.Fatalf("expected error to flag the short-prefix case, got: %s", сообщение)
		}
		if сообщение := err.Error(); !strings.Contains(сообщение, "MUL-") {
			t.Fatalf("expected error to suggest the issue key form, got: %s", сообщение)
		}
		if len(hits) != 0 {
	...
	t.Run("short UUID prefix with dashes is rejected", func(t *testing.T) {
		...
		if сообщение := err.Error(); !strings.Contains(сообщение, "short UUID prefix") {
			t.Fatalf("expected short-prefix error, got: %s", сообщение)
		}
	})
	...
	t.Run("non-hex gibberish is rejected with key/UUID guidance", func(t *testing.T) {
		...
		if сообщение := err.Error(); strings.Contains(сообщение, "short UUID prefix") {
			t.Fatalf("non-hex input should not be classified as a short prefix; got: %s", сообщение)
		}
		if сообщение := err.Error(); !strings.Contains(сообщение, "not a recognized issue reference") {
			t.Fatalf("expected generic guidance error, got: %s", сообщение)
		}
	})
...
func TestRunIssueCommentListFlagGuards(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "thread + recent still rejected when --recent explicit zero",
			setup: func(c *cobra.Command) {
...
func TestRunIssueCommentList_ThreadTailPassesThroughAndPrintsReplyCursor(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodGet && strings.HasSuffix(r.URL.Path, "/comments") {
			...
			w.Header().Set("X-Goosar-Next-Before", "2026-01-01T00:00:00.000000001Z")
			w.Header().Set("X-Goosar-Next-Before-Id", "00000000-0000-0000-0000-000000000999")
			w.Write([]byte("[]"))
			return
		}
		t.Errorf("unexpected request: %s %s", r.Method, r.URL.String())
...
func TestRunIssueCommentList_RecentStillLabelsCursorAsThread(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("X-Goosar-Next-Before", "2026-01-01T00:00:00.000000001Z")
		w.Header().Set("X-Goosar-Next-Before-Id", "00000000-0000-0000-0000-000000000777")
		w.Write([]byte("[]"))
	}))
	defer srv.Close()
...
func TestRunIssueListRejectsInvalidSortAndDirection(t *testing.T) {
	t.Setenv("GOOSAR_SERVER_URL", "http://127.0.0.1:0")
	t.Setenv("GOOSAR_WORKSPACE_ID", "ws-1")
	t.Setenv("GOOSAR_TOKEN", "test-token")
...
func TestRunIssueReorderComputesPosition(t *testing.T) {
	tests := []struct {
		...
		{"top of column", "top", "true", 0},
		{"bottom of column", "bottom", "true", 10},
		{"before another issue", "before", "MUL-2", 5},
		{"after another issue", "after", "MUL-2", 10},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
