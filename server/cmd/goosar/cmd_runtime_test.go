...
func TestRunRuntimeDeleteConflictSuggestsCascade(t *testing.T) {
	...
	сообщение := err.Error()
	if !strings.Contains(сообщение, "Codex (agent-1)") || !strings.Contains(сообщение, "--cascade") {
		t.Fatalf("error = %q, want agent name and --cascade guidance", сообщение)
	}
}
...
func TestRunRuntimeDeleteCascadeSucceedsAgainstRealResponseShape(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	t.Setenv("GOOSAR_TOKEN", "test-token")
	...
	var deleteWorkspaceHeader, cascadeWorkspaceHeader string
	var gotExpectedIDs []string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case r.Method == http.MethodDelete && r.URL.Path == "/api/runtimes/rt-1":
			deleteWorkspaceHeader = r.Header.Get("X-Workspace-ID")
			w.WriteHeader(http.StatusConflict)
			_ = json.NewEncoder(w).Encode(map[string]any{
				"error": "cannot delete runtime: it has active agents bound to it. Archive them, or have each agent's owner reassign their agent.",
				"code":  "runtime_has_active_agents",
				"active_agents": []map[string]any{
					{
						"id": agentID1, "workspace_id": "ws-1", "runtime_id": "rt-1",
						"name": "Codex", "description": "", "instructions": "",
						"runtime_mode": "cloud", "runtime_config": nil, "custom_args": nil,
						"mcp_config": json.RawMessage("null"),
					},
					{
						"id": agentID2, "workspace_id": "ws-1", "runtime_id": "rt-1",
						"name": "Claude", "description": "", "instructions": "",
						"runtime_mode": "cloud", "runtime_config": nil, "custom_args": nil,
						"mcp_config": json.RawMessage("null"),
					},
				},
		...
		case r.Method == http.MethodPost && r.URL.Path == "/api/runtimes/rt-1/archive-agents-and-delete":
			cascadeWorkspaceHeader = r.Header.Get("X-Workspace-ID")
			var body struct {
				ExpectedActiveAgentIDs []string `json:"expected_active_agent_ids"`
			}
			if err := json.NewDecoder(r.Body).Decode(&body); err != nil {
	...
	if err != nil {
		t.Fatalf("runRuntimeDelete: %v (stdout: %s)", err, out)
	}

	if deleteWorkspaceHeader != "ws-1" || cascadeWorkspaceHeader != "ws-1" {
		t.Fatalf("X-Workspace-ID header mismatch: delete=%q cascade=%q, want ws-1 on both",
			deleteWorkspaceHeader, cascadeWorkspaceHeader)
	}
	wantIDs := []string{agentID1, agentID2}
	if strings.Join(gotExpectedIDs, ",") != strings.Join(wantIDs, ",") {
		t.Fatalf("expected_active_agent_ids = %#v, want %#v", gotExpectedIDs, wantIDs)
	}
	...
	if got["id"] != "rt-1" || got["deleted"] != true {
		t.Fatalf("stdout = %#v, want a successful cascade delete result", got)
	}
}
