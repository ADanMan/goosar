...
func TestResolveAgent(t *testing.T) {
	...
	t.Run("ambiguous match", func(t *testing.T) {
		_, err := resolveAgent(ctx, client, "a")
		if err == nil {
...
func TestRunAutopilotUpdateRejectsSubscriberAndClear(t *testing.T) {
	t.Setenv("GOOSAR_SERVER_URL", "http://127.0.0.1")
	t.Setenv("GOOSAR_WORKSPACE_ID", "ws-1")
	t.Setenv("GOOSAR_TOKEN", "test-token")
...
func TestUUIDRegexp(t *testing.T) {
	tests := []struct {
		...
		{"11111111-1111-1111-1111-1111111111111", false},
		{"", false},
	}
...
func autopilotGetFixture(t *testing.T, autopilotID string, trigger map[string]any) *httptest.Server {
	t.Helper()
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/api/autopilots/"+autopilotID {
			http.NotFound(w, r)
...
func TestRunAutopilotGetRedactsWebhookCredentialsByDefault(t *testing.T) {
	...
	autopilotGetFixture(t, autopilotID, map[string]any{
		"id": "trigger-1", "kind": "webhook",
		"webhook_token": webhookToken, "webhook_path": webhookPath, "webhook_url": webhookURL,
		"has_signing_secret": true, "signing_secret_hint": "abcd",
	})
...
func TestRunAutopilotGetShowSecretsIsExplicitAndWarns(t *testing.T) {
	...
	autopilotGetFixture(t, autopilotID, map[string]any{
		"id": "trigger-1", "kind": "webhook", "webhook_token": webhookToken,
	})
	...
	stderr.restore()
	if err != nil {
		t.Fatalf("runAutopilotGet --show-secrets: %v", err)
	}
	if !strings.Contains(out, webhookToken) {
		t.Fatalf("--show-secrets did not print the token:\n%s", out)
	}
	if !strings.Contains(stderr.out.String(), "Warning: --show-secrets") {
		t.Fatalf("--show-secrets printed no warning on stderr: %q", stderr.out.String())
	}

	table := newAutopilotGetTestCmd()
	_ = table.Flags().Set("show-secrets", "true")
	_ = table.Flags().Set("output", "table")
	if err := runAutopilotGet(table, []string{autopilotID}); err == nil || !strings.Contains(err.Error(), "--output json") {
		t.Fatalf("--show-secrets with --output table = %v, want an error naming --output json", err)
	}
}
