...
func TestResolveSkillContentFlagStdinNoBlock(t *testing.T) {
	t.Run("character-device stdin fails fast with the same actionable error", func(t *testing.T) {
		...
		var resultErr error
		go func() {
			_, _, resultErr = resolveSkillContentFlag(cmd)
			close(done)
		}()
		select {
		case <-done:
		case <-time.After(2 * time.Second):
			t.Fatal("resolveSkillContentFlag did not return within 2s for a character-device stdin")
		}
		if resultErr == nil {
	...
	t.Run("noInput result skips the blocking read entirely", func(t *testing.T) {
		...
		case <-done:
		case <-time.After(2 * time.Second):
			t.Fatal("resolveSkillContentFlag blocked on io.ReadAll despite a noInput=true stat result")
		}
	...
	t.Run("empty pipe still returns an empty error", func(t *testing.T) {
		cmd := newSkillCreateTestCmd()
		_ = cmd.Flags().Set("content-stdin", "true")
		pipeStdin(t, "", func() {
			_, _, err := resolveSkillContentFlag(cmd)
	...
	t.Run("non-empty pipe is read verbatim", func(t *testing.T) {
		cmd := newSkillCreateTestCmd()
		_ = cmd.Flags().Set("content-stdin", "true")
		body := "skill body\nwith two lines\n"
		pipeStdin(t, body, func() {
			got, ok, err := resolveSkillContentFlag(cmd)
			if err != nil || !ok {
				t.Fatalf("unexpected: ok=%v err=%v", ok, err)
			}
			if got != body {
				t.Errorf("got %q, want verbatim %q", got, body)
			}
		})
	})
}
