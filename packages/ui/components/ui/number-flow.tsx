...
import NumberFlowPrimitive, { NumberFlowGroup, type NumberFlowProps } from '@number-flow/react';
...
type CurrencyNumberFlowProps = Omit<NumberFlowProps, 'format' | 'prefix' | 'suffix' | 'value'> & {
  value: number;
  prefix?: string;
...
type CompactNumberFlowProps = Omit<NumberFlowProps, 'format' | 'suffix' | 'value'> & {
  value: number;
...
function CompactNumberFlow({
  ...
  const fractionDigits = divisor > 1 && Math.abs(scaledValue % 1) >= 0.05 ? 1 : 0;
  const suffix =
    divisor === 1_000_000
      ? suffixCase === 'upper'
        ? 'M'
...
type CappedNumberFlowProps = Omit<NumberFlowProps, 'format' | 'suffix' | 'value'> & {
  value: number;
  max?: number;
};
...
export { CappedNumberFlow, CompactNumberFlow, CurrencyNumberFlow, NumberFlow, NumberFlowGroup };
export type {
  CappedNumberFlowProps,
  CompactNumberFlowProps,
  CurrencyNumberFlowProps,
