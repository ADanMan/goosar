...
import { issueChangedDims, issueMatchesListFilter, listFilterDependsOn } from './membership';
...
describe('issueMatchesListFilter', () => {
  ...
  it('degrades to unknown when the entity is missing the filtered field', () => {
    expect(issueMatchesListFilter({ title: 'partial' }, 'assigned', { assignee_id: 'me' })).toBe(
      'unknown',
    );
  });
  ...
  it('judges project filters', () => {
    expect(issueMatchesListFilter(makeIssue(), 'project:p1', { project_id: 'p1' })).toBe(true);
    expect(
  ...
  it('never decides involves_user_id — the ownership graph is server-side', () => {
    expect(issueMatchesListFilter(makeIssue(), 'agents', { involves_user_id: 'me' })).toBe(
      'unknown',
    );
  });
  ...
  it('ANDs across fields — a definitive miss beats an unknown', () => {
    expect(
      issueMatchesListFilter(makeIssue({ project_id: 'p2' }), 'scoped', {
        project_id: 'p1',
        involves_user_id: 'me',
      }),
    ).toBe(false);
...
describe('listFilterDependsOn', () => {
  ...
  it('assignee-keyed filters react to assignee changes', () => {
    ...
    expect(listFilterDependsOn('assigned', { assignee_id: 'me' }, { ...none, project: true })).toBe(
      false,
    );
  });
