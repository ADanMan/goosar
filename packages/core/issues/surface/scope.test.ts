...
import { UnsupportedIssueScopeError, issueScopeKey } from './scope';
...
describe('issue surface scope', () => {
  it('builds stable surface keys', () => {
    ...
    expect(issueScopeKey({ type: 'workspace', actorKind: 'agents' })).toBe('workspace:agents');
    expect(issueScopeKey({ type: 'my', relation: 'assigned', userId: 'u1' })).toBe(
      'my:u1:assigned',
    );
    expect(issueScopeKey({ type: 'project', projectId: 'p1' })).toBe('project:p1');
    expect(
      issueScopeKey({
        type: 'actor',
        actorType: 'agent',
  ...
  it('builds the workspace query plan', () => {
    ...
    expect(buildIssueSurfaceQueryPlan({ type: 'workspace', actorKind: 'agents' })).toMatchObject({
      kind: 'scoped',
    ...
    expect(buildIssueSurfaceQueryPlan({ type: 'workspace', actorKind: 'members' })).toMatchObject({
      kind: 'scoped',
  ...
  it('builds project and actor query plans', () => {
    expect(buildIssueSurfaceQueryPlan({ type: 'project', projectId: 'p1' })).toMatchObject({
      scopeKey: 'project:p1',
  ...
  it('throws for team until the issue API has a team filter', () => {
    ...
    expect(() => buildIssueSurfaceQueryPlan(scope)).toThrow(UnsupportedIssueScopeError);
  });
});
