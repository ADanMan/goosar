...
import type { Issue, ListIssuesCache, ListIssuesResponse } from '../types';
...
function collectParentId(parentIssueIds: Set<string>, parentId: string | null | undefined) {
...
function collectParentFromListCache(
  ...
  const parentId = data ? findIssueLocation(data, issueId)?.issue.parent_issue_id : undefined;
...
export function collectDeletedIssueCacheMetadata(
  ...
  for (const [, data] of qc.getQueriesData<InfiniteData<ListIssuesResponse, number>>({
...
export function pruneDeletedIssueFromListCaches(qc: QueryClient, wsId: string, issueId: string) {
  ...
  for (const [key, data] of qc.getQueriesData<InfiniteData<ListIssuesResponse, number>>({
    queryKey: issueKeys.flatAll(wsId),
  })) {
    if (!data?.pages) continue;
    const found = data.pages.some((page) => page.issues.some((issue) => issue.id === issueId));
    if (!found) continue;
...
export function invalidateDeletedIssueDependentCaches(qc: QueryClient, wsId: string) {
...
export function invalidateIssueScopedCaches(qc: QueryClient, wsId: string, issueId: string) {
