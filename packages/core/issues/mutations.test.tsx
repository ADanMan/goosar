...
function makeInboxItem(id: string, issueId: string, overrides: Partial<InboxItem> = {}): InboxItem {
  return {
    id,
    workspace_id: WS_ID,
...
describe('useLoadMoreByStatus', () => {
  ...
  it('targets the sorted cache key and forwards sort to the API', async () => {
    ...
    const { result } = renderHook(() => useLoadMoreByStatus('todo', undefined, sort), {
  ...
  it('ignores a stale cache entry under a different sort', async () => {
    ...
    const { result } = renderHook(() => useLoadMoreByStatus('todo', undefined, activeSort), {
      wrapper: createWrapper(qc),
    });
    ...
    expect(active?.byStatus.todo?.issues.map((i) => i.id)).toEqual(['issue-1', 'issue-2']);
  ...
  it('targets the myList scoped cache when myIssues is provided', async () => {
    ...
    const { result } = renderHook(() => useLoadMoreByStatus('in_progress', myIssues, sort), {
      wrapper: createWrapper(qc),
    });
  ...
  it('targets the project surface cache when myIssues uses a project scope key', async () => {
    ...
    const { result } = renderHook(() => useLoadMoreByStatus('todo', myIssues, sort), {
      wrapper: createWrapper(qc),
    });
    ...
    const updated = qc.getQueryData<ListIssuesCache>(activeKey);
    expect(updated?.byStatus.todo?.issues.map((i) => i.id)).toEqual(['issue-1', 'issue-2']);
  });
  ...
  it('works with no sort (matches the {} key used by sort-less callers)', async () => {
    ...
    const { result } = renderHook(() => useLoadMoreByStatus('todo', myIssues), {
      wrapper: createWrapper(qc),
    });
...
describe('useLoadMoreByAssigneeGroup', () => {
  ...
  it('forwards sort to the grouped API and appends into the right group', async () => {
    ...
    expect(updated?.groups[0]?.issues.map((i) => i.id)).toEqual(['issue-1', 'issue-2']);
  });
});
...
describe('useUpdateIssue — optimistic move keeps every bucketed board in sync', () => {
  ...
  function inboxStatus(issueId: string) {
    return qc.getQueryData<InboxItem[]>(inboxKey)?.find((item) => item.issue_id === issueId)
      ?.issue_status;
  }
  ...
  it('uses server move intent while keeping provisional position optimistic-only', async () => {
    moveIssue.mockResolvedValue(makeIssue(1, { status: 'in_progress', position: 15 }));
    const { result } = renderHook(() => useUpdateIssue(), {
      wrapper: createWrapper(qc),
    });
    ...
    expect(qc.getQueryData<ListIssuesCache>(wsKey)?.byStatus.in_progress?.issues[0]?.position).toBe(
      15,
    );
  });
  ...
  it('rolls the linked inbox row status back when the request fails', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ id: 'issue-1', status: 'in_progress' }).catch(() => {});
    });
  ...
  it('rolls the membership removal back when a project move fails', async () => {
    ...
    await act(async () => {
      await result.current.mutateAsync({ id: 'issue-1', project_id: 'project-9' }).catch(() => {});
    });
...
describe("useUpdateIssue — detaching a sub-issue prunes the old parent's children cache", () => {
  ...
  beforeEach(() => {
    ...
    qc.setQueryData<Issue[]>(childKey, [child, makeIssue(2, { parent_issue_id: PARENT_ID })]);
  });
  ...
  it('keeps the issue under its parent for a non-reparenting update', async () => {
    updateIssue.mockResolvedValue(makeIssue(1, { parent_issue_id: PARENT_ID, status: 'done' }));
