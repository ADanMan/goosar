...
describe('issue date-only helpers', () => {
  ...
  it('formats a date-only string timezone-safely (no day shift)', () => {
    expect(formatDateOnly('2026-03-01', { month: 'short', day: 'numeric' }, 'en-US')).toBe('Mar 1');
    expect(formatDateOnly(null, { month: 'short', day: 'numeric' }, 'en-US')).toBe('');
    expect(formatDateOnly('', { month: 'short', day: 'numeric' }, 'en-US')).toBe('');
  });

  it("names the month in the locale it is given, not the machine's", () => {
    expect(formatDateOnly('2026-03-01', { month: 'long' }, 'ru')).toBe('март');
    expect(formatDateOnly('2026-03-01', { month: 'long' }, 'en-US')).toBe('March');
  });
  ...
  it('round-trips a picked day back to the same displayed day', () => {
    ...
    expect(formatDateOnly(stored, { month: 'short', day: 'numeric' }, 'en-US')).toBe('Mar 1');
  });

  it('anchors a date-only value at UTC midnight', () => {
    expect(dateOnlyToUTCDate('2026-03-01')?.toISOString()).toBe('2026-03-01T00:00:00.000Z');
