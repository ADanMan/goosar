...
import type { InboxItem, Issue, IssueTableRowsResponse, ListIssuesCache } from '../types';
...
const flatSearchKey = issueKeys.flat(WS_ID, 'workspace:all', { q: 'Issue 1' }, sort);
const tableRowKey = [
  ...issueKeys.tableRows(
    WS_ID,
    {
...
const flatUpdatedSortKey = issueKeys.flat(WS_ID, 'workspace:all', {}, updatedSort);
...
describe('applyIssueChange', () => {
  ...
  it('plain field change: patches in place everywhere, no removals, no stale keys', () => {
    ...
    expect(qc.getQueryData<Issue>(issueKeys.detail(WS_ID, 'issue-1'))?.title).toBe('renamed');
  ...
  it('patches a loaded flat row without refetching an unrelated position window', () => {
    ...
    expect(qc.getQueryData<IssueFlatCache>(flatKey)?.pages[0]?.issues[0]?.title).toBe(
      'renamed in table',
    );
    expect(result.staleKeys.map(hashKey)).not.toContain(hashKey(flatKey));

    rollbackIssueChange(qc, WS_ID, 'issue-1', result);
    expect(qc.getQueryData<IssueFlatCache>(flatKey)?.pages[0]?.issues[0]?.title).toBe('Issue 1');
  });
  ...
  it('patches and rolls back loaded server Table rows', () => {
    ...
    expect(qc.getQueryData<IssueTableRowsResponse>(tableRowKey)?.rows[0]?.issue.title).toBe(
      'optimistic table title',
    );
  ...
  it('marks only flat windows whose sort or facet depends on the changed field', () => {
    for (const key of [flatKey, flatTitleKey, flatFilteredKey, flatUpdatedWindowKey]) {
  ...
  it('status change: rebuckets loaded cards, patches inbox, adjusts counts for absent-but-member lists', () => {
    ...
    expect(qc.getQueryData<InboxItem[]>(inboxKey)?.[0]?.issue_status).toBe('in_progress');
  ...
  it('assignee change me→bob: removes from my-assigned (total decremented), keeps members tab, flags union/involves scopes', () => {
    ...
    expect(
      qc.getQueryData<ListIssuesCache>(membersKey)?.byStatus.todo?.issues[0]?.assignee_id,
    ).toBe('bob');
...
describe('invalidateUpdatedAtSortedIssueLists', () => {
  ...
  it('invalidates updated_at-sorted assignee-grouped boards (workspace + My Issues)', () => {
    ...
    expect(qc.getQueryState(myAssigneeGroupsUpdatedKey)?.isInvalidated).toBe(true);
    expect(qc.getQueryState(assigneeGroupsPositionKey)?.isInvalidated).toBe(false);
  });
