...
export const ISSUE_SURFACE_VIEW_STORAGE_KEY = 'goosar_issue_surface_views';

type PersistedIssueViewState = ReturnType<ReturnType<typeof viewStorePersistOptions>['partialize']>;
...
function resetStoreFromRegistry(surfaceKey: string, store: StoreApi<IssueViewState>) {
  const persisted = issueSurfaceViewRegistryStore.getState().surfaces[surfaceKey]?.state;
...
export function getIssueSurfaceViewStore(surfaceKey: string): StoreApi<IssueViewState> {
