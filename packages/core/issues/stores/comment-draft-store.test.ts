...
describe('comment draft store — attachments in the draft', () => {
  ...
  it('persists attachments alongside content under the same key', () => {
    const { setDraft, setAttachments, getDraft, getAttachments } = useCommentDraftStore.getState();
  ...
  it('setDraft preserves already-uploaded attachments', () => {
    const { setAttachments, setDraft, getAttachments } = useCommentDraftStore.getState();
  ...
  it('keeps an attachment-only draft (empty text) instead of dropping it', () => {
    const { setAttachments, setDraft, getAttachments, getDraft } = useCommentDraftStore.getState();
...
describe('comment draft store — prune on rehydrate', () => {
  const KEY = 'goosar_comment_drafts:acme';
  ...
  it('drops an upload that was in flight at reload', async () => {
    seed({
      'new:issue-1': {
        content: '',
        attachments: [{ clientUploadId: 'c1', status: 'uploading', filename: 'shot.png', size: 9 }],
        updatedAt: Date.now(),
      },
    });
