...
describe('issue surface view store registry', () => {
  it('isolates view state by surface key inside one workspace registry', async () => {
    ...
    expect(parsed.state.surfaces['project:a'].state.priorityFilters).toEqual(['high']);
  ...
  it('persists table columns, order, widths, grouping, and calculation per surface', async () => {
    ...
    projectA.getState().reorderTableColumn('property:estimate', 'identifier');
    ...
    expect(projectA.getState().tableColumns.map((column) => column.key)).toEqual([
      'title',
      'status',
      'priority',
      'assignee',
    ...
    expect(
      projectA.getState().tableColumns.find((column) => column.key === 'property:estimate')?.width,
    ).toBe(184);
    ...
    expect(projectB.getState().viewMode).toBe('board');
    expect(
      projectB.getState().tableColumns.some((column) => column.key.startsWith('property:')),
    ).toBe(false);

    const raw = localStorage.getItem(`${ISSUE_SURFACE_VIEW_STORAGE_KEY}:acme`);
    const parsed = JSON.parse(raw as string);
    expect(parsed.state.surfaces['project:table-a'].state.tableColumns).toContainEqual({
      key: 'property:estimate',
      width: 184,
    });
  });
  ...
  it('clears one surface without touching sibling surfaces', async () => {
    ...
    expect(getIssueSurfaceViewStateRegistrySnapshot()['project:b']?.state.viewMode).toBe('gantt');
  });
  ...
  it('prunes invalid surfaces and resets live stores for pruned keys', async () => {
    ...
    expect(projectA.getState().viewMode).toBe('list');
    expect(projectB.getState().viewMode).toBe('board');
    expect(getIssueSurfaceViewStateRegistrySnapshot()['project:a']?.state.viewMode).toBe('list');
