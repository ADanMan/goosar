...
function installFakeChildApi(listChildIssues: (parentId: string) => Promise<{ issues: Issue[] }>) {
...
describe('childIssuesOptions', () => {
  it('refetches a cached snapshot when the parent issue is opened again', async () => {
    ...
    const observer = new QueryObserver(qc, childIssuesOptions(WS_ID, parentId));
...
describe('issueTableRowPageOptions', () => {
  ...
  function installFakeTableRowsApi(
    listIssueTableRows: (params: IssueTableRowsRequest) => Promise<IssueTableRowsResponse>,
  ) {
  ...
  it('refetches an invalidated page when its observer reattaches', async () => {
    ...
    const listIssueTableRows = vi
      .fn<(params: IssueTableRowsRequest) => Promise<IssueTableRowsResponse>>()
    ...
    await vi.waitFor(() => {
      ...
      expect(rowIssueIds(observer2.getCurrentResult().data)).toEqual(['issue-1', 'issue-2']);
    });
...
describe('projectGanttIssuesOptions', () => {
  ...
  it('loops through pages until total is satisfied (no silent truncation)', async () => {
    ...
    const firstPage = Array.from({ length: PROJECT_GANTT_PAGE_LIMIT }, (_, i) => makeIssue(i));
    const secondPage = Array.from({ length: 7 }, (_, i) => makeIssue(PROJECT_GANTT_PAGE_LIMIT + i));
    ...
    const listIssues = vi
      ...
      .mockImplementation(async (params) => {
        ...
        if (offset === PROJECT_GANTT_PAGE_LIMIT) return { issues: secondPage, total };
...
describe('flat issue table queries', () => {
  ...
  it('walks every page only for an explicit full CSV export', async () => {
    const first = Array.from({ length: ISSUE_FLAT_PAGE_SIZE }, (_, index) => makeIssue(index + 1));
    ...
    const issues = await qc.fetchQuery(
      issueFlatExportOptions(WS_ID, 'project:project-1', { project_id: PROJECT_ID }, undefined, {
        sort_by: 'status',
        sort_direction: 'asc',
      }),
    );
  ...
  it('does not silently truncate exports above ten thousand issues', async () => {
    ...
    const listIssues = vi
      ...
      .mockImplementation(async (params) => {
        ...
        return {
          issues: Array.from({ length: count }, (_, index) => makeIssue(offset + index + 1)),
          total,
        };
      });
    installFakeApi(listIssues);
  ...
  it('keeps paging when the server returns fewer rows than requested', async () => {
    ...
    expect(issues).toHaveLength(total);
    expect(listIssues.mock.calls.map(([params]) => params?.offset)).toEqual([0, 40, 80]);
  });

  it('fails explicitly if the export endpoint stops advancing offsets', async () => {
    const page = Array.from({ length: ISSUE_FLAT_PAGE_SIZE }, (_, index) => makeIssue(index + 1));
    const listIssues = vi
      .fn<(params?: ListIssuesParams) => Promise<ListIssuesResponse>>()
    ...
    await expect(
      qc.fetchQuery(issueFlatExportOptions(WS_ID, 'workspace:all', {}, undefined)),
  ...
  it('deduplicates the three My Issues relations and restores global sort order', async () => {
    ...
    const issues = await qc.fetchQuery(
      issueFlatExportOptions(WS_ID, 'all', {}, 'user-1', {
        sort_by: 'status',
        sort_direction: 'asc',
      }),
    );

    expect(issues.map((issue) => issue.id)).toEqual([backlog.id, todo.id, shared.id]);
...
describe('childrenByParentsOptions chunking', () => {
  ...
  it('merges children from all chunks into one grouped map', async () => {
    ...
    const grouped = await qc.fetchQuery(childrenByParentsOptions(WS_ID, parentIds, qc));
...
describe('issueIdentifierOptions', () => {
  ...
  it('returns the issue whose identifier exactly matches the query', async () => {
    ...
    expect(searchIssues).toHaveBeenCalledWith(expect.objectContaining({ q: 'MUL-7' }));
  });
  ...
  it('keys the query by workspace and identifier', () => {
    expect(issueKeys.identifier(WS_ID, 'MUL-7')).toEqual(['issues', WS_ID, 'identifier', 'MUL-7']);
  });
});
