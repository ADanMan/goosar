...
import { ALL_ACCESS_SCOPES, effectiveAccessScope, isAccessChangeReady } from './effective-access';
...
describe('ALL_ACCESS_SCOPES', () => {
  it('lists the three scopes in display order', () => {
    expect(ALL_ACCESS_SCOPES).toEqual(['workspace', 'specific-people', 'owner-only']);
  });
});
...
describe('isAccessChangeReady', () => {
  ...
  it('private with no targets is ready (clears targets)', () => {
    expect(isAccessChangeReady({ permission_mode: 'private', invocation_targets: [] })).toBe(true);
  });
  ...
  it('public_to with zero targets is NOT ready', () => {
    expect(isAccessChangeReady({ permission_mode: 'public_to', invocation_targets: [] })).toBe(
      false,
    );
  });
});
