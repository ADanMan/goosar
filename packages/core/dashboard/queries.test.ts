...
function resolvePlaceholder(
  ...
  )(previousData, previousKey ? { queryKey: previousKey } : undefined);
}
...
describe('dashboard range placeholders', () => {
  it.each(optionBuilders)(
    ...
    (buildOptions) => {
      ...
      expect(resolvePlaceholder(nextOptions, previous, previousOptions.queryKey)).toBe(previous);
    },
  );

  it('does not carry placeholder data across workspace, project, or timezone scopes', () => {
    const previous = [{ sentinel: 'previous-scope' }];
    const nextOptions = dashboardUsageDailyOptions('ws-1', 7, 'project-1', 'Asia/Shanghai');
    const previousScopes = [
      dashboardUsageDailyOptions('ws-2', 30, 'project-1', 'Asia/Shanghai'),
    ...
    for (const previousOptions of previousScopes) {
      expect(resolvePlaceholder(nextOptions, previous, previousOptions.queryKey)).toBeUndefined();
    }
  });
