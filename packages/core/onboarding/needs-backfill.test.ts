...
import { needsSourceBackfill, SOURCE_BACKFILL_MAX_DISMISSALS } from './needs-backfill';
...
describe('needsSourceBackfill', () => {
  ...
  it('returns false once dismissCount hits the cap', () => {
    const user = makeUser({ onboarding_questionnaire: {} });
    expect(needsSourceBackfill(user, SOURCE_BACKFILL_MAX_DISMISSALS)).toBe(false);
    expect(needsSourceBackfill(user, SOURCE_BACKFILL_MAX_DISMISSALS + 5)).toBe(false);
  });

  it('still returns true just below the dismiss cap', () => {
    const user = makeUser({ onboarding_questionnaire: {} });
    expect(needsSourceBackfill(user, SOURCE_BACKFILL_MAX_DISMISSALS - 1)).toBe(true);
  });
  ...
  it('treats malformed (number, null) source as missing', () => {
    expect(needsSourceBackfill(makeUser({ onboarding_questionnaire: { source: 42 } }), 0)).toBe(
      true,
    );
    expect(needsSourceBackfill(makeUser({ onboarding_questionnaire: { source: null } }), 0)).toBe(
      true,
    );
  });
});
