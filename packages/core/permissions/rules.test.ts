...
describe('canAssignAgentToIssue', () => {
  const workspaceTargets = [{ target_type: 'workspace' as const, target_id: null }];
  ...
  it('does not throw when invocation_targets is undefined', () => {
    const a = makeAgent({
      permission_mode: 'public_to',
      invocation_targets: undefined as unknown as Agent['invocation_targets'],
      owner_id: ALICE,
    });
    expect(() => canAssignAgentToIssue(a, { userId: BOB, role: 'member' })).not.toThrow();
    const d = canAssignAgentToIssue(a, { userId: BOB, role: 'member' });
    expect(d.allowed).toBe(false);
    expect(d.reason).toBe('private_visibility');
    expect(canAssignAgentToIssue(a, { userId: ALICE, role: 'member' }).allowed).toBe(true);
...
describe('canGrantPerimeterAccess', () => {
  const ctxOwner = { userId: ALICE, role: 'owner' as const };
  const ctxAdmin = { userId: ALICE, role: 'admin' as const };
  const ctxMember = { userId: ALICE, role: 'member' as const };
  ...
  it('non-managers cannot grant', () => {
    const d = canGrantPerimeterAccess(targetMember, ctxMember);
    expect(d.allowed).toBe(false);
    expect(d.reason).toBe('not_admin_role');
  });
  it("admin cannot change an owner's grant", () => {
    const d = canGrantPerimeterAccess(targetOwner, ctxAdmin);
    expect(d.allowed).toBe(false);
    expect(d.reason).toBe('owner_grant_protected');
  });
  it('admin can grant admins and members (including self)', () => {
    expect(canGrantPerimeterAccess(targetAdmin, ctxAdmin).allowed).toBe(true);
    expect(canGrantPerimeterAccess(targetMember, ctxAdmin).allowed).toBe(true);
  });
  it("owner can change an owner's grant — no last-owner analogue", () => {
    expect(canGrantPerimeterAccess(targetOwner, ctxOwner).allowed).toBe(true);
  });
  it('role null (loading / not a member) denies gracefully', () => {
    expect(canGrantPerimeterAccess(targetMember, { userId: ALICE, role: null }).allowed).toBe(
      false,
    );
  });
});
