  ...
  return useMutation({
    ...
    onSuccess: (label) => {
      qc.setQueryData<ListLabelsResponse>(
        labelKeys.list(wsId, label.resource_type ?? 'issue'),
        (old) =>
...
export function useAttachResourceLabel(resourceType: 'agent' | 'skill', resourceId: string) {
  const qc = useQueryClient();
  const wsId = useWorkspaceId();
  return useMutation({
    mutationFn: (labelId: string) => api.attachLabelToResource(resourceType, resourceId, labelId),
    onSuccess: (data: ResourceLabelsResponse) => {
      qc.setQueryData(labelKeys.byResource(wsId, resourceType, resourceId), data);
    },
    onSettled: () => {
      qc.invalidateQueries({ queryKey: labelKeys.all(wsId) });
...
export function useDetachResourceLabel(resourceType: 'agent' | 'skill', resourceId: string) {
  const qc = useQueryClient();
  const wsId = useWorkspaceId();
  return useMutation({
    mutationFn: (labelId: string) => api.detachLabelFromResource(resourceType, resourceId, labelId),
    onSuccess: (data: ResourceLabelsResponse) => {
      qc.setQueryData(labelKeys.byResource(wsId, resourceType, resourceId), data);
    },
    onSettled: () => {
