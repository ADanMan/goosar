...
import type { Issue, IssueLabelsResponse, Label, ListLabelsResponse } from '../types';
...
describe('issue label mutations', () => {
  ...
  it('prevents an older children fetch from overwriting an optimistic attach', async () => {
    ...
    await waitFor(() => expect(qc.getQueryState(childKey)?.fetchStatus).toBe('fetching'));
    ...
    await waitFor(() =>
      expect(qc.getQueryData<Issue[]>(childKey)?.[0]?.labels).toEqual([labelA, labelB]),
    );
  ...
  it('prevents an older children fetch from overwriting an optimistic detach', async () => {
    ...
    act(() => result.current.mutate(labelB.id));
    await waitFor(() => expect(qc.getQueryData<Issue[]>(childKey)?.[0]?.labels).toEqual([labelA]));
