...
export function useActorName() {
  ...
  const getSquadName = useCallback(
    (squadId: string) => {
  ...
  const getActorAvatarUrl = useCallback(
    (type: string, id: string): string | null => {
      if (type === 'member')
        return resolvePublicFileUrl(members.find((m) => m.user_id === id)?.avatar_url);
      if (type === 'agent')
        return resolvePublicFileUrl(agents.find((a) => a.id === id)?.avatar_url);
      if (type === 'squad')
        return resolvePublicFileUrl(squads.find((s) => s.id === id)?.avatar_url);
      return null;
    },
    [agents, members, squads],
  );
  ...
  return useMemo(
    ...
    [getActorAvatarUrl, getActorInitials, getActorName, getAgentName, getMemberName, getSquadName],
  );
}
