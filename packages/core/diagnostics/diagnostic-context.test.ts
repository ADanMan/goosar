...
describe('bucketDiagnosticPath', () => {
  ...
  it('templates percent-encoded ids without splitting them', () => {
    expect(bucketDiagnosticPath('/acme/runtimes/machine%2Fruntime')).toBe('/:slug/runtimes/:id');
    expect(bucketDiagnosticPath('/acme/runtimes/machine%2Fruntime/runtime/runtime%20one')).toBe(
      '/:slug/runtimes/:id/runtime/:runtimeId',
    );
    expect(bucketDiagnosticPath('/my%20workspace/skills/a%2Fb')).toBe('/:slug/skills/:id');
  });
  ...
  it('drops query string and hash — they can carry resource ids', () => {
    expect(bucketDiagnosticPath('/acme/issues?issue=MUL-1#comment-3')).toBe('/:slug/issues');
  });
  ...
  it('masks an unknown route instead of passing segments through', () => {
    expect(bucketDiagnosticPath('/acme/issues/MUL-1/secret-tab')).toBe('/:slug/issues/*');
