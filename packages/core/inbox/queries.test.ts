...
describe('deduplicateArchivedInboxItems', () => {
  it('keeps only archived rows, one per issue, newest first', () => {
    ...
    expect(merged.map((i) => i.id)).toEqual(['archived-newer', 'archived-other-issue']);
  });
...
describe('hasOtherWorkspaceUnread', () => {
  ...
  it('is true when a workspace other than the active one has unread', () => {
    expect(hasOtherWorkspaceUnread(summary([{ workspace_id: 'ws-2', count: 3 }]), 'ws-1')).toBe(
      true,
    );
  });

  it("excludes the active workspace's own unread", () => {
    expect(hasOtherWorkspaceUnread(summary([{ workspace_id: 'ws-1', count: 5 }]), 'ws-1')).toBe(
      false,
    );
  });

  it('ignores other workspaces whose count is zero', () => {
    expect(hasOtherWorkspaceUnread(summary([{ workspace_id: 'ws-2', count: 0 }]), 'ws-1')).toBe(
      false,
    );
  });
  ...
  it("counts every workspace as 'other' when there is no active workspace", () => {
    expect(hasOtherWorkspaceUnread(summary([{ workspace_id: 'ws-1', count: 2 }]), null)).toBe(true);
  });
});
