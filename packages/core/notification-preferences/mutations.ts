...
export function useUpdateNotificationPreferences() {
  ...
  >({
    ...
    onMutate: async ({ patch, workspaceId }) => {
      ...
      const previous = qc.getQueryData<NotificationPreferenceResponse>(targetKey);
      qc.setQueryData<NotificationPreferenceResponse>(targetKey, (old) => ({
        ...(old ?? { workspace_id: workspaceId }),
        preferences: applyNotificationPreferencePatch(old?.preferences ?? {}, patch),
      }));
      return { previous, key: targetKey };
    },
    onError: (_error, { patch, workspaceId }, context) => {
      const targetKey = context?.key ?? notificationPreferenceKeys.all(workspaceId);
      qc.setQueryData<NotificationPreferenceResponse>(targetKey, (old) => ({
        ...(old ?? { workspace_id: workspaceId }),
  ...
  const mutate = useCallback(
    (preferences: NotificationPreferences, options?: ExternalMutationOptions) => {
      const patch = deriveNotificationPreferencePatch(renderedPreferencesRef.current, preferences);
      mutation.mutate(
