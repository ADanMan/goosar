...
describe('notification preference patches', () => {
  ...
  it('merges optimistic patches without dropping unrelated preferences', () => {
    expect(
      applyNotificationPreferencePatch({ status_changes: 'muted' }, { comments: 'muted' }),
    ).toEqual({ status_changes: 'muted', comments: 'muted' });
  });

  it('does not roll back a key that a later mutation already changed', () => {
    expect(
      rollbackNotificationPreferencePatch({ comments: 'muted' }, { comments: 'muted' }, {}),
    ).toEqual({});

    expect(rollbackNotificationPreferencePatch({}, { comments: 'muted' }, {})).toEqual({});
  });
});
