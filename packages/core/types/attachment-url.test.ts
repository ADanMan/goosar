...
describe('attachmentIdFromDownloadURL', () => {
  ...
  it('strips a query string before matching', () => {
    expect(attachmentIdFromDownloadURL(`/api/attachments/${ID}/download?cache=1`)).toBe(ID);
  });
  ...
  it('rejects URLs that do not start with /api/attachments/', () => {
    ...
    expect(attachmentIdFromDownloadURL('https://cdn.example.com/photo.png')).toBeUndefined();
  });

  it('rejects URLs missing the /download suffix', () => {
    expect(attachmentIdFromDownloadURL(`/api/attachments/${ID}`)).toBeUndefined();
    expect(attachmentIdFromDownloadURL(`/api/attachments/${ID}/content`)).toBeUndefined();
  });

  it('rejects when the segment between the prefix and suffix is not a UUID literal', () => {
    expect(attachmentIdFromDownloadURL('/api/attachments/not-a-uuid/download')).toBeUndefined();
    expect(attachmentIdFromDownloadURL('/api/attachments//download')).toBeUndefined();
  });
...
describe('contentReferencesAttachment', () => {
  const att = {
    ...
    markdown_url: `https://api.goosar.ru/api/attachments/${ID}/download`,
  };
