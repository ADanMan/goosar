...
describe('before_send $exception pipeline', () => {
  ...
  type BeforeSend = (e: { event: string; properties: Record<string, unknown> } | null) => unknown;
  ...
  it('redacts the message, then drops repeats past the per-fingerprint limit', async () => {
    ...
    const first = beforeSend(excEvent()) as {
      properties: { $exception_list: Array<{ value: string }> };
    };
