...
export function useCreateRuntimeProfile(wsId: string) {
  ...
  return useMutation({
    mutationFn: (body: CreateRuntimeProfileRequest) => api.createRuntimeProfile(wsId, body),
    onSettled: () => {
...
export function useUpdateRuntimeProfile(wsId: string) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ profileId, patch }: { profileId: string; patch: UpdateRuntimeProfileRequest }) =>
...
export function parseRuntimeProfileBoundConflict(err: unknown): RuntimeProfileBoundConflict | null {
