...
export function useConsumeChatDraftRestore() {
  ...
  return useMutation({
    ...
    onMutate: ({ sessionId, restoreId }) => {
      qc.setQueryData<ChatDraftRestoresResponse>(chatKeys.draftRestores(sessionId), (old) =>
        old ? { ...old, restores: old.restores.filter((r) => r.id !== restoreId) } : old,
      );
    },
...
export function useSetChatSessionPinned() {
  ...
  return useMutation({
    ...
    onMutate: async ({ sessionId, pinned }) => {
      ...
      const patch = (old?: ChatSession[]) =>
        old && sortChatSessions(old.map((s) => (s.id === sessionId ? { ...s, pinned } : s)));
      qc.setQueryData<ChatSession[]>(chatKeys.sessions(wsId), patch);
