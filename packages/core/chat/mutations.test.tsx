...
describe('useSetChatSessionArchived', () => {
  ...
  it('optimistically zeroes unread when archiving', async () => {
    setChatSessionArchived.mockResolvedValue(makeSession({ status: 'archived' }));
  ...
  it('rolls back the unread patch when the request fails', async () => {
    ...
    await act(async () => {
      await expect(result.current.mutateAsync({ sessionId: 's1', archived: true })).rejects.toThrow(
        'boom',
      );
    });
...
describe('useSetChatSessionProject', () => {
  ...
  let updateChatSession: ReturnType<
    typeof vi.fn<
      (id: string, data: { title: string } | { project_id: string | null }) => Promise<ChatSession>
    >
  >;
  ...
  it('rolls the project back when the update fails', async () => {
    ...
    expect(qc.getQueryData<ChatSession[]>(chatKeys.sessions(WS_ID))![0]!.project_id).toBe(
      'project-1',
    );
  });
});
