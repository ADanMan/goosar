...
interface RecentContextState {
  ...
  recordVisit: (
    wsId: string,
    entry: Pick<RecentContextEntry, 'type' | 'id'> &
      Partial<Pick<RecentContextEntry, 'label' | 'subtitle' | 'status' | 'projectStatus' | 'icon'>>,
  ) => void;
...
export function selectRecentContexts(wsId: string | null) {
  return (state: RecentContextState) => (wsId ? (state.byWorkspace[wsId] ?? EMPTY) : EMPTY);
}
