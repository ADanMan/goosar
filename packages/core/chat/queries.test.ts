...
describe('mergeTaskMessagesBySeq', () => {
  ...
  it('drops duplicate seqs and lets the existing entry win', () => {
    ...
    const merged = mergeTaskMessagesBySeq(existing, [{ ...msg(1), content: 'refetch' }, msg(2)]);
...
describe('highestTaskMessageSeq', () => {
  it('has no watermark for an empty or absent transcript', () => {
    expect(highestTaskMessageSeq(undefined)).toBeNull();
    expect(highestTaskMessageSeq([])).toBeNull();
  });

  it('returns the highest seq regardless of the order it is stored in', () => {
    expect(highestTaskMessageSeq([msg(1), msg(2), msg(3)])).toBe(3);
    expect(highestTaskMessageSeq([msg(3), msg(1), msg(2)])).toBe(3);
  });
});
