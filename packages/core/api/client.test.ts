...
describe('ApiClient server Table query', () => {
  ...
  it('preserves future Table status and actor enum values', async () => {
    ...
    await expect(
      ...
      total: 1,
      groups: [{ value: { kind: 'assignee', actor: { type: 'service', id: 'bot-1' } } }],
    });
  });
      ...
      groups: [
        {
          ...
          secondary_groups: [{ value: { kind: 'status', status: 'todo' }, count: 2 }],
        },
      ],
    });
    expect(fetchMock.mock.calls[0]?.[1]).toMatchObject({
...
describe('ApiClient workspace working agents', () => {
  it('supports an optional source-type filter', async () => {
    ...
    const client = new ApiClient('https://api.example.test');
    await expect(client.getWorkspaceWorkingAgents('issue', 'assigned')).resolves.toEqual(payload);
    await expect(client.getWorkspaceWorkingAgents('issue')).resolves.toEqual(payload);
    await expect(client.getWorkspaceWorkingAgents()).resolves.toEqual(payload);
    await expect(client.getWorkspaceWorkingAgents('issue', undefined, 'parent-1')).resolves.toEqual(
      payload,
    );
    await expect(
...
describe('ApiClient label response schemas', () => {
  it('falls back safely for malformed label catalog, label, and resource responses', async () => {
    ...
    await expect(client.updateLabel('label-1', { name: 'Operations' })).resolves.toMatchObject({
      id: '',
    });
    ...
    await expect(client.attachLabelToResource('agent', 'agent-1', 'label-1')).resolves.toEqual({
      labels: [],
    });
    await expect(client.detachLabelFromResource('agent', 'agent-1', 'label-1')).resolves.toEqual({
      labels: [],
    });
...
describe('ApiClient agent builder runtime switch', () => {
  it('PATCHes the session runtime endpoint and returns the runtime the server bound', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ runtime_id: 'runtime-j' }), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
      }),
    );
    ...
    const client = new ApiClient('https://api.example.test');
    await expect(
      client.switchAgentBuilderRuntime('session-1', { runtime_id: 'runtime-j' }),
    ).resolves.toEqual({ runtime_id: 'runtime-j' });

    const call = fetchMock.mock.calls[0] as [string, RequestInit];
    expect(call[0]).toContain('/api/agent-builder/sessions/session-1/runtime');
    expect(call[1].method).toBe('PATCH');
    expect(JSON.parse(String(call[1].body))).toEqual({ runtime_id: 'runtime-j' });
  });
...
describe('ApiClient notification preferences', () => {
  it('sends atomic preference updates with PATCH', async () => {
    ...
    const client = new ApiClient('https://api.example.test');
    await expect(
      client.updateNotificationPreferences({ comments: 'muted' }, 'workspace-one'),
    ).resolves.toEqual({
      workspace_id: 'workspace-1',
      preferences: {
        status_changes: 'muted',
  ...
  it('falls back safely when a preference response is malformed', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        new Response(JSON.stringify({ workspace_id: 'workspace-1', preferences: [] }), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        }),
      ),
...
describe('ApiClient', () => {
  ...
  it('preserves planned and delivered comment coverage from issue task runs', async () => {
    ...
    expect(tasks[0]?.delivered_comment_ids).toEqual(['comment-1', 'comment-2', 'comment-3']);
  });
  ...
  it('uses the Cloud Runtime node API contract', async () => {
    ...
    await client.createCloudRuntimeNode({ instance_type: 'g5.xlarge', name: 'gpu-dev-01' });

    const listCall = fetchMock.mock.calls[0]!;
    const createCall = fetchMock.mock.calls[1]!;
    expect(listCall[0]).toBe('https://api.example.test/api/cloud-runtime/nodes?limit=20&offset=5');
    expect(createCall[0]).toBe('https://api.example.test/api/cloud-runtime/nodes');
  ...
  describe('getAttachment', () => {
    ...
    it('surfaces download_ticket_url so a bare navigation has a credential', async () => {
      vi.stubGlobal(
        'fetch',
        vi.fn().mockResolvedValue(
          new Response(
            JSON.stringify({
              id: 'att-1',
              url: 'https://static.example.test/ws/att-1.md',
              download_url: '/api/attachments/att-1/download?token=tkt.sig',
              download_ticket_url: '/api/attachments/att-1/download?token=tkt.sig',
              filename: 'report.md',
            }),
            { status: 200, headers: { 'Content-Type': 'application/json' } },
          ),
      ...
      expect(att.download_ticket_url).toBe('/api/attachments/att-1/download?token=tkt.sig');
    });

    it('defaults download_ticket_url to an empty string against a server that predates it', async () => {
      vi.stubGlobal(
        'fetch',
        vi.fn().mockResolvedValue(
          new Response(
      ...
      expect(att.id).toBe('att-1');
      expect(att.download_ticket_url).toBe('');
    });

    it('falls back to an empty attachment when download_ticket_url is the wrong type', async () => {
      vi.stubGlobal(
        'fetch',
        vi.fn().mockResolvedValue(
          new Response(
            JSON.stringify({
              ...
              url: 'https://static.example.test/ws/att-1.md',
              download_url: '/api/attachments/att-1/download',
              download_ticket_url: { token: 'not-a-string' },
              filename: 'report.md',
            }),
            { status: 200, headers: { 'Content-Type': 'application/json' } },
          ),
  ...
  describe('getAttachmentTextContent', () => {
    it('returns body text and the original content type from the X-* header', async () => {
      ...
      const client = new ApiClient('https://api.example.test');
      const { text, originalContentType } = await client.getAttachmentTextContent('att-1');
    ...
    it('throws PreviewTooLargeError on 413', async () => {
      ...
      vi.stubGlobal(
        ...
        vi
          .fn()
          .mockResolvedValue(new Response('', { status: 413, statusText: 'Payload Too Large' })),
      );
  ...
  describe('chat attachment wiring', () => {
    ...
    it('rejects with the fetch AbortError when the signal is already aborted', async () => {
      ...
      await expect(client.uploadFile(file, undefined, controller.signal)).rejects.toMatchObject({
        name: 'AbortError',
      });
    });
...
describe('ApiClient 401 handling (#105)', () => {
  describe('fetchRaw path (e.g. listWorkspaces)', () => {
    it('does NOT call onUnauthorized or clear the token on a 401 for a request sent without a token', async () => {
      const fetchMock = vi.fn().mockResolvedValue(
    ...
    it('DOES call onUnauthorized on a 401 for a request sent with a token', async () => {
      const fetchMock = vi.fn().mockResolvedValue(
  ...
  describe('uploadFile path (second 401 site)', () => {
    it('does NOT call onUnauthorized on a 401 for an upload sent without a token', async () => {
      const fetchMock = vi.fn().mockResolvedValue(
        new Response(JSON.stringify({ error: 'missing authorization' }), {
          status: 401,
          headers: { 'Content-Type': 'application/json' },
    ...
    it('DOES call onUnauthorized on a 401 for an upload sent with a token', async () => {
      const fetchMock = vi.fn().mockResolvedValue(
        new Response(JSON.stringify({ error: 'invalid token' }), {
          status: 401,
          headers: { 'Content-Type': 'application/json' },
...
describe('ApiClient verify-link response schema', () => {
  it('sends the token in the POST body and parses a valid response', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(
        JSON.stringify({
          token: 'jwt-1',
          user: { id: 'u1', name: 'Alice', email: 'a@example.test' },
        }),
        { status: 200, headers: { 'Content-Type': 'application/json' } },
      ),
    );
    ...
    const result = await new ApiClient('https://api.example.test').verifyLink('tok-abc');

    expect(result.token).toBe('jwt-1');
    expect(result.user?.id).toBe('u1');
    expect(result.mfa_required).toBe(false);
    const [url, init] = fetchMock.mock.calls[0]!;
    expect(String(url)).toContain('/auth/verify-link');
  ...
  it('falls back to the empty LoginResponse on a malformed body', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        new Response(JSON.stringify({ token: 42, user: 'nope' }), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        }),
      ),
    ...
    expect(result.mfa_required).toBe(false);
    expect(result.mfa_token).toBe('');
  });

  it('carries a second-factor demand instead of a session', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        new Response(JSON.stringify({ mfa_required: true, mfa_token: 'ticket-1' }), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        }),
      ),
    ...
    expect(result.mfa_token).toBe('ticket-1');
    expect(result.token).toBe('');
  });

  it('rejects with ApiError when the link is invalid or expired (400)', async () => {
    vi.stubGlobal(
      'fetch',
      vi.fn().mockResolvedValue(
        new Response(JSON.stringify({ error: 'invalid or expired link' }), {
          status: 400,
          headers: { 'Content-Type': 'application/json' },
        }),
      ),
    ...
    await expect(
      new ApiClient('https://api.example.test').verifyLink('spent'),
    ).rejects.toBeInstanceOf(ApiError);
  });
});

describe('ApiClient task messages', () => {
  ...
  it('reads the whole transcript when no watermark is given', async () => {
    const fetchMock = stubOkFetch();
    const client = new ApiClient('https://api.example.test');

    await client.listTaskMessages('task-1');

    expect(fetchMock.mock.calls[0]?.[0]).toBe('https://api.example.test/api/tasks/task-1/messages');
  });

  it('reads only what is newer than the watermark when one is given', async () => {
    const fetchMock = stubOkFetch();
    const client = new ApiClient('https://api.example.test');

    await client.listTaskMessages('task-1', { since: 42 });

    expect(fetchMock.mock.calls[0]?.[0]).toBe(
      'https://api.example.test/api/tasks/task-1/messages?since=42',
    );
  });

  it('treats seq 0 as a watermark, not as a missing one', async () => {
    const fetchMock = stubOkFetch();
    const client = new ApiClient('https://api.example.test');

    await client.listTaskMessages('task-1', { since: 0 });

    expect(fetchMock.mock.calls[0]?.[0]).toBe(
      'https://api.example.test/api/tasks/task-1/messages?since=0',
    );
  });
});

describe('ApiClient agent env response schema', () => {
  ...
  it("passes an owner's plaintext response through", async () => {
    json({
      agent_id: 'a-1',
    ...
    const result = await new ApiClient('https://api.example.test').getAgentEnv('a-1');

    expect(result.custom_env).toEqual({ JIRA_PERSONAL_TOKEN: 'pat-live' });
    expect(result.values_masked).toBe(false);
  });

  it('keeps values_masked so the UI can tell **** from a secret', async () => {
    json({
      agent_id: 'a-1',
      custom_env: { JIRA_PERSONAL_TOKEN: '****' },
      values_masked: true,
    });

    const result = await new ApiClient('https://api.example.test').getAgentEnv('a-1');

    expect(result.values_masked).toBe(true);
    expect(result.custom_env.JIRA_PERSONAL_TOKEN).toBe('****');
  });

  it('treats a missing values_masked as an older plaintext-only backend', async () => {
    json({ agent_id: 'a-1', custom_env: { K: 'v' } });

    const result = await new ApiClient('https://api.example.test').getAgentEnv('a-1');

    expect(result.values_masked).toBeUndefined();
    expect(result.custom_env).toEqual({ K: 'v' });
  });

  it('fails the read when values_masked is not a boolean', async () => {
    json({
      agent_id: 'a-1',
      custom_env: { JIRA_PERSONAL_TOKEN: '****' },
      values_masked: 'true',
    ...
    await expect(new ApiClient('https://api.example.test').getAgentEnv('a-1')).rejects.toThrow();
  });

  it('fails the read when custom_env is malformed instead of defaulting to {}', async () => {
    json({ agent_id: 'a-1', custom_env: ['JIRA_PERSONAL_TOKEN'] });

    await expect(new ApiClient('https://api.example.test').getAgentEnv('a-1')).rejects.toThrow();
  });

  it('fails the read when a value is not a string', async () => {
    json({ agent_id: 'a-1', custom_env: { K: 42 } });

    await expect(new ApiClient('https://api.example.test').getAgentEnv('a-1')).rejects.toThrow();
  });

  it('keeps the plaintext body out of the schema-mismatch log', async () => {
    const warn = vi.fn();
    setSchemaLogger({
      debug: vi.fn(),
      info: vi.fn(),
    ...
    json({
      agent_id: 'a-1',
      custom_env: { JIRA_PERSONAL_TOKEN: 'pat-live-secret', PORT: 8080 },
    });
    ...
    expect(logged).not.toContain('pat-live-secret');
    expect(logged).toContain('/api/agents/{id}/env');
    expect(logged).toContain('PORT');
    setSchemaLogger(noopLogger);
  });

  it('fails the save when the response body is malformed', async () => {
    json({ agent_id: 'a-1' });

    await expect(
      new ApiClient('https://api.example.test').updateAgentEnv('a-1', {
        custom_env: { K: 'v' },
      }),
    ).rejects.toThrow();
  });
