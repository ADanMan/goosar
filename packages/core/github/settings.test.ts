...
describe('deriveGitHubSettings', () => {
  ...
  it('each sub-flag can be flipped independently when master is on', () => {
    expect(deriveGitHubSettings(ws({ github_pr_sidebar_enabled: false }))).toMatchObject({
      enabled: true,
      prSidebar: false,
      coAuthor: true,
      autoLinkPRs: true,
    ...
    expect(deriveGitHubSettings(ws({ co_authored_by_enabled: false }))).toMatchObject({
      enabled: true,
      prSidebar: true,
      coAuthor: false,
      autoLinkPRs: true,
    ...
    expect(deriveGitHubSettings(ws({ github_auto_link_prs_enabled: false }))).toMatchObject({
      enabled: true,
      prSidebar: true,
      coAuthor: true,
      autoLinkPRs: false,
  ...
  it('treats non-false values (true, null, missing) as enabled', () => {
    expect(
      deriveGitHubSettings(ws({ github_enabled: true, github_pr_sidebar_enabled: null })),
