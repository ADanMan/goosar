...
describe('deriveChecksStatus', () => {
  ...
  it('preserves legacy provider passed, pending, and failed conclusions', () => {
    expect(deriveChecksStatus({ checks_conclusion: 'passed', checks_total: 3 })).toEqual({
      kind: 'passed',
      total: 3,
    });
    expect(
...
describe('deriveMergeStatus', () => {
  ...
  it('never infers ready from `mergeable === mergeable` alone', () => {
    ...
    expect(deriveMergeStatus({ mergeable: 'mergeable', merge_state_status: null }).kind).toBe(
      'none',
    );
  });
...
describe('shouldShowPullRequestStats', () => {
  it('hides when every field is 0 or missing (legacy backend)', () => {
    ...
    expect(shouldShowPullRequestStats({ additions: 0, deletions: 0, changed_files: 0 })).toBe(
      false,
    );
  });
  ...
  it('shows when at least one number is non-zero', () => {
    ...
    expect(shouldShowPullRequestStats({ additions: 437, deletions: 6, changed_files: 6 })).toBe(
      true,
    );
  });
});
