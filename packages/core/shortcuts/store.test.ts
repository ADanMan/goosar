...
describe('shortcut store', () => {
  ...
  it('removes structurally equal overrides when restoring a default', () => {
    useShortcutStore
      .getState()
      .setShortcut('createIssue', createShortcutChord('I', { primary: true }));
    useShortcutStore.getState().setShortcut('createIssue', createShortcutChord('C'));
  ...
  it('rejects unsafe programmatic overrides as a store invariant', () => {
    useShortcutStore.getState().setShortcut('send', createShortcutChord('C'));
    useShortcutStore.getState().setShortcut('send', createShortcutChord('Enter', { shift: true }));
    useShortcutStore
      .getState()
      .setShortcut('send', createShortcutChord('Enter', { primary: true, shift: true }));
    expect(getShortcut('send')).toEqual(createShortcutChord('Enter', { primary: true }));
  });
  ...
  it('finds conflicts against defaults and overrides', () => {
    expect(findShortcutConflict('createIssue', createShortcutChord('K', { primary: true }))).toBe(
      'openSearch',
    );
  ...
  it('drops malformed current-version storage and unknown actions', () => {
    ...
    expect(migrateShortcutState({ overrides: 'corrupt' }, 2)).toEqual({ overrides: {} });
  });
