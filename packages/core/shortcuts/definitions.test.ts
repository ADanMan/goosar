...
describe('keyboard shortcut definitions', () => {
  it('keeps every shipped default inside the action safety policy', () => {
    for (const action of SHORTCUT_ACTIONS) {
      ...
      expect(isShortcutAllowedForAction(action.id, action.defaultShortcut, 'macos')).toBe(true);
      expect(isShortcutAllowedForAction(action.id, action.defaultShortcut, 'windows')).toBe(true);
    }
  });
  ...
  it('requires every modifier to match exactly', () => {
    ...
    expect(
      shortcutMatchesEvent(shortcut, keyEvent('k', { ctrlKey: true, shiftKey: true }), 'linux'),
    ).toBe(false);
  });
  ...
  it('ignores synthetic events without a key, such as Chrome autofill', () => {
    ...
    expect(shortcutMatchesEvent(createShortcutChord('K', { primary: true }), event, 'macos')).toBe(
      false,
    );
  });
  ...
  it('uses platform-specific reserved shortcuts', () => {
    expect(isReservedShortcut(createShortcutChord('Space', { primary: true }), 'macos')).toBe(true);
    expect(isReservedShortcut(createShortcutChord('K', { meta: true }), 'windows')).toBe(true);
    expect(isReservedShortcut(createShortcutChord('K', { primary: true }), 'windows')).toBe(false);
  });
  ...
  it('prevents unsafe plain keys from hijacking editors and global navigation', () => {
    expect(isShortcutAllowedForAction('openSearch', createShortcutChord('J'), 'macos')).toBe(false);
    expect(isShortcutAllowedForAction('send', createShortcutChord('J'), 'macos')).toBe(false);
    expect(
      isShortcutAllowedForAction('send', createShortcutChord('J', { shift: true }), 'macos'),
    ).toBe(false);
    expect(
      isShortcutAllowedForAction('openSearch', createShortcutChord('J', { alt: true }), 'macos'),
    ).toBe(false);
    expect(isShortcutAllowedForAction('send', createShortcutChord('Enter'), 'macos')).toBe(true);
    expect(
      isShortcutAllowedForAction('send', createShortcutChord('Enter', { primary: true }), 'macos'),
    ).toBe(true);
    expect(
      isShortcutAllowedForAction('send', createShortcutChord('Enter', { shift: true }), 'macos'),
    ).toBe(false);
    expect(
      isShortcutAllowedForAction(
        'send',
    ...
    expect(isShortcutAllowedForAction('goInbox', createShortcutChord('Enter'), 'macos')).toBe(
      false,
    );
    expect(isShortcutAllowedForAction('goInbox', createShortcutChord('G'), 'macos')).toBe(true);
  });

  it('protects fundamental editing shortcuts from reassignment', () => {
    expect(
      isShortcutAllowedForAction('send', createShortcutChord('C', { primary: true }), 'macos'),
    ).toBe(false);
  });
