...
export function sanitizeShortcutOverrides(value: unknown): ShortcutOverrides {
  ...
  for (const [actionId, rawShortcut] of Object.entries(value)) {
    ...
    if (
      shortcut !== undefined &&
      (shortcut === null || isShortcutAllowedForAction(actionId as ShortcutActionId, shortcut))
    ) {
...
export function migrateShortcutState(
  ...
  const previous = isRecord(persisted) && isRecord(persisted.overrides) ? persisted.overrides : {};
  const overrides: ShortcutOverrides = {};
...
export const useShortcutStore = create<ShortcutState>()(
  persist(
    ...
    {
      name: 'goosar_keyboard_shortcuts',
...
export function findShortcutConflict(
  ...
  for (const action of SHORTCUT_ACTIONS) {
    if (action.id !== actionId && shortcutChordEquals(getShortcut(action.id), shortcut)) {
      return action.id;
    }
  }
  return null;
