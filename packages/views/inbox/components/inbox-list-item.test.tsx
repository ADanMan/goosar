...
describe('InboxListItem issue activity', () => {
  it('shows issue-specific agent activity without an availability dot', () => {
    ...
    expect(getByTestId('actor-avatar').getAttribute('data-show-status-dot')).toBe('false');
    expect(getByTestId('issue-agent-activity').getAttribute('data-issue-id')).toBe('issue-1');
  });

  it('shows the activity badge without its hover card', () => {
    const { getByTestId } = renderRow({ item: item(), view: 'inbox' });

    expect(getByTestId('issue-agent-activity').getAttribute('data-hover-card')).toBe('false');
  });
