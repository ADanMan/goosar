...
const polling = {
  interval: false as number | false,
  captured: { active: undefined as unknown, archived: undefined as unknown },
};

vi.mock('@tanstack/react-query', () => ({
  useQuery: (options: { queryKey: readonly unknown[]; refetchInterval?: unknown }) => {
    const isArchived = options.queryKey.includes('archived');
    polling.captured[isArchived ? 'archived' : 'active'] = options.refetchInterval;
    return {
      data: isArchived ? listData.archived : listData.active,
      isLoading: false,
      isError: false,
    };
  },
...
vi.mock('@goosar/core/realtime', () => ({
  useRealtimePollingInterval: () => polling.interval,
}));
...
vi.mock('@goosar/core/inbox/mutations', () => {
...
vi.mock('@goosar/ui/hooks/use-mobile', () => ({ useIsMobile: () => true }));
vi.mock('./inbox-list', () => ({
  InboxList: ({
    items,
    view,
...
function reset() {
  ...
  replace.mockClear();
  polling.interval = false;
  polling.captured = { active: undefined, archived: undefined };
}

describe('InboxPage — degraded-mode REST polling (#257)', () => {
  it('does not poll while the realtime connection is healthy', () => {
    reset();
    render(<InboxPage />);
    expect(polling.captured.active).toBe(false);
  });

  it('polls the active list while the realtime connection is degraded', () => {
    reset();
    polling.interval = 15_000;
    render(<InboxPage />);
    expect(polling.captured.active).toBe(15_000);
  });

  it('leaves the archive un-polled — nothing new lands there on its own', () => {
    reset();
    polling.interval = 15_000;
    render(<InboxPage />);
    expect(polling.captured.archived).toBeUndefined();
  });
});
...
describe('InboxPage', () => {
  it('keeps the title unread count static', () => {
    ...
    const titleCount = container
      .querySelector('h1')
      ?.parentElement?.querySelector('number-flow-react') as
      (HTMLElement & { animated?: boolean }) | null;
  ...
  it('keeps the archived view in the URL when selecting an item there', () => {
    reset();
    searchParams = new URLSearchParams('view=archived');
    listData.archived = [item({ id: 'archived-1', issue_id: 'issue-9', archived: true })];
