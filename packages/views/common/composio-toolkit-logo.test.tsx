...
describe('ComposioToolkitLogo', () => {
  ...
  it('uses backend logo for light mode and Composio dark logo for dark mode', () => {
    ...
    expect(images[1]?.getAttribute('src')).toBe('https://logos.composio.dev/api/slack?theme=dark');
  });
  ...
  it('keeps the dark logo when the hidden light logo fails', () => {
    ...
    expect(dark?.getAttribute('src')).toBe('https://logos.composio.dev/api/notion?theme=dark');
  });
});
