...
import type { AgentRuntime, AgentTask } from '@goosar/core/types/agent';
import { useTranscriptViewStore } from '@goosar/core/agents/stores';
...
vi.mock('@goosar/ui/lib/clipboard', () => ({
...
vi.mock('@goosar/ui/components/ui/collapsible', async () => {
  const React = await import('react');
...
describe('AgentTranscriptDialog', () => {
  it('explains unavailable live events for an empty runtime-a transcript', async () => {
    vi.mocked(api.listRuntimes).mockResolvedValue([runtimeFor('runtime-a')]);

    renderDialog([], { task: liveTask, isLive: true });

    expect(
      await screen.findByText(
        'Runtime A does not currently provide live execution events. The transcript will be available after the task completes.',
      ),
    ).toBeInTheDocument();
  ...
  it('keeps waiting for live events from other runtimes', async () => {
    vi.mocked(api.listRuntimes).mockResolvedValue([runtimeFor('runtime-j')]);
