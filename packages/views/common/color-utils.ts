// Цветовая математика общего выбора цвета (color-picker.tsx).
...
export function randomOptionColor(avoid?: string): string {
  ...
  const avoidHue = avoidHsv && avoidHsv.s > 0 && avoidHsv.v > 0 ? avoidHsv.h : null;
  const hue =
    avoidHue === null
      ? Math.random() * 360
      : (avoidHue + MIN_HUE_DISTANCE + Math.random() * (360 - MIN_HUE_DISTANCE * 2)) % 360;
