...
describe('githubShortLabel', () => {
  it('extracts owner/repo from an https URL', () => {
    expect(githubShortLabel('https://github.com/octocat/hello-world')).toBe('octocat/hello-world');
  });

  it('ignores extra path segments after owner/repo', () => {
    expect(githubShortLabel('https://github.com/octocat/hello-world/tree/main')).toBe(
      'octocat/hello-world',
    );
  });
  ...
  it('handles ssh:// URLs and strips .git', () => {
    expect(githubShortLabel('ssh://git@github.com/octocat/hello-world.git')).toBe(
      'octocat/hello-world',
    );
  });

  it('handles scp-style shorthand (git@github.com:owner/repo.git)', () => {
    expect(githubShortLabel('git@github.com:octocat/hello-world.git')).toBe('octocat/hello-world');
  });

  it('handles scp-style shorthand without a .git suffix', () => {
    expect(githubShortLabel('git@github.com:octocat/hello-world')).toBe('octocat/hello-world');
  });
