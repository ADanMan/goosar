...
import type { PersonalAccessToken } from '@goosar/core/types';
...
export function TokensTab() {
  ...
  const handleCopyCommand = async () => {
    if (!newToken) return;
    if (await copyText(`goosar login --token ${newToken}`)) {
        ...
        <Card>
          <CardContent className="space-y-3">
            <div className="grid gap-3 sm:grid-cols-[1fr_120px_auto]">
              ...
              >
                <SelectTrigger size="sm" aria-label={t(($) => $.tokens.title)}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {t(($) => $.tokens.metadata_prefix, {
                        ...
                        created: new Date(token.created_at).toLocaleDateString(uiLocale),
                        lastUsed: token.last_used_at
                          ? t(($) => $.tokens.last_used_with_date, {
                              date: new Date(token.last_used_at!).toLocaleDateString(uiLocale),
                            })
                          : t(($) => $.tokens.last_used_never),
                      })}
                      {token.expires_at &&
                        t(($) => $.tokens.expires_with_date, {
                          date: new Date(token.expires_at!).toLocaleDateString(uiLocale),
                        })}
                    </div>
                  </div>
                  <Tooltip>
          ...
          <div className="min-w-0 space-y-1.5">
            ...
            <div className="flex min-w-0 items-center gap-2">
              ...
              <Tooltip>
                ...
                />
                <TooltipContent>
                  {t(($) => $.tokens.created_dialog.copy_command_tooltip)}
                </TooltipContent>
              </Tooltip>
            </div>
          </div>
