...
import { RESOURCE_LABELS_FLAG } from '@goosar/core/feature-flags';
...
import type { Label, LabelResourceType } from '@goosar/core/types';
...
export function LabelsTab() {
  ...
  return (
    <SettingsTab title={t(($) => $.labels.title)} description={t(($) => $.labels.description)}>
              ...
              {filteredLabels.map((label) => (
                ...
                >
                  ...
                  <span className="text-xs text-muted-foreground">
                    {new Date(label.updated_at).toLocaleDateString(uiLocale)}
                  </span>
                  <DropdownMenu>
                    <DropdownMenuTrigger
                      render={
      ...
      <LabelEditorDialog
        ...
        label={editing}
      />
      <DeleteLabelDialog label={pendingDelete} onClose={() => setPendingDelete(null)} />
    </SettingsTab>
  );
}
...
function DeleteLabelDialog({ label, onClose }: { label: Label | null; onClose: () => void }) {
  const { t } = useT('settings');
