...
import { propertyListOptions, useCreateProperty, useUpdateProperty } from '@goosar/core/properties';
import type { IssueProperty, IssuePropertyOption, IssuePropertyType } from '@goosar/core/types';
import { ISSUE_PROPERTY_TYPES } from '@goosar/core/types';
...
import { Label as FieldLabel } from '@goosar/ui/components/ui/label';
...
import { PropertyIcon, PropertyIconGlyph, PropertyIconPicker } from '../../common/property-icon';
...
export function PropertiesTab() {
  ...
  const activeCount = useMemo(() => properties.filter((p) => !p.archived).length, [properties]);
  ...
  return (
    ...
    >
          ...
          {isLoading ? (
            ...
            <div className="px-4 py-12 text-center">
              ...
              <p className="mt-3 text-sm font-medium">
                {query ? t(($) => $.properties.no_results) : t(($) => $.properties.empty)}
              </p>
              {!query && (
            ...
            <div className="divide-y divide-surface-border">
              {visible.map((property) => (
                ...
                >
                  ...
                  <span className="text-xs text-muted-foreground">
                    {new Date(property.updated_at).toLocaleDateString(uiLocale)}
                  </span>
                  {canManage ? (
                    <DropdownMenu>
                      <DropdownMenuTrigger
                        render={
                          <Button
                            variant="ghost"
                            size="icon-sm"
                            aria-label={t(($) => $.properties.actions.open, {
                              name: property.name,
                            })}
                          >
      ...
      <PropertyEditorDialog
        ...
        property={editing}
      />
      <ArchivePropertyDialog property={pendingArchive} onClose={() => setPendingArchive(null)} />
    </SettingsTab>
  );
}
...
function PropertyEditorDialog({
  ...
  const canSubmit = draft.name.trim().length > 0 && (!showOptions || validOptions.length > 0);
  ...
  const submit = () => {
    ...
    const onError = (error: unknown) =>
      toast.error(error instanceof Error ? error.message : t(($) => $.properties.save_failed));
    if (property) {
      update.mutate(
        {
          id: property.id,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>{t(($) => $.properties.editor.admin_hint)}</DialogDescription>
        </DialogHeader>
        <div className="space-y-5 py-2">
          <div className="grid grid-cols-[4.25rem_minmax(0,1fr)_10rem] gap-3">
            ...
            <div className="space-y-2">
              <FieldLabel htmlFor="property-name">{t(($) => $.properties.editor.name)}</FieldLabel>
              <Input
                id="property-name"
                autoFocus
                maxLength={32}
                      ...
                      ...current.options,
                      {
                        ...
                        color:
                          COLOR_PICKER_PRESETS[
                            current.options.length % COLOR_PICKER_PRESETS.length
                          ] ?? COLOR_PICKER_PRESETS[6],
                      },
                    ],
                  }))
                }
