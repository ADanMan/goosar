...
import { memberListOptions, workspaceKeys } from '@goosar/core/workspace/queries';
import {
  githubInstallationRepositoriesOptions,
  githubInstallationsOptions,
} from '@goosar/core/github';
import { api } from '@goosar/core/api';
import type { GitHubRepository, Workspace, WorkspaceRepo } from '@goosar/core/types';
import { useNavigation } from '../../navigation';
import { useT } from '../../i18n';
import { SettingsCard, SettingsSaveState, SettingsSection, SettingsTab } from './settings-layout';
...
export function RepositoriesTab() {
  ...
  const [selectedRepositories, setSelectedRepositories] = useState<Map<number, GitHubRepository>>(
    new Map(),
  );
  ...
  const githubBrowseConfigured = githubData?.repository_browse_configured === true;
  const githubRepositoriesQuery = useInfiniteQuery({
    ...githubInstallationRepositoriesOptions(wsId, selectedInstallationID),
    enabled:
      githubPickerOpen && canManageWorkspace && githubBrowseConfigured && !!selectedInstallationID,
  });
  ...
  useEffect(() => {
    if (
      ...
      githubInstallations.some((installation) => installation.id === selectedInstallationID)
    ) {
      return;
    }
  ...
  useEffect(() => {
    ...
    if (!githubError && (githubInstallationsPending || githubInstallationsFetching)) {
      return;
    }
  ...
  const autoSave = useAutoSave({
    ...
    onError: (error) =>
      toast.error(
        error instanceof Error ? error.message : t(($) => $.repositories.toast_save_failed),
      ),
    enabled: !!workspace && canManageWorkspace && allUrlsValid,
    isEqual: repositoriesEqual,
  });

  const updateRepository = (index: number, field: keyof WorkspaceRepo, value: string) => {
    setRepositories((current) =>
      current.map((repo, repoIndex) => (repoIndex === index ? { ...repo, [field]: value } : repo)),
    );
  };
  ...
  const openGitHubPicker = () => {
    setSelectedInstallationID(selectedInstallationID || githubInstallations[0]?.id || '');
    setGitHubPickerOpen(true);
  };
  ...
  const handleGitHubAction = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : t(($) => $.repositories.github_connect_failed),
      );
    } finally {
  ...
  const toggleGitHubRepository = (repository: GitHubRepository, checked: boolean) => {
  ...
  const importGitHubRepositories = () => {
    ...
    for (const repository of selectedRepositories.values()) {
      ...
      additions.push({
        ...
        ...(repository.description?.trim() ? { description: repository.description.trim() } : {}),
      });
    }
  ...
  return (
    <SettingsTab title={t(($) => $.page.tabs.repositories)}>
          ...
          {repositories.map((repository, index) => (
            ...
            >
              <Input
                ...
                onChange={(event) => updateRepository(index, 'url', event.target.value)}
              ...
              <Input
                ...
                onChange={(event) => updateRepository(index, 'description', event.target.value)}
                onBlur={autoSave.flush}
                disabled={!canManageWorkspace}
      ...
      >
        <DialogContent className="flex max-h-[85vh] flex-col gap-0 p-0 sm:max-w-2xl">
          <DialogHeader className="border-b px-6 py-5">
            <DialogTitle>{t(($) => $.repositories.github_picker_title)}</DialogTitle>
            <DialogDescription>
          ...
          <div className="space-y-3 px-6 py-4">
            {githubInstallations.length > 1 ? (
              ...
              >
                <SelectTrigger aria-label={t(($) => $.repositories.github_account)}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  ...
                  return (
                    ...
                    >
                      <Checkbox
                        ...
                        checked={alreadyAdded || selectedRepositories.has(repository.id)}
                        disabled={disabled}
                      ...
                      <span className="min-w-0 flex-1 space-y-1">
                        <span className="flex flex-wrap items-center gap-2">
                          ...
                          {alreadyAdded ? (
                            <Badge variant="outline">{t(($) => $.repositories.github_added)}</Badge>
                          ) : null}
                        </span>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t(($) => $.repositories.delete_confirm_title)}</AlertDialogTitle>
            <AlertDialogDescription>
              {t(($) => $.repositories.delete_confirm_description)}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t(($) => $.repositories.delete_confirm_cancel)}</AlertDialogCancel>
            <AlertDialogAction
              variant="destructive"
              onClick={() => {
