...
vi.mock('@goosar/core/i18n/react', async () => {
  const actual =
    await vi.importActual<typeof import('@goosar/core/i18n/react')>('@goosar/core/i18n/react');
  return {
    ...actual,
    useLocaleAdapter: () => ({
      persist: mockPersist,
...
import { useCommentComposerStore } from '@goosar/core/issues/stores';
...
describe('PreferencesTab — Timezone section', () => {
  ...
  async function pickTimezone(user: ReturnType<typeof userEvent.setup>, name: RegExp | string) {
  ...
  it('renders the stored timezone in the trigger', () => {
    ...
    expect(screen.getByRole('combobox', { name: 'Viewing Timezone' }).textContent).toContain(
      'Asia/Shanghai',
    );
  });
