...
const workspaceRef = vi.hoisted(() => ({
  current: {
    ...
    name: 'Test Workspace',
    slug: 'test-workspace',
    repos: [{ url: 'https://github.com/adanman/goosar' }] as {
      url: string;
      description?: string;
    }[],
  },
...
describe('RepositoriesTab — automatic updates', () => {
  beforeEach(() => {
    ...
    workspaceRef.current = {
      ...
      name: 'Test Workspace',
      slug: 'test-workspace',
      repos: [{ url: 'https://github.com/adanman/goosar' }],
    };
  ...
  it('renders persisted repositories as the same shared input controls used for editing', () => {
    ...
    expect(inputs[0]!.value).toBe('https://github.com/adanman/goosar');
  ...
  it('updates a changed URL automatically on blur', async () => {
    ...
    await user.type(urlInput, 'https://github.com/adanman/edited');
    await user.tab();

    await waitFor(() => {
      expect(mockUpdateWorkspace).toHaveBeenCalledWith('workspace-1', {
        repos: [{ url: 'https://github.com/adanman/edited' }],
      });
  ...
  it('does not persist a new row until its URL is non-empty', async () => {
    ...
    await user.type(newUrlInput, 'git@github.com:adanman/second.git');
    await user.tab();
    ...
    await waitFor(() => {
      expect(mockUpdateWorkspace).toHaveBeenCalledWith('workspace-1', {
        repos: [
          ...
          { url: 'git@github.com:adanman/second.git' },
        ],
      });
    });
  });
  ...
  it('accepts scp-like repository shorthand', async () => {
    ...
    await user.clear(urlInput);
    await user.type(urlInput, 'git@github.com:adanman/goosar.git');
    ...
    await waitFor(() => {
      expect(mockUpdateWorkspace).toHaveBeenCalledWith('workspace-1', {
        repos: [{ url: 'git@github.com:adanman/goosar.git' }],
      });
    });
  });
  ...
  it('includes the description in the automatic update payload', async () => {
    workspaceRef.current = {
      ...workspaceRef.current,
      repos: [{ url: 'https://github.com/adanman/goosar', description: 'Main app' }],
    };
    const user = setupUser();
    render(<RepositoriesTab />, { wrapper: I18nWrapper });
  ...
  it('starts GitHub connection with the signed repository return target', async () => {
    ...
    mockGetGitHubConnectURL.mockResolvedValue({
      configured: true,
      url: 'https://github.com/apps/acme-bot/installations/new',
    });
  ...
  it('imports selected GitHub repositories and deduplicates HTTPS against SSH', async () => {
    ...
    githubRef.current = {
      installations: [{ id: 'installation-row-1', account_login: 'adanman' }],
      configured: true,
      repository_browse_configured: true,
      can_manage: true,
    };
    githubRepositoriesRef.current = [
      {
        id: 1,
        full_name: 'adanman/goosar',
        html_url: 'https://github.com/adanman/goosar',
        clone_url: 'https://github.com/adanman/goosar.git',
      ...
      {
        id: 2,
        full_name: 'adanman/console',
        html_url: 'https://github.com/adanman/console',
        clone_url: 'https://github.com/adanman/console.git',
    ...
    await waitFor(() => {
      expect(mockUpdateWorkspace).toHaveBeenCalledWith('workspace-1', {
        repos: [
          { url: 'git@github.com:adanman/goosar.git' },
          {
            url: 'https://github.com/adanman/console.git',
            description: 'Console app',
          },
        ],
      });
