...
export function KeyboardShortcutsTab() {
  ...
  const visibleActions = useMemo(() => {
    ...
    return SHORTCUT_ACTIONS.filter((action) => {
      ...
      return `${label} ${description}`.toLocaleLowerCase().includes(needle);
    });
  }, [query, t]);
  ...
  return (
    ...
    >
      ...
      {groups.map((category) => {
        ...
        return (
          <SettingsSection key={category} title={t(($) => $.shortcuts.categories[category])}>
            <SettingsCard>
              {actions.map((action) => (
                <ShortcutRow
                  key={action.id}
      ...
      >
        <SettingsCard>
          <FixedShortcutRow
            label={t(($) => $.shortcuts.fixed.close_tab)}
            shortcut={createShortcutChord('W', { primary: true })}
          />
          <FixedShortcutRow
            label={t(($) => $.shortcuts.fixed.zoom_in)}
            shortcut={createShortcutChord('Plus', { primary: true })}
          />
          <FixedShortcutRow
            label={t(($) => $.shortcuts.fixed.zoom_out)}
            shortcut={createShortcutChord('Minus', { primary: true })}
          />
          <FixedShortcutRow
            label={t(($) => $.shortcuts.fixed.reset_zoom)}
            shortcut={createShortcutChord('0', { primary: true })}
          />
          <FixedShortcutRow
            label={t(($) => $.shortcuts.fixed.close_dialog)}
      ...
      <AlertDialog open={resetConfirmOpen} onOpenChange={setResetConfirmOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t(($) => $.shortcuts.reset_confirm.title)}</AlertDialogTitle>
            <AlertDialogDescription>
              {t(($) => $.shortcuts.reset_confirm.description)}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>{t(($) => $.shortcuts.reset_confirm.cancel)}</AlertDialogCancel>
            <AlertDialogAction
              variant="destructive"
              onClick={() => {
                resetAll();
    ...
    <SettingsRow
      ...
      description={
        action.id === 'send' && isPlainShortcut(shortcut, 'Enter') ? (
          <>
            {description}
            <span className="mt-1 block text-brand">{t(($) => $.shortcuts.send_enter_hint)}</span>
          </>
        ) : (
          description
        )
