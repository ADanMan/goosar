...
describe('KeyboardShortcutsTab', () => {
  ...
  it('records a shortcut and applies it immediately', () => {
    ...
    expect(getShortcut('openSearch')).toEqual(createShortcutChord('J', { primary: true }));
  ...
  it('only allows Enter or Primary+Enter for Send', () => {
    ...
    expect(screen.getByRole('alert')).toHaveTextContent('Send can only use Enter or Mod+Enter.');
  ...
  it('rejects shortcuts already assigned to another action', () => {
    ...
    expect(screen.getByRole('alert')).toHaveTextContent('Already used by Open search.');
  });
  ...
  it('can disable and restore an action', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Reset Create issue' }));
    expect(getShortcut('createIssue')).toEqual(createShortcutChord('C'));
  });
  ...
  it('confirms before restoring all shortcut defaults', () => {
    useShortcutStore
      .getState()
      .setShortcut('openSearch', createShortcutChord('J', { primary: true }));
    renderWithI18n(<KeyboardShortcutsTab />);
