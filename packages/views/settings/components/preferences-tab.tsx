...
import { DEFAULT_LOCALE, SUPPORTED_LOCALES, type SupportedLocale } from '@goosar/core/i18n';
import { useLocaleAdapter } from '@goosar/core/i18n/react';
import { useAuthStore } from '@goosar/core/auth';
import { useCommentComposerStore, useIssueLinkStore } from '@goosar/core/issues/stores';
  ...
  return (
    <SettingsTab title={t(($) => $.page.tabs.preferences)}>
      <SettingsSection title={t(($) => $.preferences.general_title)}>
        <SettingsCard>
          <SettingsRow label={t(($) => $.preferences.theme.title)} size="select">
            <Select
              items={themeOptions}
              value={theme}
              onValueChange={(next) => {
          ...
          <SettingsRow label={t(($) => $.preferences.language.title)} size="select">
            <Select
...
function TimezoneRow() {
  ...
  const options = useMemo(() => timezoneOptions(stored ?? browser), [stored, browser]);
