...
describe('DeleteWorkspaceDialog', () => {
  ...
  it('submits on Enter when matched; ignores Enter when not matched', async () => {
    ...
    await user.type(input, 'acm{Enter}'); 
  ...
  it('fails closed when the workspace has no name instead of pre-satisfying itself', () => {
    const onConfirm = vi.fn();
    render(
      <DeleteWorkspaceDialog workspaceName="" open onOpenChange={vi.fn()} onConfirm={onConfirm} />,
    );

    expect(screen.queryByRole('textbox')).toBeNull();
    expect(
      screen.getByText('There is no exact value to type here, so this action cannot be confirmed.'),
    ).toBeTruthy();
  ...
  it('never spells the answer out in the placeholder', () => {
    ...
    expect(input.placeholder).not.toContain('acme');
  });
});
