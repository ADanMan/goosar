...
import { agentListOptions, memberListOptions, workspaceKeys } from '@goosar/core/workspace/queries';
import { AGENT_DESCRIPTION_MAX_LENGTH } from '@goosar/core/agents';
import { isImeComposing } from '@goosar/core/utils';
import type { Agent, MemberWithUser } from '@goosar/core/types';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function CreateSquadModal({ onClose }: { onClose: () => void }) {
  ...
  const handleLeaderChange = (id: string) => {
    setLeaderId(id);
    if (id) {
      setSelectedMembers((prev) => prev.filter((m) => !(m.type === 'agent' && m.id === id)));
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : t(($) => $.create_squad.toast_failed));
      setCreating(false);
    }
  };
      ...
      <DialogContent className="p-0 gap-0 flex flex-col overflow-hidden !top-1/2 !left-1/2 !-translate-x-1/2 !-translate-y-1/2 !w-full !max-w-2xl !h-[85vh]">
        ...
        <div className="flex items-center justify-end gap-2 border-t bg-background px-5 py-3">
          ...
          <Button onClick={() => void handleSubmit()} disabled={!canSubmit}>
            {creating ? t(($) => $.create_squad.submitting) : t(($) => $.create_squad.submit)}
          </Button>
        </div>
      </DialogContent>
    </Dialog>
...
function LeaderPicker({
  ...
  const otherAgents = useMemo(
    () => (currentUserId ? agents.filter((a) => a.owner_id !== currentUserId) : agents),
    [agents, currentUserId],
  );

  const q = filter.trim().toLowerCase();
  const matches = (a: Agent) => !q || a.name.toLowerCase().includes(q) || matchesPinyin(a.name, q);
        ...
        >
          ...
          <PopoverContent align="start" className="w-[var(--anchor-width)] p-0">
            ...
            <div className="max-h-72 overflow-y-auto p-1">
              ...
              {filteredMine.length === 0 && filteredOthers.length === 0 && <PickerEmpty />}
            </div>
          </PopoverContent>
        </Popover>
      )}
...
function AdditionalMembersPicker({
  ...
  const myAgents = useMemo(
    () =>
      currentUserId ? agents.filter((a) => a.owner_id === currentUserId && a.id !== leaderId) : [],
    [agents, currentUserId, leaderId],
  );
  const otherAgents = useMemo(
    () =>
      agents.filter(
        (a) => a.id !== leaderId && (currentUserId ? a.owner_id !== currentUserId : true),
      ),
    [agents, currentUserId, leaderId],
  );
  ...
  const anyResults = filteredMine.length + filteredOthers.length + filteredMembers.length > 0;
              ...
              {value.length === 0 ? (
                ...
                <div className="flex min-w-0 flex-1 flex-wrap items-center gap-1.5">
                  {value.slice(0, CHIP_DISPLAY_LIMIT).map((m) => (
                    <MemberChip key={`${m.type}:${m.id}`} m={m} onRemove={() => remove(m)} />
                  ))}
              ...
              <PickerSection label={t(($) => $.create_squad.group_my_agents)}>
                {filteredMine.map((a) => (
                  <PickerItem
                    ...
                    onClick={() => toggle({ type: 'agent', id: a.id, name: a.name })}
                  >
                    <ActorAvatar actorType="agent" actorId={a.id} size="sm" showStatusDot />
                    <span className="truncate">{a.name}</span>
                  </PickerItem>
              ...
              <PickerSection label={t(($) => $.create_squad.group_members)}>
                {filteredMembers.map((m) => (
                  <PickerItem
                    ...
                    onClick={() => toggle({ type: 'member', id: m.user_id, name: m.name })}
                  >
...
function MemberChip({ m, onRemove }: { m: SelectedMember; onRemove: () => void }) {
  const { t } = useT('modals');
  return (
