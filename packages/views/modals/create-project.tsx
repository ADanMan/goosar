...
import { useCreateProject } from '@goosar/core/projects/mutations';
import { useProjectDraftStore } from '@goosar/core/projects';
import {
...
import { isDesktopShell, pickDirectory, validateLocalDirectory } from '../platform/local-directory';
...
export function CreateProjectModal({ onClose }: { onClose: () => void }) {
    ...
    try {
      ...
      if (!picked.ok || !picked.path) {
        if (picked.reason && picked.reason !== 'cancelled') {
          setLocalPickError(picked.error ?? t(($) => $.create_project.local_pick_failed));
        }
        return;
      }
      const validation = await validateLocalDirectory(picked.path);
      if (!validation.ok) {
        setLocalPickError(validation.error ?? t(($) => $.create_project.local_invalid_dir));
        return;
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(
        err instanceof Error && err.message ? err.message : t(($) => $.create_project.toast_failed),
      );
    } finally {
      setSubmitting(false);
    }
          ...
          <DropdownMenu>
            <DropdownMenuTrigger
              render={
                <PillButton>
                  <span
                    className={cn('size-2 rounded-full', PROJECT_STATUS_CONFIG[status].dotColor)}
                  />
