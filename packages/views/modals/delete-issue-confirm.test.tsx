...
async function deleteWith(adapter: NavigationAdapter, data: Record<string, unknown> | null) {
  const onClose = vi.fn();
  render(
...
describe('DeleteIssueConfirmModal', () => {
  ...
  it('falls back to the workspace list when the issue was opened cold', async () => {
    ...
    await waitFor(() => expect(adapter.replace).toHaveBeenCalledWith('/acme/issues'));
