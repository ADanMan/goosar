...
vi.mock('@goosar/core/paths', () => ({ useCurrentWorkspace: () => ({ id: 'ws1' }) }));
vi.mock('@goosar/core/hooks/use-file-upload', () => ({
  useFileUpload: () => ({ uploadWithToast: vi.fn() }),
}));
vi.mock('@goosar/core/api', () => ({ api: {} }));
vi.mock('sonner', () => ({ toast: { info: vi.fn(), error: vi.fn(), success: vi.fn() } }));
vi.mock('@goosar/core/feedback', () => ({
  ...
  const ContentEditor = forwardRef(({ defaultValue, onSubmit, onUploadingChange }: any, ref) => {
    ...
    return (
      <textarea
        ...
        onChange={(event) => {
          liveEditorMarkdown = event.currentTarget.value;
        }}
        onKeyDown={(event) => {
