...
import { runtimeProfileListOptions } from '@goosar/core/runtimes';
...
import { buildRuntimeMachines, sharedCustomName } from './runtime-machines';
...
function findMachine(machines: ReturnType<typeof buildRuntimeMachines>, locator: string) {
  return (
    machines.find(
      (candidate) =>
        candidate.id === locator || candidate.runtimes.some((runtime) => runtime.id === locator),
    ) ??
...
export function RuntimeDetailPage({
  ...
  const { data: runtimeProfiles = [] } = useQuery(runtimeProfileListOptions(wsId));
  const now = useNowTick();
  const machineLocator = decodeRouteParam(runtimeId);
  const [renameOpen, setRenameOpen] = useState(false);
  const [createProfileOpen, setCreateProfileOpen] = useState(false);
  const workloadIndex = useMemo(() => buildWorkloadIndex(agents, tasks), [agents, tasks]);
  const baseMachines = useMemo(
    ...
    [runtimes, now, localDaemonId, localMachineName, currentUserId, workloadIndex, hasLocalMachine],
  );
  ...
  const canAddRuntime = isAdmin && machine?.mode === 'local' && !!machine.daemonId;
    ...
    return (
      <div className="flex min-h-0 flex-1 flex-col">
        <div className="flex flex-1 flex-col items-center justify-center gap-3 px-6 py-16 text-center">
          ...
          <div>
            <p className="text-sm font-medium">{t(($) => $.machine.not_found_title)}</p>
          ...
          <div className="mt-4 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex min-w-0 items-start gap-4">
              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border bg-card sm:h-14 sm:w-14">
                <Icon aria-hidden="true" className="h-5 w-5 text-muted-foreground sm:h-6 sm:w-6" />
              </div>
              <div className="min-w-0 pt-0.5">
                ...
                {machine.subtitle && (
                  <p className="mt-1 text-sm text-muted-foreground">{machine.subtitle}</p>
                )}
                <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-muted-foreground">
                  ...
                  />
                  {machine.lastSeenAt && <span>{timeAgo(machine.lastSeenAt)}</span>}
                </div>
              </div>
            </div>
        ...
        <div className="mx-auto w-full max-w-[1440px] p-4 sm:p-6">
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            <div className="min-w-0">
              <h2 className="text-sm font-semibold">{t(($) => $.machine.metrics.runtimes)}</h2>
              <p className="mt-1 text-xs text-muted-foreground">
                {t(($) => $.machine.select_runtime)}
              </p>
            </div>
            {canAddRuntime && (
              <Button type="button" size="sm" onClick={() => setCreateProfileOpen(true)}>
          ...
          {machineRuntimes.length > 0 ? (
            <div className="overflow-hidden rounded-lg border bg-card">
              <RuntimeList
                ...
                runtimeHref={(childRuntimeId) => paths.runtimeSettings(machine.id, childRuntimeId)}
              />
            </div>
          ) : (
            <div className="flex min-h-48 flex-col items-center justify-center rounded-lg border border-dashed px-6 text-center">
              <Server aria-hidden="true" className="h-7 w-7 text-muted-foreground/40" />
