...
import type { RuntimeProfile, RuntimeProtocolFamily } from '@goosar/core/types';
import {
...
export function RuntimeProfilesDialog({
  ...
  const { data: profiles = [], isLoading } = useQuery(runtimeProfileListOptions(wsId));
  ...
  const [draftFamily, setDraftFamily] = useState<RuntimeProtocolFamily>(
    PROTOCOL_FAMILIES[0] ?? 'runtime-c',
  );

  const catalog = useMemo(() => buildRuntimeCatalog(profiles), [profiles]);
  const entries = useMemo(() => [...catalog.customs, ...catalog.builtins], [catalog]);
  const selectedEntry = entries.find((entry) => entry.id === selectedId) ?? null;
  const openCreateForm = () => setState({ surface: 'form', mode: 'create', step: 'family' });
      ...
      >
        <DialogHeader className="border-b px-6 py-5">
          <div className="flex items-start justify-between gap-3">
            <DialogTitle className="flex min-w-0 items-center gap-2 text-base">
              <Server aria-hidden="true" className="h-4 w-4 shrink-0 text-muted-foreground" />
            ...
            <div className="flex shrink-0 items-center gap-2">
              {state.surface === 'browse' && catalog.customs.length > 0 && (
                <Button type="button" size="sm" className="h-8 px-2.5" onClick={openCreateForm}>
        ...
        {state.surface === 'form' ? (
          <ProfileFormView
            ...
            family={state.mode === 'edit' ? state.profile.protocol_family : draftFamily}
          ...
          <div className="grid min-h-0 flex-1 grid-cols-1 md:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
            ...
            <DetailPanel
              entry={selectedEntry}
              wsId={wsId}
              onEdit={(profile) => setState({ surface: 'form', mode: 'edit', profile })}
...
function CatalogList({
  ...
  const selectedIsBuiltin = catalog.builtins.some((entry) => entry.id === selectedId);

  return (
    <div className="flex min-h-0 flex-col border-b md:border-b-0 md:border-r">
      <div className="flex shrink-0 items-center justify-between border-b bg-background px-4 py-3">
        <h3 className="text-sm font-medium">{t(($) => $.profiles.list_title)}</h3>
      </div>
      {loading ? (
    ...
    <div className="rounded-md border bg-muted/20 p-4">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <Button type="button" size="sm" className="mt-3 h-8 px-2.5" onClick={onAddNew}>
            <Plus aria-hidden="true" className="h-3.5 w-3.5" />
            {t(($) => $.profiles.add_new)}
          </Button>
        </div>
...
function CatalogRow({
  ...
  const { t } = useT('runtimes');
  const label = entry.kind === 'custom' ? entry.profile.display_name : entry.protocolFamily;
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="flex items-center gap-1.5">
          <span
            className={cn('truncate text-sm font-medium', entry.kind === 'builtin' && 'capitalize')}
          >
            {label}
          </span>
          {disabled && (
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-6">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h3 className="truncate text-base font-semibold capitalize">{entry.protocolFamily}</h3>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-6">
        <div className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            <span className="flex h-10 w-10 items-center justify-center rounded-md border bg-background">
              <ProviderLogo provider={profile.protocol_family} className="h-5 w-5" />
            </span>
            <div className="min-w-0">
...
function ProfileDetailsForm({
  ...
  const [values, setValues] = useState<ProfileFormValues>({
    ...
    commandLine: profile ? formatCommandLine(profile.command_name, profile.fixed_args) : '',
  ...
  const parsedCommand = useMemo(() => parseCommandLine(values.commandLine), [values.commandLine]);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setFormError(
        err instanceof Error && err.message ? err.message : t(($) => $.profiles.form.error_generic),
      );
    }
  };
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      >
        ...
        <div className="space-y-1.5">
          <Label htmlFor={`${idPrefix}-display-name`} className="text-xs text-muted-foreground">
          ...
          {hasError('displayName') && (
            <p id={`${idPrefix}-display-name-error`} className="text-xs text-destructive">
        ...
        <div className="space-y-1.5">
          <Label htmlFor={`${idPrefix}-command`} className="text-xs text-muted-foreground">
          ...
          <Input
            ...
            aria-describedby={hasError('commandLine') ? `${idPrefix}-command-error` : undefined}
      ...
      <div className="flex shrink-0 justify-between gap-2 border-t bg-muted/30 px-6 py-3">
        ...
        <div className="flex gap-2">
          ...
          <Button type="submit" size="sm" form={formId} disabled={submitting}>
            {submitting && <Loader2 aria-hidden="true" className="h-3.5 w-3.5 animate-spin" />}
            {mode === 'create'
              ? submitting
