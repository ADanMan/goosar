import { deriveRuntimeState, type RuntimeHealth, type RuntimeState } from '@goosar/core/runtimes';
...
export function runtimeRowLabel(runtime: AgentRuntime, machineTitle: string): string {
...
function placeholderLocalMachine(options: RuntimeMachineOptions): RuntimeMachine {
...
function finalizeRuntimeMachine(
  ...
  const runtimes = draft.runtimes.toSorted((a, b) => a.provider.localeCompare(b.provider));
  const first = runtimes[0];
  const providerNames = Array.from(new Set(runtimes.map((r) => r.provider))).sort();
  const ownsLocalRuntime =
    !!options.currentUserId && runtimes.some((r) => r.owner_id === options.currentUserId);
  ...
  const states = runtimes.map((runtime) => deriveRuntimeState(runtime, options.now));
  const liveHealth = states
    .map((state) => state.health)
    .filter((health): health is RuntimeHealth => health !== null);
  const onlineCount = liveHealth.filter((h) => h === 'online').length;
  const issueCount = states.filter((state) => state.needsAttention).length;
  const health = worstHealth(liveHealth);
  ...
  return {
    ...
    providerNames,
    lastSeenAt: latestLastSeenAt(states),
  };
}

function worstHealth(healths: RuntimeHealth[]): RuntimeHealth {
  return healths.reduce<RuntimeHealth>(
    (worst, current) => (HEALTH_SEVERITY[current] > HEALTH_SEVERITY[worst] ? current : worst),
    healths.length === 0 ? 'offline' : 'online',
  );
}
...
function compactDeviceInfo(deviceInfo: string | null, title: string): string | null {
...
function latestLastSeenAt(states: RuntimeState[]): string | null {
  let latest: string | null = null;
  for (const state of states) {
    if (!state.lastSeenAt) continue;
    if (!latest || new Date(state.lastSeenAt) > new Date(latest)) {
