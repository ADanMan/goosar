...
function runtime(id: string, daemonId: string, provider: string): AgentRuntime {
  return {
    id,
    workspace_id: 'ws-1',
    daemon_id: daemonId,
...
describe('resolveRuntimeSettingsTarget', () => {
  ...
  const machineBRuntime = runtime('runtime-j', 'daemon-b', 'claude');
  const runtimes = [machineARuntime, machineBRuntime];

  it('resolves a runtime inside its owning machine', () => {
    const target = resolveRuntimeSettingsTarget(runtimes, 'local:daemon-a', 'runtime-a');
  ...
  it('rejects a runtime that belongs to a different machine', () => {
    expect(resolveRuntimeSettingsTarget(runtimes, 'local:daemon-a', 'runtime-j')).toBeNull();
  });
});
