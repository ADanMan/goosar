...
export function RuntimeSettingsPage({
  ...
  const target = resolveRuntimeSettingsTarget(runtimes, decodedMachineId, decodedRuntimeId);
  if (!target) {
    return (
      <div className="flex h-full flex-col items-center justify-center text-muted-foreground">
        <Server aria-hidden="true" className="h-10 w-10 text-muted-foreground/30" />
  ...
  return (
    <RuntimeDetail
      ...
      afterDeleteHref={
        machine.runtimes.length > 1 ? paths.runtimeDetail(decodedMachineId) : paths.runtimes()
      }
    />
  );
}
...
export function resolveRuntimeSettingsTarget(
  ...
  const machine = buildRuntimeMachines(runtimes, { now: Date.now() }).find(
    (candidate) =>
      candidate.id === machineId || candidate.runtimes.some((item) => item.id === machineId),
  );
