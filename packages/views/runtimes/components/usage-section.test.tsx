...
vi.mock('@goosar/core/runtimes/custom-pricing-store', () => {
  const useCustomPricingStore = Object.assign(
    (sel?: (s: typeof pricingState) => unknown) => (sel ? sel(pricingState) : pricingState),
...
vi.mock('./charts', () => ({
  ...
  ActivityHeatmap: ({ tz }: { tz: string }) => <div data-testid="heatmap-tz">{tz}</div>,
}));
...
describe('UsageSection — custom-pricing entry point', () => {
  ...
  it('stays hidden when every model resolves and nothing is overridden', () => {
    ...
    expect(screen.queryByRole('button', { name: 'Edit custom prices' })).toBeNull();
  });
  ...
  it('warns and offers the dialog while a model is unpriced', () => {
    ...
    expect(screen.getByRole('button', { name: 'Set custom prices' })).toBeInTheDocument();
  });
  ...
  it('keeps the dialog reachable after the last override is saved', () => {
    ...
    expect(screen.getByRole('button', { name: 'Edit custom prices' })).toBeInTheDocument();
  });
});
