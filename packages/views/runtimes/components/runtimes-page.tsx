...
import { ChevronRight, Cloud, Monitor, Plus, Server } from 'lucide-react';
...
function OrphanRuntimeProfiles({
  ...
  return (
    <section className={hasMachines ? 'mt-6' : undefined}>
      <div className="mb-3">
        <h2 className="text-sm font-semibold">{t(($) => $.profiles.unassigned_title)}</h2>
...
function PageHeaderBar({
  ...
  return (
    <CollectionPageHeader
      ...
      learnMore={{
        href: installRuntimeDocsHref(i18n.language),
...
function MachineList({
  ...
  if (machines.length === 0) {
    return (
      <CollectionPageState
        icon={Server}
        title={bootstrapping ? t(($) => $.page.bootstrapping.title) : t(($) => $.page.empty.title)}
        description={
          bootstrapping ? t(($) => $.page.bootstrapping.hint) : t(($) => $.page.empty.hint)
        }
      />
    );
  }
