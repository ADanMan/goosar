...
import { Loader2, CheckCircle2, XCircle, ArrowUpCircle, Check, Lock } from 'lucide-react';
import { Button } from '@goosar/ui/components/ui/button';
import { api } from '@goosar/core/api';
import { useConfigStore, isPerimeterDeliveryProfile } from '@goosar/core/config';
import type { RuntimeUpdateStatus } from '@goosar/core/types';
import { useT } from '../../i18n';

const GITHUB_RELEASES_URL = 'https://api.github.com/repos/adanman/goosar/releases/latest';
...
export function UpdateSection({
  ...
  const isOperatorManaged = useConfigStore((state) =>
    ...
    <div className="space-y-2">
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {isManaged ? (
          ...
          <span
            className="inline-flex items-center gap-1 text-xs text-muted-foreground"
            title={t(($) => $.update.managed_by_operator_title)}
          >
            <Lock className="h-3 w-3" />
            {t(($) => $.update.managed_by_operator)}
          </span>
        ) : (
          <>
