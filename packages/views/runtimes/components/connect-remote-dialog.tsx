...
import { daemonCommands, installCommand } from '../install-command';
...
function InstructionsStep({ onClose }: { onClose: () => void }) {
  ...
  const installCmd = installCommand(daemonServerUrl, daemonAppUrl);
  return (
    <>
      <DialogHeader className="px-6 pt-6 pb-2">
        <DialogTitle className="text-base text-balance">{t(($) => $.connect.title)}</DialogTitle>
...
function LiveListening() {
  ...
  return (
    ...
    >
      ...
      <span className="font-medium text-foreground">{t(($) => $.connect.live_listening)}</span>
      <span className="text-muted-foreground">{t(($) => $.connect.live_listening_hint)}</span>
    </div>
  );
}
