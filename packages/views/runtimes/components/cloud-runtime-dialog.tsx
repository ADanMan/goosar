...
import type { CloudRuntimeNode } from '@goosar/core/runtimes';
import {
...
export function CloudRuntimeDialog({ onClose }: { onClose: () => void }) {
  ...
  const [instanceType, setInstanceType] = useState<string>(DEFAULT_INSTANCE_TYPE);
  const [diskSizeGB, setDiskSizeGB] = useState(String(DEFAULT_DISK_SIZE_GB));

  const nodesQuery = useQuery(cloudRuntimeNodeListOptions(wsId, { limit: 20, offset: 0 }));
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const diskSize = diskSizeGB.trim() ? Number(diskSizeGB.trim()) : DEFAULT_DISK_SIZE_GB;
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : t(($) => $.cloud_runtime.toast_create_failed),
      );
    }
  };
    ...
    <Dialog open onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="flex max-h-[88vh] flex-col gap-0 p-0 sm:max-w-3xl">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
          <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.82fr)]">
            <form id={formId} onSubmit={handleSubmit} className="space-y-4">
              <div>
                <h3 className="text-sm font-medium">{t(($) => $.cloud_runtime.create_title)}</h3>
            ...
            <section className="min-h-0 rounded-md border bg-muted/20">
              <div className="flex items-center justify-between border-b bg-background px-3 py-2.5">
                <h3 className="text-sm font-medium">{t(($) => $.cloud_runtime.nodes_title)}</h3>
                <Button
                  type="button"
                  variant="ghost"
                  size="sm"
                ...
                >
                  <RefreshCw
                    className={cn('h-3.5 w-3.5', nodesQuery.isFetching && 'animate-spin')}
                  />
              ...
              {nodesQuery.isLoading ? (
                ...
                <div className="flex h-40 flex-col items-center justify-center px-5 text-center">
                  <p className="text-sm font-medium">{t(($) => $.cloud_runtime.nodes_failed)}</p>
                  <p className="mt-1 text-xs text-muted-foreground">
        ...
        <DialogFooter className="m-0 border-t bg-muted/30 px-6 py-3">
          ...
          <Button type="submit" size="sm" form={formId} disabled={createNode.isPending}>
      ...
      <div className="space-y-1.5">
        ...
        >
          <SelectTrigger id={id} className="h-9 w-full rounded-md text-sm">
            <SelectValue>{() => <span className="truncate">{value}</span>}</SelectValue>
          </SelectTrigger>
...
function CloudRuntimeNodeRow({ node, wsId }: { node: CloudRuntimeNode; wsId: string }) {
  ...
  const title =
    node.name.trim() || node.instance_id.trim() || t(($) => $.cloud_runtime.node_fallback_name);
  const created = formatDateTime(node.created_at, uiLocale);
  return (
