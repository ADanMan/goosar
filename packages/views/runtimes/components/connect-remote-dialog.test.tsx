...
function renderDialog(config?: { daemonServerUrl?: string; daemonAppUrl?: string }) {
  resetConfigStore();
  if (config) {
...
describe('ConnectRemoteDialog', () => {
  ...
  it('uses cloud setup commands by default', () => {
    ...
    expect(baseElement).toHaveTextContent('goosar config set server_url https://goosar.ru');
    expect(baseElement).toHaveTextContent('goosar config set app_url https://goosar.ru');
  });
  ...
  it('uses self-host daemon URLs from runtime config', () => {
    ...
    expect(baseElement).toHaveTextContent('goosar config set server_url https://api.example.com');
    expect(baseElement).toHaveTextContent('goosar config set app_url https://app.example.com');
  });
  ...
  it('disables font ligatures in setup command code', () => {
    ...
    const setupCode = Array.from(baseElement.querySelectorAll('code')).find((node) =>
      node.textContent?.includes('goosar setup'),
    );
  ...
  it('disables font ligatures in fallback token command code', () => {
    ...
    const tokenCode = Array.from(baseElement.querySelectorAll('code')).find((node) =>
      node.textContent?.includes('goosar login --token <YOUR_TOKEN>'),
    );
  ...
  it('transitions from setup instructions to the connected state', async () => {
    ...
    expect(baseElement).not.toHaveTextContent('goosar setup');
  });
});
