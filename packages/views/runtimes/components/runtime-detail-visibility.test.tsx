...
vi.mock('@goosar/core/api', () => ({
  api: {
    ...
    deleteRuntimeProfile: (...args: unknown[]) => mockDeleteRuntimeProfile(...args),
  },
...
vi.mock('@goosar/core/runtimes', async () => {
  const { deriveRuntimeState } = await vi.importActual<typeof import('@goosar/core/runtimes')>(
    '../../../core/runtimes/derive-health',
  );
  return {
...
describe('RuntimeDetail visibility section', () => {
  ...
  it('hides the Delete runtime button entirely for callers who cannot edit', () => {
    ...
    expect(screen.queryByRole('button', { name: /Delete runtime/i })).not.toBeInTheDocument();
  });

  it('routes custom runtime deletion through the profile delete dialog for admins', async () => {
    const profile = makeProfile();
    mockQueryData.members = [{ user_id: 'user-me', role: 'owner', name: 'Me' }];
    ...
    expect(screen.getByText('Delete custom runtime from workspace?')).toBeInTheDocument();
    expect(
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Delete from workspace' }));
    await waitFor(() => expect(mockDeleteRuntimeProfile).toHaveBeenCalledWith(profile.id));
  });
  ...
  it('presents a runtime bound to another machine as unavailable, never as freshly seen', () => {
    const profile = makeProfile();
    mockQueryData.profiles = [profile];
    ...
    expect(screen.getByText('command not found on PATH: hermes')).toBeInTheDocument();
  });
});
