...
const COLUMN_KEYS: SkillColumnKey[] = ['usedBy', 'source', 'creator', 'updated', 'created'];

const SORT_FIELDS: SkillSortField[] = ['name', 'usedBy', 'updated', 'created'];

export function countActiveFilterDimensions(filters: SkillListFilters): number {
  let count = 0;
...
const ORIGIN_TYPES: OriginType[] = ['manual', 'runtime_local', 'clawhub', 'skills_sh', 'github'];
...
export function SkillListToolbar({
  ...
  const creatorOptions = new Map<string, { member: MemberWithUser; count: number }>();
      ...
      <div className="flex shrink-0 items-center gap-1">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-auto">
            ...
            <DropdownMenuSub>
              <DropdownMenuSubTrigger>
                <span className="flex-1">{t(($) => $.toolbar.section_usage)}</span>
                {filters.usage.length > 0 && (
                  <span className="text-xs font-medium text-primary">{filters.usage.length}</span>
                )}
              </DropdownMenuSubTrigger>
            ...
            <DropdownMenuSub>
              <DropdownMenuSubTrigger>
                ...
                {filters.origins.length > 0 && (
                  <span className="text-xs font-medium text-primary">{filters.origins.length}</span>
                )}
              </DropdownMenuSubTrigger>
              <DropdownMenuSubContent className="w-auto min-w-48">
                {ORIGIN_TYPES.filter((type) => originCounts.has(type)).map((type) => (
                  <DropdownMenuCheckboxItem
                    key={type}
            ...
            <DropdownMenuSub>
              <DropdownMenuSubTrigger>
                ...
                {filters.agents.length > 0 && (
                  <span className="text-xs font-medium text-primary">{filters.agents.length}</span>
                )}
              </DropdownMenuSubTrigger>
              ...
              <DropdownMenuSubContent className="max-h-72 w-auto min-w-48 overflow-y-auto">
                {[...creatorOptions.values()].map(({ member, count }) => (
                  <DropdownMenuCheckboxItem
                    ...
                    onCheckedChange={() => onToggleFilter('creators', member.user_id)}
                    className={FILTER_ITEM_CLASS}
                  >
              ...
              <div className="mt-2 flex items-center gap-1.5">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="start" className="w-auto">
                    <DropdownMenuRadioGroup
                      value={sortField}
                      onValueChange={(v) => onSortFieldChange(v as SkillSortField)}
                    >
