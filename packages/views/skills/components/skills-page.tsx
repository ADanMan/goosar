...
import type { Agent, AgentRuntime, MemberWithUser, Skill, SkillSummary } from '@goosar/core/types';
...
import {
  ...
  type SkillColumnKey,
  type SkillSortField,
} from '@goosar/core/skills/stores';
import { SkillListToolbar } from './skill-list-toolbar';
import { SkillBatchToolbar, SkillRowActions, type SkillActionsContext } from './skill-list-actions';
...
function columnTrackVars(isVisible: (key: SkillColumnKey) => boolean): React.CSSProperties {
  const width = (key: SkillColumnKey) => (isVisible(key) ? `${COLUMN_WIDTHS[key]}px` : '0px');
  const minWidth =
    FIXED_TRACKS_WIDTH +
...
function NameCell({ row }: { row: SkillRow }) {
  ...
  return (
    <ListGridCell className="gap-1.5">
      ...
      {!canEdit && (
        <Tooltip>
          <TooltipTrigger render={<Lock className="h-3 w-3 shrink-0 text-muted-foreground/60" />} />
...
function UsedByCell({ agents }: { agents: Agent[] }) {
  ...
  if (agents.length === 0) {
    return (
      <ListGridCell>
        <span className="text-xs text-muted-foreground/70">{t(($) => $.table.unused)}</span>
      </ListGridCell>
    );
  }
    ...
    return (
      <ListGridCell className="gap-1.5">
        <ActorAvatar
          ...
          size="md"
        />
        <span className="min-w-0 truncate text-xs text-muted-foreground">{agent.name}</span>
      </ListGridCell>
    );
  }
...
function SourceCell({ skill, runtime }: { skill: SkillSummary; runtime: AgentRuntime | null }) {
  const { t } = useT('skills');
      ...
      {creator && (
        <>
          <ActorAvatar
            ...
            size="md"
          />
          <span className="min-w-0 truncate text-xs text-muted-foreground">{creator.name}</span>
        </>
      )}
    </ListGridCell>
  );
...
function SkillListHeader({
  ...
  const { t } = useT('skills');
  const sorted = (field: SortField) => (sortField === field ? sortDirection : false);
  ...
  return (
    <ListGridHeader>
      ...
      {isColVisible('usedBy') ? (
        <ListGridHeaderCell sorted={sorted('usedBy')} onSort={() => onSort('usedBy')}>
          {t(($) => $.table.used_by)}
        </ListGridHeaderCell>
      ) : (
...
export default function SkillsPage() {
  ...
  const myRole = members.find((m: MemberWithUser) => m.user_id === currentUserId)?.role ?? null;
  ...
  const rows = useMemo<SkillRow[]>(() => {
    ...
    const filtered = allRows.filter((row) => {
      ...
      if (filters.agents.length > 0 && !row.agents.some((a) => filters.agents.includes(a.id))) {
        return false;
      }
      if (
    ...
    filtered.sort((a, b) => {
      ...
      if (sortField === 'usedBy') {
        return (
          (a.agents.length - b.agents.length) * dir || a.skill.name.localeCompare(b.skill.name)
        );
      }
      if (sortField === 'created') {
        return (Date.parse(a.skill.created_at) - Date.parse(b.skill.created_at)) * dir;
      }
      return (Date.parse(a.skill.updated_at) - Date.parse(b.skill.updated_at)) * dir;
    });
    return filtered;
  ...
  const handleToggleAll = () => {
    setSelectedIds(allSelected ? new Set() : new Set(rows.map((r) => r.skill.id)));
  };
    ...
    return (
      <div className="flex flex-1 min-h-0 flex-col">
        ...
        <CollectionPageState
          ...
          description={
            listError instanceof Error ? listError.message : t(($) => $.page.list_error.fallback)
          }
          actions={
