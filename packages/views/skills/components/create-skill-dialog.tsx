...
import { anySkillSourceEnabled, isSkillSourceEnabled, useConfigStore } from '@goosar/core/config';
import type { Skill } from '@goosar/core/types';
import { useWorkspaceId } from '@goosar/core/hooks';
import { isImeComposing } from '@goosar/core/utils';
import { skillDetailOptions, workspaceKeys } from '@goosar/core/workspace/queries';
...
function MethodChooser({ onChoose }: { onChoose: (m: Method) => void }) {
  const { t } = useT('skills');
  const skillSources = useConfigStore((s) => s.skillSources);
  const allMethods: {
    key: Method;
    icon: typeof Plus;
  ...
  const methods = allMethods.filter(
    ({ key }) => key !== 'url' || anySkillSourceEnabled(skillSources),
  );
  return (
...
function UrlForm({
  ...
  const sourceCards = [
    {
      key: 'clawhub' as const,
      enabled: isSkillSourceEnabled(skillSources, 'clawhub'),
      label: 'ClawHub',
    ...
    {
      key: 'skillssh' as const,
      enabled: isSkillSourceEnabled(skillSources, 'skillssh'),
      label: 'Skills.sh',
    ...
    {
      key: 'github' as const,
      enabled: isSkillSourceEnabled(skillSources, 'github'),
      label: 'GitHub',
  ...
  const gridColsClass =
    sourceCards.length === 1
      ...
      : sourceCards.length === 2
        ? 'grid-cols-2'
        : 'grid-cols-3';
  const urlPlaceholder = sourceCards[0]?.placeholder ?? 'https://clawhub.ai/owner/skill';
          ...
          <div className={cn('grid gap-2', gridColsClass)}>
            {sourceCards.map((card) => (
              <SourceCard
                key={card.key}
                label={card.label}
                exampleHost={card.exampleHost}
                browseUrl={card.browseUrl}
                active={card.active}
              />
            ))}
          </div>
  ...
  return (
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      ...
      >
        ...
        {method === 'runtime' && (
          <RuntimeLocalSkillImportPanel onImported={handleCreated} onBulkDone={onClose} />
        )}
      </DialogContent>
    </Dialog>
  );
