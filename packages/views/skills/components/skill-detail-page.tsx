...
import { runtimeDisplayLabel, runtimeDisplayName, runtimeListOptions } from '@goosar/core/runtimes';
import { ActorAvatar } from '@goosar/ui/components/common/actor-avatar';
import { Button, buttonVariants } from '@goosar/ui/components/ui/button';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import { useSkillPermissions } from '@goosar/core/permissions';
...
import { AddToAgentDialog, type SkillActionsContext } from './skill-list-actions';
...
function OriginSidebarCard({
  ...
  return (
    <div className="rounded-md border bg-muted/30 p-3">
      ...
      {runtime && (
        <div className="mt-1 break-all text-xs text-foreground">{runtimeDisplayName(runtime)}</div>
      )}
      {origin.source_path && (
  ...
  return (
    <div className="flex flex-1 min-h-0 flex-col">
      <BreadcrumbHeader
        ...
        leaf={<span className="truncate font-mono text-xs text-foreground">{skill.name}</span>}
        actions={
          <>
            {!canEdit && (
      ...
      <div className="flex flex-1 min-h-0 flex-col overflow-y-auto md:flex-row md:overflow-hidden">
        ...
        <section className="flex min-h-[32rem] min-w-0 shrink-0 flex-col md:min-h-0 md:flex-1 md:shrink">
          ...
          <div className="space-y-2 border-b px-4 py-4 sm:px-5">
            ...
            <ResourceLabelPicker resourceType="skill" resourceId={skill.id} canEdit={canEdit} />
