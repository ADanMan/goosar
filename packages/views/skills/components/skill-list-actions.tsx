...
function partitionAgents(ctx: SkillActionsContext) {
  ...
  const mine = active.filter((a) => a.owner_id !== null && a.owner_id === ctx.currentUserId);
  const others = ctx.isAdmin
    ? active.filter((a) => a.owner_id === null || a.owner_id !== ctx.currentUserId)
    : [];
  return { mine, others };
}
...
function AgentPickerRow({
  ...
  const { t } = useT('skills');
  const owned = skillIds.filter((id) => agent.skills.some((s) => s.id === id)).length;
  ...
  return (
    <button
      ...
      className={cn(
        ...
        hasAll ? 'opacity-50' : selected ? 'bg-accent' : 'hover:bg-accent/50',
      )}
    >
      {/* Indicator only — the wrapping <button> handles clicks. */}
      <Checkbox checked={hasAll || selected} tabIndex={-1} className="pointer-events-none" />
      <ActorAvatar
        name={agent.name}
  ...
  return (
    <div className="flex flex-wrap items-center gap-1">
      ...
      {overflow.length > 0 && (
        <Tooltip>
          <TooltipTrigger
            render={<span className={cn(chipClass, 'font-medium')}>+{overflow.length}</span>}
          />
...
export function AddToAgentDialog({
  ...
  const matchesQuery = (a: Agent) => !searching || a.name.toLowerCase().includes(trimmedQuery);
  ...
  const handleConfirm = async () => {
    ...
    try {
      for (const agent of targets) {
        const missing = skillIds.filter((id) => !agent.skills.some((s) => s.id === id));
        if (missing.length > 0) {
      ...
      toast.error(
        e instanceof Error && e.message ? e.message : t(($) => $.actions.add_failed_toast),
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="flex h-[32rem] max-h-[85svh] max-w-lg flex-col">
        <DialogHeader>
          <DialogTitle className="text-sm">{t(($) => $.actions.add_to_agent)}</DialogTitle>
        ...
        <DialogFooter>
          ...
          <Button type="button" onClick={handleConfirm} disabled={saving || count === 0}>
            {saving ? (
              <>
...
export function DeleteSkillsDialog({
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      toast.error(
        e instanceof Error && e.message ? e.message : t(($) => $.actions.delete_failed_toast),
      );
    } finally {
      setDeleting(false);
    }
...
export function SkillRowActions({ row, ctx }: { row: SkillRow; ctx: SkillActionsContext }) {
  const { t } = useT('skills');
  ...
  return (
    <span onClick={(e) => e.stopPropagation()} className="flex items-center">
      ...
      <AddToAgentDialog skills={[row.skill]} ctx={ctx} open={addOpen} onOpenChange={setAddOpen} />
      <DeleteSkillsDialog rows={[row]} ctx={ctx} open={deleteOpen} onOpenChange={setDeleteOpen} />
    </span>
  );
}
    ...
    <>
      ...
      <div className="absolute bottom-6 left-1/2 z-50 flex -translate-x-1/2 items-center gap-1 rounded-lg border bg-background px-2 py-1.5 shadow-lg">
        ...
        {allDeletable ? (
          ...
          <Tooltip>
            <TooltipTrigger render={<span className="inline-flex">{deleteButton}</span>} />
            <TooltipContent side="top">{t(($) => $.actions.delete_no_permission)}</TooltipContent>
          </Tooltip>
        )}
      </div>
