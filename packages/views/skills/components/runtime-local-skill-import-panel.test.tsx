...
function renderPanel(
  props: { onImported?: (skill: unknown) => void; onBulkDone?: () => void } = {},
) {
...
describe('RuntimeLocalSkillImportPanel', () => {
  ...
  it('surfaces the runtime alias and provider in the picker, not the raw daemon name (MUL-5248)', async () => {
    mockRuntimeListOptions.mockReturnValue({
      queryKey: ['runtimes', 'ws-1', 'list'],
      queryFn: () => Promise.resolve([{ ...MOCK_RUNTIME, custom_name: 'Dev Box' }]),
    });

    renderPanel();

    const labels = await screen.findAllByText('Dev Box (Claude)', {}, { timeout: 5000 });
    expect(labels.length).toBeGreaterThan(0);
    expect(screen.queryByText('Claude (MacBook) (claude)')).not.toBeInTheDocument();
  });
  ...
  it('shows an empty search state when no local skills match', async () => {
    ...
    expect(screen.getByText('No local skills match "terraform".')).toBeInTheDocument();
    expect(screen.queryByText('Review Helper')).not.toBeInTheDocument();
  ...
  it('handles partial failures gracefully', async () => {
    ...
    const selectAllCheckbox2 = selectAllLabel2
      .closest('label')!
      .querySelector("input[type='checkbox']")!;
  ...
  it('calls onBulkDone when multiple skills succeed', async () => {
    ...
    const selectAllCheckbox3 = selectAllLabel3
      .closest('label')!
      .querySelector("input[type='checkbox']")!;
  ...
  it('resolves a creator conflict by overwriting the existing skill', async () => {
    ...
    expect(await screen.findByText(/A skill with this name already exists/i)).toBeInTheDocument();
    ...
    await waitFor(
      () => {
        expect(mockResolveRuntimeLocalSkillImport).toHaveBeenLastCalledWith('runtime-1', {
          skill_key: 'review-helper',
          name: 'Review Helper',
          description: 'Review pull requests',
          supports_conflict: true,
  ...
  it("renders the creator's display name for non-overwritable conflicts", async () => {
    ...
    expect(await screen.findByText(/created by Bob/i, {}, { timeout: 5000 })).toBeInTheDocument();
  ...
  it('falls back to the unbranded locked message when the creator left the workspace', async () => {
    ...
    expect(
      await screen.findByText(/Only the creator can overwrite this skill/i, {}, { timeout: 5000 }),
    ).toBeInTheDocument();
