...
function makeAgent(id: string, ownerId: string | null, overrides: Record<string, unknown> = {}) {
  return {
    id,
    workspace_id: 'ws-1',
    runtime_id: 'rt-1',
  ...
  const ui = (nextRows: AgentListRow[]) => (
    <QueryClientProvider client={qc}>
      <I18nProvider locale="en" resources={TEST_RESOURCES}>
        <AgentBatchToolbar rows={nextRows} members={[]} currentUserId="user-1" onClear={() => {}} />
      </I18nProvider>
    </QueryClientProvider>
  );
...
describe('AgentBatchToolbar — presence', () => {
  it('closes dialogs and removes the toolbar when selection becomes empty', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.queryByRole('button', { name: 'Set access scope' })).not.toBeInTheDocument();
    });
  });
});
...
describe('AgentBatchToolbar — bulk Set access scope', () => {
  it('shows the Set access scope button for active owned agents', () => {
    ...
    expect(screen.getByRole('button', { name: 'Set access scope' })).toBeInTheDocument();
  });
  ...
  it('settles after picking a scope: Apply enables and stays enabled', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      expect(screen.getByRole('radio', { name: /Entire workspace/ })).toBeChecked();
    }
  });
  ...
  it('applies Entire workspace to each owned agent exactly once', async () => {
    ...
    expect(updateAgentSpy.mock.calls.map((call) => call[0]).sort()).toEqual(['a', 'b']);
  ...
  it('does not retain the previous selection after close and reopen', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Apply' })).toBeDisabled();
    expect(screen.getByRole('radio', { name: /Entire workspace/ })).not.toBeChecked();
