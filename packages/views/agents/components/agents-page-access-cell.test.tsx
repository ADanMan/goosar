...
function makeRow(overrides: Partial<AgentListRow['agent']> = {}): AgentListRow {
  return {
    agent: {
      id: 'agent-1',
      workspace_id: 'ws-1',
...
describe('AccessCell', () => {
  ...
  it("renders 'Specific people' for a public_to agent with a member target", () => {
    renderCell(
      makeRow({
        permission_mode: 'public_to',
        invocation_targets: [{ target_type: 'member', target_id: 'user-1' }],
      }),
    );
  ...
  it("renders 'Specific people' for a public_to agent with no targets", () => {
    renderCell(makeRow({ permission_mode: 'public_to', invocation_targets: [] }));
    expect(screen.getByText('Specific people')).toBeInTheDocument();
  });
