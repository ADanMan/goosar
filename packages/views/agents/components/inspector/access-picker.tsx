...
import { effectiveAccessScope } from '@goosar/core/agents';
...
export function AccessPicker({
  ...
  const teamIds = useMemo(() => selectedTargetIds(invocationTargets, 'team'), [invocationTargets]);
  ...
  useEffect(() => {
    ...
    const membersChanged =
      ...
      persistedMembers.some((id, i) => id !== prevPersistedMembersRef.current[i]);
  ...
  const dirty = draftScope !== persistedScope || (draftScope === 'members' && !sameMembers);
        ...
        <div className="border-t border-surface-border bg-muted/20 px-4 py-5 sm:px-6">
          <div>
            {editableMembers.length === 0 ? (
              <p className="text-xs text-muted-foreground">{t(($) => $.access.members_empty)}</p>
            ) : (
                  ...
                  return (
                    ...
                    >
                      <Checkbox
                        ...
                        onCheckedChange={(value) => toggleMember(member.user_id, value === true)}
                      />
                      <label
                        htmlFor={id}
