...
import type { AgentRuntime, MemberWithUser } from '@goosar/core/types';
...
export function RuntimePicker({
  ...
  const isPrivateToSomeoneElse = (r: AgentRuntime): boolean => {
    if (!currentUserId) return false;
    if (r.owner_id === currentUserId) return false;
    return r.visibility !== 'public';
  };
  const isForeignSecretDestination = (r: AgentRuntime): boolean => {
    if (!agentCarriesSecrets) return false;
    if (agentOwnerId === null) return false;
    if (currentUserId !== null && agentOwnerId === currentUserId) return false;
    if (r.id === value) return false; 
    if (!r.owner_id) return false; 
    return r.owner_id !== agentOwnerId;
  };
  const isDisabled = (r: AgentRuntime): boolean =>
    isPrivateToSomeoneElse(r) || isForeignSecretDestination(r);
  ...
  const selectedLabel = selected ? runtimeRowLabel(selected, selectedMachine?.title ?? '') : null;
  ...
  const machineOwner = (machine: RuntimeMachine): MemberWithUser | null => {
    const ownerIds = new Set(machine.runtimes.map((r) => r.owner_id).filter(Boolean));
  ...
  const handleOpenChange = (next: boolean) => {
    if (next) {
      const nextFilter: Filter =
        selected && currentUserId && selected.owner_id !== currentUserId ? 'all' : filter;
  ...
  const drilled = machineId ? (machines.find((m) => m.id === machineId) ?? null) : null;
        ...
        <>
          {selected ? (
            <ProviderLogo
              ...
              className={variant === 'field' ? 'h-4 w-4 shrink-0' : 'h-3 w-3 shrink-0'}
            />
          ) : (
            <Monitor
              className={
          ...
          {variant === 'field' && selected ? (
            ...
            <span
              className={
                variant === 'field' ? 'min-w-0 flex-1 truncate' : 'min-w-0 truncate font-mono'
              }
            >
              {combinedLabel}
            </span>
      ...
      header={
        drilled ? (
          ...
          >
            ...
            <span className="min-w-0 flex-1 truncate font-medium">{drilled.title}</span>
      ...
      {drilled ? (
        drilled.runtimes.map((rt) => {
          ...
          const tooltip = [
            ...
            isForeignSecretDestination(rt)
              ? t(($) => $.pickers.runtime_foreign_secret_locked_tooltip)
              : locked
        ...
        machines.map((machine) => {
          ...
          const containsSelection = machine.runtimes.some((r) => r.id === value);
          const extraProviders = machine.providerNames.length - MACHINE_PROVIDER_PREVIEW;
          return (
            <button
              key={machine.id}
              type="button"
                ...
                <div className="mt-0.5 flex items-center gap-1.5 text-xs text-muted-foreground">
                  ...
                  {machine.providerNames.length > 0 && (
                    <span className="flex shrink-0 items-center gap-1">
                      {machine.providerNames.slice(0, MACHINE_PROVIDER_PREVIEW).map((provider) => (
