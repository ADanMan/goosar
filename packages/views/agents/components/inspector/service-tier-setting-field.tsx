...
import type { RuntimeModel, RuntimeModelServiceTier } from '@goosar/core/types';
...
export function ServiceTierSettingField({
  ...
  return (
    <SettingsRow label={label} size="select-wide">
      <ServiceTierPicker value={value} tiers={tiers} canEdit={canEdit} onChange={onChange} />
    </SettingsRow>
  );
}
...
function ServiceTierPicker({
  ...
  const triggerLabel = selected?.name || value || t(($) => $.pickers.service_tier_default);
