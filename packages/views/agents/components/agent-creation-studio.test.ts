...
import type { Agent, RuntimeDevice } from '@goosar/core/types';
import {
  AgentNameField,
  ModeChooser,
  StudioFooter,
...
Return findings."}</agent_draft>`;
  ...
  it("round-trips only the user's natural-language request for chat display", () => {
    ...
    expect(JSON.parse(content.slice(content.indexOf('\n') + 1))).toMatchObject({
      ...
      available_runtime_models: [{ id: 'gpt-5.5', label: 'GPT-5.5', provider: 'openai' }],
    });
    expect(decodeBuilderInput('ordinary chat message')).toBe('ordinary chat message');
  });
  ...
  it('accepts catalog models and rejects invented or cross-runtime models', () => {
    ...
    expect(
      mergeBuilderDraft(draft(), { model: 'gpt-5.5' }, new Set(), new Set(), validModelIds).model,
    ).toBe('gpt-5.5');
    expect(
      mergeBuilderDraft(
        draft(),
    ...
    expect(
      mergeBuilderDraft(draft(), { model: 'invented-model' }, new Set(), new Set(), validModelIds)
        .model,
    ).toBe('model-1');
    expect(
      mergeBuilderDraft(draft(), { model: '' }, new Set(), new Set(), validModelIds).model,
    ).toBe('');
  });

  it('preserves a user-selected custom model when the catalog is unavailable', () => {
    expect(
      mergeBuilderDraft(draft(), { model: 'invented-model' }, new Set(), new Set(), null).model,
    ).toBe('model-1');
    expect(mergeBuilderDraft(draft(), { model: 'model-1' }, new Set(), new Set(), null).model).toBe(
      'model-1',
    );
    expect(
      mergeBuilderDraft(draft(), { model: 'invented-model' }, new Set(), new Set(), new Set())
        .model,
    ).toBe('model-1');
    expect(mergeBuilderDraft(draft(), { model: '' }, new Set(), new Set(), new Set()).model).toBe(
      'model-1',
    );
  });
...
describe('Agent creation studio execution overrides', () => {
  ...
  it('omits empty overrides instead of sending an empty string', () => {
    ...
    expect(JSON.parse(JSON.stringify(request))).not.toHaveProperty('thinking_level');
  });
  ...
  it('keeps the overrides while the AI builder leaves the model alone', () => {
    ...
    expect(
      mergeBuilderDraft(current, { name: 'Renamed' }, new Set(), new Set(), new Set(['model-1'])),
    ).toMatchObject({
      name: 'Renamed',
      model: 'model-1',
      thinkingLevel: 'high',
