...
export function AgentBatchToolbar({
  ...
  const handleAccessReadyChange = useCallback((ready: boolean, change?: AccessChange) => {
  ...
  const invalidate = () => qc.invalidateQueries({ queryKey: workspaceKeys.agents(wsId) });
  ...
  const runBatch = async (
    ...
    setBusy(true);
    const settled = await Promise.allSettled(targets.map((row) => fn(row.agent.id)));
    ...
    <>
            ...
            <DialogHeader>
              <DialogTitle>
                {t(($) => $.row_actions.archive_dialog_title, {
                  name: rows.length === 1 && rows[0] ? rows[0].agent.name : String(rows.length),
                })}
              </DialogTitle>
              <DialogDescription>
      ...
      {rows.length > 0 && (
        <Dialog open={confirmAccess} onOpenChange={setAccessDialogOpen}>
          <DialogContent className="sm:max-w-md" aria-describedby="bulk-access-summary">
            <DialogHeader>
              <DialogTitle>{t(($) => $.row_actions.set_access_dialog_title)}</DialogTitle>
