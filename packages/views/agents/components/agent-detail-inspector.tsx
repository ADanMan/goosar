...
import { buildModelChangeUpdate, type ModelCatalog } from './inspector/model-change-cleanup';
...
function agentHasGatewayToken(agent: Agent): boolean {
  const gateway = agent.runtime_config?.gateway;
...
export function AgentDetailInspector({
  ...
  const profileDraft = useMemo(() => ({ name: name.trim(), description }), [description, name]);
  ...
  const modelsQuery = useQuery(runtimeModelsOptions(isOnline ? agent.runtime_id : null));
  const modelCatalog = useMemo<ModelCatalog>(
    () =>
      modelsQuery.isSuccess ? (modelsQuery.data.supported ? modelsQuery.data.models : []) : null,
    ...
    <div className="space-y-8">
      ...
      >
        <SettingsCard>
          ...
          <SettingsRow label={t(($) => $.inspector.description_label)} size="text" align="start">
            <div>
              <Textarea
          ...
          <SettingsRow
            ...
            align="start"
          >
            <ResourceLabelPicker resourceType="agent" resourceId={agent.id} canEdit={canEdit} />
          </SettingsRow>
        </SettingsCard>
      </SettingsSection>
      ...
      >
        <SettingsCard>
          <SettingsRow label={t(($) => $.inspector.prop_runtime)} size="select-wide">
            <RuntimePicker
              variant="field"
              showLabel={false}
          ...
          <SettingsRow label={t(($) => $.inspector.prop_model)} size="select-wide">
            <ModelPicker
              variant="field"
              showLabel={false}
          ...
          <ThinkingSettingField
            ...
            canEdit={canEdit}
            onChange={(thinkingLevel) => update({ thinking_level: thinkingLevel })}
          />
          ...
          />
          <SettingsRow label={t(($) => $.inspector.prop_concurrency)} size="select-wide">
            <ConcurrencyField
