...
import { effectiveAccessScope, type AccessScope } from '@goosar/core/agents';
import { EMPTY_AGENT_FILTERS, type AgentListFilters } from '@goosar/core/agents/stores';
...
function withAccess(filters: AgentListFilters, value: AccessScope): AgentListFilters {
...
describe('rowMatchesFilters — access dimension', () => {
  ...
  it('access filter combines with other dimensions (AND)', () => {
    const ownerUser1 = makeRow({ owner_id: 'user-1' }, { isOwnedByMe: true });
    const ownerUser2 = makeRow({ owner_id: 'user-2' }, { isOwnedByMe: false });
  ...
  it('access + runtimes filter: both must pass (AND)', () => {
    const localAgent = makeRow({ permission_mode: 'private', runtime_id: 'rt-local' });
    const cloudAgent = makeRow({ permission_mode: 'private', runtime_id: 'rt-cloud' });
    const filters: AgentListFilters = {
      ...noFilters,
      access: ['owner-only'],
      runtimes: ['rt-local'],
