...
const RUNTIME_MACHINE_SECTIONS: RuntimeMachineSection[] = ['local', 'remote', 'cloud'];
...
export function RuntimeMachineFilterDropdown({
  ...
  const { t } = useT('agents');
  const selected =
    value === null ? null : (machines.find((machine) => machine.id === value) ?? null);

  const triggerLabel = selected ? selected.title : t(($) => $.runtime_filter.all);
  const triggerCount = selected ? (agentCountByMachine.get(selected.id) ?? 0) : totalAgentCount;
  ...
  return (
    <DropdownMenu>
      ...
      >
        ...
        <span className="font-mono tabular-nums text-muted-foreground/70">{triggerCount}</span>
