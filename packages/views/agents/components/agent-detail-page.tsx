...
import type { Agent, AgentRuntime, UpdateAgentRequest } from '@goosar/core/types';
...
import { useAgentPermissions } from '@goosar/core/permissions';
import { Button } from '@goosar/ui/components/ui/button';
import { CapabilityBanner } from '@goosar/ui/components/common/capability-banner';
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function AgentDetailPage({ agentId }: AgentDetailPageProps) {
  ...
  const { canAssign, canEdit, isLoading: permissionsLoading } = useAgentPermissions(agent, wsId);
  ...
  const handleUpdate = async (id: string, data: Record<string, unknown>) => {
    ...
    try {
      ...
      if (prevAgent) {
        qc.setQueryData<Agent[]>(queryKey, (old) =>
          old?.map((a) => (a.id === id ? ({ ...a, ...prevFields } as Agent) : a)),
        );
      }
      qc.invalidateQueries({ queryKey });
  ...
  return (
    <div className="flex flex-1 min-h-0 flex-col">
      <DetailHeader
        ...
        onDm={handleDm}
        onAssign={() => useModalStore.getState().open('quick-create-issue', { agent_id: agent.id })}
      ...
      <div className="mx-auto max-w-[1440px]">
        ...
        <div className="mt-4 flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="min-w-0 pt-0.5">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-muted-foreground">
                ...
                <span className="inline-flex min-w-0 items-center gap-1.5">
                  ...
                  <span className="truncate">
                    {runtime ? runtimeDisplayLabel(runtime) : t(($) => $.pickers.runtime_none)}
                  </span>
                </span>
          ...
          <div className="flex shrink-0 items-center gap-2 self-end lg:self-start">
            {!isArchived && (
              <Button type="button" variant="outline" size="sm" disabled={dmPending} onClick={onDm}>
