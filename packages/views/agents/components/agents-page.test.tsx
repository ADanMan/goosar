...
import type { AgentActivity } from '@goosar/core/agents';
...
vi.mock('@goosar/core/agents', () => ({
  ...
  effectiveAccessScope: (pm: unknown, it: unknown) => {
    ...
    if (
      (Array.isArray(it) ? it : []).some(
        (t) => (t as { target_type?: string })?.target_type === 'workspace',
      )
    )
      return 'workspace';
...
vi.mock('@goosar/core/agents/stores', () => ({
  useAgentsViewStore: (selector: (state: unknown) => unknown) => selector(mocks.viewState),
...
vi.mock('@goosar/ui/components/ui/skeleton', () => ({
...
function betaPrecedesAlpha(): boolean {
  ...
  return Boolean(beta.compareDocumentPosition(alpha) & Node.DOCUMENT_POSITION_FOLLOWING);
}
