...
import { chatKeys, chatMessagesOptions, pendingChatTaskOptions } from '@goosar/core/chat/queries';
...
type StudioScreenKey = 'choose' | 'templates' | 'configure' | 'ai-setup' | 'ai-builder';
...
const BUILDER_INPUT_PREFIX = 'GOOSAR_AGENT_BUILDER_INPUT\n';
...
export function AgentCreationStudio() {
  ...
  const usableRuntimes = useMemo(
    () =>
      runtimes.filter(
        (runtime) =>
          runtime.status === 'online' && isRuntimeUsableForUser(runtime, currentUser?.id ?? null),
      ),
  ...
  const builderModelsQuery = useQuery(
    runtimeModelsOptions(
      mode === 'ai' && selectedRuntime?.status === 'online' ? selectedRuntime.id : null,
    ),
  );
  ...
  const validBuilderModelIds = useMemo(
    () =>
      builderModelCatalog === null ? null : new Set(builderModelCatalog.map((model) => model.id)),
    [builderModelCatalog],
  );
  ...
  const latestBuilderDraftMessage = [...builderMessages]
    .reverse()
    .find((message) => message.role === 'assistant' && parseBuilderDraft(message.content));
  ...
  useEffect(() => {
    ...
    setDraft((current) =>
      mergeBuilderDraft(current, payload, skillIdSet, memberIdSet, validBuilderModelIds),
    );
  }, [
    latestBuilderDraftMessageContent,
    latestBuilderDraftMessageId,
  ...
  const accessInvalid =
    draft.permissionScope === 'members' && draft.memberIds.size === 0 && draft.teamIds.size === 0;
  const canCreate =
    draft.name.trim().length > 0 &&
  ...
  const deleteBuilderSession = async () => {
    ...
    try {
      ...
      setBuilderError(
        error instanceof Error ? error.message : t(($) => $.creation_studio.builder.stop_failed),
      );
      return false;
    } finally {
  ...
  const sendBuilderMessage = async (content: string): Promise<boolean> => {
    ...
    try {
      ...
      const result = await api.sendChatMessage(builderSessionId, encodedContent);
      const createdAt = new Date().toISOString();
      qc.setQueryData<ChatMessage[]>(chatKeys.messages(builderSessionId), (current = []) =>
  ...
  const screenVariants = {
    ...
    animate: {
      ...
      transition: {
        duration: shouldReduceMotion ? UI_MOTION_DURATION.fast : UI_MOTION_DURATION.standard,
        ease: UI_EASE_OUT,
      },
    },
    ...
    <div className="flex min-h-0 flex-1 flex-col bg-background">
      <header className="flex h-14 shrink-0 items-center gap-3 border-b px-5">
        ...
        <div className="min-w-0">
          ...
          <p className="truncate text-xs text-muted-foreground">{currentModeLabel}</p>
        </div>
      ...
      <AnimatePresence mode="wait" initial={false} custom={transitionDirection}>
        ...
        >
          {mode === 'choose' && <ModeChooser onBlank={chooseBlank} onAI={chooseAI} />}
...
export function ModeChooser({ onBlank, onAI }: { onBlank: () => void; onAI: () => void }) {
  const { t } = useT('agents');
  const modes = [
    {
      icon: FileText,
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-3">
          {loading ? (
            ...
            <div className="py-16 text-center text-sm text-muted-foreground">
              {t(($) => $.creation_studio.templates.empty)}
            </div>
          ) : (
      ...
      <section className="min-h-0 overflow-y-auto">
        {!selected ? (
          ...
          <div className="mx-auto max-w-3xl p-6 sm:p-8">
            ...
            <div className="mt-7">
              <h3 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                {t(($) => $.creation_studio.templates.instructions)}
              </h3>
              <div className="mt-3 max-h-80 overflow-y-auto whitespace-pre-wrap rounded-lg border bg-muted/30 p-4 text-sm leading-6">
                {detailLoading ? <Loader2 className="size-4 animate-spin" /> : detail?.instructions}
              </div>
            </div>
            <div className="mt-7 flex justify-end">
              <Button onClick={onUse} disabled={detailLoading || !detail}>
                {t(($) => $.creation_studio.templates.use)}
...
function ConfigurationPanel({
  ...
  return (
    <div className={cn('space-y-8', compact && 'space-y-6')}>
      ...
      >
        <SettingsCard>
          <DraftFieldRow compact={compact} label={t(($) => $.create_dialog.avatar.change_aria)}>
      ...
      >
        <SettingsCard>
          ...
          >
            {(['private', 'workspace', 'members'] as PermissionScope[]).map((scope) => (
              <button
                key={scope}
                type="button"
                role="radio"
...
function BuilderSetup({
  ...
  const hasOnline = runtimes.some(
    (runtime) => runtime.status === 'online' && isRuntimeUsableForUser(runtime, currentUserId),
  );
  return (
    <main className="flex min-h-0 flex-1 items-center justify-center overflow-y-auto px-5 py-10">
      <div className="w-full max-w-xl rounded-xl border bg-card p-6 shadow-sm">
        ...
        <div className="mt-6 flex justify-end">
          {hasOnline ? (
            <Button onClick={onStart} disabled={starting || selectedRuntime?.status !== 'online'}>
...
export function StudioFooter({
  ...
  return (
    <div className="sticky bottom-0 mt-8 flex items-center justify-between gap-3 border-t bg-background/95 px-5 py-3 backdrop-blur">
      ...
      <Button type="button" className="ml-auto shrink-0" onClick={onCreate} disabled={!canCreate}>
...
export function buildDuplicateDraft(
  ...
  const keepsRuntime =
    ...
    options.runtimes.some(
      (runtime) =>
        runtime.id === source.runtime_id && isRuntimeUsableForUser(runtime, options.currentUserId),
    );
  return {
    ...EMPTY_DRAFT,
...
export function useDuplicateDraftSeed({
  ...
  }, [currentUserId, fallbackRuntimeId, nameSuffix, runtimes, runtimesSettled, source]);
}
...
export function mergeBuilderDraft(
  ...
  return {
    ...
    instructions:
      typeof payload.instructions === 'string' ? payload.instructions : current.instructions,
    model,
