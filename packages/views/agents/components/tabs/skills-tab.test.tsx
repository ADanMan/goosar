...
vi.mock('@goosar/core/api', () => ({
  api: {
    ...
    setAgentRuntimeSkillEnabled: (...args: unknown[]) => mockSetAgentRuntimeSkillEnabled(...args),
...
function renderSkillsTab(agentOverrides: Partial<Agent> = {}, runtime: AgentRuntime | null = null) {
...
describe('SkillsTab', () => {
  ...
  it('separates workspace assignments from inherited runtime skills', async () => {
    ...
    expect(await screen.findByText('Assigned to agent')).toBeInTheDocument();
  ...
  it('disables an assigned skill without removing it', async () => {
    ...
    expect(mockSetAgentSkillEnabled).toHaveBeenCalledWith('agent-1', 'skill-1', false);
  ...
  it('shows a permission notice when capability discovery is forbidden', async () => {
    ...
    expect(
      await screen.findByText("You don't have permission to view this runtime's skills."),
    ).toBeInTheDocument();
  });
