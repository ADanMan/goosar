...
import type { Agent, AgentTask, Issue } from '@goosar/core/types';
import {
  type AgentActivity,
...
export function ActivityTab({ agent, showPerformance = true }: ActivityTabProps) {
  ...
  return (
    <div className="flex min-w-0 flex-col gap-6">
      ...
      {showPerformance && <Last30dSection activity={activity} avgDurationMs={avgDurationMs} />}
      <RecentWorkSection
        tasks={recentTasks}
...
export function AgentPerformanceSummary({ agent }: { agent: Agent }) {
  ...
  return (
    <section className="mt-5 border-t pt-5">
      <h2 className="text-sm font-medium">{t(($) => $.tab_body.activity.section_last_30d)}</h2>
      {summary.totalRuns === 0 ? (
        ...
        <>
          <div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-3">
            ...
            />
            <Metric value={`${successPct}%`} label={t(($) => $.tab_body.activity.success_label)} />
            <Metric
...
function RecentWorkSkeleton() {
  ...
  return (
    <div className="overflow-hidden rounded-lg border divide-y" aria-hidden="true">
...
function TaskList({
  ...
  return (
    <div
      className={
        timeMode === 'completed' ? 'overflow-hidden rounded-lg border divide-y' : 'space-y-2'
      }
    >
      {tasks.map((task) => (
