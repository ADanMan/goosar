...
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@goosar/ui/components/ui/tabs';
import { Textarea } from '@goosar/ui/components/ui/textarea';
import { useT } from '../../../i18n';
import type {
  ManagedMcpServer,
  McpConfigErrorCode,
  McpConfigWarningCode,
} from './mcp-config-model';
import { isRecord, mcpTransport, validateMcpServerConfig } from './mcp-config-model';
...
function formFromConfig(config: Record<string, unknown>): McpFormState {
  const extras = { ...config };
  for (const key of ['type', 'command', 'args', 'env', 'environment', 'url', 'headers']) {
    delete extras[key];
  }
  ...
  if (typeof config.command === 'string') command = config.command;
    ...
    const tokens = config.command.filter((value): value is string => typeof value === 'string');
    command = tokens[0] ?? '';
    args = tokens.slice(1);
  }
  if (Array.isArray(config.args)) {
    args = config.args.filter((value): value is string => typeof value === 'string');
  }
...
function formValidationProbe(form: McpFormState): Record<string, unknown> {
  return {
    ...configFromForm(form),
...
export function McpServerDialog({
  ...
  const validation = useMemo(() => {
    if (mode === 'form') return validateMcpServerConfig(formValidationProbe(form));
    if (!jsonResult.ok) return { errors: [], warnings: [] };
    return validateMcpServerConfig(jsonResult.value);
  }, [form, jsonResult, mode]);

  const jsonSyntaxError = mode === 'json' && !jsonResult.ok ? jsonResult.error : null;
  const canSave =
    !saving && nameError === null && jsonSyntaxError === null && validation.errors.length === 0;

  const translateError = (code: McpConfigErrorCode): string => {
    switch (code) {
      case 'not_object':
        return t(($) => $.tab_body.mcp_config.dialog_json_object);
      case 'missing_target':
        return t(($) => $.tab_body.mcp_config.dialog_json_target);
      case 'command_required':
        return t(($) => $.tab_body.mcp_config.dialog_command_required);
      case 'command_has_args':
        return t(($) => $.tab_body.mcp_config.dialog_command_args_error);
      case 'command_shell_syntax':
        return t(($) => $.tab_body.mcp_config.dialog_command_shell_error);
      case 'url_required':
        return t(($) => $.tab_body.mcp_config.dialog_url_required);
      case 'url_invalid':
        return t(($) => $.tab_body.mcp_config.dialog_url_invalid);
      default:
        return '';
    }
  };

  const translateWarning = (code: McpConfigWarningCode): string => {
    switch (code) {
      case 'command_registry_runner':
        return t(($) => $.tab_body.mcp_config.dialog_warning_registry);
      case 'command_unverified':
        return t(($) => $.tab_body.mcp_config.dialog_warning_unverified);
      default:
        return '';
    }
  };
  ...
  const errorMessage =
    ...
    (jsonSyntaxError !== null
      ? t(($) => $.tab_body.mcp_config.invalid_json, { error: jsonSyntaxError })
      : validation.errors[0]
        ? translateError(validation.errors[0])
        : '');

  const commandInvalid = validation.errors.some((code) => code.startsWith('command_'));
  const urlInvalid = validation.errors.some((code) => code.startsWith('url_'));
  ...
  const handleModeChange = (next: string | number | null) => {
    ...
    if (next === 'json' && mode === 'form') {
      ...
      if (isRecord(jsonResult.value)) setForm(formFromConfig(jsonResult.value));
    }
    setMode(next);
  };
  ...
  const handleSave = async () => {
    ...
    else if (jsonResult.ok && isRecord(jsonResult.value)) config = jsonResult.value;
    else return;
    setSaving(true);
    try {
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => !saving && onOpenChange(next)}>
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
              ...
              <>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder={t(($) => $.tab_body.mcp_config.dialog_command_placeholder)}
                    aria-invalid={commandInvalid || undefined}
                    aria-describedby="mcp-server-command-hint"
                  />
                  <p
                    id="mcp-server-command-hint"
                    className="text-pretty break-words text-xs leading-5 text-muted-foreground"
                  >
        ...
        {errorMessage && (
          <p
            className="text-pretty break-words text-xs leading-5 text-destructive"
            aria-live="polite"
          >
            {errorMessage}
          </p>
        ...
        {validation.warnings.length > 0 && (
          <ul
            className="space-y-1.5 rounded-md border border-warning/40 bg-warning/5 px-3 py-2"
            aria-label={t(($) => $.tab_body.mcp_config.dialog_warnings_aria)}
            aria-live="polite"
          >
            {validation.warnings.map((code) => (
              <li key={code} className="flex items-start gap-2 text-xs leading-5 text-warning">
                <AlertTriangle className="mt-0.5 size-3.5 shrink-0" aria-hidden="true" />
                <span className="min-w-0 text-pretty break-words">{translateWarning(code)}</span>
              </li>
            ))}
          </ul>
        )}

        <p className="text-pretty break-words text-xs leading-5 text-muted-foreground">
          {t(($) => $.tab_body.mcp_config.dialog_verification_note)}
        </p>
