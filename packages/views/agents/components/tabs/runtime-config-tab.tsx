...
import {
  RUNTIME_N_GATEWAY_TOKEN_MASK,
  type RuntimeNRoutingMode,
  type RuntimeNConfig,
  runtimeNConfigEquals,
  parseRuntimeNConfig,
...
function configToForm(cfg: RuntimeNConfig): FormState {
  const masked = cfg.gateway?.token === RUNTIME_N_GATEWAY_TOKEN_MASK;
  return {
    mode: cfg.mode ?? 'local',
...
function formToConfig(state: FormState): RuntimeNConfig {
  const cfg: RuntimeNConfig = { mode: state.mode };
  if (state.mode === 'gateway') {
    const gw: NonNullable<RuntimeNConfig['gateway']> = {};
    ...
    if (state.tokenWasMasked && state.token === '') {
      gw.token = RUNTIME_N_GATEWAY_TOKEN_MASK;
    } else if (state.token !== '') {
...
export function RuntimeConfigTab({
  ...
  const original = useMemo<RuntimeNConfig>(
    () => parseRuntimeNConfig(agent.runtime_config),
  ...
  useEffect(() => {
    setState((current) => (formEquals(current, previousFormRef.current) ? originalForm : current));
    previousFormRef.current = originalForm;
  }, [originalForm]);

  const currentCfg = useMemo(() => formToConfig(state), [state]);
  const dirty = !runtimeNConfigEquals(original, currentCfg);
  ...
  const canSave = isOwner && portValid && !saving;
  ...
  const handleSave = async () => {
    ...
    try {
      await onSave({
        runtime_config: serializeRuntimeNConfig(currentCfg),
      });
  ...
  return (
    <div className="flex h-full flex-col space-y-4">
      <p className="text-xs text-muted-foreground">{t(($) => $.tab_body.runtime_config.intro)}</p>
      ...
      >
        ...
        <div className="space-y-1.5">
          <Label htmlFor="runtime-n-gw-host" className="text-xs">
            {t(($) => $.tab_body.runtime_config.host_label)}
          </Label>
          <Input
            id="runtime-n-gw-host"
            value={state.host}
        ...
        <div className="space-y-1.5">
          <Label htmlFor="runtime-n-gw-port" className="text-xs">
            {t(($) => $.tab_body.runtime_config.port_label)}
          </Label>
          <Input
            id="runtime-n-gw-port"
            value={state.port}
        ...
        <div className="space-y-1.5">
          <Label htmlFor="runtime-n-gw-token" className="text-xs">
            {t(($) => $.tab_body.runtime_config.token_label)}
          </Label>
          <Input
            id="runtime-n-gw-token"
            type="password"
            value={state.token}
            onChange={(e) =>
              setState((s) => ({
        ...
        <div className="flex items-center justify-between gap-2 pt-1">
          <div>
            <Label htmlFor="runtime-n-gw-tls" className="text-xs">
          ...
          <Switch
            ...
            checked={state.tls}
            disabled={!isGateway}
            onCheckedChange={(checked: boolean) => setState((s) => ({ ...s, tls: checked }))}
          />
        </div>
      </fieldset>
