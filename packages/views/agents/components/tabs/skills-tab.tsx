...
import { FileText, Loader2, Plus, RefreshCw, Server, Trash2 } from 'lucide-react';
...
type SelectedSkill =
  { kind: 'workspace'; id: string } | { kind: 'runtime'; skill: RuntimeLocalSkillSummary } | null;
...
export function SkillsTab({
  ...
  const handleRemove = async (skillId: string) => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : t(($) => $.tab_body.skills.remove_failed_toast),
      );
    } finally {
      setBusyId(null);
    }
  ...
  const handleToggle = async (skillId: string, enabled: boolean) => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : t(($) => $.tab_body.skills.toggle_failed_toast),
      );
    } finally {
      setBusyId(null);
    }
  ...
  const handleRuntimeToggle = async (skill: RuntimeLocalSkillSummary, enabled: boolean) => {
  ...
  return (
    <div className="space-y-8">
      <p className="text-sm leading-6 text-muted-foreground">{t(($) => $.tab_body.skills.intro)}</p>
      ...
      >
        {!runtime ? (
          ...
          <RuntimeNotice loading text={t(($) => $.tab_body.skills.runtime_discovering)} />
                ...
                >
                  ...
                  {canEdit &&
                    ...
                    (busy ? (
                      ...
                      <Switch
                        checked={!disabled}
                        onCheckedChange={(checked) => handleRuntimeToggle(skill, checked)}
                        aria-label={t(($) => $.tab_body.skills.runtime_toggle_aria, {
                          name: skill.name,
                        })}
                      />
                    ))}
...
function SkillDetailDialog({
  ...
  const title =
    runtimeSkill?.name || workspaceSkill?.name || t(($) => $.tab_body.skills.detail_title);
            ...
            {runtimeSkill.plugin && (
              <>
                <dt className="text-muted-foreground">
                  {t(($) => $.tab_body.skills.detail_plugin)}
                </dt>
            ...
            {(workspaceSkill.files ?? []).length > 0 && (
              <div>
                <h4 className="text-xs font-medium">
                  {t(($) => $.tab_body.skills.detail_supporting_files)}
                </h4>
