...
import { Loader2, Pencil, Plus, Save, Terminal, Trash2 } from 'lucide-react';
...
type EditorState = { kind: 'add' } | { kind: 'edit'; entryId: string } | null;
...
export function CustomArgsTab({
  ...
  const commitEditor = () => {
    ...
    if (editor.kind === 'add') {
      ...
      setEntries((current) =>
        current.map((entry) => (entry.id === editor.entryId ? { ...entry, value } : entry)),
      );
    }
    closeEditor();
  };
    ...
    <div className="space-y-6">
        ...
        <SettingsSection title={t(($) => $.tab_body.custom_args.command_preview_label)}>
          <SettingsCard>
            <div className="flex min-w-0 items-start gap-3 p-4">
              ...
              <code className="min-w-0 break-all pt-1.5 font-mono text-xs leading-5" translate="no">
                {launchCommand}
              </code>
            </div>
          </SettingsCard>
      ...
      {isOwner && (
        <div className="flex items-center justify-end gap-3 pt-1">
          ...
          <Button onClick={handleSave} disabled={!dirty || saving || editor !== null} size="sm">
            {saving ? (
              <Loader2
