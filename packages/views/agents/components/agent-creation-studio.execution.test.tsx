...
import type { RuntimeDevice, RuntimeModel, RuntimeModelListRequest } from '@goosar/core/types';
...
import { AgentExecutionOverrides, type AgentDraft } from './agent-creation-studio';
...
describe('AgentExecutionOverrides', () => {
  ...
  it('offers Speed for a model whose catalog advertises a tier', async () => {
    ...
    expect(onChange).toHaveBeenCalledWith(expect.objectContaining({ serviceTier: 'priority' }));
  });
  ...
  it("offers the exact model's thinking levels", async () => {
    ...
    expect(onChange).toHaveBeenCalledWith(expect.objectContaining({ thinkingLevel: 'high' }));
  });
