...
export function AgentOverviewSummary({ agent, runtime, owner }: AgentOverviewSummaryProps) {
  const { t } = useT('agents');
  ...
  return (
    <aside className="self-start rounded-xl border border-surface-border bg-surface p-5 shadow-[var(--surface-shadow)] xl:sticky xl:top-6">
      <section>
        <h2 className="text-sm font-medium">{t(($) => $.overview.agent_context)}</h2>
      ...
      <section className="mt-5 border-t pt-5">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-sm font-medium">{t(($) => $.inspector.section_skills)}</h2>
