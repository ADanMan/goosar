...
const {
  ...
  mockGetShareableUrl: vi.fn((p: string) => `https://app.goosar/${p}`),
  mockMembers: {
    current: [] as Array<{
      id: string;
      workspace_id: string;
      ...
      actorType === 'member'
        ...
        : actorType === 'agent'
          ...
          : actorType === 'squad'
            ...
            : undefined;
    return <span data-testid="issue-assignee-avatar" title={name ?? `${actorType}:${actorId}`} />;
  },
}));

vi.mock('@goosar/core/issues/stores', () => {
...
vi.mock('@goosar/ui/components/common/theme-provider', () => ({
...
describe('SearchCommand', () => {
  beforeEach(() => {
    ...
    mockGetShareableUrl.mockReset().mockImplementation((p: string) => `https://app.goosar/${p}`);
    mockMembers.current = [];
  ...
  it('copies issue link and identifier when on an issue detail route', async () => {
    ...
    expect(mockClipboardWrite).toHaveBeenCalledWith('https://app.goosar//ws-test/issues/issue-1');
  ...
  it('shows the assignee avatar instead of status text for issue search results', async () => {
    ...
    await waitFor(
      () => {
        expect(
          screen.getByText(
            (_, el) => el?.textContent === 'Assigned search result' && el?.tagName === 'SPAN',
          ),
        ).toBeInTheDocument();
      },
      { timeout: 2000 },
  ...
  it('renders description and comment snippets regardless of match_source', async () => {
    ...
    await waitFor(
      () => {
        expect(
          screen.getByText(
            (_, el) => el?.textContent === 'HTML rendering pipeline' && el?.tagName === 'SPAN',
          ),
        ).toBeInTheDocument();
      },
      { timeout: 2000 },
