...
import type { MemberWithUser, SearchIssueResult, SearchProjectResult } from '@goosar/core/types';
...
import { issueDetailOptions, issueTimelineOptions } from '@goosar/core/issues/queries';
...
import { createShortcutChord } from '@goosar/core/shortcuts';
...
import type { ProjectStatus } from '@goosar/core/types';
...
import { useTheme } from '@goosar/ui/components/common/theme-provider';
...
type NavKey =
  'inbox' | 'myIssues' | 'issues' | 'projects' | 'agents' | 'runtimes' | 'skills' | 'settings';
  ...
  const filteredMembers = useMemo(() => {
    ...
    const wantsAllMembers =
      ...
      ('members'.startsWith(q) ||
        ...
        'users'.startsWith(q) ||
        'team'.startsWith(q));
    return members.filter((member) => wantsAllMembers || matchesMember(member, q)).slice(0, 10);
  }, [members, query]);

  const hasResults =
    results.issues.length > 0 || results.projects.length > 0 || filteredMembers.length > 0;
                  ...
                  >
                    ...
                    {issue.matched_description_snippet && (
                      <div className="flex items-start gap-2 pl-[26px]">
                        ...
                        <span className="text-xs text-muted-foreground truncate">
                          <HighlightText text={issue.matched_description_snippet} query={query} />
                        </span>
                      </div>
                    )}
