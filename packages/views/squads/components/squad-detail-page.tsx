...
export function SquadDetailPage() {
  ...
  const memberStatusPollingInterval = useRealtimePollingInterval(
    BACKGROUND_DEGRADED_POLL_INTERVAL_MS,
  );
  const { data: memberStatusResp } = useQuery({
    ...squadMemberStatusOptions(wsId, squadId),
    enabled: !!workspace?.id && !!squadId,
    refetchInterval: memberStatusPollingInterval,
  });
  ...
  const canManage = isWorkspaceAdmin || (!!currentUser && squad?.creator_id === currentUser.id);
  ...
  const addMemberMut = useMutation({
    ...
    onError: (err) =>
      toast.error(
        err instanceof Error && err.message ? err.message : t(($) => $.toasts.member_add_failed),
      ),
  });

  const removeMemberMut = useMutation({
    mutationFn: (m: SquadMember) =>
      api.removeSquadMember(squadId, { member_type: m.member_type, member_id: m.member_id }),
    onSuccess: () => {
      refetchMembers();
    ...
    onError: (err) =>
      toast.error(
        err instanceof Error && err.message ? err.message : t(($) => $.toasts.member_remove_failed),
      ),
  });
  ...
  const updateRoleMut = useMutation({
    ...
    onSuccess: () => {
      refetchMembers();
      toast.success(t(($) => $.toasts.role_updated));
    },
    onError: (err) =>
      toast.error(
        err instanceof Error && err.message ? err.message : t(($) => $.toasts.role_update_failed),
      ),
  });
  ...
  const setLeaderMut = useMutation({
    ...
    onError: (err) =>
      toast.error(
        err instanceof Error && err.message ? err.message : t(($) => $.toasts.leader_update_failed),
      ),
  });
  ...
  const deleteMut = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: workspaceKeys.squads(wsId) });
      push(p.squads());
      toast.success(t(($) => $.toasts.squad_archived));
    },
    onError: (err) =>
      toast.error(
        err instanceof Error && err.message ? err.message : t(($) => $.toasts.squad_archive_failed),
      ),
  });
  ...
  const availableAgents = agents.filter(
    (a: Agent) =>
      !a.archived_at && !members.some((m) => m.member_type === 'agent' && m.member_id === a.id),
  );
  const availableMembers = wsMembers.filter(
    (m) => !members.some((sm) => sm.member_type === 'member' && sm.member_id === m.user_id),
  );
    ...
    <div className="flex flex-1 min-h-0 flex-col">
      ...
      <div className="flex flex-1 min-h-0 flex-col gap-3 overflow-y-auto p-3 md:grid md:grid-cols-[280px_minmax(0,1fr)] md:gap-4 md:overflow-hidden md:p-6 lg:grid-cols-[320px_minmax(0,1fr)]">
        <SquadDetailInspector
          ...
          onUpdateDescription={async (next) => {
        ...
        <SquadOverviewPane
          ...
          onCreateAgentClick={
            canManage
              ? () => push(`${p.newAgent()}?squad=${encodeURIComponent(squadId)}`)
              : undefined
          }
          onSetLeader={(id) => setLeaderMut.mutate(id)}
          onRemoveMember={(m) => removeMemberMut.mutate(m)}
          onUpdateRole={async (m, role) => {
            await updateRoleMut.mutateAsync({ member: m, role });
          }}
          onSaveInstructions={async (next) => {
            await updateSquadMut.mutateAsync({ instructions: next });
            toast.success(t(($) => $.toasts.instructions_saved));
          }}
      ...
      {showAddMember && (
        <AddMemberDialog
          ...
          onSubmit={async (input) => {
            await addMemberMut.mutateAsync(input);
          }}
        />
      )}
      ...
      {confirmArchive && (
        <AlertDialog
          open
          onOpenChange={(v) => {
            if (!v && !deleteMut.isPending) setConfirmArchive(false);
          }}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
...
function SquadNameEditor({
  ...
  return (
    <InlineEditPopover
      ...
      validate={(v) => (v.trim().length > 0 ? null : t(($) => $.name_editor.required_error))}
    >
      {(triggerProps) => (
        <button
          type="button"
...
function InlineEditPopover({
  ...
  const commit = async () => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : t(($) => $.toasts.save_failed));
    } finally {
      setSaving(false);
    }
  };
              ...
              <PopoverContent align="start" className="w-[var(--anchor-width)] p-0">
                ...
                <div className="p-1 max-h-72 overflow-y-auto">
                  {filteredMembers.length > 0 && (
                    <PickerSection label={t(($) => $.add_member_dialog.section_members)}>
                  ...
                  {filteredAgents.length > 0 && (
                    <PickerSection label={t(($) => $.add_member_dialog.section_agents)}>
          ...
          <div>
            <Label className="text-xs text-muted-foreground">
              ...
              <span className="text-muted-foreground/60">
                {t(($) => $.add_member_dialog.label_optional)}
              </span>
            </Label>
            <Input
              type="text"
        ...
        <DialogFooter>
          ...
          <Button onClick={() => void handleSubmit()} disabled={!canSubmit}>
            {submitting ? (
              ...
              t(($) => $.add_member_dialog.add_button)
            )}
          </Button>
        </DialogFooter>
      </DialogContent>
  ...
  if (editing) {
    return (
      <Input
        ...
        disabled={saving}
        placeholder={t(($) => $.add_member_dialog.role_input_placeholder)}
    ...
    >
      {value || (
        <span className="italic opacity-60">
          {t(($) => $.add_member_dialog.placeholder_role_inline)}
        </span>
      )}
    </button>
  );
  ...
  return (
    <>
      ...
      <textarea
        ...
        value={draft}
        onChange={(e) => setDraft(e.target.value)}
        placeholder={t(($) => $.description_dialog.input_placeholder)}
        rows={6}
        onKeyDown={(e) => {
          if (e.key === 'Escape') {
            onClose();
                ...
                <div className="flex items-center gap-2">
                  ...
                  {m.member_type === 'agent' && statusLabel && (
                    <span className="inline-flex items-center gap-1 text-xs text-muted-foreground">
                      <span
                        className={`h-1.5 w-1.5 rounded-full ${dotClass ?? 'bg-muted-foreground/40'}`}
                      />
                      {statusLabel}
                    </span>
                  )}
              ...
              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity">
                {m.member_type === 'agent' && (
                  <Tooltip>
                    ...
                    />
                    <TooltipContent>{t(($) => $.members_tab.view_agent_tooltip)}</TooltipContent>
                  </Tooltip>
                )}
                {canManage && m.member_type === 'agent' && !isLeader(m) && !isArchived(m) && (
                  <Tooltip>
                    ...
                    />
                    <TooltipContent>{t(($) => $.members_tab.make_leader_tooltip)}</TooltipContent>
                  </Tooltip>
                )}
                {canManage && !isLeader(m) && (
                  <Tooltip>
                    ...
                    />
                    <TooltipContent>{t(($) => $.members_tab.remove_member_tooltip)}</TooltipContent>
                  </Tooltip>
                )}
              </div>
            </div>
...
function SquadInstructionsTab({
  ...
  return (
    <div className="flex h-full flex-col gap-4">
      <p className="text-xs text-muted-foreground">{t(($) => $.instructions_tab.description)}</p>
      ...
      >
        <ContentEditor
          ...
          placeholder={canManage ? t(($) => $.instructions_tab.placeholder) : ''}
          debounceMs={150}
          disableMentions
