...
import {
  ...
  type SquadsScope,
  type SquadSortField,
} from '@goosar/core/squads/stores';
...
function columnTrackVars(
  ...
  const width = (key: SquadColumnKey) => (isVisible(key) ? `${COLUMN_WIDTHS[key]}px` : '0px');
  const minWidth =
    FIXED_TRACKS_WIDTH +
...
function SquadAvatar({ squad }: { squad: Squad }) {
  ...
  return (
    <ActorAvatarBase name={squad.name} initials={initials} isSquad size="lg" className="shrink-0" />
  );
}
  ...
  return (
    <ListGridCell className="gap-3">
      ...
      <div className="min-w-0 flex-1">
        <span className="block min-w-0 truncate text-sm font-medium">{squad.name}</span>
        {squad.description ? (
...
function LeaderCell({ leaderId, leader }: { leaderId: string; leader: Agent | undefined }) {
  return (
...
function SquadRowActions({ squad }: { squad: Squad }) {
  ...
  return (
    <span onClick={(e) => e.stopPropagation()} className="flex items-center">
      ...
      <ArchiveSquadDialog squad={squad} open={archiveOpen} onOpenChange={setArchiveOpen} />
    </span>
  );
}
...
function SquadListHeader({
  ...
  const { t } = useT('squads');
  const sorted = (field: SquadSortField) => (sortField === field ? sortDirection : false);
  return (
    <ListGridHeader>
...
function SquadListToolbar({
  ...
  const activeFilterCount =
    (filters.leaders.length > 0 ? 1 : 0) + (filters.creators.length > 0 ? 1 : 0);
              ...
              <div className="mt-2 flex items-center gap-1.5">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="start" className="w-auto">
                    <DropdownMenuRadioGroup
                      value={sortField}
                      onValueChange={(v) => onSortFieldChange(v as SquadSortField)}
                    >
...
export function SquadsPage() {
  ...
  const rows = useMemo<Squad[]>(() => {
    const inScope = scopeRows.filter((s) => {
      ...
      if (filters.creators.length > 0 && !filters.creators.includes(s.creator_id)) {
        return false;
      }
      return true;
    });
  ...
  const canManageAnyRow = useMemo(
    () => isWorkspaceAdmin || (!!currentUser && rows.some((s) => s.creator_id === currentUser.id)),
                  ...
                  >
                    ...
                    {isColVisible('creator') ? (
                      <ListGridCell className="hidden gap-1.5 @2xl:flex">
                        <ActorAvatar actorType="member" actorId={squad.creator_id} size="sm" />
                        <span className="min-w-0 truncate text-xs text-muted-foreground">
                    ...
                    {isColVisible('created') ? (
                      <ListGridCell className="hidden whitespace-nowrap text-xs tabular-nums text-muted-foreground @2xl:flex">
                        {new Date(squad.created_at).toLocaleDateString(uiLocale)}
                      </ListGridCell>
                    ) : (
                      <ListGridCell className="hidden px-0 @2xl:flex" />
                    )}
...
function LoadingSkeleton() {
  return (
    <div className="min-h-0 flex-1 overflow-auto @container">
      <ListGrid
        className={GRID_COLS}
        style={columnTrackVars((key) => !SQUAD_DEFAULT_HIDDEN_COLUMNS.includes(key), true)}
      >
        <ListGridHeader>
          <ListGridHeaderCell>
