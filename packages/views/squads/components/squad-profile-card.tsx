...
import type { SquadMemberPreview } from '@goosar/core/types';
...
export function SquadProfileCard({ squadId }: SquadProfileCardProps) {
  ...
  const { data: squads = [], isLoading: squadsLoading } = useQuery(squadListOptions(wsId));
  ...
  return (
    <div className="flex flex-col gap-1.5 text-xs">
      ...
      <div className="flex flex-col gap-0.5">
        {visible.map((m) => {
          const isLeader = m.member_type === 'agent' && m.member_id === leaderId;
          const name =
            m.member_type === 'agent'
              ? (agents.find((a) => a.id === m.member_id)?.name ?? m.member_id.slice(0, 8))
              : (wsMembers.find((u) => u.user_id === m.member_id)?.name ?? m.member_id.slice(0, 8));
          const href =
