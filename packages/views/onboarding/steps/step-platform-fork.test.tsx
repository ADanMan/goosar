...
describe('StepPlatformFork', () => {
  ...
  it('opens the download page and flips the card to a post-click state', async () => {
    ...
    expect(screen.getByText(/opening the download page/i)).toBeInTheDocument();
  });
  ...
  it("CLI dialog: opens with instructions + 'waiting' and a disabled Connect button", async () => {
    ...
    expect(within(dialog).getByText(/waiting for your computer/i)).toBeInTheDocument();
    expect(within(dialog).getByRole('button', { name: /start exploring/i })).toBeDisabled();
  });

  it('CLI dialog with a selected runtime: Connect enables and fires onNext(runtime)', async () => {
    const rt = makeRuntime({ id: 'rt_bee' });
    resetPicker({
      runtimes: [rt],
      selected: rt,
      selectedId: rt.id,
    ...
    expect(within(dialog).getByText(/selected: hermes \(dev-box\)/i)).toBeInTheDocument();
