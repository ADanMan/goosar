...
import type { QuestionnaireAnswers } from '@goosar/core/onboarding';
...
function renderStep(answers: QuestionnaireAnswers = EMPTY) {
  ...
  render(
    <I18nProvider locale="en" resources={TEST_RESOURCES}>
      <StepAboutYou answers={answers} onChange={onChange} onAdvance={onAdvance} onSkip={onSkip} />
    </I18nProvider>,
  );
...
describe('StepAboutYou', () => {
  ...
  it('renders both question groups on one screen', () => {
    renderStep();
    expect(screen.getByText('Which best describes you?')).toBeInTheDocument();
    expect(screen.getByText('What do you want to use Goosar for?')).toBeInTheDocument();
  });
  ...
  it('toggling a use case patches the array and clears skip', async () => {
    ...
    await user.click(screen.getByRole('checkbox', { name: /ship code with ai agents/i }));
  ...
  it('pre-fills both groups from stored answers on re-entry', () => {
    ...
    expect(screen.getByRole('radio', { name: /designer/i })).toHaveAttribute(
      'aria-checked',
      'true',
    );
    expect(screen.getByRole('checkbox', { name: /plan, brainstorm, research/i })).toHaveAttribute(
      'aria-checked',
      'true',
    );
    expect(screen.getByRole('checkbox', { name: /write, edit, publish/i })).toHaveAttribute(
      'aria-checked',
      'true',
    );
  });
