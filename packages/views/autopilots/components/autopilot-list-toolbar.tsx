...
const SORT_FIELDS: AutopilotSortField[] = ['name', 'lastRun', 'nextRun', 'created'];

const MODES = ['create_issue', 'run_only'] as const;
const TRIGGER_KINDS = ['schedule', 'webhook', 'api'] as const;

export function countActiveFilterDimensions(filters: AutopilotListFilters): number {
  let count = 0;
...
export function AutopilotListToolbar({
  ...
  for (const row of allRows) {
    ...
    modeCounts.set(row.execution_mode, (modeCounts.get(row.execution_mode) ?? 0) + 1);
      ...
      <div className="flex min-w-0 items-center gap-2">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start" className="w-auto">
            <DropdownMenuRadioGroup
              value={scope}
              onValueChange={(value) => onScopeChange(value as AutopilotScope)}
            >
              {AUTOPILOT_SCOPES.map((s) => (
      ...
      <div className="flex shrink-0 items-center gap-1">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-auto">
            ...
            <DropdownMenuSub>
              <DropdownMenuSubTrigger>
                <span className="flex-1">{t(($) => $.toolbar.section_assignee)}</span>
              ...
              <DropdownMenuSubContent className="max-h-72 w-auto min-w-48 overflow-y-auto">
                {[...assigneeOptions.entries()].map(([value, { type, id, count }]) => (
                  <DropdownMenuCheckboxItem
                    key={value}
            ...
            <DropdownMenuSub>
              <DropdownMenuSubTrigger>
                <span className="flex-1">{t(($) => $.toolbar.section_trigger)}</span>
              ...
              <DropdownMenuSubContent className="w-auto min-w-44">
                {TRIGGER_KINDS.filter((kind) => triggerKindCounts.has(kind)).map((kind) => (
                  <DropdownMenuCheckboxItem
                    key={kind}
            ...
            <DropdownMenuSub>
              <DropdownMenuSubTrigger>
                ...
                {filters.modes.length > 0 && (
                  <span className="text-xs font-medium text-primary">{filters.modes.length}</span>
                )}
              </DropdownMenuSubTrigger>
              <DropdownMenuSubContent className="w-auto min-w-44">
                {MODES.map((mode) => (
            ...
            <DropdownMenuSub>
              <DropdownMenuSubTrigger>
                <span className="flex-1">{t(($) => $.toolbar.section_creator)}</span>
              ...
              <DropdownMenuSubContent className="max-h-72 w-auto min-w-48 overflow-y-auto">
                {[...creatorOptions.entries()].map(([value, { type, id, count }]) => (
                  <DropdownMenuCheckboxItem
                    key={value}
              ...
              <div className="mt-2 flex items-center gap-1.5">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="start" className="w-auto">
                    <DropdownMenuRadioGroup
                      value={sortField}
                      onValueChange={(v) => onSortFieldChange(v as AutopilotSortField)}
                    >
