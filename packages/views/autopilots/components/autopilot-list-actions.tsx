...
import { useDeleteAutopilot, useUpdateAutopilot } from '@goosar/core/autopilots';
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>{t(($) => $.actions.delete_dialog.title)}</DialogTitle>
          <DialogDescription>
        ...
        <p className="text-xs text-muted-foreground">{t(($) => $.actions.delete_dialog.warning)}</p>
        <DialogFooter>
          <Button
            type="button"
            variant="outline"
...
export function AutopilotRowActions({ row }: { row: Autopilot }) {
  ...
  return (
    <span onClick={(e) => e.stopPropagation()} className="flex items-center">
      ...
      <DeleteAutopilotsDialog rows={[row]} open={deleteOpen} onOpenChange={setDeleteOpen} />
    </span>
  );
}
  ...
  return (
    <>
      ...
      <div className="absolute bottom-6 left-1/2 z-50 flex -translate-x-1/2 items-center gap-1 rounded-lg border bg-background px-2 py-1.5 shadow-lg">
        ...
        {anyActive && (
          <Button variant="ghost" size="sm" onClick={() => setStatus(rows, 'paused')}>
            <Pause className="mr-1 size-3.5" />
            {t(($) => $.actions.pause)}
          </Button>
        )}
        {anyPaused && (
          <Button variant="ghost" size="sm" onClick={() => setStatus(rows, 'active')}>
