...
import { autopilotListOptions } from '@goosar/core/autopilots/queries';
import {
...
} from '@goosar/core/autopilots/stores';
...
import type { Autopilot } from '@goosar/core/types';
...
import { AutopilotBatchToolbar, AutopilotRowActions } from './autopilot-list-actions';
import type { ScheduleConfig } from './schedule-editor/model';
import { useT, useTimeAgo, useUiLocale } from '../../i18n';
...
function columnTrackVars(isVisible: (key: AutopilotColumnKey) => boolean): React.CSSProperties {
  const width = (key: AutopilotColumnKey) => (isVisible(key) ? `${COLUMN_WIDTHS[key]}px` : '0px');
  const minWidth =
    FIXED_TRACKS_WIDTH +
...
function NameCell({ autopilot }: { autopilot: Autopilot }) {
  ...
  return (
    <ListGridCell className="gap-1.5">
      <span className="min-w-0 truncate text-sm font-medium">{autopilot.title}</span>
...
function ModeCell({ autopilot }: { autopilot: Autopilot }) {
  ...
  const label =
    mode === 'create_issue' || mode === 'run_only' ? t(($) => $.execution_mode[mode]) : mode;
  return (
    <ListGridCell className="hidden @2xl:flex">
...
function AutopilotListHeader({
  ...
  const { t } = useT('autopilots');
  const sorted = (field: AutopilotSortField) => (sortField === field ? sortDirection : false);
  ...
  return (
    <ListGridHeader>
      ...
      {isColVisible('assignee') ? (
        <ListGridHeaderCell>{t(($) => $.page.table.assignee)}</ListGridHeaderCell>
      ) : (
...
function LoadingSkeleton() {
  return (
    <ListGrid
      className={GRID_COLS}
      style={columnTrackVars((key) => !AUTOPILOT_DEFAULT_HIDDEN_COLUMNS.includes(key))}
    >
      <ListGridHeader>
        <span aria-hidden="true" />
        <ListGridHeaderCell>
...
export function AutopilotsPage() {
  ...
  const listPollingInterval = useRealtimePollingInterval(BACKGROUND_DEGRADED_POLL_INTERVAL_MS);
  const {
    data: autopilots = [],
    isLoading,
    error: listError,
    ...
    refetchInterval: listPollingInterval,
  });
  ...
  const isColVisible = (key: AutopilotColumnKey) => !hiddenColumns.includes(key);
  ...
  const rows = useMemo<Autopilot[]>(() => {
    const filtered = scopeRows.filter((a) => {
      if (
        ...
        !filters.assignees.includes(actorFilterValue(a.assignee_type, a.assignee_id))
      ) {
        return false;
      }
      ...
      if (
        ...
        !filters.creators.includes(actorFilterValue(a.created_by_type, a.created_by_id))
      ) {
        return false;
      }
      return true;
    ...
    <div className="relative flex flex-1 min-h-0 flex-col">
      ...
      {listError ? (
        <CollectionPageState
          ...
          tone="destructive"
          icon={AlertCircle}
          title={listError instanceof Error ? listError.message : String(listError)}
          actions={
        ...
        <div className="flex flex-col items-center px-5 py-16">
          ...
          <p className="mb-6 mt-1 text-xs text-muted-foreground">{t(($) => $.page.empty.hint)}</p>
              ...
              return (
                ...
                >
                  ...
                  <div className="min-w-0">
                    <div className="text-sm font-medium">{t(($) => $.templates[tpl.id].title)}</div>
                    ...
                    >
                      ...
                      {isColVisible('created') ? (
                        <ListGridCell className="hidden whitespace-nowrap text-xs tabular-nums text-muted-foreground @2xl:flex">
                          {new Date(autopilot.created_at).toLocaleDateString(uiLocale)}
                        </ListGridCell>
                      ) : (
                        <ListGridCell className="hidden px-0 @2xl:flex" />
                      )}
      ...
      <AutopilotBatchToolbar rows={selectedRows} onClear={() => setSelectedIds(new Set())} />
