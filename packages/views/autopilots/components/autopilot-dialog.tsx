...
import {
  ...
  PopoverTitle,
  PopoverDescription,
} from '@goosar/ui/components/ui/popover';
...
import type { WebhookEventFilter } from '@goosar/core/types';
...
function serializeEventFilters(filters: WebhookEventFilter[]): string {
  return JSON.stringify(filters.map((f) => ({ event: f.event, actions: f.actions ?? [] })));
}
...
export function AutopilotDialog(props: AutopilotDialogProps) {
  ...
  const scheduleDirty =
    toCron(schedule) !== initialCronRef.current || schedule.timezone !== initialTimezoneRef.current;
  const eventFiltersDirty = serializeEventFilters(eventFilters) !== initialEventFiltersRef.current;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isCreate) {
        ...
        if (triggerKind === 'webhook' && eventFiltersDirty && firstTriggerIdRef.current) {
          try {
            await updateTrigger.mutateAsync({
              autopilotId: props.autopilotId,
          ...
          <>
            ...
            >
              ...
              <aside className="w-full lg:w-[380px] shrink-0 overflow-visible lg:overflow-y-auto px-5 py-5 space-y-5 bg-muted/30">
                ...
                {isCreate && <TriggerKindSection kind={triggerKind} onChange={setTriggerKind} />}
    ...
    <div className="space-y-3">
      <div>
        ...
        <p className="rounded-md border bg-background px-3 py-2 text-xs text-muted-foreground leading-relaxed">
          {isCreate ? t(($) => $.dialog.webhook_help_create) : t(($) => $.dialog.webhook_help_edit)}
        </p>
      </div>
      <WebhookEventFilterSection filters={eventFilters} onChange={onEventFiltersChange} />
    </div>
  );
}
