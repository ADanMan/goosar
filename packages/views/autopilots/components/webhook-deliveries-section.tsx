      ...
      {isLoading ? (
        ...
        <div className="rounded-md border overflow-hidden">
          {deliveries.map((delivery) => (
            <DeliveryRow key={delivery.id} delivery={delivery} autopilotId={autopilotId} />
          ))}
        </div>
      )}
    </section>
...
function DeliveryRow({
  ...
  const statusLabel =
    t(($) => $.deliveries.status[delivery.status as WebhookDeliveryStatus]) ?? delivery.status;
  ...
  return (
    <>
      ...
      >
        ...
        />
        <span className={cn('w-24 shrink-0 text-xs font-medium', visual.color)}>{statusLabel}</span>
        ...
        <span className="w-32 shrink-0 text-right text-xs text-muted-foreground tabular-nums">
          {formatDate(delivery.received_at || delivery.created_at, uiLocale)}
        </span>
      </button>
      {open && (
        ...
        <div className="space-y-4 pt-1">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className={cn('text-sm font-medium', visual.color)}>
                {t(($) => $.deliveries.status[full.status as WebhookDeliveryStatus]) ?? full.status}
              </span>
            </div>
          ...
          <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
            <MetaRow
              ...
              value={formatDate(full.received_at, uiLocale)}
            />
            <MetaRow
              label={t(($) => $.deliveries.detail.last_attempt_at)}
              value={formatDate(full.last_attempt_at, uiLocale)}
            />
            <MetaRow
            ...
            {full.status === 'queued' && (
              <MetaRow
                ...
                value={formatDate(full.available_at, uiLocale)}
              />
            )}
            <MetaRow
          ...
          {full.error && (
            <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-xs text-destructive">
              <div className="font-medium">{t(($) => $.deliveries.detail.error_label)}</div>
...
function DetailSections({
  ...
  return (
    <div className="space-y-3">
      {detail.raw_body && (
        <CodeBlock label={t(($) => $.deliveries.detail.raw_body)} value={detail.raw_body} />
      )}
...
function ReplayButton({
  ...
  const handleClick = async () => {
    try {
      ...
      const message = e instanceof Error ? e.message : t(($) => $.deliveries.replay.toast_failed);
      toast.error(message);
    }
  };

  return (
    <Button size="sm" variant="outline" onClick={handleClick} disabled={!enabled}>
      <RotateCw className={cn('h-3.5 w-3.5 mr-1', replay.isPending && 'animate-spin')} />
      {replay.isPending
