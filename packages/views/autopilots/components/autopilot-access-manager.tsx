...
import type { AutopilotCollaborator } from '@goosar/core/types';
...
export function AutopilotAccessManager({
  ...
  const grantedIds = useMemo(() => new Set(collaborators.map((c) => c.user_id)), [collaborators]);
    ...
    () =>
      members.filter(
        (m) =>
          ...
          (query === '' || m.name.toLowerCase().includes(query) || matchesPinyin(m.name, query)),
      ),
  ...
  return (
    <div className="space-y-2">
          ...
          {collaborators.map((c) => (
            ...
            >
              <span className="flex min-w-0 items-center gap-2">
                ...
                <span className="truncate text-sm">{getActorName('member', c.user_id)}</span>
              </span>
              <button
                type="button"
      ...
      <p className="text-xs text-muted-foreground">{t(($) => $.access.owner_note)}</p>
    </div>
  );
}
