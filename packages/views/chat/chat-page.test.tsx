...
vi.mock('./components/chat-thread-list', () => ({
  ChatThreadList: ({
    ...
    <button type="button" onClick={() => onSelectSession({ id: 'session-9', agent_id: 'agent-9' })}>
      select-thread
    </button>
  ),
}));
...
vi.mock('@goosar/ui/components/ui/resizable', () => ({
...
vi.mock('./components/use-chat-controller', async () => {
  ...
  return {
    useChatController: () => ({
      ...
      sessions: [],
      activeSessionId: useSyncExternalStore(subscribeToStore, () => storeRef.current)
        .activeSessionId,
