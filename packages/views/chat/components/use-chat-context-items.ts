...
function hydrateRecentEntry(
  entry: RecentContextEntry,
  data: Issue | Project | undefined,
): MentionItem {
...
export function parseCurrentContextRoute(
  pathname: string,
  searchParams: URLSearchParams,
): { type: 'issue' | 'project'; id: string } | null {
...
export function useChatContextItems(wsId: string): MentionItem[] {
  ...
  return useMemo(() => {
    ...
    const recentItems = visibleRecentEntries
      .map((entry, index) =>
        hydrateRecentEntry(entry, recentQueries[index]?.data as Issue | Project | undefined),
      )
