...
import { useWorkspacePresenceMap } from '@goosar/core/agents';
import { api } from '@goosar/core/api';
import { pendingChatTasksOptions, chatKeys, sortChatSessions } from '@goosar/core/chat/queries';
import {
...
import type { Agent, ChatSession, PendingChatTasksResponse } from '@goosar/core/types';
...
export function ChatThreadList({
    ...
    api
      ...
      .then(
        (result) => {
          ...
          apiLogger.info('cancelTask.success (list row)', {
            taskId: task.task_id,
  ...
  const renderRow = (session: ChatSession) => {
    ...
    const agentOffline = agent ? presence.byAgent.get(agent.id)?.availability === 'offline' : false;
    ...
    const timeText = last
      ? formatChatTime(last.created_at, uiLocale)
      : formatChatTime(session.updated_at, uiLocale);
    ...
    if (isRunning && agentOffline) {
      ...
      previewNode = (
        <span className="block truncate text-destructive">{t(($) => $.list.failed)}</span>
      );
