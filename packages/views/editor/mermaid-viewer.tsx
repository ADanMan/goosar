...
import { Check, Code as CodeIcon, Copy, Download, Image as ImageIcon, X } from 'lucide-react';
      ...
      <header className="flex shrink-0 items-center gap-1 border-b border-border bg-muted/30 px-3 py-2">
        ...
        <div className="ml-auto flex items-center gap-0.5">
          <ToolbarButton
            ...
            label={showSource ? t(($) => $.mermaid.show_diagram) : t(($) => $.mermaid.show_source)}
          >
            <CodeIcon className={cn('size-4', showSource && 'text-foreground')} />
          </ToolbarButton>
          <ToolbarButton onClick={handleCopySource} label={t(($) => $.mermaid.copy_source)}>
          ...
          <ToolbarButton onClick={onClose} label={t(($) => $.mermaid.close_viewer)}>
            <X className="size-4" />
          </ToolbarButton>
        </div>
      </header>
