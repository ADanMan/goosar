...
describe('highlightToHtml', () => {
  ...
  it('handles multiple highlights on one line', () => {
    expect(highlightToHtml('==a== and ==b==')).toBe('<mark>a</mark> and <mark>b</mark>');
  });
  ...
  it('highlights outside code while leaving code untouched', () => {
    expect(highlightToHtml('==hi== `x ==y==`')).toBe('<mark>hi</mark> `x ==y==`');
  });
  ...
  it('does not let a == inside inline code close the highlight', () => {
    expect(highlightToHtml('==a `b==c` d==')).toBe('<mark>a `b==c` d</mark>');
  });
