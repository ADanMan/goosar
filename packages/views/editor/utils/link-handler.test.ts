...
const APP_ORIGIN = 'https://app.goosar.ru';
...
describe('toInternalAppPath', () => {
  it('returns the path (with search and hash) for a URL on the app origin', () => {
    expect(toInternalAppPath(`${APP_ORIGIN}/acme/issues/MUL-1?tab=a#c`, APP_ORIGIN)).toBe(
      '/acme/issues/MUL-1?tab=a#c',
    );
  });
  ...
  it('keeps backend-served paths external so downloads and assets still work', () => {
    expect(toInternalAppPath(`${APP_ORIGIN}/api/attachments/abc/download`, APP_ORIGIN)).toBeNull();
    expect(toInternalAppPath(`${APP_ORIGIN}/uploads/2026/07/report.pdf`, APP_ORIGIN)).toBeNull();
...
describe('openLink', () => {
  ...
  it('opens an external URL in a new window', () => {
    openLink('https://github.com/adanman/goosar/pull/1', 'acme', APP_ORIGIN);
    expect(dispatched).toHaveLength(0);
    expect(openSpy).toHaveBeenCalledWith(
      'https://github.com/adanman/goosar/pull/1',
      '_blank',
      'noopener,noreferrer',
    );
  });
