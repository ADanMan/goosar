...
import { preprocessIssueIdentifiers, isIssueIdentifier } from '@goosar/ui/markdown';
...
describe('preprocessIssueIdentifiers', () => {
  ...
  it('skips identifiers inside inline code', () => {
    expect(preprocessIssueIdentifiers('use `MUL-1` here')).toBe('use `MUL-1` here');
  });
...
describe('isIssueIdentifier', () => {
  ...
  it('rejects a UUID (so real mentions are not treated as identifiers)', () => {
    expect(isIssueIdentifier('00000000-0000-0000-0000-000000000001')).toBe(false);
  });
