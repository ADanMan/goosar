...
const { openInNewTabMock, getShareableUrlMock, navState, slugState } = vi.hoisted(() => ({
...
import { AttachmentPreviewModal, useAttachmentPreview } from './attachment-preview-modal';
...
describe('AttachmentPreviewModal — server-relative download_url resolution (MUL-2976)', () => {
  ...
  it('passes an already-absolute CloudFront/presigned download_url through unchanged', () => {
    ...
    const img = document.querySelector('img');
    expect(img?.getAttribute('src')).toBe('https://cdn.example.test/att-1.png?Signature=s');
  });
});
...
function stubCanvasViewport() {
  vi.spyOn(HTMLElement.prototype, 'offsetWidth', 'get').mockReturnValue(CANVAS_VIEWPORT.width);
  vi.spyOn(HTMLElement.prototype, 'offsetHeight', 'get').mockReturnValue(CANVAS_VIEWPORT.height);
...
function stubNaturalSize(size: { width: number; height: number }) {
  vi.spyOn(HTMLImageElement.prototype, 'naturalWidth', 'get').mockReturnValue(size.width);
  vi.spyOn(HTMLImageElement.prototype, 'naturalHeight', 'get').mockReturnValue(size.height);
}
...
describe('AttachmentPreviewModal — image zoom', () => {
  ...
  it('letterboxes an image with no intrinsic size and hides the zoom controls', () => {
    ...
    renderImagePreview(makeAttachment({ filename: 'chart.svg', content_type: 'image/svg+xml' }));
