...
function makeCodePasteRuleEditor(): Editor {
  ...
  return new Editor({
    element,
    extensions: [StarterKit, Markdown.configure({ indentation: { style: 'space', size: 3 } })],
  });
}
...
describe('inline code paste rule', () => {
  it('preserves the character before an inline code paste match', () => {
    ...
    editor.view.dispatch(editor.state.tr.insertText('abcd`123`').setMeta('uiEvent', 'paste'));
...
describe('fenced code language', () => {
  it('keeps a newly-created unlabelled code block unlabelled in Markdown', () => {
    ...
    expect(editor.getMarkdown().trim()).toBe(['```', 'const answer = 42;', '```'].join('\n'));
  });
});
