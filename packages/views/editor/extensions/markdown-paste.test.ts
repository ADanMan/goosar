...
import { createMarkdownPasteExtension, escapeRawHtmlTagsOutsideCode } from './markdown-paste';
...
describe('markdownPaste — code block context', () => {
  ...
  it('lets semantic rich HTML paste natively instead of reparsing plain text as Markdown', () => {
    ...
    const text = 'viewFiltersToApiParams(filters) maps to Partial<ListIssuesParams>.';
    const html =
  ...
  it('keeps ordered numbering when rich HTML fragments every item into its own list', () => {
    ...
    const json = editor.getJSON() as JsonNode;
    const orderedLists = (json.content ?? []).filter((node) => node.type === 'orderedList');
    expect(orderedLists).toHaveLength(1);
    expect(orderedLists[0]?.content?.map(nodeText)).toEqual(['First', 'Second', 'Third']);
  ...
  it('repairs fragmented ordered lists nested inside another list item', () => {
    ...
    pasteThroughEditorDom(
      editor,
      'Plan\n1. First\n2. Second',
      '<ul><li>Plan' + '<ol><li>First</li></ol>' + '<ol><li>Second</li></ol>' + '</li></ul>',
    );
  ...
  it('keeps ordered lists separate when real content divides them', () => {
    ...
    pasteThroughEditorDom(
      editor,
      '1. First\nSeparate section\n1. Restarted',
      '<ol><li>First</li></ol>' + '<p>Separate section</p>' + '<ol><li>Restarted</li></ol>',
    );

    const json = editor.getJSON() as JsonNode;
    expect((json.content ?? []).filter((node) => node.type === 'orderedList')).toHaveLength(2);
  });
  ...
  it('preserves standard HTML element names as literal pasted text', () => {
    ...
    expect(handled).toBe(true);
    expect(editor.getText()).toBe(text);
    expect(editor.getMarkdown()).toBe('foo &lt;button&gt; &lt;img src="x"&gt; baz');
  });
...
describe('escapeRawHtmlTagsOutsideCode', () => {
  it('escapes HTML-like tags', () => {
    ...
    expect(escapeRawHtmlTagsOutsideCode('<MyComponent>')).toBe('&lt;MyComponent&gt;');
    expect(escapeRawHtmlTagsOutsideCode('</tag>')).toBe('&lt;/tag&gt;');
  });
  ...
  it('escapes standard HTML element names too', () => {
    ...
    expect(escapeRawHtmlTagsOutsideCode('</div>')).toBe('&lt;/div&gt;');
    expect(escapeRawHtmlTagsOutsideCode('<img src="x">')).toBe('&lt;img src="x"&gt;');
  });

  it('does not escape inside inline code spans', () => {
    expect(escapeRawHtmlTagsOutsideCode('`<tag>`')).toBe('`<tag>`');
    expect(escapeRawHtmlTagsOutsideCode('text `<T>` more')).toBe('text `<T>` more');
    expect(escapeRawHtmlTagsOutsideCode('``<tag>``')).toBe('``<tag>``');
  });
  ...
  it('escapes all tag-like runs in mixed content', () => {
    expect(escapeRawHtmlTagsOutsideCode('<T> and <div>')).toBe('&lt;T&gt; and &lt;div&gt;');
  });
