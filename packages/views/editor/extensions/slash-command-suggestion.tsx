...
export const SlashCommandList = forwardRef<SlashCommandListRef, SlashCommandListProps>(
  function SlashCommandList({ items, query, command, hideOnEmpty = false }, ref) {
    ...
    if (items.length === 0) {
      ...
      return (
        <div className="rounded-md border bg-popover p-2 text-xs text-muted-foreground shadow-md">
          {t(($) =>
            query.trim() ? $.slash_command.no_results : $.slash_command.no_skills_configured,
          )}
        </div>
      );
    }
    ...
    const describe = (item: SlashCommandItem): string | undefined =>
      item.descriptionKey === 'note' ? t(($) => $.slash_command.commands.note) : item.description;
...
function buildItems(qc: QueryClient, query: string): SlashCommandItem[] {
  ...
  const availableAgents = agents.filter(
    (a) => !a.archived_at && canAssignAgentToIssue(a, { userId, role: memberRole }).allowed,
  );
  const activeAgent =
