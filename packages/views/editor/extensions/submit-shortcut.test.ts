...
function makeEditor(content: Content, onSubmit: () => void, extraExtensions: AnyExtension[] = []) {
  return new Editor({
...
describe('submit shortcut matching', () => {
  it('handles the configured shortcut exactly', () => {
    ...
    expect(
      shouldHandleSubmitShortcut(event('Enter', { ctrlKey: true, shiftKey: true }), {
        configuredShortcut: shortcut,
        composing: false,
      }),
    ).toBe(false);
  ...
  it("does not submit on Safari's IME commit event or a repeated keydown", () => {
    ...
    expect(
      shouldHandleSubmitShortcut(event('Enter', { ctrlKey: true, repeat: true }), {
        configuredShortcut: createShortcutChord('Enter', { primary: true }),
        composing: false,
      }),
    ).toBe(false);
