// Расширение вставки: вставляемый текст разбирается как Markdown, а не берётся
// из text/html, который ProseMirror предпочитает по умолчанию.
...
function escapeRawHtmlTagsInSegment(segment: string): string {
  return segment.replace(RAW_HTML_TAG_RE, (match) =>
...
function canJoinOrderedLists(left: ProseMirrorNode, right: ProseMirrorNode): boolean {
  const leftType = left.attrs.type ?? '1';
  const rightType = right.attrs.type ?? '1';
  if (left.type.name !== 'orderedList' || right.type !== left.type || leftType !== rightType) {
    return false;
  }
...
function repairOrderedListsInFragment(fragment: Fragment): Fragment {
  ...
  fragment.forEach((node) => {
    const content = node.isLeaf ? node.content : repairOrderedListsInFragment(node.content);
    ...
    if (previous && canJoinOrderedLists(previous, repairedNode)) {
      repaired[repaired.length - 1] = previous.copy(previous.content.append(repairedNode.content));
      return;
    }
