...
import { getCurrentWsId } from '@goosar/core/platform';
import { flattenIssueBuckets, issueKeys } from '@goosar/core/issues/queries';
...
import type { Issue, ListIssuesCache, MemberWithUser, Agent, Squad } from '@goosar/core/types';
...
import type { IssueStatus, ProjectStatus } from '@goosar/core/types';
import { PROJECT_STATUS_CONFIG } from '@goosar/core/projects/config';
...
function mergeMentionItems(...itemGroups: MentionItem[][]): MentionItem[] {
...
export const MentionList = forwardRef<MentionListRef, MentionListProps>(function MentionList(
  { items, query, command, includeProjectSearch = false },
  ref,
) {
  const { t } = useT('editor');
  ...
  if (orderedItems.length === 0) {
    const isWaitingForServer =
      normalizedQuery !== '' && (isSearching || searchedQuery !== normalizedQuery);
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="flex min-w-0 items-center gap-2">
            ...
            {item.description && (
              <span className={`truncate text-foreground ${isClosed ? 'line-through' : ''}`}>
                {item.description}
              </span>
            )}
          </span>
...
export function createMentionSuggestion(
  ...
  function buildSyncItems(query: string): MentionItem[] {
    ...
    const squadItems: MentionItem[] = squads
      .filter(
        (s) => !s.archived_at && (s.name.toLowerCase().includes(q) || matchesPinyin(s.name, q)),
      )
