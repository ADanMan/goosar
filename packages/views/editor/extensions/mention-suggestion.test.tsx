...
import { issueKeys, PAGINATED_STATUSES } from '@goosar/core/issues/queries';
import { I18nProvider } from '@goosar/core/i18n/react';
import type { IssueStatus, ListIssuesCache } from '@goosar/core/types';
...
function fakeQc(data: {
  ...
  const agentsWithPermissions = (data.agents ?? []).map((a) => ({
    ...a,
    permission_mode: a.permission_mode ?? (a.visibility === 'private' ? 'private' : 'public_to'),
    invocation_targets:
      a.invocation_targets ??
      (a.visibility === 'private' ? [] : [{ target_type: 'workspace' as const, target_id: null }]),
  }));
    ...
    getQueriesData: <T,>(filter: { queryKey: readonly unknown[] }) => {
      ...
      for (const [k, v] of map) {
        ...
        if (
          ...
          prefix.every((seg, i) => JSON.stringify(seg) === JSON.stringify(parsed[i]))
        ) {
...
describe('createMentionSuggestion', () => {
  ...
  it('loads server issue and project matches when project search is enabled', async () => {
    ...
    expect(searchProjectsMock).toHaveBeenCalledWith(
      expect.objectContaining({ q: 'road', limit: 8 }),
    );
  });
  ...
  it('captures Tab while the popup has no selectable items, like Enter', () => {
    ...
    expect(ref.current?.onKeyDown({ event: new KeyboardEvent('keydown', { key: 'Tab' }) })).toBe(
      true,
    );
  });
  ...
  it('shows only current/recent chat context before the user types a query', () => {
    const qc = fakeQc({
      members: [{ user_id: 'u1', name: 'Alice', role: 'member' }],
      agents: [
        { id: 'a1', name: 'Aegis', archived_at: null, visibility: 'workspace', owner_id: null },
      ],
  ...
  it('prepends current/recent chat context without removing normal mention targets after the user types', () => {
    ...
    expect(result.map((item) => `${item.type}:${item.id}`).slice(0, 2)).toEqual([
      'issue:i1',
      'project:p1',
    ]);
