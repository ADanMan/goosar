...
import { ChevronRight, FolderGit, FolderOpen, Pencil, Plus, Search, Trash2 } from 'lucide-react';
...
export function ProjectResourcesSection({ projectId }: { projectId: string }) {
  ...
  const attachedUrls = new Set(resources.filter(isGithubRef).map((r) => r.resource_ref.url));
  ...
  const hasLocalDirectoryForCurrentDaemon = localDaemonId !== null && attachedLocalPaths.size > 0;
    ...
    try {
      ...
      if (!picked.ok) {
        if (picked.reason && picked.reason !== 'cancelled') {
          toast.error(picked.error ?? t(($) => $.resources.toast_local_pick_failed));
        }
        return;
      }
      ...
      const msg =
        err instanceof Error ? err.message : t(($) => $.resources.toast_local_pick_failed);
      toast.error(msg);
    } finally {
      setPicking(false);
    }
  ...
  const handleRenameLocalDirectory = async (
    ...
    try {
      ...
      const msg =
        err instanceof Error ? err.message : t(($) => $.resources.toast_local_rename_failed);
      toast.error(msg);
    }
  };
...
function ResourceRow({
  ...
  if (isGithubRef(resource)) {
    ...
    const display =
      resource.label ||
      (ref.ref ? `${githubShortLabel(ref.url)} @ ${ref.ref}` : githubShortLabel(ref.url));
...
function LocalDirectoryRow({
  ...
  const { t } = useT('projects');
  const ref = resource.resource_ref;
  const display = (ref.label || resource.label || ref.local_path).trim() || ref.local_path;
  const isForeignDaemon = localDaemonId !== null && ref.daemon_id !== localDaemonId;
  ...
  return (
    <div className={`flex items-center gap-2 text-xs group ${mismatch ? 'opacity-60' : ''}`}>
