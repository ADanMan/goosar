...
describe('ProjectStartDatePicker', () => {
  ...
  it('shows the formatted calendar day when a date is set', () => {
    renderWithI18n(<ProjectStartDatePicker startDate="2026-03-01" onUpdate={vi.fn()} />);
  ...
  it('emits start_date: null when the date is cleared', async () => {
    ...
    renderWithI18n(<ProjectStartDatePicker startDate="2026-03-01" onUpdate={onUpdate} />);
...
describe('ProjectDueDatePicker', () => {
  ...
  it('emits due_date: null when the date is cleared', async () => {
    const onUpdate = vi.fn();
    const user = userEvent.setup();
    renderWithI18n(<ProjectDueDatePicker dueDate="2026-03-01" onUpdate={onUpdate} />);
    await user.click(screen.getByText('Mar 1')); 
    await user.click(screen.getByRole('button', { name: 'Clear date' }));
