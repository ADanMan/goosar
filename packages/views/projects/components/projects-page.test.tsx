...
vi.mock('@goosar/core/projects', () => ({
  ...
  useProjectViewStore: (selector: (state: unknown) => unknown) => selector(mocks.projectViewState),
}));

vi.mock('@goosar/core/pins', () => ({
...
describe('ProjectsPage compact row navigation', () => {
  it('renders the project name as text, not a title link', () => {
    ...
    expect(within(row).queryByRole('link', { name: PROJECT.title })).not.toBeInTheDocument();
  });
