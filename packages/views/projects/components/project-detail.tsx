...
import { useUpdateProject, useDeleteProject } from '@goosar/core/projects/mutations';
import { pinListOptions } from '@goosar/core/pins';
import { useCreatePin, useDeletePin } from '@goosar/core/pins';
...
import { EmojiPicker } from '@goosar/ui/components/common/emoji-picker';
...
export function ProjectDetail({ projectId }: { projectId: string }) {
  ...
  const projectPollingInterval = useRealtimePollingInterval(BACKGROUND_DEGRADED_POLL_INTERVAL_MS);
  const { data: project, isLoading } = useQuery({
    ...projectDetailOptions(wsId, projectId),
    refetchInterval: projectPollingInterval,
  });
  ...
  const issueScope = useMemo(() => ({ type: 'project' as const, projectId }), [projectId]);
  ...
  const { defaultLayout, onLayoutChanged } = useDefaultLayout({
    id: 'goosar_project_detail_layout',
  });
  const sidebarRef = usePanelRef();
  const desktopSidebarInitialOpen = getAnimatedRightSidebarInitialOpen(true, defaultLayout);
  const {
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start" className="w-44">
                  {PROJECT_STATUS_ORDER.map((s) => (
                    ...
                    >
                      <span
                        className={cn('size-2 rounded-full', PROJECT_STATUS_CONFIG[s].dotColor)}
                      />
