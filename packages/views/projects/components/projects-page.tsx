...
import { pinListOptions, useCreatePin, useDeletePin } from '@goosar/core/pins';
...
const progressOf = (p: Project) => (p.issue_count > 0 ? p.done_count / p.issue_count : -1);
...
function columnTrackVars(isVisible: (key: ProjectColumnKey) => boolean): React.CSSProperties {
  const width = (key: ProjectColumnKey) => (isVisible(key) ? `${COLUMN_WIDTHS[key]}px` : '0px');
  const minWidth =
    FIXED_TRACKS_WIDTH +
...
function ProjectTableHeader({
  ...
  const { t } = useT('projects');
  const sorted = (field: ProjectSortField) => (sortField === field ? sortDirection : false);
...
function ProjectCard({
  ...
  const progressPercent =
    project.issue_count > 0 ? Math.round((project.done_count / project.issue_count) * 100) : 0;
      ...
      <div className="mt-0 flex items-center justify-between border-t px-3 pb-3 pt-2">
        <ProjectLeadPicker
          ...
          renderTrigger={(leadName) => (
            <button
              type="button"
              className="-mx-1.5 flex items-center gap-1.5 rounded px-1.5 py-0.5 transition-colors hover:bg-accent/60"
            >
              {project.lead_type && project.lead_id ? (
                <ActorAvatar
                  actorType={project.lead_type}
...
export function ProjectsPage() {
  ...
  const visible = useMemo(() => {
    ...
    sorted.sort((a, b) => {
      ...
      if (sortField === 'status') {
        return (
          (STATUS_ORDER[a.status] - STATUS_ORDER[b.status]) * dir || a.title.localeCompare(b.title)
        );
      }
