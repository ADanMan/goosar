...
const ws = vi.hoisted(() => ({
  current: { id: 'ws1', slug: 'acme' } as { id: string; slug: string } | null,
}));
...
function presentationOf(url: string, fallback?: string) {
  ...
  return renderHook(() => useTabPresentation(url, fallback), { wrapper }).result.current;
}
...
describe('ResourceLeadingVisual', () => {
  ...
  it('renders the actor avatar for an actor visual', () => {
    const { getByTestId } = render(
      <ResourceLeadingVisual visual={{ kind: 'actor', actorType: 'agent', id: 'ag1' }} />,
    );
