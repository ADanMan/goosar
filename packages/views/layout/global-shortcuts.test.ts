...
describe('global shortcut event guard', () => {
  ...
  it('ignores repeats and both standard and Safari IME composition events', () => {
    expect(
      shouldIgnoreGlobalShortcutEvent(new KeyboardEvent('keydown', { key: 'k', repeat: true })),
    ).toBe(true);
    expect(
  ...
  it('allows a fresh unhandled keydown through', () => {
    expect(
      shouldIgnoreGlobalShortcutEvent(new KeyboardEvent('keydown', { key: 'k', metaKey: true })),
    ).toBe(false);
  });
});
