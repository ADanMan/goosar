/**
 * Ссылка, ведущая на этот же деплой, — это внутренний переход, а не внешний.
 *
 * Чат и комментарии рендерят контент с абсолютными URL. Если ссылка указывает
 * на этот же домен, её не должен обрабатывать `window.open` как внешнюю (на
 * десктопе это открывает системный браузер). Тест использует реальный
 * `openLink`, а не мок, — иначе решение о маршрутизации не проверяется.
 */
...
beforeEach(() => {
  navigatedPaths = [];
  window.addEventListener('goosar:navigate', captureNavigate);
  openSpy = vi.spyOn(window, 'open').mockImplementation(() => null);
});

afterEach(() => {
  window.removeEventListener('goosar:navigate', captureNavigate);
  vi.restoreAllMocks();
});
...
describe('RichContent link routing', () => {
  ...
  it('still hands a genuinely external link to the browser', () => {
    const external = 'https://github.com/adanman/goosar/pull/1';
    renderContent(`[#1](${external})`);

    screen.getByText('#1').click();

    expect(navigatedPaths).toEqual([]);
    expect(openSpy).toHaveBeenCalledWith(external, '_blank', 'noopener,noreferrer');
  });
  ...
  it('keeps a same-origin /uploads file external — the backend serves it, not the router', () => {
    ...
    expect(navigatedPaths).toEqual([]);
    expect(openSpy).toHaveBeenCalledWith(upload, '_blank', 'noopener,noreferrer');
  });
});
