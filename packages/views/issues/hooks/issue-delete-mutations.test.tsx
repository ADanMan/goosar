...
import { agentTaskSnapshotKeys, agentTasksKeys } from '@goosar/core/agents/queries';
import { useBatchDeleteIssues, useDeleteIssue } from '@goosar/core/issues/mutations';
import { issueKeys } from '@goosar/core/issues/queries';
import { labelKeys } from '@goosar/core/labels/queries';
...
describe('useDeleteIssue', () => {
  ...
  it('cleans list, my-list, issue-scoped, and dependent agent caches after a successful single delete', async () => {
    ...
    qc.setQueryData<Issue[]>(issueKeys.children(WS_ID, PARENT_ISSUE_ID), [baseIssue, otherIssue]);
    ...
    expect(ids(qc.getQueryData(issueKeys.myList(WS_ID, 'assigned', assignedFilter)))).toEqual([
      OTHER_ISSUE_ID,
    ]);
    expect(ids(qc.getQueryData(issueKeys.myList(WS_ID, 'created', createdFilter)))).toEqual([]);
  ...
  it('optimistically prunes parent children without invalidating them before delete settles', async () => {
    ...
    expect(qc.getQueryState(issueKeys.children(WS_ID, PARENT_ISSUE_ID))?.isInvalidated).not.toBe(
      true,
    );
  ...
  it('invalidates child progress when a single delete removes a parent issue', async () => {
    ...
    qc.setQueryData<Issue[]>(issueKeys.children(WS_ID, ISSUE_ID), [childIssue]);
  ...
  it('restores optimistic snapshots when a single delete fails', async () => {
    ...
    qc.setQueryData<ListIssuesCache>(issueKeys.myList(WS_ID, 'assigned', assignedFilter), myList);
    qc.setQueryData<Issue>(issueKeys.detail(WS_ID, ISSUE_ID), baseIssue);
    qc.setQueryData<Issue[]>(issueKeys.children(WS_ID, PARENT_ISSUE_ID), children);
    qc.setQueryData<IssueUsageSummary>(issueKeys.usage(ISSUE_ID), usage);
    ...
    expect(qc.getQueryData(issueKeys.myList(WS_ID, 'assigned', assignedFilter))).toEqual(myList);
    expect(qc.getQueryData(issueKeys.detail(WS_ID, ISSUE_ID))).toEqual(baseIssue);
    expect(qc.getQueryData(issueKeys.children(WS_ID, PARENT_ISSUE_ID))).toEqual(children);
...
describe('useBatchDeleteIssues', () => {
  ...
  it('cleans list, my-list, issue-scoped, parent, and dependent agent caches after a fully successful batch delete', async () => {
    ...
    qc.setQueryData<ListIssuesCache>(issueKeys.list(WS_ID), makeListCache(baseIssue, childIssue));
    qc.setQueryData<ListIssuesCache>(
      issueKeys.myList(WS_ID, 'assigned', assignedFilter),
    ...
    qc.setQueryData<Issue[]>(issueKeys.children(WS_ID, PARENT_ISSUE_ID), [baseIssue, childIssue]);
    qc.setQueryData(issueKeys.childProgress(WS_ID), {});
    qc.setQueryData<IssueUsageSummary>(issueKeys.usage(ISSUE_ID), usage);
    qc.setQueryData<IssueUsageSummary>(issueKeys.usage(OTHER_ISSUE_ID), usage);
    qc.setQueryData<AgentTask[]>(issueKeys.tasks(ISSUE_ID), [makeTask()]);
    qc.setQueryData<AgentTask[]>(issueKeys.tasks(OTHER_ISSUE_ID), [makeTask(OTHER_ISSUE_ID)]);
    qc.setQueryData<AgentTask[]>(agentTaskSnapshotKeys.list(WS_ID), [makeTask()]);
    qc.setQueryData<AgentTask[]>(agentTasksKeys.detail(WS_ID, AGENT_ID), [makeTask()]);
    ...
    expect(ids(qc.getQueryData(issueKeys.myList(WS_ID, 'created', createdFilter)))).toEqual([]);
    expect(qc.getQueryData(issueKeys.detail(WS_ID, ISSUE_ID))).toBeUndefined();
    expect(qc.getQueryData(issueKeys.detail(WS_ID, OTHER_ISSUE_ID))).toBeUndefined();
    expect(qc.getQueryData(issueKeys.usage(ISSUE_ID))).toBeUndefined();
    ...
    expect(qc.getQueryData<Issue[]>(issueKeys.children(WS_ID, PARENT_ISSUE_ID))).toEqual([]);
    expectInvalidated(qc, issueKeys.children(WS_ID, PARENT_ISSUE_ID));
    expectInvalidated(qc, issueKeys.childProgress(WS_ID));
    expectInvalidated(qc, agentTaskSnapshotKeys.list(WS_ID));
    expectInvalidated(qc, agentTasksKeys.detail(WS_ID, AGENT_ID));
  ...
  it('restores optimistic list snapshots on partial batch delete before invalidating caches', async () => {
    ...
    qc.setQueryData<Issue>(issueKeys.detail(WS_ID, OTHER_ISSUE_ID), childIssue);
    qc.setQueryData<IssueUsageSummary>(issueKeys.usage(ISSUE_ID), usage);
    qc.setQueryData<Attachment[]>(issueKeys.attachments(ISSUE_ID), [attachment]);
    qc.setQueryData<TimelineEntry[]>(issueKeys.timeline(ISSUE_ID), timeline);
    qc.setQueryData<IssueLabelsResponse>(labelKeys.byIssue(WS_ID, ISSUE_ID), issueLabels);
    qc.setQueryData<IssueUsageSummary>(issueKeys.usage(OTHER_ISSUE_ID), usage);
    ...
    qc.setQueryData<IssueLabelsResponse>(labelKeys.byIssue(WS_ID, OTHER_ISSUE_ID), issueLabels);
    qc.setQueryData<AgentTask[]>(agentTaskSnapshotKeys.list(WS_ID), [makeTask()]);
    qc.setQueryData<AgentTask[]>(agentTasksKeys.detail(WS_ID, AGENT_ID), [makeTask()]);
    ...
    try {
      ...
      expect(batchDeleteIssues).toHaveBeenCalledWith(idsToDelete);
      expect(qc.getQueryData(issueKeys.list(WS_ID))).toEqual(list);
      expect(qc.getQueryData(issueKeys.myList(WS_ID, 'assigned', assignedFilter))).toEqual(
        assignedMyList,
      );
      expect(qc.getQueryData(issueKeys.myList(WS_ID, 'created', createdFilter))).toEqual(
        createdMyList,
      );
      expect(qc.getQueryData(issueKeys.children(WS_ID, PARENT_ISSUE_ID))).toEqual(children);
      expect(qc.getQueryData(issueKeys.detail(WS_ID, ISSUE_ID))).toEqual(baseIssue);
      expect(qc.getQueryData(issueKeys.detail(WS_ID, OTHER_ISSUE_ID))).toEqual(childIssue);
      expect(qc.getQueryData(issueKeys.usage(ISSUE_ID))).toEqual(usage);
      expect(qc.getQueryData(issueKeys.attachments(ISSUE_ID))).toEqual([attachment]);
      expect(qc.getQueryData(issueKeys.timeline(ISSUE_ID))).toEqual(timeline);
      expect(qc.getQueryData(labelKeys.byIssue(WS_ID, ISSUE_ID))).toEqual(issueLabels);
      ...
      expect(qc.getQueryData(labelKeys.byIssue(WS_ID, OTHER_ISSUE_ID))).toEqual(issueLabels);
