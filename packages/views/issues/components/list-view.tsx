...
import type { Issue, IssueStatus, Project } from '@goosar/core/types';
...
function ListViewImpl({
  ...
  const groups = useMemo(() => buildListGroups(visibleStatuses), [visibleStatuses]);
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      const resetColumns = () => setColumns(buildColumns(issues, groups, 'status'));
    ...
    <Accordion.Item value={status} ref={dragEnabled ? setDroppableRef : undefined}>
      <Accordion.Header
        className={`group/header sticky top-0 z-10 flex h-10 items-center rounded-lg bg-muted transition-colors hover:bg-accent ${
          isOver && !isExpanded ? 'ring-2 ring-brand/25 bg-accent/15' : ''
        }`}
      >
