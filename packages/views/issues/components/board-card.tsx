...
import type { Issue, IssueProperty, Project, UpdateIssueRequest } from '@goosar/core/types';
...
export const BoardCardContent = memo(function BoardCardContent({
  ...
  return (
    <div className="relative overflow-hidden rounded-lg border-[0.5px] border-surface-border bg-surface py-3 pl-3 pr-2.5 shadow-[var(--surface-shadow)] transition-colors group-hover/card:border-foreground/15 group-hover/card:bg-surface-hover group-data-[popup-open]/card:border-foreground/15 group-data-[popup-open]/card:bg-surface-hover">
      ...
      {showDescription &&
        (() => {
          ...
          return <p className="mt-1 text-xs text-muted-foreground line-clamp-1">{preview}</p>;
        })()}
      ...
      {showMetaRow && (
        <div className="mt-2 flex items-center justify-between gap-2">
          {showAssigneeSection && <div className="min-w-0 flex-1">{assigneeNode}</div>}
          {showRightMeta && (
                ...
                (canEdit ? (
                  ...
                  <span
                    className={`flex shrink-0 items-center gap-1 text-xs ${
                      isPastDateOnly(issue.due_date) ? 'text-destructive' : 'text-muted-foreground'
                    }`}
                  >
                    <CalendarDays className="size-3" />
                    {formatDate(issue.due_date!)}
