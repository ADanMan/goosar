...
describe('getIssueTableSelectionRange', () => {
  ...
  it('returns an inclusive range in either direction', () => {
    expect(getIssueTableSelectionRange(issueIds, 'issue-1', 'issue-4')).toEqual(issueIds);
    expect(getIssueTableSelectionRange(issueIds, 'issue-4', 'issue-2')).toEqual([
      'issue-2',
      'issue-3',
      'issue-4',
    ]);
  ...
  it('returns null when the anchor or target is not visible', () => {
    ...
    expect(getIssueTableSelectionRange(issueIds, 'missing', 'issue-2')).toBeNull();
    expect(getIssueTableSelectionRange(issueIds, 'issue-1', 'missing')).toBeNull();
  });
});
...
describe('refreshFrozenTableRows', () => {
  ...
  it('keeps structure and keys while swapping in live issue objects', () => {
    ...
    expect(refreshed.map((row) => row.key)).toEqual(['status:todo', 'issue-1', 'issue-2']);
...
describe('table calculations and CSV', () => {
  it('calculates numeric custom-property sums, averages, and counts', () => {
    ...
    expect(calculateIssueTableColumn(issues, 'property:estimate', 'sum')).toBe(8);
    expect(calculateIssueTableColumn(issues, 'property:estimate', 'average')).toBe(4);
    expect(calculateIssueTableColumn(issues, 'property:estimate', 'count')).toBe(2);
  });

  it('escapes commas, quotes, and newlines in CSV output', () => {
    expect(buildIssueTableCsv(['Identifier', 'Title'], [['MUL-1', 'Ship, "verify"\nnext']])).toBe(
      'Identifier,Title\r\nMUL-1,"Ship, ""verify""\nnext"',
    );
  });
