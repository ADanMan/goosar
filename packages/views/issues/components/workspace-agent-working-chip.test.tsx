...
vi.mock('@goosar/core/agents', () => ({
  workspaceWorkingAgentsOptions: (wsId: string, type?: string, mineRelation?: string) => {
...
vi.mock('@goosar/ui/components/ui/button', async () => {
  const actual = await vi.importActual<typeof import('@goosar/ui/components/ui/button')>(
    '@goosar/ui/components/ui/button',
  );
  return {
    ...actual,
...
import { WorkspaceAgentWorkingChip, chipAppearance } from './workspace-agent-working-chip';

function makeAgent(id: string, runningTaskCount = 1): WorkspaceWorkingAgent {
  return {
    id,
    name: `Agent ${id}`,
    avatar_url: null,
...
describe('WorkspaceAgentWorkingChip', () => {
  it('shows every agent returned by the independent workspace API', () => {
    mockState.agents = [makeAgent('agent-1'), makeAgent('agent-2', 3), makeAgent('agent-3')];

    renderWithI18n(<WorkspaceAgentWorkingChip value={false} onToggle={() => {}} />);

    expect(screen.getByRole('button', { name: '3 agents working' })).toBeTruthy();
    expect(mockState.avatarAgentIds).toEqual(['agent-1', 'agent-2', 'agent-3']);
  ...
  it('requests the selected My Issues relation when the header is scoped', () => {
    renderWithI18n(
      <WorkspaceAgentWorkingChip value={false} onToggle={() => {}} mineRelation="involved" />,
    );
  ...
  it('shows a known zero instead of an indeterminate Table value', () => {
    ...
    expect(screen.getByRole('button', { name: '0 agents working' })).toBeTruthy();
  ...
  it('keeps the active filter visually selected after the final agent stops', () => {
    renderWithI18n(<WorkspaceAgentWorkingChip value onToggle={() => {}} />);
