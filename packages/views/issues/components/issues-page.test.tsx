...
const mockListIssueTableGroups = vi.hoisted(() =>
  vi.fn(async (request: any) => {
    ...
    return {
      query_fingerprint: 'test',
      total: response.groups.reduce((total: number, group: any) => total + group.total, 0),
...
const mockListIssueTableRows = vi.hoisted(() =>
  vi.fn(async (request: any) => {
    if (request.group.kind === 'assignee') {
      const response = await mockListGroupedIssues();
      const group = response.groups.find((candidate: any) => candidate.id === request.group_key);
...
vi.mock('@goosar/core/issues/stores/view-store', () => ({
...
vi.mock('@goosar/core/issues/stores/issues-scope-store', () => ({
...
vi.mock('@goosar/core/issues/stores/selection-store', () => ({
...
vi.mock('@goosar/core/issues/stores/recent-issues-store', () => ({
...
function mockAssigneeGroups(issues: Issue[]) {
  const groups = new Map<
    string,
    { assignee_type: Issue['assignee_type']; assignee_id: string | null; issues: Issue[] }
  >();
...
describe('IssuesPage (shared)', () => {
  ...
  it('shows empty state when there are no issues', async () => {
    ...
    expect(screen.getByText('Create an task to get started.')).toBeInTheDocument();
  });
      ...
      const matches = mockIssues.filter(
        (i) =>
          ...
          (!params?.assignee_types ||
            (i.assignee_type !== null && params.assignee_types.includes(i.assignee_type))),
      );
