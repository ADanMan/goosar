...
import { Sheet, SheetContent } from '@goosar/ui/components/ui/sheet';
...
import {
  ...
  CommandGroup,
  CommandItem,
} from '@goosar/ui/components/ui/command';
import { AvatarGroup, AvatarGroupCount } from '@goosar/ui/components/ui/avatar';
...
import { STATUS_CONFIG, PRIORITY_CONFIG } from '@goosar/core/issues/config';
import { formatDateOnly, isPastDateOnly } from '@goosar/core/issues/date';
import { useUpdateIssue } from '@goosar/core/issues/mutations';
...
import { useGitHubSettings } from '@goosar/core/github';
...
import { useRecentContextStore } from '@goosar/core/chat';
import {
  issueListOptions,
  issueDetailOptions,
  childIssuesOptions,
...
import { issueLabelsOptions } from '@goosar/core/labels';
import { propertyListOptions } from '@goosar/core/properties';
import { memberListOptions, agentListOptions } from '@goosar/core/workspace/queries';
import {
...
import { ReactionBar } from '@goosar/ui/components/common/reaction-bar';
import { useTimeAgo, useUiLocale } from '../../i18n';
            ...
            <div className="flex min-w-0 flex-1 items-center gap-1">
              ...
              <Tooltip>
                ...
                <TooltipContent side="top">
                  {new Date(entry.created_at).toLocaleString(uiLocale)}
                </TooltipContent>
              </Tooltip>
            </div>
          </div>
    ...
    (updates: Partial<UpdateIssueRequest>) => {
      updateIssue.mutate(
        { id: child.id, ...updates },
        {
          onError: (err) =>
            toast.error(
              err instanceof Error && err.message ? err.message : t(($) => $.detail.update_failed),
            ),
        },
      );
    },
...
function SubIssueDisplayPopover({ workspaceProperties }: { workspaceProperties: IssueProperty[] }) {
  const { t } = useT('issues');
...
export function IssueDetail({
  ...
  layoutId = 'goosar_issue_detail_layout',
  highlightCommentId,
}: IssueDetailProps) {
  const { t } = useT('issues');
  ...
  const detailPollingInterval = useRealtimePollingInterval();
  const railPollingInterval = useRealtimePollingInterval(BACKGROUND_DEGRADED_POLL_INTERVAL_MS);
  const { data: issue = null, isLoading: issueLoading } = useQuery({
    ...
    refetchInterval: detailPollingInterval,
  });
  ...
  const minimapThreads = useMemo<ThreadMinimapThread[]>(
    () =>
      items.flatMap((it) =>
        it.kind === 'comment' || it.kind === 'resolved-bar' ? [{ id: it.id, entry: it.entry }] : [],
      ),
    [items],
  );
  ...
  const { data: usage } = useQuery({
    ...
    refetchInterval: railPollingInterval,
  });
  ...
  const handleDescriptionUpload = useCallback(
    async (file: File) => {
      ...
      if (result) {
        descPendingAttachmentsRef.current = [...descPendingAttachmentsRef.current, result];
        ...
        {propertiesOpen && (
          <div className="grid grid-cols-[auto_1fr] gap-x-2 gap-y-0.5 pl-2">
            ...
            {(OPTIONAL_PROP_KEYS.some(
              (k) =>
                !visibleOptionalProps.has(k) && (k !== 'stage' || issue.parent_issue_id != null),
            ) ||
      ...
      {Object.keys(issue.metadata ?? {}).length > 0 && (
        <>
          ...
          >
            ...
            <span className="tabular-nums">· {Object.keys(issue.metadata ?? {}).length}</span>
          </button>
