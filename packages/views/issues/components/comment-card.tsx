...
import type { TimelineEntry, Attachment } from '@goosar/core/types';
import { contentReferencesAttachment } from '@goosar/core/types';
import { useCommentCollapseStore, useCommentDraftStore } from '@goosar/core/issues/stores';
...
function useEditAttachmentState(
  ...
  const { submitting: saving, submit: saveEdit } = useComposerSubmit({
    ...
    onSubmit: async (trimmed) => {
      ...
      const attachmentsChanged = !sameIdSet(
        activeIds,
        (entry.attachments ?? []).map((a) => a.id),
      );
        ...
        >
          ...
          <div className="flex items-center justify-between gap-2 mt-2">
            ...
            <div className="flex shrink-0 items-center gap-2">
              ...
              />
              <Button size="sm" variant="ghost" onClick={edit.cancelEdit} disabled={edit.saving}>
  ...
  return (
    <Card className="!py-0 !gap-0 overflow-clip transition-colors duration-700">
      ...
      >
        ...
        <StickyHeaderShell sticky={stickyHeader} highlighted={isHighlighted} className="px-4 py-3">
              ...
              {resolutionReply && (
                <div
                  ...
                  className={cn(
                    ...
                    highlightedCommentId === resolutionReply.id &&
                      highlightedCommentBackgroundClass,
                  )}
                >
                  <CommentRow
                    issueId={issueId}
              ...
              <div className="border-t border-border/50 px-4 py-2.5">
                <ReplyInput
                  ...
                  onSubmit={(content, attachmentIds, suppressAgentIds) =>
                    onReply(entry.id, content, attachmentIds, suppressAgentIds)
                  }
                />
              </div>
            </>
