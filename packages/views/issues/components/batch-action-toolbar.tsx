...
import type { Issue, UpdateIssueRequest } from '@goosar/core/types';
import { commonIssueFields } from '@goosar/core/issues/batch';
import { useBatchUpdateIssues, useBatchDeleteIssues } from '@goosar/core/issues/mutations';
...
export function BatchActionToolbar({
  ...
  const loading = surfaceActions?.isPending ?? (batchUpdate.isPending || batchDelete.isPending);
  ...
  const handleBatchUpdate = async (updates: Partial<UpdateIssueRequest>) => {
    try {
      ...
      toast.error(
        err instanceof Error && err.message ? err.message : t(($) => $.batch.update_failed),
      );
    }
  };
  ...
  const handleBatchDelete = async () => {
    try {
      ...
      toast.error(
        err instanceof Error && err.message ? err.message : t(($) => $.batch.delete_failed),
      );
    } finally {
      setDeleteOpen(false);
    }
