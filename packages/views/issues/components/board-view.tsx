...
import { useLoadMoreByAssigneeGroup, useLoadMoreByStatus } from '@goosar/core/issues/mutations';
import type {
...
import type { IssueGrouping } from '@goosar/core/issues/stores/view-store';
...
function buildGroups(
  ...
  if (groupingProperty) {
    const columns: BoardColumnGroup[] = (groupingProperty.config.options ?? []).map((option) => ({
...
function BoardViewImpl({
  ...
  const propertyColumnsPollingInterval = useRealtimePollingInterval(
    BACKGROUND_DEGRADED_POLL_INTERVAL_MS,
  );
  const { data: workspaceProperties = [] } = useQuery({
    ...propertyListOptions(boardWsId),
    refetchInterval: groupingPropertyId ? propertyColumnsPollingInterval : false,
  });
  ...
  const applyPropertyGroupValue = useCallback(
    (group: BoardColumnGroup, issueId: string) => {
      ...
      const onError = (err: unknown) => {
        toast.error(
          err instanceof Error && err.message ? err.message : t(($) => $.page.move_failed),
        );
      };
      if (group.propertyOptionId === null) {
        unsetIssuePropertyMutation.mutate({ issueId, propertyId: group.propertyId }, { onError });
    ...
    if (grouping === 'assignee' && groupBranches?.enabled) {
      return groupBranches.descriptors.flatMap((descriptor): BoardColumnGroup[] => {
        ...
        return [
          {
            id: descriptor.key,
            title: actor ? getActorName(actor.type, actor.id) : t(($) => $.filters.no_assignee),
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      const finalCol =
        sortBy === 'position' ? findColumn(finalColumns, activeId, groupIds) : overCol;
      if (!finalCol) {
        resetColumns();
        return;
      }
