...
import type { Issue, IssueTableGroupDescriptor } from '@goosar/core/types';
...
const mockListChildrenByParents = vi.hoisted(() => vi.fn().mockResolvedValue({ issues: [] }));
...
const useLoadMoreByStatusMock = vi.fn((_status: string, _opts?: unknown, _sort?: unknown) => ({
  total: 0,
  loaded: 0,
  hasMore: false,
  isLoading: false,
...
vi.mock('@goosar/core/issues/mutations', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@goosar/core/issues/mutations')>();
  return {
    ...actual,
...
const mockToggleSwimlaneCollapsed = mockViewState.toggleSwimlaneCollapsed as ReturnType<
  typeof vi.fn
>;

vi.mock('@goosar/core/issues/stores/view-store-context', () => ({
...
vi.mock('@goosar/core/modals', () => ({
  useModalStore: Object.assign(() => ({ open: mockOpenModal }), {
      ...
      (descriptor.secondary_groups ?? []).map((cell) => [
        cell.key,
        {
          ...
          loaded: issues.filter(
            (issue) => cell.value.kind === 'status' && issue.status === cell.value.status,
          ).length,
          hasMore: false,
          isLoading: false,
          isFetching: false,
...
describe('SwimLaneView', () => {
  ...
  it('renders a Cancelled column and its cards by default', () => {
    renderWithI18n(
      <SwimLaneView issues={[...mockIssues, cancelledOrphan]} onMoveIssue={vi.fn()} />,
    );
  ...
  it('omits the Cancelled column when the status filter narrows to a subset without cancelled', () => {
    renderWithI18n(
      <SwimLaneView
        ...
        visibleStatuses={['backlog', 'todo', 'in_progress', 'in_review', 'done', 'blocked']}
        onMoveIssue={vi.fn()}
      />,
    );
  ...
  it("renders children whose parent is not in the loaded set under an 'Other parents' fallback lane", () => {
    renderWithI18n(<SwimLaneView issues={[orphanChild]} onMoveIssue={vi.fn()} />);
  ...
  it("does not call onMoveIssue when a card is dropped onto the empty whitespace of an 'Other parents' cell", () => {
    ...
    renderWithI18n(
      <SwimLaneView issues={[...mockIssues, orphanChild]} onMoveIssue={mockOnMoveIssue} />,
    );
  ...
  it('renders HiddenColumnsPanel only when hiddenStatuses is non-empty', () => {
    const { unmount } = renderWithI18n(<SwimLaneView issues={mockIssues} onMoveIssue={vi.fn()} />);
  ...
  it('calls onMoveIssue on drag-and-drop end', () => {
    const mockOnMoveIssue = vi.fn();
    renderWithI18n(<SwimLaneView issues={mockIssues} onMoveIssue={mockOnMoveIssue} />);
  ...
  it('passes a settle callback that releases the lock without error', () => {
    ...
    const onSettled = mockOnMoveIssue.mock.calls[0]?.[2] as (() => void) | undefined;
  ...
  it('persists lane order via setSwimlaneOrder when a lane is dragged onto another', () => {
    renderWithI18n(<SwimLaneView issues={multiParentIssues} onMoveIssue={vi.fn()} />);
  ...
  it('does not call onMoveIssue when a lane drag ends (no card mutation)', () => {
    const mockOnMoveIssue = vi.fn();
    renderWithI18n(<SwimLaneView issues={multiParentIssues} onMoveIssue={mockOnMoveIssue} />);
  ...
  it('renders parent lanes in stored swimlaneOrder when set', () => {
    ...
    expect(
      parentB.compareDocumentPosition(parentA) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
  });
  ...
  it("keeps 'No parent' lane pinned at top regardless of stored order", () => {
    ...
    const parentB = screen.getByText('Parent B');
    expect(
      noParent.compareDocumentPosition(parentB) & Node.DOCUMENT_POSITION_FOLLOWING,
    ).toBeTruthy();
  });
  ...
  it("groups by project when swimlaneGrouping is 'project'", () => {
    ...
    renderWithI18n(<SwimLaneView issues={projectIssues} onMoveIssue={vi.fn()} />);
  ...
  it('emits project_id when a card is dropped into a project lane', () => {
    ...
    renderWithI18n(<SwimLaneView issues={projectIssues} onMoveIssue={mockOnMoveIssue} />);
  ...
  it("groups by assignee when swimlaneGrouping is 'assignee'", () => {
    ...
    renderWithI18n(<SwimLaneView issues={assigneeIssues} onMoveIssue={vi.fn()} />);
  ...
  it('emits assignee_type + assignee_id when a card is dropped into an actor lane', () => {
    ...
    renderWithI18n(<SwimLaneView issues={assigneeIssues} onMoveIssue={mockOnMoveIssue} />);
  ...
  it('does not fire listChildrenByParents when there are no parent lanes', async () => {
    ...
    renderWithI18n(<SwimLaneView issues={flatIssues} onMoveIssue={vi.fn()} />);
...
describe('SwimLaneView tab-session scroll restoration (MUL-4741)', () => {
  it('registers the outer scroller for memento capture and restores the saved offset at attach', () => {
    const adapter = {
      get: (key: string) => (key === 'swimlane' ? { top: 240, height: 2000 } : undefined),
    };
    const qc = new QueryClient({
      defaultOptions: { queries: { retry: false, gcTime: 0 } },
    });
    ...
    const scroller = container.querySelector<HTMLElement>('[data-tab-scroll-root="swimlane"]');
