...
import { ALL_STATUSES, PRIORITY_ORDER } from '@goosar/core/issues/config';
...
import { labelListOptions } from '@goosar/core/labels/queries';
...
import {
  useIssuesScopeStore,
  type IssuesScope,
} from '@goosar/core/issues/stores/issues-scope-store';
...
function shortDateLabel(dateOnly: string, locale: string) {
  return formatDateOnly(dateOnly, { month: 'short', day: 'numeric' }, locale) || dateOnly;
}
...
function useIssueCounts(allIssues: Issue[], serverFacets?: IssueTableFacetsResponse) {
  return useMemo(() => {
...
function ActorSubContent({
  ...
  const filteredAgents = agents.filter(
    (a) => !a.archived_at && (a.name.toLowerCase().includes(query) || matchesPinyin(a.name, query)),
  );
  const filteredSquads = squads.filter(
    (s) => !s.archived_at && (s.name.toLowerCase().includes(query) || matchesPinyin(s.name, query)),
  );
            ...
            {filteredSquads.map((s) => {
              ...
              return (
                <DropdownMenuCheckboxItem
                  key={s.id}
                  checked={checked}
                  onCheckedChange={() => onToggle({ type: 'squad', id: s.id })}
                  className={FILTER_ITEM_CLASS}
                >
                  <HoverCheck checked={checked} />
...
export function IssueDisplayControls({
  ...
  const tableGroupableProperties = useMemo(
    () => workspaceProperties.filter((p) => ['select', 'checkbox'].includes(p.type)),
    [workspaceProperties],
  );
    ...
    showDateFilter && dateFilter
      ? `${t(($) => $.filters[DATE_FIELD_LABEL_KEY[dateFilter.field]])}: ${
          dateFilter.from === dateFilter.to
            ? shortDateLabel(dateFilter.from, uiLocale)
            : `${shortDateLabel(dateFilter.from, uiLocale)} - ${shortDateLabel(dateFilter.to, uiLocale)}`
        }`
      : null;
  ...
  const effectiveTableGrouping =
    tableGroupingPropertyId && !propertyById.has(tableGroupingPropertyId) ? 'none' : tableGrouping;
    ...
    <div className="flex shrink-0 items-center gap-1">
      ...
      >
        ...
        <DropdownMenuContent align="end" className="w-auto">
          {/* Status */}
          <DropdownMenuSub
            onOpenChange={(open) => onTableFacetChange?.(open ? { kind: 'status' } : null)}
          >
            <DropdownMenuSubTrigger>
          ...
          {/* Priority */}
          <DropdownMenuSub
            onOpenChange={(open) => onTableFacetChange?.(open ? { kind: 'priority' } : null)}
          >
            <DropdownMenuSubTrigger>
          ...
          {showDateFilter && onDateFilterChange && (
            <DropdownMenuSub>
              ...
              <DropdownMenuSubContent className="w-56">
                <DateSubContent value={dateFilter} onChange={onDateFilterChange} />
              </DropdownMenuSubContent>
            </DropdownMenuSub>
          )}
          ...
          {/* Assignee */}
          <DropdownMenuSub
            onOpenChange={(open) => onTableFacetChange?.(open ? { kind: 'assignee' } : null)}
          >
            <DropdownMenuSubTrigger>
          ...
          {/* Creator */}
          <DropdownMenuSub
            onOpenChange={(open) => onTableFacetChange?.(open ? { kind: 'creator' } : null)}
          >
            <DropdownMenuSubTrigger>
          ...
          {/* Project */}
          <DropdownMenuSub
            onOpenChange={(open) => onTableFacetChange?.(open ? { kind: 'project' } : null)}
          >
            <DropdownMenuSubTrigger>
          ...
          {/* Label */}
          <DropdownMenuSub
            onOpenChange={(open) => onTableFacetChange?.(open ? { kind: 'label' } : null)}
          >
            <DropdownMenuSubTrigger>
          ...
          {filterableProperties.map((property) => {
            ...
            return (
              <DropdownMenuSub
                key={property.id}
                onOpenChange={(open) =>
                  onTableFacetChange?.(open ? { kind: 'property', property_id: property.id } : null)
                }
              >
                <DropdownMenuSubTrigger>
                  {property.icon ? (
      ...
      {viewMode === 'table' && (
        <DropdownMenu open={tableGroupMenuOpen} onOpenChange={setTableGroupMenuOpen}>
          <DropdownMenuTrigger
            render={
        ...
        <TableColumnPicker
          ...
          trigger={
            <Button variant="outline" size="sm" className={controlButtonClass}>
              ...
              <span className="hidden md:inline">{t(($) => $.table.columns.section)}</span>
              <ChevronDown className="size-3 text-muted-foreground" />
            </Button>
          }
        />
                  ...
                  >
                    {SORT_OPTIONS.map((opt) => (
                      <DropdownMenuRadioItem key={opt.value} value={opt.value}>
                        {t(
                          ($) =>
                            $.display[SORT_LABEL_KEY[opt.value as keyof typeof SORT_LABEL_KEY]],
                        )}
                      </DropdownMenuRadioItem>
                    ))}
