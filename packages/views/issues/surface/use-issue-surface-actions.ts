...
import { type IssueSurfaceActions, type IssueSurfaceMutationOptions } from './actions-context';
import type { IssueCreateDefaults } from './types';
import { useT } from '../../i18n';

export type MoveIssueUpdates = Pick<
  UpdateIssueRequest,
  'status' | 'assignee_type' | 'assignee_id' | 'position' | 'parent_issue_id' | 'project_id'
> & {
...
export function useIssueSurfaceActions({
  ...
  const openCreateIssue = useCallback(
    (defaults?: IssueCreateDefaults) => {
      useModalStore.getState().open('create-issue', { ...createDefaults, ...defaults });
    },
    [createDefaults],
  );
