...
describe('useIssueGroupBranches', () => {
  ...
  it('pages group headers only when the surface sentinel asks', async () => {
    const listIssueTableGroups = vi.fn(async (request: IssueTableGroupsRequest) => {
      ...
      return {
        ...
        groups: [
          {
            key: secondPage ? 'assignee:unassigned' : 'assignee:member:user-1',
            value: {
              kind: 'assignee' as const,
              actor: secondPage ? null : { type: 'member' as const, id: 'user-1' },
            },
            count: 1,
          },
        ],
  ...
  it('keeps exact compound descriptors while paging only visible cells', async () => {
    ...
    const todoKey = result.current.descriptors[0]?.secondary_groups?.[0]?.key ?? '';
    expect(result.current.pagination[todoKey]?.total).toBe(2);
    act(() => result.current.pagination[todoKey]?.loadMore());
    await waitFor(() =>
      expect(result.current.issues.map((issue) => issue.id)).toEqual(['child-1']),
    );
    expect(listIssueTableRows).toHaveBeenCalledTimes(1);
    expect(listIssueTableRows.mock.calls[0]?.[0].group_key).toContain(':status:todo');
    act(() => result.current.pagination[todoKey]?.loadMore());
