...
import { pruneIssueSurfaceViewStates } from '@goosar/core/issues/stores/surface-view-store';
import type {
  AgentTask,
  Issue,
...
describe('IssueSurface — scope switch loading semantics', () => {
  ...
  beforeEach(() => {
    ...
    const listIssues = vi.fn((params?: ListIssuesParams) => {
      ...
      const issues = params?.status === 'todo' ? [makeIssue('i1', 'P1 issue', 'p1')] : [];
  ...
  it("shows loading — not the previous project's issues — while the next project is fetching", async () => {
    ...
    rerender(<QueryClientProvider client={qc}>{projectSurface('p2')}</QueryClientProvider>);
  ...
  it('shows a cached project instantly on switch-back (no loading flash)', async () => {
    ...
    rerender(<QueryClientProvider client={qc}>{projectSurface('p1')}</QueryClientProvider>);
    await waitFor(() => expect(screen.getByText('P1 issue')).toBeInTheDocument());
  ...
  it('shows loading on a workspace switch even though the scope key is identical', async () => {
    ...
    const listIssues = vi.fn((params?: ListIssuesParams) => {
      const issues = params?.status === 'todo' ? [makeIssue('i1', 'WS1 issue', 'p1')] : [];
      return Promise.resolve({ issues, total: issues.length });
    });
    setApiInstance({
      listIssues,
    ...
    rerender(<QueryClientProvider client={qc}>{workspaceSurface()}</QueryClientProvider>);
...
describe('IssueSurface — table pagination ownership', () => {
  ...
  it('does not materialize the legacy offset window and starts one cursor root branch', async () => {
    const { getIssueSurfaceViewStore } =
      await import('@goosar/core/issues/stores/surface-view-store');
  ...
  it('keeps loaded rows when a continuation page reports zero', async () => {
    ...
    const loadMoreButton = document.querySelector<HTMLButtonElement>('tbody button.sticky');
