...
function isQueuedTaskStatus(status: AgentTask['status']) {
  return status === 'queued' || status === 'dispatched' || status === 'waiting_local_directory';
}
...
export function selectIssueTasks(snapshot: readonly AgentTask[], issueId: string): IssueTaskGroups {
...
export function deriveIssueSurfaceActivity(tasks: readonly AgentTask[]): IssueSurfaceActivity {
