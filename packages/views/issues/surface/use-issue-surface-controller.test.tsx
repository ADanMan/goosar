...
function makeIssue(overrides: Partial<Issue> & Pick<Issue, 'id' | 'status'>): Issue {
  return {
    workspace_id: 'ws-1',
    number: 1,
    identifier: 'MUL-1',
...
describe('useIssueSurfaceController', () => {
  ...
  >;
  let getAgentTaskSnapshot: ReturnType<typeof vi.fn<() => Promise<AgentTask[]>>>;
  let getWorkspaceWorkingAgents: ReturnType<typeof vi.fn<() => Promise<WorkspaceWorkingAgent[]>>>;
  ...
  beforeEach(() => {
    ...
    getWorkspaceWorkingAgents = vi.fn(() => Promise.resolve([] satisfies WorkspaceWorkingAgent[]));
  ...
  it.each([
    ...
    'merges $name create defaults into the create modal payload',
    ({ scope, surfaceKey, expected }) => {
      const { result } = renderHook(
        () =>
          useIssueSurfaceController({
            scope,
  ...
  it('delegates drag movement as a server-owned relative intent', () => {
    ...
    const options = updateIssueMutate.mock.calls[0]?.[1] as { onSettled?: () => void } | undefined;
    options?.onSettled?.();
  ...
  it('debounces table search into the canonical server query without fetching the legacy flat window', async () => {
    ...
    await waitFor(() => expect(result.current.tableQuerySpec.search).toBe('Release train'));
  ...
  it('loads only the Table facet whose filter submenu is active', async () => {
    ...
    await waitFor(() => expect(result.current.tableFacetCounts?.facets[0]?.kind).toBe('status'));
  ...
  it.each([
    ...
    async ({ configure, properties }) => {
      ...
      setApiInstance({
        ...
        listGroupedIssues: vi.fn(() => never()),
        listProjects: vi.fn(() => never()),
        listProperties: vi.fn(() => Promise.resolve({ properties, total: properties.length })),
        getAgentTaskSnapshot: vi.fn(() => Promise.resolve([])),
        getChildIssueProgress: vi.fn(() => Promise.resolve([])),
      } as unknown as ApiClient);
  ...
  it('exports every stable cursor page exactly once', async () => {
    ...
    await expect(result.current.exportTableIssues()).resolves.toEqual([first, second]);
  ...
  it('uses the active My Issues relation for the Table working-agent filter', async () => {
    ...
    await waitFor(() =>
      expect(getWorkspaceWorkingAgents).toHaveBeenCalledWith('issue', 'assigned', undefined),
    );
    expect(result.current.tableQuerySpec.filters.working_issue_ids).toEqual([]);
  });
  ...
  it('treats a cancelled-only filter like any other narrowing status filter', async () => {
    ...
    expect(result.current.surfaceIssues.map((i) => i.id)).toContain('cancelled-1');
  });
  ...
  it('sends working issue ids to the server without reconstructing a local scope', async () => {
    ...
    await waitFor(() => expect(result.current.isLoading).toBe(false));
    expect(result.current.tableQuerySpec.filters.working_issue_ids).toEqual(['todo-1', 'prog-1']);
    expect(result.current.tableQuerySpec.filters.assignees).toBeUndefined();
    expect(result.current.tableQuerySpec.filters.working_only).toBeUndefined();
    expect(getAgentTaskSnapshot).not.toHaveBeenCalled();
  ...
  it('keeps the activity-chip scope unknown before the server filter is enabled', async () => {
    ...
    getWorkspaceWorkingAgents.mockResolvedValue([makeWorkingAgent('agent-1', ['todo-1'])]);
  ...
  it('sends the sub-issue display rule to the same server query', async () => {
    ...
    getWorkspaceWorkingAgents.mockResolvedValue([makeWorkingAgent('agent-1', ['parent-1'])]);
  ...
  it('filters Gantt by running-task issue ids rather than issue assignees', async () => {
    ...
    await waitFor(() => expect(result.current.filteredGanttIssues.length).toBe(1));

    expect(result.current.workingScopeIssues?.map((i) => i.id)).toEqual(['gantt-open']);
  ...
  it.each([
  ...
  ])('keeps Gantt empty when $name', async ({ workingAgents, selectedAssigneeId }) => {
    mockGanttIssues(ganttFixture);
    ...
    expect(result.current.tableQuerySpec.filters.assignees).toEqual(
      selectedAssigneeId ? [{ type: 'agent', id: selectedAssigneeId }] : undefined,
    );
  ...
  it('widens the gantt working scope when show-completed is turned on', async () => {
    ...
    await waitFor(() => expect(result.current.filteredGanttIssues.length).toBe(2));
