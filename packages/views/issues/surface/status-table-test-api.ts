...
type LegacyListIssues = (params?: ListIssuesParams) => Promise<ListIssuesResponse>;

function legacyParamsForStatus(query: IssueTableQuerySpec, status: IssueStatus): ListIssuesParams {
...
async function rowsForStatus(
  ...
  if (query.filters.statuses && !query.filters.statuses.includes(status)) {
    return [];
  }
  const response = await listIssues(legacyParamsForStatus(query, status));
  return response.issues.filter((issue) => {
    if (query.filters.include_sub_issues === false && issue.parent_issue_id !== null) {
      return false;
    }
...
async function allRows(listIssues: LegacyListIssues, query: IssueTableQuerySpec) {
...
function primaryDescriptor(
  ...
  if (primary === 'assignee') {
    const actor =
      issue.assignee_type && issue.assignee_id
        ...
        : null;
    return {
      key: actor ? `assignee:${actor.type}:${actor.id}` : 'assignee:unassigned',
  ...
  return {
    key: issue.parent_issue_id ? `parent:${issue.parent_issue_id}` : 'parent:none',
    value: {
      kind: 'parent',
      ...
      parent: parent
        ...
        : null,
      value_state: issue.parent_issue_id ? (parent ? 'value' : 'unavailable') : 'unset',
    },
  };
}
  ...
  return {
    listIssueTableGroups: async (request: IssueTableGroupsRequest) => {
      if (request.group.kind === 'compound') {
        ...
        for (const issue of issues) {
          const descriptor = primaryDescriptor(issue, request.group.primary, issueById);
    ...
    listIssueTableRows: async (request: IssueTableRowsRequest) => {
      if (request.group.kind === 'compound') {
        ...
        const status =
          marker >= 0 ? request.group_key?.slice(marker + ':status:'.length) : undefined;
        const primaryKey =
          marker >= 0 ? request.group_key?.slice('compound:'.length, marker) : undefined;
        ...
        const issues = all.filter((issue) => {
          const descriptor = primaryDescriptor(issue, primary, issueById);
      ...
      const issues = status ? await rowsForStatus(listIssues, request.query, status) : [];
      return {
        query_fingerprint: 'test',
        group_key: request.group_key,
        parent_id: request.parent_id,
