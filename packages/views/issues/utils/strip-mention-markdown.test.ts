...
describe('stripMentionMarkdown', () => {
  ...
  it('strips simple member mention', () => {
    expect(stripMentionMarkdown('[@Alice](mention://member/abc-123)')).toBe('@Alice');
  });

  it('strips issue mention (no @ prefix)', () => {
    expect(stripMentionMarkdown('[MUL-123](mention://issue/some-uuid)')).toBe('MUL-123');
  });

  it('handles escaped brackets in names', () => {
    expect(stripMentionMarkdown('[@David\\[TF\\]](mention://agent/id-123)')).toBe('@David[TF]');
  });
  ...
  it('does NOT strip regular markdown links', () => {
    expect(stripMentionMarkdown('[docs](https://example.com)')).toBe('[docs](https://example.com)');
  });

  it('does NOT strip non-mention parenthetical links', () => {
    expect(stripMentionMarkdown('[click here](http://foo.bar/baz)')).toBe(
      '[click here](http://foo.bar/baz)',
    );
  });

  it('handles backslash-escaped content that is NOT a mention', () => {
    expect(stripMentionMarkdown('\\[@Literal](mention://agent/id)')).toBe(
      '\\[@Literal](mention://agent/id)',
    );
  });
